...
def jax_docs_dir():
  ...
  try:
    # In bazel, access docs files via data depndencies of a jax.docs package.
    return importlib.resources.files('jax.docs')
  except ImportError:
    # Outside of bazel, assume we are running tests from the github repository.
...
@functools.lru_cache()
def get_all_documented_jax_apis() -> dict[str, list[str]]:
...
class DocumentationCoverageTest(jtu.JaxTestCase):
  ...
  def setUp(self):
    if 'bazel-out' in sys.executable:
