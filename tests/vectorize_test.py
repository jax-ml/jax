...
class VectorizeTest(jtu.JaxTestCase):
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_leftshape={}_rightshape={}".format(
    ...
    def test_matmat(self, left_shape, right_shape, result_shape):
        self.assertEqual(
            matmat(np.zeros(left_shape), np.zeros(right_shape)).shape, result_shape
        )
    ...
    def test_matvec(self, left_shape, right_shape, result_shape):
        self.assertEqual(
            matvec(np.zeros(left_shape), np.zeros(right_shape)).shape, result_shape
        )
    ...
    def test_vecvec(self, left_shape, right_shape, result_shape):
        self.assertEqual(
            vecvec(np.zeros(left_shape), np.zeros(right_shape)).shape, result_shape
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            ...
            for shape, result_shape in [((3,), ()), ((2, 3), (2,)), ((1, 2, 3), (1, 2))]
        )
    )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            ...
            for shape, result_shape in [((), (2,)), ((3,), (3, 2))]
        )
    )
