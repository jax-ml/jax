...
class BatchingTest(jtu.JaxTestCase):
    ...
    def testConcatenate(self):
        ...
        expected_ans = onp.concatenate(
            [x, onp.swapaxes(y, 0, 1), onp.broadcast_to(z, (10, 4, 3))], 1
        )
        self.assertAllClose(ans, expected_ans, check_dtypes=False)
        ...
        expected_ans = onp.concatenate(
            [x, onp.broadcast_to(y, (10, 2, 3)), onp.moveaxis(z, 2, 0)], 2
        )
        self.assertAllClose(ans, expected_ans, check_dtypes=False)
    ...
    def testHessian(self):
        ...
        x = onp.array([-1.0, -0.5, 0.0, 0.5, 1.0])
    ...
    def testRandom(self):
        ...
        expected = onp.stack(
            [random.normal(random.PRNGKey(seed), (3, 2)) for seed in onp.arange(10)]
        )
        self.assertAllClose(ans, expected, check_dtypes=False)
    ...
    def testConvGeneralDilatedBatchNotMajor(self):
        ...
        per_example = np.reshape(
            np.transpose(per_example, (1, 2, 0, 3, 4)), (5, 5, 21, 4)
        )
    ...
    def testGatherBatchedOperand(
        self, axis, shape, dtype, idxs, dnums, slice_sizes, rng, rng_idx
    ):
    ...
    def testGatherGradBatchedOperand(
        self, axis, shape, dtype, idxs, dnums, slice_sizes, rng, rng_idx
    ):
        fun = partial(lax.gather, dimension_numbers=dnums, slice_sizes=slice_sizes)
    ...
    def testGatherGradBatchedIndices(
        self, axis, shape, dtype, idxs, dnums, slice_sizes, rng, rng_idx
    ):
        fun = partial(lax.gather, dimension_numbers=dnums, slice_sizes=slice_sizes)
        gfun = grad(lambda x, idx: np.sum(np.sin(fun(x, idx))))
    ...
    def testGatherBatchedBoth(
        self, op_axis, idxs_axis, shape, dtype, idxs, dnums, slice_sizes, rng, rng_idx
    ):
        fun = partial(lax.gather, dimension_numbers=dnums, slice_sizes=slice_sizes)
        operand = rng(shape, dtype)
    ...
    def testIssue489(self):
        def f(key):
            ...
            u, _ = lax.while_loop(lambda uk: uk[0] > 0.5, body_fn, (1.0, key))
            return u
    ...
    def testIndexAddBatchedIndexesOnly(self):
        ...
        result = vmap(f, (None, 0, None))(onp.zeros((10,)), onp.arange(10), 1.0)
