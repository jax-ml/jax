...
# TODO: Branched from https://github.com/sholtodouglas/minformer/blob/main/minformer/model_test.py
# All the changes are marked with a TODO
#
"""Minimal model definition."""
...
# TODO: introduce a global flag to workaround the fact that some of the code
# cannot be processed symbolically.
TODO = True

# TODO: I would like to use a device mesh with a symbolic number of devices
#  but don't know how.
def create_mesh():
...
def segment_ids_to_positions(segment_ids):
  ...
  # TODO: lax.associative_scan is not supported for symbolic shapes
  if TODO:
...
def attention(
  ...
  """
  # Div sqrt(key_dim)
  # TODO: symbolic dimensions cannot be raised to non-integer powers
...
def attention_kernel(q, k, v, q_segment_ids, kv_segment_ids, cfg: Config):
  ...
  if TODO:
    # TODO: the flashattention needs scale to be a static, but we cannot
...
def init_optimizer_state(weights: Weights):
  def _zeros_like(old):
    # TODO: we need this because _zeros_like does not work inside a jit; old.sharding is not defined for DynamicJaxprTracer
    if TODO:
...
def sample_from_prompt(
  ...
  # TODO: in presence of symbolic shapes, use tokens.shape[0] instead of len(tokens)
  assert tokens.shape[0] <= cfg.max_seq_len
  # TODO: the following is not supported with symbolic shapes, i.e., we can
...
#
# TODO: everything below is added to exercise different configurations
#
import numpy as np
...
class MinformerSymbolicShapesTest(parameterized.TestCase):
  ...
  def setup_config(self, causal: bool):
    B, T, N, K, V, D, F, H, S = export.symbolic_shape(
        ...
        constraints=[
            ...
            "mod(S, 512) == 0",
            # TODO: The following should all follow from mod(H, 2) == 0, but the
            # reasoning engine is not powerful enough.
            # Needed for some element wise op.
            "floordiv(H + 1, 2) == floordiv(H , 2)",
            # without this we get Incompatible shapes for broadcasting: shapes=[(H,), (1, N, 16, 2*floordiv(H, 2))]
            "2*floordiv(H, 2) == H",
            # without this we get Symbolic dimension comparison 'H' >= 'floordiv(H, 2)' is inconclusive in a slice
            "H >= floordiv(H, 2)",
        ])
    ...
    cfg = Config(
        ...
        query_heads=N,
        key_heads=K,
        # TODO: the code uses a Python for loop over range(num_layers) to initialize
  ...
  def test_training(self, use_attn_kernel_training: bool = True):
    ...
    if use_attn_kernel_training:
      # TODO: currently, the attn kernel requires the key_dim to be non-symbolic
    ...
    def harness():
      ...
      # TODO: in the original this was 256, but results in error when using the
      ...
      prompt = jnp.arange(0, 6)  # TODO: we would like to use input_size instead
  ...
  # TODO: This is the actual test from the original minformer
  def test_overtrain_and_sample_simple_sequence(self):
    ...
    # TODO: the original test had 256 here, but that does not work with blocks
    # that are 512
