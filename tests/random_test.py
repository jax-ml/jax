...
class LaxRandomTest(jtu.JaxTestCase):
    ...
    def testNumpyAndXLAAgreeOnFloatEndianness(self, dtype):
        ...
        numpy_bits = onp.array(1.0, dtype).view(bits_dtype)
        xla_bits = api.jit(
            lambda: lax.bitcast_convert_type(onp.array(1.0, dtype), bits_dtype)
        )()
    ...
    def testDirichlet(self, alpha, dtype):
        ...
        for samples in [uncompiled_samples, compiled_samples]:
            self.assertAllClose(
                samples.sum(-1), onp.ones(10000, dtype=dtype), check_dtypes=True
            )
            alpha_sum = sum(alpha)
            for i, a in enumerate(alpha):
                self._CheckKolmogorovSmirnovCDF(
                    samples[..., i], scipy.stats.beta(a, alpha_sum - a).cdf
                )
    ...
    def testGammaGrad(self, alpha):
        ...
        cdf_dot = (
            scipy.stats.gamma.cdf(z, alpha + eps)
            - scipy.stats.gamma.cdf(z, alpha - eps)
        ) / (2 * eps)
    ...
    def testStaticShapeErrors(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            re.compile(r".*requires a concrete.*"),
    ...
    def testNoOpByOpUnderHash(self):
        ...
        try:
            out = random.threefry_2x32(
                onp.zeros(2, onp.uint32), onp.arange(10, dtype=onp.uint32)
            )
        finally:
