...
class ShardingInTypesTest(jtu.JaxTestCase):
  ...
  def test_cond_fori_reduced_fwd(self, mesh):
    k = jax.device_put(np.arange(8.0), jax.P('x'))
    v = jax.device_put(np.arange(8.0), jax.P('x'))
    ...
    kv_obj = KV(jax.reshard(k, jax.P(reduced={'x'})),
                jax.reshard(v, jax.P(reduced={'x'})))
