...
class ArrayPjitTest(jtu.JaxTestCase):
  ...
  def test_abstract_mesh_context_trace(self, axis_type):
    abstract_mesh = jax.sharding.AbstractMesh((2,), ('x',), (axis_type,))
    inp = jax.ShapeDtypeStruct((8, 2), np.float32)
  ...
  def test_abstract_mesh_context_mismatch_error(self):
    abstract_mesh = jax.sharding.AbstractMesh((2,), ('x',), (AxisType.Auto,))

    @jax.jit
    def f(x):
      return x * 2

    abstract_mesh2 = jax.sharding.AbstractMesh((1,), ('x',), (AxisType.Auto,))
    ...
    mesh3 = jtu.create_mesh((1,), 'x', axis_types=(AxisType.Auto,))
    inp = jax.ShapeDtypeStruct(
