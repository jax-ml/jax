...
class FftTest(jtu.JaxTestCase):
  @parameterized.named_parameters(
      jtu.cases_from_list({
          "testcase_name":
              "_shape={}_axes={}".format(jtu.format_shape_dtype_string(shape, dtype), axes),
          "axes":
              axes,
          "shape":
              shape,
          ...
          "rng":
              rng
      } for rng in [jtu.rand_default()] for dtype in all_dtypes
  ...
  def testFftnErrors(self):
    ...
    self.assertRaisesRegexp(
        ValueError, "jax.np.fftn only supports 1D, 2D, and 3D FFTs over the innermost axes. "
