...
def maybe_skip_known_failure():
  if os.getenv("SKIP_KNOWN_FAILURES", ""):
    raise unittest.SkipTest("TODO: SKIP_KNOWN_FAILURES is set")
...
class ReproTest(jtu.JaxTestCase):
  ...
  def test_pallas_blockspec_outside_collect(self):
    ...
    log_output = "\n".join(logs.output)
    self.assertRegex(log_output, r"Repro error: Duplicate function definition")
  ...
  def test_pallas_emit_pipeline_tpu(self, *, interpret: bool):
    ...
    fake_info = tpu_info._get_tpu_info_impl(tpu_info.ChipVersion.TPU_V5E, 1)
    run_native = interpret or jtu.is_device_tpu_at_least(5)
    mock_context = (contextlib.nullcontext() if jtu.is_device_tpu_at_least(5)
                    else mock.patch(
    ...
    def f(x):
      if run_native:
        ...
        exp = export.export(g, platforms=("tpu",))(x)
        return 0.
