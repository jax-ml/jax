...
from jax._src.repro.tracker import (
  _thread_local_state as _repro_thread_local_state,
  flags_override as _repro_flags_override)
from jax._src.repro.emitter import (
  initialize_operand_emitter as _repro_initialize_operand_emitter,
  _operand_emitter as _repro_operand_emitter)
...
class ReproTest(jtu.JaxTestCase):
  ...
  def setUp(self):
    if not traceback_util.repro_enabled():
      self.skipTest("JAX_REPRO_DIR not set")
    self.repro_state_snapshot = _repro_thread_local_state.snapshot_state()
    super().setUp()

  def tearDown(self) -> None:
    _repro_thread_local_state.restore_state(self.repro_state_snapshot)
    super().tearDown()
  ...
  def collect_and_check(self, func: Callable, *args,
    ...
    with context:
      direct_result = repro.save_repro(
          lambda: func(*args, **kwargs),
          repro_name_prefix=f"repro_{self._testMethodName}")

    self.assertIsNotNone(repro.last_repro())
    repro_path, repro_source = repro.last_repro()
    if skip_repro_eval:
      return

    with traceback_util.enable_repro(False):
      with context:
  ...
  def test_with_repro_first_error(self):
    def f1(x, y):
      res = x + y  # Cannot broadcast together
      assert False
  ...
  @jtu.parameterized_filterable(
      kwargs=[dict(use_save_repro=use_save_repro)
              for use_save_repro in (True, False)]
  )
  def test_non_error(self, use_save_repro: bool):
    def f1(x, y):
      drop1 = jnp.sin(x)
    ...
    with self.assertRaisesRegex(TypeError, "got incompatible shapes"):
      if use_save_repro:
        repro.save_repro(lambda: f1(x, y))
      else:
        f1(x, y)
    _, repro_source = repro.last_repro()
    # We don't have any of the sin, cos, lax.cond if we saved the repro
    # due to an uncaught exception; otherwise we keep everything
    if use_save_repro:
  ...
  def test_keep_non_error(self):
    # We keep all if we use repro.save_repro
    def f1(x, y):
      drop1 = jnp.sin(x)
      drop2 = lax.cond(x[0] > 0., lambda: jnp.sin(y), lambda: jnp.cos(y))
      err = drop1 + drop2  # Raises the error
  ...
  def test_printing_constants(self):
    ...
    with _repro_flags_override("fake_array_threshold", x.size + 1):
  ...
  def test_traceback_error_missing_custom_emitter(self):
    ...
    _repro_initialize_operand_emitter()
    ...
    old_lax_Precision_emitter = _repro_operand_emitter[lax.Precision]
    try:
      del _repro_operand_emitter[lax.Precision]

      with _repro_flags_override("log_traceback_frames", 40,
                                 "enable_checks", True,
                                 "enable_checks_as_errors", True):
        with self.assertLogs(level=logging.ERROR) as logs:
          with self.assertRaisesRegex(repro.ReproError,
                                      "errors during repro emitting"):
            self.collect_and_check(my_func_with_distinct_name,
                                  jnp.ones((8,), dtype=np.float32),
                                  skip_repro_eval=True)
        log_output = "\n".join(logs.output)
        self.assertIn("<locals>.my_func_with_distinct_name", log_output)
        self.assertIn("non-tracer without custom emitter: HIGHEST", log_output)

    finally:
      _repro_operand_emitter[lax.Precision] = old_lax_Precision_emitter
  ...
  def test_forgotten_api_boundary(self):
    ...
    with _repro_flags_override("enable_checks_as_errors", True,
                               "enable_checks", True):
      with self.assertRaisesRegex(repro.ReproError,
  ...
  def test_cleanup_main_body(self):
    ...
    s = _repro_thread_local_state.call_stack[-1]
  ...
  def test_remat_custom_vjp_policy(self):
    ...
    with _repro_flags_override("enable_checks_as_errors", False):
  ...
  def test_pallas_call_scalar_prefetch(self):
    ...
    with _repro_flags_override("fake_array_threshold", 1024):
  ...
  def test_pallas_call_custom_fusion(self):
    @fuser.custom_fusion
    def c(x, y):
      return x + y
