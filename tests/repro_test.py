...
class ReproTest(jtu.JaxTestCase):
  ...
  def collect_repro(self, func: Callable, *args, **kwargs):
    ...
    prev = repro._thread_local_state.collect_repro_on_success
    try:
      repro._thread_local_state.collect_repro_on_success = True
      res = func(*args, **kwargs)
    finally:
      repro._thread_local_state.collect_repro_on_success = prev
    repro_stack_entry = repro._thread_local_state.call_stack[-1]
    assert repro_stack_entry.func.is_main
    repro._emit_repro(repro_stack_entry, "repro_test.collect_repro")
    return res
  ...
  def collect_and_check(self, func: Callable, *args,
    ...
    # Now try to evaluate the repro
    compiled = compile(repro_source, repro_path, "exec")
    custom_namespace = {}
    custom_namespace['__builtins__'] = __builtins__
    try:  # Disable repros for this exec
      prev = repro._thread_local_state.collect_repro_enabled
      repro._thread_local_state.collect_repro_enabled = False
      if expect_exception is None:
      ...
      repro._thread_local_state.collect_repro_enabled = prev
    return repro_source
  ...
  def test_traceback_error_missing_custom_emitter(self):
    ...
    repro.initialize_operand_emitter()
    ...
    old_lax_Precision_emitter = repro._operand_emitter[lax.Precision]
    try:
      del repro._operand_emitter[lax.Precision]

      old_traceback_frames = repro._thread_local_state.flags.log_traceback_frames
      try:
        repro._thread_local_state.flags.log_traceback_frames = 40
        ...
        self.assertTrue(any("<local>.my_func_with_distinct_name" for o in logs.output))
      finally:
        repro._thread_local_state.flags.log_traceback_frames = old_traceback_frames
    finally:
      repro._operand_emitter[lax.Precision] = old_lax_Precision_emitter
  ...
  def test_cleanup_main_body(self):
    ...
    s = repro._thread_local_state.call_stack[-1]
  ...
  def test_forgotten_jax_boundary(self):
    self.skipTest("We got rid of the checking of traceback")
    # Simulate forgetting to declare a jax_boundary. We use
    ...
    with self.assertRaisesRegex(repro.ReproError,
                                "Repro collection invariants broken"):
