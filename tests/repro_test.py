...
class ReproTest(jtu.JaxTestCase):
  ...
  def collect_and_check(self, func: Callable, *args,
    ...
    direct_result = repro.collect_repro(func, *args, **kwargs)

    if len(_repro_thread_local_state.emitted_repros) == 0:
      assert False, "No repros have been collected"
    elif len(_repro_thread_local_state.emitted_repros) > 1:
      assert False, (
          "Multiple repros have been collected, perhaps you are running the "
          "function in eager mode? Seen: " +
          ", ".join(str(p) for p, _ in _repro_thread_local_state.emitted_repros))
    repro_path, repro_source = _repro_thread_local_state.emitted_repros[0]

    with _repro_flag_override("collect_repro_enabled", False):
      result = repro.eval_repro(repro_path, repro_source)
    self.assertAllClose(_repro_EmitFuncContext.flatten_custom_pytree(direct_result),
                        result)
  ...
  def test_printing_constants(self):
    ...
    with _repro_flag_override("fake_array_threshold", x.size + 1):
  ...
  def test_traceback_error_missing_custom_emitter(self):
    ...
    try:
      ...
      with _repro_flag_override("log_traceback_frames", 40):
        with self.assertLogs(level=logging.ERROR) as logs:
          with self.assertRaisesRegex(repro.ReproError,
                                      "errors during repro emitting"):
            self.collect_and_check(my_func_with_distinct_name, jnp.ones((8,), dtype=np.float32))
        log_output = "\n".join(logs.output)
  ...
  def test_forgotten_repro_boundary(self):
    ...
    with _repro_flag_override("enable_checks_as_errors", True):
      with self.assertRaisesRegex(repro.ReproError,
  ...
  def test_remat_custom_vjp_policy(self):
    ...
    with _repro_flag_override("enable_checks_as_errors", False):
  ...
  def test_pallas_call_scalar_prefetch(self):
    ...
    with _repro_flag_override("fake_array_threshold", 1024):
