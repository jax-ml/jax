...
class APITest(jtu.JaxTestCase):
    ...
    def test_grad_unit_output(self):
        jtu.check_raises(
            lambda: grad(lambda x: ())(onp.zeros(3)),
            TypeError,
            "Gradient only defined for scalar-output functions. ",
        )
    ...
    def test_grad_nonscalar_output(self):
        jtu.check_raises(
            lambda: grad(lambda x: x)(onp.zeros(3)),
            TypeError,
            "Gradient only defined for scalar-output functions. ",
        )
    ...
    def test_device_put_and_get(self):
        x = onp.arange(12.0).reshape((3, 4)).astype("float32")
        dx = api.device_put(x)
    ...
    def test_std_basis(self):
        ...
        basis = api._std_basis([0.0, (np.zeros(3), np.zeros((3, 4)))])
    ...
    def test_hessian_on_pytrees(self):
        ans = hessian(lambda x: np.array(x) ** 2)((1.0, 2.0))
        expected = (
            (onp.array([2.0, 0.0]), onp.array([0.0, 0.0])),
            (onp.array([0.0, 0.0]), onp.array([0.0, 2.0])),
        )
        self.assertAllClose(ans, expected, check_dtypes=False)
    ...
    def test_large_device_constant(self):
        ...
        self.assertAllClose(ans, onp.ones(int(2e6)) * 2.0, check_dtypes=False)

    def test_grad_and_aux_basic(self):
        g, aux = grad(lambda x: (x ** 3, [x ** 2]), has_aux=True)(3.0)
        self.assertAllClose(g, grad(lambda x: x ** 3)(3.0), check_dtypes=True)
        self.assertAllClose(aux, [9.0], check_dtypes=True)
    ...
    def test_jarrett_jvps(self):
        ...
        for x in [3.0, onp.array([2.0, 3.0, 4.0])]:
    ...
    def test_jarrett_jvps2(self):
        ...
        for x, y in [(3.0, 4.0), (onp.array([5.0, 6.0]), onp.array([7.0, 8.0]))]:
    ...
    def test_defvjp_all(self):
        ...
        def foo(x):
            return 2.0 * foo_p.bind(x)

        ad.defvjp_all(foo_p, lambda x: (x ** 2, lambda g: (4 * g * np.sin(x),)))
        val_ans, grad_ans = api.value_and_grad(foo)(3.0)
        self.assertAllClose(val_ans, 2 * 3.0 ** 2, check_dtypes=False)
        self.assertAllClose(grad_ans, 4 * 2 * onp.sin(3.0), check_dtypes=False)
    ...
    def test_defvjp_all_const(self):
        ...
        ad.defvjp_all(foo_p, lambda x: (x ** 2, lambda g: (12.0,)))
        val_ans, grad_ans = api.value_and_grad(foo)(3.0)
        self.assertAllClose(val_ans, 9.0, check_dtypes=False)
        self.assertAllClose(grad_ans, 12.0, check_dtypes=True)
    ...
    def test_defvjp_all_higher_order_revmode(self):
        ...
        ans = api.grad(api.grad(foo))(3.0)
        self.assertAllClose(ans, 2 * 2 * 3.0, check_dtypes=False)
    ...
    def test_defvjp_all_multiple_arguments(self):
        ...
        def vjpfun(x, y):
            out = x ** 2 + y ** 3
            vjp = lambda g: (g + x + y, g * x * 9.0)
            return out, vjp

        ad.defvjp_all(foo_p, vjpfun)
        val_ans, grad_ans = api.value_and_grad(foo)(3.0, 4.0)
        self.assertAllClose(val_ans, 3.0 ** 2 + 4.0 ** 3, check_dtypes=False)
        self.assertAllClose(grad_ans, 1.0 + 3.0 + 4.0, check_dtypes=False)

        ans = api.grad(foo, (0, 1))(3.0, 4.0)
        self.assertAllClose(ans, (1.0 + 3.0 + 4.0, 1.0 * 3.0 * 9.0), check_dtypes=False)
    ...
    def test_defvjp_all(self):
        ...
        val_ans, grad_ans = api.value_and_grad(foo)(3.0)
        self.assertAllClose(val_ans, onp.sin(3.0), check_dtypes=False)
        self.assertAllClose(grad_ans, 3.0, check_dtypes=False)
    ...
    def test_defvjp(self):
        ...
        val_ans, grad_ans = api.value_and_grad(foo)(3.0, 4.0)
        self.assertAllClose(val_ans, onp.sin(3.0 * 4.0), check_dtypes=False)
        self.assertAllClose(grad_ans, 0.0, check_dtypes=False)

        ans_0, ans_1 = api.grad(foo, (0, 1))(3.0, 4.0)
        self.assertAllClose(ans_0, 0.0, check_dtypes=False)
        self.assertAllClose(ans_1, 3.0 * 4.0, check_dtypes=False)
    ...
    def test_defvjp_higher_order(self):
        ...
        ans = api.grad(api.grad(foo))(2.0)
        expected = api.grad(api.grad(np.sin))(2.0)
        self.assertAllClose(ans, expected, check_dtypes=False)
    ...
    def test_defvjp_use_ans(self):
        ...
        val_ans, grad_ans = api.value_and_grad(foo, 1)(3.0, 4.0)
        self.assertAllClose(val_ans, onp.sin(3.0 * 4.0), check_dtypes=False)
        self.assertAllClose(
            grad_ans, 3.0 * 4.0 + onp.cos(onp.sin(3.0 * 4)), check_dtypes=False
        )
    ...
    def test_custom_gradient(self):
        ...
        self.assertAllClose(f(3.0), 9.0, check_dtypes=False)
        self.assertAllClose(api.grad(f)(3.0), 3.0, check_dtypes=False)
    ...
    def test_devicearray_delete(self):
        ...
        x.delete()
        jtu.check_raises_regexp(
            lambda: repr(x), ValueError, "DeviceValue has been deleted."
        )
    ...
    def test_namedtuple_subclass_transparency(self):
        ...
        def f(pt):
            return 0.0 if pt.is_zero() else np.sqrt(pt.x ** 2 + pt.y ** 2)
    ...
    def test_partial_eval_lower(self):
        ...
        subjaxpr = next(
            eqn.bound_subjaxprs[0][0] for eqn in jaxpr.eqns if eqn.bound_subjaxprs
        )
    ...
    def test_xla_computation(self):
        ...
        axis_env = [("i", 4), ("j", 2)]
        c = api.xla_computation(g, axis_env=axis_env)(5.0)
        self.assertIn("all-reduce", c.GetHloText())
    ...
    def test_xla_computation_args(self):
        ...
        c = api.xla_computation(foo)(1.0, 2.0, 3.0)
        self.assertEqual(len(c.GetProgramShape().parameter_shapes()), 3)

        c = api.xla_computation(foo, tuple_args=True)(1.0, 2.0, 3.0)
    ...
    def test_jit_device(self):
        device = xb.devices()[-1]
        x = api.jit(lambda x: x, device=device)(3.0)
        self.assertIsInstance(x, DeviceArray)
        self.assertEqual(x.device_buffer.device(), device)

    def test_jit_of_noncallable(self):
        jtu.check_raises_regexp(
            lambda: api.jit(3), TypeError, "Expected a callable value.*"
        )
    ...
    def test_concurrent_jit(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            ys = [f.result() for f in futures]
        for x, y in zip(xs, ys):
            self.assertAllClose(x * 2 - 3.0, y, check_dtypes=True)
    ...
    def test_dtype_warning(self):
        ...
        check_warning(
            lambda: np.ones_like(3, dtype=onp.int64),
            lambda: np.ones_like(3, dtype=onp.int32),
        )
        check_warning(
            lambda: np.zeros(3, dtype="int64"), lambda: np.zeros(3, dtype="int32")
        )
        check_warning(
            lambda: np.zeros_like(3, dtype="float64"),
            lambda: np.zeros_like(3, dtype="float32"),
        )
        check_warning(
            lambda: np.full((2, 3), 1, dtype="int64"), lambda: np.full((2, 3), 1)
        )
        check_warning(
            lambda: np.ones(3).astype("float64"), lambda: np.ones(3).astype("float32")
        )
        check_warning(lambda: np.eye(3, dtype=onp.float64), lambda: np.eye(3))
        check_warning(
            lambda: np.arange(3, dtype=onp.float64),
            lambda: np.arange(3, dtype=onp.float32),
        )
        check_warning(
            lambda: np.linspace(0, 3, dtype=onp.float64),
            lambda: np.linspace(0, 3, dtype=onp.float32),
        )
        check_warning(
            lambda: np.tri(2, dtype="float64"), lambda: np.tri(2, dtype="float32")
        )
    ...
    def test_custom_vjp_zeros(self):
        ...
        api.grad(lambda x, y: f(x, y)[0])(1.0, 2.0)  # doesn't crash
