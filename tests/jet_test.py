...
class JetTest(jtu.JaxTestCase):
  ...
  def test_conv(self):
    ...
    x = rng.randn(*input_shape).astype(jnp.float_)
    primals = (W, b, x)

    series_in1 = [rng.randn(*W.shape).astype(jnp.float_) for _ in range(order)]
    series_in2 = [rng.randn(*b.shape).astype(jnp.float_) for _ in range(order)]
    series_in3 = [rng.randn(*x.shape).astype(jnp.float_) for _ in range(order)]
