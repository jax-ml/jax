...
class LaxControlFlowTest(jtu.JaxTestCase):
  ...
  def testCond(self):
    ...
    def cfun(x):
      def false_fun(x):
        y = lax.mul(2, x)
        return y, lax.mul(2, y)
      return lax.cond(lax.lt(x, 3), x, lambda x: (x, x), false_fun)
  ...
  def testIssue1379(self):
    ...
    def fun(pred):
      return lax.cond(pred, pred, lambda x: (True, x), lambda x: (False, x))
  ...
  def testCondOneBranchConstant(self):
    ...
    @api.jit
    def cfun(x):
      return lax.cond(lax.lt(x, 3), x, lambda x: 5, lambda x: x)
  ...
  def testCondJVP(self):
    ...
    def fun(x):
      def false_fun(x):
        y = 2 * x
        return y, 2 * y
      return lax.cond(x < 3, x, lambda x: (x, x), false_fun)
  ...
  def testCondGrad(self):
    ...
    def f(x):
      return lax.cond(x < 2, x, lambda x: 3. * x, lambda x: jnp.sin(x))
  ...
  def testCondLinearize2(self):
    ...
    def f(x):
      return lax.cond(
          x[0] < 2, x,
          lambda x: jnp.array([1., 2.]) * x,
          lambda x: jnp.cos(jnp.sin(x))).sum()
  ...
  def testCondVmapGrad(self):
    ...
    def f(x): return lax.cond(x > 0, x, f_1, f_2)
