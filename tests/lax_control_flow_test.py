...
class LaxControlFlowTest(jtu.JaxTestCase):
    ...
    def testLoopWithConjunctionCondition(self):
        ...
        for num in [0, 5, 10, 15]:
            self.assertAllClose(
                sum_first_n(x, num), onp.sum(x[:num]), check_dtypes=False
            )
    ...
    def testNestedCond(self):
        ...
        def cfun(x):
            return lax.cond(
                ...
                x,
                lambda x: lax.cond(
                    lax.lt(x, 5), x, lambda x: lax.mul(3, x), 4, lambda y: lax.mul(y, x)
                ),
            )
    ...
    def testCondOneBranchConstantTuple(self):
        ...
        def cfun(x):
            return lax.cond(
                lax.lt(x, 3), x, lambda x: (1, 2.0, 3.0), x, lambda x: (x, 2.0, 4.0)
            )
    ...
    def testIssue1263(self):
        def f(rng, x):
            ...
            return lax.cond(cond, x, lambda x: x, np.abs(x) - 1.0, lambda x: x)
    ...
    def testScanLinearize(self, jit_scan, jit_f):
        ...
        expected = api.linearize(lambda c, as_: scan_reference(f, c, as_), c, as_)[1](
            c, as_
        )
        self.assertAllClose(ans, expected, check_dtypes=False)
    ...
    def testScanGrad(self, jit_scan, jit_f):
        ...
        expected = api.grad(lambda c, as_: list(scan_reference(f, c, as_))[0].sum())(
            c, as_
        )
        self.assertAllClose(ans, expected, check_dtypes=False)

        jtu.check_grads(
            partial(scan, f), (c, as_), order=2, modes=["rev"], atol=1e-3, rtol=1e-3
        )
    ...
    def testScanRnn(self):
        ...
        # linearize works
        _, expected = api.jvp(
            loss, (params, inputs, targets), (params, inputs, targets)
        )
    ...
    def testScanHigherOrderDifferentiation(self):
        ...
        as_ = np.arange(6.0).reshape((3, 2))
        c = 1.0

        jtu.check_grads(
            lambda c, as_: lax.scan(f, c, as_), (c, as_), modes=["rev"], order=2
        )
    ...
    def testIssue804(self):
        ...
        f = partial(lax.scan, lambda c, x: (c + lax.psum(x, "i"), c), 0.0)
    ...
    def testIssue1316(self):
        ...
        api.grad(lambda c: lax.scan(f, (c, key), onp.ones(3))[0][0])(
            0.0
        )  # doesn't crash
    ...
    def testIssue1361(self):
        ...
        def jit_run_scan(x):
            ...
            (x, _), _ = lax.scan(fun, (x, 0.0), np.arange(3))
            return x

        api.grad(lambda x: jit_run_scan(x))(0.0)  # doesn't crash
