...
class LaxControlFlowTest(jtu.JaxTestCase):
    ...
    def testWhileTypeErrors(self):
        ...
        with self.assertRaisesRegex(TypeError,
                                    re.escape(
                                        "cond_fun must return a boolean scalar, but got pytree PyTreeDef(tuple, [*,*]).")):
            lax.while_loop(lambda c: (1., 1.), lambda c: c, 0.)
        with  self.assertRaisesRegex(TypeError,
                                     re.escape(
                                         "cond_fun must return a boolean scalar, but got output type(s) [ShapedArray(float32[])].")):
            lax.while_loop(lambda c: np.float32(1.), lambda c: c, np.float32(0.))
        with self.assertRaisesRegex(TypeError,
                                    re.escape(
                                        "body_fun output and input must have same type structure, got PyTreeDef(tuple, [*,*]) and *.")):
    ...
    def testCondTypeErrors(self):
        ...
        with self.assertRaisesRegex(TypeError,
                                    re.escape(
                                        "true_fun and false_fun output must have same type structure, got * and PyTreeDef(tuple, [*,*]).")):
    ...
    def testSwitchErrors(self):
        ...
        with self.assertRaisesRegex(TypeError,
                                    re.escape(
                                        "branch 0 and 1 outputs must have same type structure, got * and PyTreeDef(tuple, [*,*]).")):
    ...
    @parameterized.named_parameters(
        {"testcase_name": f"_{shape}",
         "shape": shape}
        for shape in range(2, 100))
    def testAssociativeScanFailing(self, shape):
