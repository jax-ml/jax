...
def _skip_if_unsupported_type(dtype):
    ...
    if not FLAGS.jax_enable_x64 and dtype in (
        onp.dtype("float64"),
...
class NumpyLinalgTest(jtu.JaxTestCase):
    ...
    def testDet(self, n, dtype, rng):
        ...
        self._CheckAgainstNumpy(
            onp.linalg.det, np.linalg.det, args_maker, check_dtypes=True, tol=1e-3
        )
        self._CompileAndCheck(np.linalg.det, args_maker, check_dtypes=True)

    def testDetOfSingularMatrix(self):
        x = np.array([[-1.0, 3.0 / 2], [2.0 / 3, -1.0]], dtype=onp.float32)
    ...
    def testIssue1213(self):
        for n in range(5):
            mat = np.array([onp.diag(onp.ones([5], dtype=onp.float32)) * (-0.01)] * 2)
    ...
    def testEig(self, shape, dtype, rng):
        ...
        self._CompileAndCheck(
            partial(np.linalg.eig), args_maker, check_dtypes=True, rtol=1e-3
        )
    ...
    def testEigBatching(self, shape, dtype, rng):
        ...
        self.assertTrue(
            onp.all(
                onp.linalg.norm(onp.matmul(args, vs) - ws[..., None, :] * vs) < 1e-3
            )
        )
    ...
    def testEigh(self, n, dtype, lower, rng):
        ...
        a = (a + onp.conj(a.T)) / 2
        w, v = np.linalg.eigh(
            onp.tril(a) if lower else onp.triu(a), UPLO=uplo, symmetrize_input=False
        )
    ...
    def testEighGrad(self, shape, dtype, rng, lower):
        ...
        if dtype not in complex_types:
            ...
            f = lambda a: partial(np.linalg.eigh, UPLO=uplo, symmetrize_input=True)(a)[
                0
            ]
        ...
        assert (
            onp.max(
                onp.abs(
                    (
                        onp.diag(
                            onp.dot(onp.conj((v + dv).T), onp.dot(new_a, (v + dv)))
                        )
                        - new_w
                    )
                    / new_w
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            ...
            for ord in (
                ...
                or (isinstance(axis, tuple) and len(axis) == 1)
    ...
    def testNorm(self, shape, dtype, ord, axis, keepdims, rng):
        ...
        if ord in ("nuc", 2, -2) and (
            ...
            or (isinstance(axis, tuple) and len(axis) == 2)
        ):
        ...
        self._CheckAgainstNumpy(
            onp_fn, np_fn, args_maker, check_dtypes=check_dtypes, tol=1e-3
        )
    ...
    def testSVD(self, b, m, n, dtype, full_matrices, compute_uv, rng):
        ...
        if compute_uv:
            ...
            if full_matrices:
                ...
                self.assertTrue(
                    onp.all(
                        norm(a - onp.matmul(out[1][..., None, :] * out[0], out[2]))
                        < 300
                    )
                )
            ...
            self.assertTrue(
                onp.all(
                    norm(
                        ...
                        - onp.matmul(onp.conj(T(out[0])), out[0])
                    )
                    < 10
                )
            )
                ...
                self.assertTrue(
                    onp.all(
                        norm(
                            ...
                            - onp.matmul(onp.conj(T(out[2])), out[2])
                        )
                        < 10
                    )
                )
                ...
                self.assertTrue(
                    onp.all(
                        norm(
                            ...
                            - onp.matmul(out[2], onp.conj(T(out[2])))
                        )
                        < 20
                    )
                )
            ...
            self.assertTrue(
                onp.allclose(
                    onp.linalg.svd(a, compute_uv=False),
                    onp.asarray(out),
                    atol=1e-4,
                    rtol=1e-4,
                )
        ...
        self._CompileAndCheck(
            partial(np.linalg.svd, full_matrices=full_matrices, compute_uv=compute_uv),
            args_maker,
            check_dtypes=True,
        )
    ...
    def testSolve(self, lhs_shape, rhs_shape, dtype, rng):
        ...
        self._CheckAgainstNumpy(
            onp.linalg.solve, np.linalg.solve, args_maker, check_dtypes=True, tol=1e-3
        )
    ...
    def testInv(self, shape, dtype, rng):
        ...
        self._CheckAgainstNumpy(
            onp.linalg.inv, np.linalg.inv, args_maker, check_dtypes=True, tol=1e-3
        )
    ...
    def testIssue1151(self):
        ...
        self.assertAllClose(
            vmap(np.dot)(A, x), b, atol=1e-3, rtol=1e-3, check_dtypes=True
        )
...
class ScipyLinalgTest(jtu.JaxTestCase):
    ...
    def testLuSolve(self, lhs_shape, rhs_shape, dtype, trans, rng):
        ...
        self._CheckAgainstNumpy(
            osp_fun, jsp_fun, args_maker, check_dtypes=True, tol=1e-3
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            ...
            for sym_pos, lower in [(False, False), (True, False), (True, True)]
            for dtype in float_types + complex_types
            for rng in [jtu.rand_default()]
        )
    )
    ...
    def testSolveTriangular(
        self, lower, transpose_a, unit_diagonal, lhs_shape, rhs_shape, dtype, rng
    ):
        _skip_if_unsupported_type(dtype)
