...
class NumpyLinalgTest(jtu.JaxTestCase):
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "rng": rng
                          ...
                          for dtype in float_types + complex_types for rng in [jtu.rand_default()]))
  def testCholesky(self, shape, dtype, rng):
    ...
    if np.issubdtype(dtype, np.complexfloating) and (len(shape) > 2
                                                     or jtu.device_under_test() != "cpu"):
      self.skipTest("Unimplemented case for complex Cholesky decomposition.")

    self._CheckAgainstNumpy(onp.linalg.cholesky, np.linalg.cholesky, args_maker, check_dtypes=True,
                            tol=1e-3)
  ...
  def testDet(self, n, dtype, rng):
    ...
    self._CheckAgainstNumpy(onp.linalg.det, np.linalg.det, args_maker, check_dtypes=True, tol=1e-3)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(0, 0), (1, 1), (3, 3), (4, 4), (10, 10), (200, 200), (2, 2, 2), (2, 3,
                                                                                        3), (3, 2,
                                                                                             2)]
                          for dtype in float_types + complex_types for rng in [jtu.rand_default()]))
  ...
  def testSlogdet(self, shape, dtype, rng):
    ...
    self._CheckAgainstNumpy(onp.linalg.slogdet, np.linalg.slogdet, args_maker, check_dtypes=True,
                            tol=1e-3)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(1, 1), (4, 4), (5, 5), (2, 7, 7)] for dtype in float_types
  ...
  def testEig(self, shape, dtype, rng):
    ...
    self._CompileAndCheck(partial(np.linalg.eig), args_maker, check_dtypes=True, rtol=1e-3)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(4, 4), (5, 5), (50, 50)] for dtype in float_types + complex_types
                          for rng in [jtu.rand_default()]))
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(1, 1), (4, 4), (5, 5)] for dtype in float_types + complex_types
                          for rng in [jtu.rand_default()]))
  @jtu.skip_on_devices("gpu", "tpu")
  def testEigBatching(self, shape, dtype, rng):
    ...
    self.assertTrue(onp.all(onp.linalg.norm(onp.matmul(args, vs) - ws[..., None, :] * vs) < 1e-3))
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "lower": lower,
          "rng": rng
      } for n in [0, 4, 5, 50] for dtype in float_types + complex_types for lower in [False, True]
                          for rng in [jtu.rand_default()]))
  def testEigh(self, n, dtype, lower, rng):
    ...
    a, = args_maker()
    a = (a + onp.conj(a.T)) / 2
    w, v = np.linalg.eigh(onp.tril(a) if lower else onp.triu(a), UPLO=uplo, symmetrize_input=False)
    self.assertTrue(norm(onp.eye(n) - onp.matmul(onp.conj(T(v)), v)) < 5)
    self.assertTrue(norm(onp.matmul(a, v) - w * v) < tol)

    self._CompileAndCheck(
        partial(np.linalg.eigh, UPLO=uplo), args_maker, check_dtypes=True, rtol=1e-3)
  ...
  def testEigvalsh(self, shape, dtype, rng):
    ...
    self._CheckAgainstNumpy(onp.linalg.eigvalsh, np.linalg.eigvalsh, args_maker, check_dtypes=True,
                            tol=1e-3)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "lower": lower,
          "eps": eps
      } for shape in [(1, 1), (4, 4), (5, 5), (50, 50)] for dtype in complex_types
                          for rng in [jtu.rand_default()] for lower in [True, False]
                          for eps in [1e-4]))
  ...
  def testEighGradVectorComplex(self, shape, dtype, rng, lower, eps):
    ...
    RTOL = 1e-2
    assert onp.max(
        onp.abs((onp.diag(onp.dot(onp.conj(
            (v + dv).T), onp.dot(new_a, (v + dv)))) - new_w) / new_w)) < RTOL
  ...
  def testEighBatching(self, shape, dtype, rng):
    _skip_if_unsupported_type(dtype)
    if (jtu.device_under_test() == "tpu" and np.issubdtype(dtype, onp.complexfloating)):
      raise unittest.SkipTest("No complex eigh on TPU")
    shape = (10,) + shape
    args = rng(shape, dtype)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "ord": ord,
          "rng": rng
      } for axis, shape in [(None, (1,)), (None, (7,)), (None, (5, 8)), (0, (9,)), (0, (
          4, 5)), ((1,), (10, 7,
                          3)), ((-2,),
                                (4, 8)), (-1, (6, 3)), ((0, 2),
                                                        (3, 4, 5)), ((2, 0),
                          ...
                          for ord in ([None] if axis is None and len(shape) > 2 else
                                      [None, 0, 1, 2, 3, -1, -2, -3, np.inf, -np.inf] if
                                      (axis is None and len(shape) == 1) or isinstance(axis, int) or
  ...
  def testNorm(self, shape, dtype, ord, axis, keepdims, rng):
    _skip_if_unsupported_type(dtype)
    if (ord in ('nuc', 2, -2)
        and (jtu.device_under_test() != "cpu" or (isinstance(axis, tuple) and len(axis) == 2))):
    ...
    self._CheckAgainstNumpy(onp_fn, np_fn, args_maker, check_dtypes=check_dtypes, tol=1e-3)
    self._CompileAndCheck(np_fn, args_maker, check_dtypes=check_dtypes)

  @parameterized.named_parameters(
      jtu.cases_from_list({
          "testcase_name": "_n={}_full_matrices={}_compute_uv={}".format(
              jtu.format_shape_dtype_string(b + (m, n), dtype), full_matrices, compute_uv),
          "b": b,
          "m": m,
          "n": n,
          "dtype": dtype,
          ...
          "rng": rng
      } for b in [(), (3,), (2, 3)] for m in [2, 7, 29, 53] for n in [2, 7, 29, 53]
                          for dtype in float_types + complex_types
                          for full_matrices in [False, True] for compute_uv in [False, True]
                          for rng in [jtu.rand_default()]))
  @jtu.skip_on_devices("tpu")
  def testSVD(self, b, m, n, dtype, full_matrices, compute_uv, rng):
    ...
    if compute_uv:
      ...
      if full_matrices:
        ...
        if m < n:
          self.assertTrue(
              onp.all(norm(a - onp.matmul(out[1][..., None, :] * out[0], out[2][..., :k, :])) < 50))
        else:
          self.assertTrue(
              onp.all(
                  norm(a - onp.matmul(out[1][..., None, :] * out[0][..., :, :k], out[2])) < 350))
      else:
        self.assertTrue(onp.all(norm(a - onp.matmul(out[1][..., None, :] * out[0], out[2])) < 300))

      # Check the unitary properties of the singular vector matrices.
      self.assertTrue(
          onp.all(norm(onp.eye(out[0].shape[-1]) - onp.matmul(onp.conj(T(out[0])), out[0])) < 10))
      if m >= n:
        self.assertTrue(
            onp.all(norm(onp.eye(out[2].shape[-1]) - onp.matmul(onp.conj(T(out[2])), out[2])) < 10))
      else:
        self.assertTrue(
            onp.all(norm(onp.eye(out[2].shape[-2]) - onp.matmul(out[2], onp.conj(T(out[2])))) < 20))

    else:
      self.assertTrue(
          onp.allclose(onp.linalg.svd(a, compute_uv=False), onp.asarray(out), atol=1e-4, rtol=1e-4))

    self._CompileAndCheck(
        partial(np.linalg.svd, full_matrices=full_matrices, compute_uv=compute_uv), args_maker,
        check_dtypes=True)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "rng": rng
                          ...
                          for dtype in float_types + complex_types
                          for full_matrices in [False, True] for rng in [jtu.rand_default()]))
  def testQr(self, shape, dtype, full_matrices, rng):
    _skip_if_unsupported_type(dtype)
    if (np.issubdtype(dtype, onp.complexfloating)
        and (jtu.device_under_test() == "tpu" or jax.lib.version <= (0, 1, 27))):
    ...
    self.assertTrue(onp.all(norm(onp.eye(k) - onp.matmul(onp.conj(T(lq)), lq)) < 5))
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(10, 4, 5), (5, 3, 3), (7, 6, 4)] for dtype in float_types + complex_types
                          for rng in [jtu.rand_default()]))
  ...
  def testSolve(self, lhs_shape, rhs_shape, dtype, rng):
    ...
    self._CheckAgainstNumpy(onp.linalg.solve, np.linalg.solve, args_maker, check_dtypes=True,
                            tol=1e-3)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(1, 1), (4, 4), (2, 5, 5), (200, 200), (5, 5, 5)] for dtype in float_types
                          for rng in [jtu.rand_default()]))
  def testInv(self, shape, dtype, rng):
    ...
    self._CheckAgainstNumpy(onp.linalg.inv, np.linalg.inv, args_maker, check_dtypes=True, tol=1e-3)
  ...
  def testIssue1151(self):
    ...
    self.assertAllClose(vmap(np.dot)(A, x), b, atol=1e-3, rtol=1e-3, check_dtypes=True)
...
class ScipyLinalgTest(jtu.JaxTestCase):
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(1, 1), (4, 5), (10, 5), (50, 50)] for dtype in float_types + complex_types
                          for rng in [jtu.rand_default()]))
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          ...
          "dtype": dtype,
          "rng": rng
      } for shape in [(4, 5), (6, 5)] for dtype in [np.float32] for rng in [jtu.rand_default()]))
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          "testcase_name": "_lhs={}_rhs={}_trans={}".format(
              jtu.format_shape_dtype_string(lhs_shape, dtype),
  ...
  def testLuSolve(self, lhs_shape, rhs_shape, dtype, trans, rng):
    ...
    self._CheckAgainstNumpy(osp_fun, jsp_fun, args_maker, check_dtypes=True, tol=1e-3)
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          "testcase_name": "_lhs={}_rhs={}_sym_pos={}_lower={}".format(
              jtu.format_shape_dtype_string(lhs_shape, dtype),
              jtu.format_shape_dtype_string(rhs_shape, dtype), sym_pos, lower),
          "lhs_shape": lhs_shape,
          "rhs_shape": rhs_shape,
          "dtype": dtype,
          "sym_pos": sym_pos,
  ...
  def testSolve(self, lhs_shape, rhs_shape, dtype, sym_pos, lower, rng):
    _skip_if_unsupported_type(dtype)
    if (sym_pos and np.issubdtype(dtype, onp.complexfloating) and jtu.device_under_test() == "tpu"):
      raise unittest.SkipTest("Complex Cholesky decomposition not implemented on TPU")
  ...
  @parameterized.named_parameters(
      jtu.cases_from_list({
          "testcase_name": "_lhs={}_rhs={}_lower={}_transposea={}_unit_diagonal={}".format(
              jtu.format_shape_dtype_string(lhs_shape, dtype),
              jtu.format_shape_dtype_string(rhs_shape, dtype), lower, transpose_a, unit_diagonal),
          "lower": lower,
          ...
          "rng": rng
                          ...
                          for unit_diagonal in [False, True] for lhs_shape, rhs_shape in [
                              ((4, 4), (4,)),
                              ((4, 4), (4, 3)),
                              ((2, 8, 8), (2, 8, 10)),
                          ] for dtype in float_types for rng in [jtu.rand_default()]))
  def testSolveTriangular(self, lower, transpose_a, unit_diagonal, lhs_shape, rhs_shape, dtype,
                          rng):
    _skip_if_unsupported_type(dtype)
      ...
      jtu.cases_from_list(
          {
              "testcase_name":
                  "_A={}_B={}_lower={}_transposea={}_conja={}_unitdiag={}_leftside={}".format(
                      ...
                      jtu.format_shape_dtype_string(b_shape, dtype), lower, transpose_a,
                      conjugate_a, unit_diagonal, left_side),
              "lower": lower,
              "transpose_a": transpose_a,
          ...
          for dtype in float_types + complex_types for transpose_a in [False, True]
          for conjugate_a in ([False] if np.issubdtype(dtype, np.floating) else [False, True])
  ...
  def testTriangularSolveGrad(self, lower, transpose_a, conjugate_a, unit_diagonal, left_side,
                              a_shape, b_shape, dtype, rng):
    _skip_if_unsupported_type(dtype)
    ...
    A = A if lower else T(A)
    B = rng(b_shape, dtype)
    f = partial(lax_linalg.triangular_solve, lower=lower, transpose_a=transpose_a,
                conjugate_a=conjugate_a, unit_diagonal=unit_diagonal, left_side=left_side)
