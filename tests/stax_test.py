...
class StaxTest(jtu.JaxTestCase):
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_channels={}_filter_shape={}_padding={}_strides={}_input_shape={}".format(
                    channels, filter_shape, padding, strides, input_shape
                ),
                "channels": channels,
    ...
    def testConvShape(self, channels, filter_shape, padding, strides, input_shape):
        init_fun, apply_fun = stax.Conv(
            channels, filter_shape, strides=strides, padding=padding
        )
    ...
    def testConvTransposeShape(
        self, channels, filter_shape, padding, strides, input_shape
    ):
        init_fun, apply_fun = stax.ConvTranspose(
            channels, filter_shape, strides=strides, padding=padding  # 2D
        )
        _CheckShapeAgreement(self, init_fun, apply_fun, input_shape)
    ...
    def testConv1DTransposeShape(
        self, channels, filter_shape, padding, strides, input_shape
    ):
        init_fun, apply_fun = stax.Conv1DTranspose(
            channels, filter_shape, strides=strides, padding=padding
        )
        _CheckShapeAgreement(self, init_fun, apply_fun, input_shape)
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_out_dim={}_input_shape={}".format(
                    out_dim, input_shape
                ),
                "out_dim": out_dim,
                "input_shape": input_shape,
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_input_shape={}_nonlinear={}".format(
                    input_shape, nonlinear
                ),
                "input_shape": input_shape,
                "nonlinear": nonlinear,
            }
        ...
        jtu.cases_from_list(
            {
                ...
                "_maxpool={}".format(
                    window_shape, padding, strides, input_shape, max_pool
                ),
    ...
    def testIssue182(self):
        ...
        input_shape = (10, 3)
        inputs = onp.arange(30.0).astype("float32").reshape(input_shape)

        out_shape, params = init_fun(key, input_shape)
        out = apply_fun(params, inputs)

        assert out_shape == out.shape
        assert onp.allclose(onp.sum(onp.asarray(out), -1), 1.0)
