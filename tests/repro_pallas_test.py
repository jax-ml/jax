...
class ReproTest(rtu.ReproTestBase):
    ...
    def matmul_block_spec(x, y, *, bm, bn, bk, interpret, debug=False):
      ...
      def matmul_kernel(x_ref, y_ref, o_ref):
        ...
        def body(i, acc):
          ...
          return acc + jnp.dot(x_block, y_block)
  ...
  @jtu.ignore_warning(category=DeprecationWarning,
                      message=".*core_map is deprecated.*")
  ...
  def test_kernel_multiple_bodies(self):
    ...
    def g():
      ...
      def scalar_subcore_fn(_, tec_sem):
        for i in range(v_mesh.num_subcores):
          pl.semaphore_signal(
              tec_sem,
              device_id={"s_core": jax.lax.axis_index("s_core"), "subcore": i},
          )
  ...
  def test_pallas_async_remote_copy(self, mesh):
    ...
    def body(x_ref, idx_ref, o_ref):
      ...
      def g(x, idx):
        compiler_params = pltpu.CompilerParams()
        pallas_mesh = pltpu.TensorCoreMesh(axis_name="x", num_cores=2)
  ...
  def test_pallas_gpu_interpret_params_emitter(self):
    from jax._src.pallas.mosaic_gpu.interpret import params as plgpu_interpret_params
    from jax._src.repro import emitter

    ip = plgpu_interpret_params.InterpretGPUParams(detect_races=True, num_tma_threads_per_device=2)

    tracker.lazy_init()
    global_ctx = emitter.EmitGlobalContext()
    func_ctx = emitter.EmitFunctionDefContext("test", global_ctx, None)

    func_ctx.traverse_value(ip)
    text = "\n".join(func_ctx.emitted_function.lines)
    self.assertIn("plgpu_interpret_params.InterpretGPUParams(detect_races=True, num_tma_threads_per_device=2)", text)

  def test_pallas_gpu_interpret_params(self):
    import jax.experimental.pallas.mosaic_gpu as plgpu
    from jax._src.pallas.mosaic_gpu.interpret import params as plgpu_interpret_params
    ...
    def g(x):
      return plgpu.kernel(
          ...
          grid=(1,),
          grid_names=("sm",),
          interpret=plgpu_interpret_params.InterpretGPUParams(detect_races=True),
      )(x)
    ...
    with tracker.flags_override(fake_array_threshold=x.size + 1):
      self.collect_and_check(f, x, skip_repro_eval=True)
  ...
  def test_pallas_gpu_wgmma_transpose(self, *, interpret: bool):
    ...
    def kernel(x_gmem, y_gmem, o_gmem):
      def acc_scope(acc_ref):
        def pipeline_body(step, x_smem, y_smem):
          plgpu.wgmma(acc_ref, x_smem.transpose((1, 0)), y_smem)
          plgpu.wgmma_wait(0)
