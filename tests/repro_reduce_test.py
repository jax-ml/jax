...
class DDTest(jtu.JaxTestCase):
  ...
  def setUp(self):
    self.enter_context(tracker.flags_override(error_mode="raise"))
    super().setUp()
...
def main_repro_func():
  @jax.jit
  def f(x):
    return x + jnp.concatenate([x, x])  # This triggers "got incompatible shapes"
  f(np.array([1, 2, 3]))

main_repro = dict(
    func=main_repro_func,
    uncaught_exception_type_str="ValueError")
"""
    src_no_repro_no_uncaught_exception = """
main_repro = dict(
    func=lambda: 0,
    uncaught_exception_type_str=None)
"""
    src_no_repro_success = """
main_repro = dict(
    func=lambda: 0,
    uncaught_exception_type_str="NotImplementedError")
"""
    src_invalid_repro_other_error = """
def main_repro_func():
...
main_repro = dict(
    ...
    uncaught_exception_type_str="NotImplementedError")
"""
    def validate_repro_fun(repro_fun):
      try:
        repro_fun()
        return "No error"
      except TypeError as e:
        str_e = str(e)
        if "add got incompatible shapes" in str_e:
          return ""
        else:
          return str_e

    # Test successful repro
    res = reducer.Repro.check_repro(src_valid_repro, pathlib.Path("<test>"),
                                    validate_repro_fun,
    ...
    with self.assertRaisesRegex(ValueError, "Not a reproducer"):
      reducer.Repro.check_repro(src_no_repro_no_uncaught_exception,
                                pathlib.Path("<test>"),
                                validate_repro_fun,
                                reducer.DropFunctionCallsStrategy)
    ...
    with self.assertRaisesRegex(ValueError, "Not a reproducer"):
      reducer.Repro.check_repro(src_no_repro_success, pathlib.Path("<test>"),
                                validate_repro_fun,
                                reducer.DropFunctionCallsStrategy)

    # Test failed repro with raise_on_failure=False
    msg = reducer.Repro.check_repro(src_no_repro_success, pathlib.Path("<test>"),
                                    validate_repro_fun,
                                    reducer.DropFunctionCallsStrategy,
    ...
    with self.assertRaisesRegex(ValueError, "Not a reproducer, it raises: unexpected error"):
      reducer.Repro.check_repro(src_invalid_repro_other_error, pathlib.Path("<test>"),
                                validate_repro_fun,
                                reducer.DropFunctionCallsStrategy)

  @jtu.thread_unsafe_test()
  def test_check_repro_with_generic_validator(self):
    src_valid_repro = """
import jax
import numpy as np
from jax import numpy as jnp
...
"""
    ...
    validate_repro_fun = reducer.load_validator("expected_exception")

    validator_module = importlib.import_module(
      "jax._src.repro.validators.expected_exception")

    original_exception = validator_module.expected_exception
    original_re = validator_module.expected_re

    try:
      validator_module.expected_exception = TypeError
      validator_module.expected_re = re.compile(r'add got incompatible shapes')
      ...
      validator_module.expected_exception = original_exception
      validator_module.expected_re = original_re
...
main_repro = dict(func=main_repro_func)
"""
    col = emitter.collector(repro.load(src, pathlib.Path("<here>")).run)
    col()
  ...
  def test_reduce_drop_calls(self):
    src = """
import jax
from jax import numpy as jnp

def fun_main_repro_func_0():
  def g():
    return 5.
...
"""
    def validate_repro_fun(repro_fun):
      try:
        ...
        if "add got incompatible shapes for broadcasting" in str(e):
          return ""
        else:
          return str(e)
      return ""
    ...
    r = reducer.Repro.make(src, pathlib.Path("<here>"),
                           validate_repro_fun=validate_repro_fun,
                           strategy_factory=reducer.DropFunctionCallsStrategy)
...
"""
    ...
    true_branches = [(c, *rest)
                     ...
                     if tracker.func_api_name(c.func) == "cond" and rest[0] == 0]
    ...
    false_branches = [(c, *rest)
                      for c, *rest in collect_inlining.all_candidates
                      if tracker.func_api_name(c.func) == "cond" and rest[0] == 1]
...
"""
    ...
    self.assertIn("jit_call(fun_my_f", source1)
    jit_my_f = [(c, *rest)
                 for c, *rest in collect_inlining.all_candidates
                 if tracker.func_api_name(c.func) == "jit_call"
    ...
    self.assertNotIn("jit_call(fun_my_f", source4)
...
"""
    ...
    self.assertIn("checkpoint_call(fun_my_f", source1)
    checkpoint_my_f = [(c, *rest)
                 for c, *rest in collect_inlining.all_candidates
                 if tracker.func_api_name(c.func) == "checkpoint_call"
                    and c.args[0].fun_name.startswith("my_f")]
    ...
    self.assertNotIn("checkpoint_call(fun_my_f", source2)
...
"""
      ...
      _ = col.to_source(strategy=collect_reductions)
      args_to_jit_call_my_f = [
         c
         for c in collect_reductions.all_candidates
         if (tracker.func_api_name(c[0].func) == "jit_call" and
             "my_f" in c[0].args[0].fun_name and
             c[1]  # for_args
             )]
      # 7 = jit_call(fun_my_f_3, None, {}, v_0, v_1, {'x': v_0, 'y': v_1})
