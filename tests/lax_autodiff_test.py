...
class LaxAutodiffTest(jtu.JaxTestCase):
  ...
  def testAsinhGradLargeValues(self):
    # float16: x^2 overflows for x > ~256
    x16 = jnp.float16(300.0)
    g16 = jax.grad(lax.asinh)(x16)
    _, t16 = jax.jvp(lax.asinh, (x16,), (jnp.ones_like(x16),))
    self.assertAllClose(g16, np.float16(1.0 / 300.0), rtol=1e-2)
    self.assertAllClose(t16, np.float16(1.0 / 300.0), rtol=1e-2)

    # float32: x^2 overflows for x > ~1.84e19
    x32 = jnp.float32(1e25)
    g32 = jax.grad(lax.asinh)(x32)
    _, t32 = jax.jvp(lax.asinh, (x32,), (jnp.ones_like(x32),))
    self.assertAllClose(g32, jnp.float32(1e-25), rtol=1e-5)
    self.assertAllClose(t32, jnp.float32(1e-25), rtol=1e-5)

  @jtu.skip_on_devices("tpu")
  def testAsinhGradLargeValuesFloat64(self):
    # float64: x^2 overflows for x > ~1.34e154
    with jax.enable_x64():
      x64 = jnp.float64(1e200)
      g64 = jax.grad(lax.asinh)(x64)
      _, t64 = jax.jvp(lax.asinh, (x64,), (jnp.ones_like(x64),))
      self.assertAllClose(g64, jnp.float64(1e-200), rtol=1e-5)
      self.assertAllClose(t64, jnp.float64(1e-200), rtol=1e-5)

  def testAsinhDerivatives(self):
    # 1. Near x = 0: standard value
    x_near0 = jnp.float32(0.0001)
    true_val = np.float32(1.0 / np.sqrt(np.float64(x_near0) ** 2 + 1.0))
    self.assertAllClose(jax.grad(lax.asinh)(x_near0), true_val, rtol=1e-6)

    # 2. Complex plane with negative real part: same factorization as acosh
    z = jnp.complex64(-2.0 + 1.0j)
    expected_asinh = 1.0 / (np.sqrt(complex(z) + 1.0j) * np.sqrt(complex(z) - 1.0j))
    _, t_asinh = jax.jvp(lax.asinh, (z,), (jnp.complex64(1.0),))
    self.assertAllClose(t_asinh, expected_asinh, rtol=1e-5, check_dtypes=False)
    ...
    # 4. Second derivative
    d2 = jax.grad(jax.grad(lax.asinh))(jnp.float32(1.0))
    self.assertAllClose(d2, jnp.float32(-1.0 / 2.0 ** 1.5), rtol=1e-5)
