...
class LapaxTest(jtu.JaxTestCase):
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_lhs={}_rhs={}_lower={}_leftside={}_transposea={}".format(
            ...
            for lhs_shape, rhs_shape in [((2, 4, 4), (2, 4, 6) if left else (2, 6, 4))]
            for dtype in float_types
            for rng in [jtu.rand_default()]
        )
    )
    def testSolveTriangular(
        self, lower, left_side, transpose_a, lhs_shape, rhs_shape, dtype, rng
    ):
    ...
    def testSolveTriangularBlocked(
        self, lower, left_side, transpose_a, lhs_shape, rhs_shape, dtype, rng
    ):
        # pylint: disable=invalid-name
        T = lambda X: onp.swapaxes(X, -1, -2)
