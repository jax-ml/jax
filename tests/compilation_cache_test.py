...
class CompilationCacheTest(CompilationCacheTestCase):
  ...
  def test_diff_executables(self):
    ...
    backend = xla_bridge.get_backend()
    execution_devices = xc.DeviceList(tuple(backend.local_devices()))
    if jax._src.lib.jaxlib_extension_version < 331:
      ...
      executable1 = backend.compile(
          computation1, execution_devices, compile_options)
      executable2 = backend.compile(
          computation2, execution_devices, compile_options)
    ...
    self.assertNotEqual(
        cc.get_executable_and_time(
            "key1", compile_options, backend, execution_devices)[0],
        cc.get_executable_and_time(
            "key2", compile_options, backend, execution_devices)[0]
    )
