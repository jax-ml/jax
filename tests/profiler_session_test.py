...
class ProfilerSessionTest(jtu.JaxTestCase):
  ...
  def setUp(self):
    super().setUp()
    # Ensure any lingering trace from previous tests is stopped.
    try:
      jax.profiler.stop_trace()
    except RuntimeError:
      pass

  def test_programmatic_profiling_with_custom_session_id(self):
  ...
  def test_programmatic_profiling(self, session_id: Optional[str]):
    tmpdir = pathlib.Path(self.create_tempdir().full_path)
