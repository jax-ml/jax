...
class IndexedUpdateTest(jtu.JaxTestCase):
  ...
  def testWrapNegativeIndices1D(self, op, mode, wrap_negative_indices, shape, update_shape):
    rng = jtu.rand_default(self.rng())
    idx_rng = jtu.rand_unique_int(self.rng(), high=shape[0] + 2)

    def args_maker():
      data = rng(shape, 'float32').round(1)
      update = rng(update_shape, 'float32').round(1)
      # we need indices to be unique, so we generate unique values in [0, N + 2]
      # and then subtract N from half of them
