...
class StringArrayTest(jtu.JaxTestCase):
  ...
  def setUp(self):
    super().setUp()
    self.host_devices = jax.devices("cpu")
  ...
  def test_single_device_array(self, asarray):
    ...
    if asarray:
      jax_string_array = jnp.asarray(
          numpy_string_array, device=self.host_devices[0]
      )
    else:
  ...
  def test_multi_device_array(self, asarray):
    if len(self.host_devices) < 2:
      self.skipTest(
          f"Skipping this test because only {len(self.host_devices)} host"
    ...
    sharding = jax.sharding.PositionalSharding(self.host_devices).reshape(2,1)
    if asarray:
