...
class StringArrayTest(jtu.JaxTestCase):
  ...
  def test_single_device_array(self, asarray):
    ...
    if asarray:
      jax_string_array = jnp.asarray(
          numpy_string_array, device=jax.devices("cpu")[0]
      )
    else:
  ...
  def test_multi_device_array(self, asarray):
    ...
    if xla_extension_version < 304:
      self.skipTest(
          "Skipping this test because the current XLA extenstion version:"
          f" {xla_extension_version} is older than 304, the oldest version with"
          " string array support."
      )
    if not hasattr(np_dtypes, "StringDType"):
  ...
  def test_string_array_cannot_be_non_cpu_devices(self, asarray):
    ...
    with self.assertRaisesRegex(
        TypeError, "StringDType arrays can only be sharded to CPU devices"
    ):
      if asarray:
  ...
  def test_jit_fails_with_string_arrays(self):
    ...
    self.assertRaisesRegex(
        TypeError,
        "StringDType arrays are not supported by jit",
  ...
  def test_vmap_fails_with_string_arrays(self):
    if xla_extension_version < 304:
      self.skipTest(
          "Skipping this test because the current XLA extension version:"
          f" {xla_extension_version} is older than 304, the oldest version with"
    ...
    def f(x):
      logging.error("2DO Something happened %s", traceback.extract_stack())
      return x

    g = jax.vmap(f)

    numpy_string_array = np.array(
        ["one", "two", "three", "four"], dtype=np_dtypes.StringDType()  # type: ignore
    ).reshape(2, 2)
    input_array = jax.device_put(
        numpy_string_array, device=jax.devices("cpu")[0]
    )
    self.assertRaisesRegex(
        TypeError,
        "StringDType arrays are not supported for vmap",
        lambda: g(input_array),
    )
