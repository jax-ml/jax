...
class CheckifyTransformTests(jtu.JaxTestCase):
  ...
  def test_image_resize_nan(self):
    resize = partial(jax.image.resize, shape=(1,), method="linear")
    err, _ = checkify.checkify(resize, errors=checkify.nan_checks)(
        jnp.array([jnp.inf, -jnp.inf]))
    self.assertEqual(err.get(), "nan generated by primitive: image_resize.")

    resize = partial(jax.image.resize, shape=(4,), method="nearest")
    err, _ = checkify.checkify(resize, errors=checkify.nan_checks)(
        jnp.array([jnp.nan, 1.]))
    self.assertIsNone(err.get())
