...
class DebugTest(jtu.JaxTestCase):
  def setUp(self):
    super().setUp()
    self.prev = config.jax_debug_mode
    config.update("jax_debug_mode", True)

  def tearDown(self):
    config.update("jax_debug_mode", self.prev)
    super().tearDown()

  def test_debug_mode_cache_miss(self):
    @jax.jit
    def f(x):
  ...
  def test_debug_mode_no_jit(self):
    def f(x):
      return jnp.divide(x, x)
