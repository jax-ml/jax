...
class CoreTest(jtu.JaxTestCase):
  ...
  def test_tracer_reprs(self):
    ...
    # By default, with dev_mode=False, we get concise string representations:
    ...
    # If we set dev_mode=True, we get more verbose string representations:
    with config.dev_mode(True):
      jax.jit(f)(jnp.arange(10.0, dtype='float32'))
