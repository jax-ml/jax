...
class ThreadGuardTest(jt_multiprocess.MultiProcessTest):
  ...
  def test_thread_guard_raises_slow_path_jit(self):
  ...
  def test_thread_guard_raises_fast_path_jit(self):
    x = _make_multiprocess_array()
    x = f(x)
    x = f(x)
    with self.assertRaisesRegex(
  ...
  def test_local_devices_only_no_error(self):
    mesh = jax.make_mesh(
  ...
  def test_nested_thread_guards(self):
    x = _make_multiprocess_array()
    with config.thread_guard(True):
      y = f(x)
      with config.thread_guard(True):
  ...
  def test_guard_in_subthread(self):
    x = _make_multiprocess_array()
  ...
  def test_guard_in_subthread_with_nested_guard_raises(self):
    x = _make_multiprocess_array()
