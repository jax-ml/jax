...
class MosaicGpuDialectTCGen05Test(TestCase, jtu.JaxTestCase):
  ...
  @parameterized.parameters(jnp.int32, jnp.int8)
  def test_tma_gather(self, index_dtype):
    ...
    def body(ctx, src, indices, dst, smem):
      ...
      # Compute
      val = mgpu_dialect.vector_load(smem_ref)
      one = arith.constant(ir.BF16Type.get(), 1.0)
      one_vec = vector.broadcast(val.type, one)
      res = arith.addf(val, one_vec)
      mgpu_dialect.vector_store(res, smem_ref)
    ...
    # Verification
    gathered = src[indices.astype(jnp.int32)]
    computed = gathered + 1
    np.testing.assert_allclose(result, computed, atol=1e-5, rtol=1e-5)
