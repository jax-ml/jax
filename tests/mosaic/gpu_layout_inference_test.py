...
class LayoutInferenceTest(parameterized.TestCase):
  ...
  def test_custom_primitive_op_with_conflicting_transforms_is_unsat(self):
    with ir.InsertionPoint(self.module.body):
      ...
      [ref] = undefs(ref_ty)
      mgpu.dialect.with_transforms(ref, transforms_a)
      mgpu.dialect.with_transforms(ref, transforms_b)
  ...
  def test_infer_transforms_for_memref_transpose(self, annotate_input):
    ...
    with ir.InsertionPoint(self.module.body):
      ...
      permutaiton = ir.AffineMap.get_permutation((1, 0))
      transpose_op = memref.TransposeOp(out_ref_ty, in_ref, permutaiton)
