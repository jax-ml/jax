...
class LayoutInferenceTest(parameterized.TestCase):
  ...
  def test_conjure_smem_assignment_from_is_transferrable(self, transposed):
    ...
    self.assertEqual(conjured, [
            (var, eqns.SMEMTiling(lc.TileTransform(t(8, 32)))),
    ...
    conjured = conjure(constraints)
    self.assertEqual(conjured, [
            (var, eqns.SMEMTiling(lc.TileTransform((32, 8) if transposed else (8, 16)))),
            (var, eqns.SMEMTiling(None)),
        ]
    )

    # Yield also 1-tiling with Divides constraints with ir.Value.
    i32 = ir.IntegerType.get_signless(32)
    ...
    self.assertEqual(conjured, [
            (var, eqns.SMEMTiling(lc.TileTransform(t(8, 32)))),
            (var, eqns.SMEMTiling(lc.TileTransform((1, 8) if transposed else (1, 32)))),
            (var, eqns.SMEMTiling(None)),
        ]
    )

  def test_conjure_smem_assignment_from_transposed(self):
    shape = (128, 128)
    f32 = ir.F32Type.get()
    ref_ty = ir.MemRefType.get(shape, f32, memory_space=mgpu.utils.smem())
    [ref] = undefs(ref_ty)
    ...
    var = eqns.Variable(op_or_result)
    constraints = [eqns.Transposed(eqns.SMEMTiling(lc.TileTransform((8, 16))), var)]
    s = eqns.EquationSystem(constraints=constraints)
    conjured = list(layout_inference.conjure_assignment(set([var]), s, []))
    self.assertEqual(conjured, [
            (var, eqns.SMEMTiling(lc.TileTransform((16, 8)))),
            (var, eqns.SMEMTiling(None)),
        ]
    )
  ...
  @parameterized.product(
      op_type=(vector.LoadOp, vector.StoreOp),
      layout=(
          fa.WGMMA_LAYOUT,
          fa.WGMMA_ROW_LAYOUT,
          fa.WGMMA_COL_LAYOUT,
  ...
  def test_infer_transforms_for_vector_load_store_op(self, op_type, layout):
    shape = (64, 64)
    elt_ty = ir.BF16Type.get()
    ...
    with ir.InsertionPoint(self.module.body):
      ...
      if op_type == vector.LoadOp:
        ...
        [value_to_store] = undefs(ir.VectorType.get(shape, elt_ty))
        vector_op = vector.StoreOp(value_to_store, smem_ref, [zero] * len(shape))
    ...
    if op_type == vector.LoadOp and isinstance(layout, fa.WGSplatFragLayout):
  ...
  def test_infer_transforms_for_slice_smem_op_two_identical_consumers(self):
    shape = (64, 64)
    elt_ty = ir.BF16Type.get()
    ...
    with ir.InsertionPoint(self.module.body):
      ...
      mgpu.dialect.with_transforms(slice_smem_op.result, transforms)
      mgpu.dialect.with_transforms(slice_smem_op.result, transforms)
  ...
  def test_infer_transforms_for_slice_smem_op_two_mismatching_consumers(self):
    shape = (64, 64)
    elt_ty = ir.BF16Type.get()
    ...
    with ir.InsertionPoint(self.module.body):
      ...
      mgpu.dialect.with_transforms(slice_smem_op.result, transforms1)
      mgpu.dialect.with_transforms(slice_smem_op.result, transforms2)
