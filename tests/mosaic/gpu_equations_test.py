...
class EquationSystemTest(parameterized.TestCase):
  ...
  def test_equation_system_known_assignmentss_can_not_be_unknowns(self):
    v0 = V(0)
    layout = C(mgpu.WGSplatFragLayout((1, 1)))
    system = equations.EquationSystem(assignments={v0: layout}, unknowns={v0})
    self.assertEmpty(system.unknowns)

  def test_union_of_conflicting_systems_is_unsatisfiable(self):
    v0 = V(0)
    layout0, layout1 = [C(mgpu.WGSplatFragLayout((1, i))) for i in (1, 2)]
  ...
  def test_union_of_compatible_systems_is_union_of_fields(self):
    v0, v1, v2 = V(0), V(1), V(2)
    layout0, layout1, layout2 = [C(mgpu.WGSplatFragLayout((1, i))) for i in (1, 2, 3)]
    system0 = equations.EquationSystem(unknowns={v0}, equations={Eq(v0, layout0)})
