...
class ShardedJitTest(jtu.JaxTestCase):
  ...
  def testInfeed(self, partition_input):
    ...
    # Transfer data to infeed first before executing the function. For some
    # devices, the infeed is blocking, so transferring data to infeed before
    # executing ensures that the execution does not deadlock waiting for
    # the infeed data.
    result = f(x)
