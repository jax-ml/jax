...
class PallasCallTest(PallasTest):
  ...
  def test_swiglu(self):
    ...
    wgmma_config = plgpu.WGMMAConfig(jnp.float16, swizzle=min(128, k))
    lhs_smem = wgmma_config.lhs_smem_config((64, k))
    gate_smem = rhs_smem = wgmma_config.rhs_smem_config((k, 128))
    out_smem = wgmma_config.out_smem_config((64, 128))

    def kernel(a_ref, gate_ref, b_ref, o_ref):
      def body(acc0, acc1):
        plgpu_primitives.wgmma(acc0, a_ref, b_ref)
        plgpu_primitives.wgmma(acc1, a_ref, gate_ref)
        plgpu_primitives.wgmma_wait(0)
        return jax.nn.swish(acc0[...]) * acc1[...]

      # Create a mutable and scoped accumulator for wgmma.
      acc_cfg = wgmma_config.accumulator_config(64, 128)
      acc_arr = pl.run_scoped(body, acc_cfg, acc_cfg)
      o_ref[...] = acc_arr

    a = np.ones((64, k), jnp.float16)
    gate = np.ones((k, 128), jnp.float16)
    b = np.ones((k, 128), jnp.float16)
    _blockspec = lambda smem_config: pl.BlockSpec(smem_config.shape, None, memory_space=smem_config.memory_space)
    res = pl.pallas_call(
        kernel,
        in_specs=[_blockspec(lhs_smem), _blockspec(gate_smem), _blockspec(rhs_smem)],
        out_specs=_blockspec(out_smem),
        out_shape=jax.ShapeDtypeStruct((64, 128), jnp.float32),
    )(a, gate, b)
    np.testing.assert_allclose(res, jax.nn.swish(a @ b) * (a @ gate))
