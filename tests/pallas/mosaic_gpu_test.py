...
class PallasCallSm100ATest(PallasSm100ATest):
  ...
  @parameterized.product(m_n_k=[(256, 256, 256),
                                (256, 128, 128),
                                (256, 256, 64)],
                         swizzle=[128, 64, 32],
                         dtype=[jnp.float16, jnp.bfloat16],
  ...
  def test_simple_collective_matmul(self, m_n_k, swizzle, dtype,
                                    lhs_tmem=True):
    self.skip_if_wg_semantics()
    full_lhs_shape = (m_n_k[0], m_n_k[2])  # M, K
    full_rhs_shape = (m_n_k[2], m_n_k[1])  # K, N
    full_acc_shape = (m_n_k[0], m_n_k[1])  # M, N
    ...
    block_acc_shape = (full_lhs_shape[0] // 2, full_rhs_shape[1])
    block_lhs_shape = (full_lhs_shape[0] // 2, full_lhs_shape[1])
    block_rhs_shape = (full_rhs_shape[0], full_rhs_shape[1] // 2)
    ...
    if lhs_tmem:
      scratch_shapes.append(plgpu.TMEM(block_lhs_shape, dtype, collective=True,
                                       packed=True))
    else:
      scratch_shapes.append(None)
