...
class PallasCallTest(PallasTest):
  ...
  def test_wgmma_sliced(self, dtype):
    def kernel(a_ref, b_ref, o_ref):
      def scope(acc_ref):
    ...
    key1, key2 = jax.random.split(jax.random.key(42), 2)
    a = jax.random.uniform(key1, shape=(2, 64, 128), dtype=dtype)
    b = jax.random.uniform(key2, shape=(2, 128, 192), dtype=dtype)
    ...
    res = pl.pallas_call(
        ...
        out_specs=plgpu.GPUBlockSpec((64, 198), lambda *i: i),
        out_shape=jax.ShapeDtypeStruct((64, 198), jnp.float32),
    )(a, b)
