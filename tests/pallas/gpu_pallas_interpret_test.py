...
class InterpretTest(jtu.JaxTestCase):
  ...
  def test_tmem_ref_with_leading_batch_dimensions(self):
    # A TMEM ref of rank > 2 carries an implicit collapse of its leading
    # dimensions. Indexing is written against the ref's declared shape, so the
    # reshape does not move the accessed elements and can be ignored.
    @functools.partial(
        plgpu.kernel,
        out_type=jax.ShapeDtypeStruct((128, 64), jnp.float32),
  ...
  def test_copy_gmem_to_smem_out_of_bounds(self, oob_mode):
    # A TMA copy may name a window extending past the end of its source --
    # kernels rely on this to handle a trailing partial tile -- and `oob_mode`
    # says what the hardware supplies for the part that is out of bounds.
