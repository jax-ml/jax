...
class PallasCallPipelineTest(jtu.JaxTestCase):
  ...
  def test_prefetched_input(
    ...
    def kernel(x_hbm_ref, x_prefetched_vmem, o_hbm_ref):
          ...
          in_specs=[
              pl.BlockSpec(
                  ...
                  pipeline_mode=pl.Buffered(
                      buffer_count=buffer_count, use_lookahead=use_lookahead
                  ),
              )
          ],
      ...
      x_bref, o_bref = make_allocs(x_hbm_ref, o_hbm_ref)
      x_bref = dataclasses.replace(x_bref, window_ref=None)
      pl.run_scoped(
          lambda x_bref, o_bref: pipeline(
              x_hbm_ref,
              o_hbm_ref,
              allocations=(
                  x_bref.with_prefetched_ref(x_prefetched_vmem, 1),
                  o_bref,
              ),
          ),
          x_bref,
