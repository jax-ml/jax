...
class PallasCallCollectivePipelineTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    if jtu.is_device_tpu(7) and jax.device_count() < 4:
      # v7 consists of pairs of chips that share the same ICI connection,
      # so we need at least 4 chips to test collectives.
      self.skipTest('Only >=4 devices are supported on TPU v7.')
    if not jtu.is_device_tpu_at_least(5):
      self.skipTest('Only works with TPU v5')
