...
class SchedulersGoldenTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      ...
      (schedulers.static_rolled_scheduler,),
  )
  def test_2_async_stages(self, scheduler):
  ...
  def test_async_inputs_with_different_buffering(self, scheduler):
    ...
    output = stdout().strip().split("\n")
    expected = [
        # Note: The optimal schedule is to issue all itr=0 prefetches first,
        # then itr=1, and so on. But the current schedulers cannot do this
        # optimization.
        test_util.AnyOrder([
          "[itr=0] stage1a_start",
          "[itr=0] stage1b_start",
  ...
  def test_synchronous_3_stage(self, scheduler):
  ...
  def test_standard_emit_pipeline(self, scheduler):
  ...
  def test_pipelined_prefetch(self, scheduler):
