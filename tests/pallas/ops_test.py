...
class OpsTest(PallasBaseTest):
  ...
  def test_matrix_vector_like_dot_general(
    ...
    if jtu.test_device_matches(["tpu"]):
      if not jtu.if_cloud_tpu_at_least(2025, 7, 24):
        self.skipTest("Requires libtpu built after 2025-07-24")
      if not jtu.is_device_tpu_at_least(6) and rhs_is_vector:
        self.skipTest("Requires TPUv5+ for sublane gather")

    contracting_shape = (128,)
    lhs_shape = lhs_non_contracting_shape + contracting_shape
    rhs_shape = contracting_shape if rhs_is_vector else (1,) + contracting_shape
    batch_shape = (batch_size,) if batch_size is not None else ()
    batch_dim = [0] if batch_size else []
    lhs_shape = batch_shape + lhs_shape
    rhs_shape = batch_shape + rhs_shape
    k1, k2 = random.split(jax.random.key(0))
