...
class PallasCallPrintTest(PallasBaseTest):
  ...
  def test_debug_print_vector(self, shape, dtype):
    def has_incremental_sequence(s: str, length: int) -> bool:
      """Verify if the string contains incremental numbers from 0 to n-1.

      Args:
        s (str): Input string containing numbers.
        length (int): Expected number of incremental numbers, starting from 0.

      Returns:
        True if such a contiguous incremental sequence exists, False otherwise.
      """
    ...
    with jtu.capture_stderr() as get_output:
      jax.block_until_ready(compiled_kernel(x))
    output = get_output()
    # Check if the output matches the values generated by `arange`.
    self.assertTrue(has_incremental_sequence(output, n))
