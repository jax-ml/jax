...
from jax.interpreters.masking import Poly, Mon
...
class PolyTest(jtu.JaxTestCase):
  ...
  def test_parse_spec(self, spec, ans):
    self.assertEqual(str(masking.parse_spec(spec)), ans)
    self.assertEqual(str(masking.remap_ids(masking.UniqueIds(),
                                           masking.parse_spec(spec))),
                     ans)

  def test_poly_equal(self):
    self.assertEqual(constant_poly(4), constant_poly(4))
    self.assertEqual(Poly({Mon({}): 3, Mon({'n': 1}): 4}),
                     Poly({Mon({'n': 1}): 4, Mon({}): 3}))
    self.assertNotEqual(Poly({Mon({}): 3, Mon({'n': 1}): 4}),
                        Poly({Mon({}): 3, Mon({'n': 2}): 4}))
    self.assertNotEqual(Poly({Mon({}): 3, Mon({'m': 1}): 4}),
                        Poly({Mon({}): 3, Mon({'n': 1}): 4}))

  def test_poly_equal_integer(self):
    self.assertEqual(constant_poly(3), 3)
    self.assertEqual(3, constant_poly(3))
    self.assertEqual(np.array(3, np.int64), constant_poly(3))
    self.assertEqual(np.array(3, np.int64)[()], constant_poly(3))
    self.assertEqual(np.array(3, np.int64), constant_poly(3))
    self.assertNotEqual(constant_poly(4), 3)
    self.assertNotEqual(4, constant_poly(3))
    self.assertNotEqual(constant_poly(3), constant_poly(4))

  # TODO(mattjj): need these?
  # def test_poly_hash(self):
  #   self.assertLen(set(hash(Poly({Mon({}): i})) for i in range(3)), 3)
  #   self.assertEqual(hash(Poly({Mon({}): 3, Mon({'n': 1}): 4})),
  #                    hash(Poly({Mon({'n': 2}): 4, Mon({}): 3})))

  def test_mon_hash(self):
    self.assertLen(set(hash(Mon({'a': i})) for i in range(3)), 3)
    self.assertEqual(hash(Mon({'a': 1, 'b': 1})), hash(Mon({'b': 1, 'a': 1})))
...
class MaskingTest(jtu.JaxTestCase):
  ...
  def check(self, fun, in_shapes, out_shape, logical_env, padded_in_shapes,
    ...
    logical_out_shapes = [masking.eval_polymorphic_shape(logical_env, s)
                          for s in out_specs]
    ...
    logical_in_shapes = [masking.eval_polymorphic_shape(logical_env, s)
                         for s in in_specs]
  ...
  #   self.assertRaisesWithLiteralMatch(masking.ShapeError, message, thunk)
  ...
  #   message = "Masking rule for unsupported_op not implemented."
