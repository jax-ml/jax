  ...
  def testPmapConstantError(self):
    ...
    if config.pmap_shmap_merge.value:
      if jtu.device_under_test() == "cpu" or jax.device_count() > 1:
        expected_regex = r"Sharding.*implies.*partitioned.*but the dimension.*"
      else:
...
class CppPmapTest(PythonPmapTest):
  ...
  def setUp(self):
    super().setUp()
    if config.pmap_shmap_merge.value:
      raise SkipTest('Not testing cpp_pmap when `pmap_shmap_merge=True`.')
  ...
  def test_cache_hits_across_threads(self):
    ...
    # NOTE(dsuo): pmap cache is now global (i.e., either `_cached_shard_map` or
    # `jit`'s cache.), so there might not be an equivalent test.
    if config.pmap_shmap_merge.value:
      self.assertIn(f, shard_map._cached_shard_map.cache)
    else:
  ...
  def test_cache_uses_jax_key(self):
    ...
    if config.pmap_shmap_merge.value:
      ...
      self.assertEqual(pmapped_f._cache_size, 1)
...
class VmapPmapCollectivesTest(jtu.JaxTestCase):
  ...
  def testCollectivesWithVmap(self, collective):
    ...
    if config.pmap_shmap_merge.value:
      # NOTE(dsuo): Ignore nested pmap when `pmap_shmap_merge=True`.
  ...
  def testAllToAllMultipleAxesVsVmap(self, axes, split_axis, concat_axis):
    ...
    shape = (2, 2, 4, 4, 4)
    x = jnp.arange(math.prod(shape)).reshape(shape)
    self.assertAllClose(pmap(vmap(f, axis_name='j'), axis_name='i')(x),
...
class ArrayTest(jtu.JaxTestCase):
  ...
  def testThreadsafeIndexing(self):
    if config.pmap_shmap_merge.value:
      # NOTE(dsuo): This passes when not CPU, but fails for all platforms under
      # disable_jit=True.
