...
class PmapTest(jtu.JaxTestCase):
    ...
    def testTwoArgsGrad(self):
        def f(x, y):
            return lax.psum(5.0 * np.cos(x) * np.sin(y), "i")

        f = pmap(f, "i")

        def g(x, y):
            tot = np.sum(5.0 * np.cos(x) * np.sin(y))
    ...
    def testNestedWithClosure(self, device_mesh_shape):
        ...
        def test_fun(x):
            ...
            def g(z):
                return 3.0 * np.exp(np.sin(x).sum() * np.cos(y) * np.tan(z))
    ...
    def testReplicaGroups(self):
        ...
        self.assertEqual(
            (tuple(sorted(groups[0])),), ((0, 1, 2, 3, 4, 5, 6, 7),)
    ...
    def testDeviceCountError(self):
        ...
        self.assertRaisesRegexp(ValueError, ".*requires.*replicas", lambda: f(x))
    ...
    def testVmapOfPmapTuple(self):
        ...
        bx, by, bz = vmap(f1, in_axes=(1, 2, (None, 0)), out_axes=(1, 2, 0))(
            ax, ay, (az1, az2)
        )
    ...
    def testCompositionWithJitTwice(self):
        ...
        f(onp.arange(1.0).reshape((1, 1)))  # doesn't crash
        ...
        def multi_step_pmap(state, count):
            ...
            def time_evolution(state):
                return lax.fori_loop(
                    0, count, lambda i, s: exchange_and_multi_step(s), state
                )
...
class PmapWithDevicesTest(jtu.JaxTestCase):
    def testAllDevices(self):
        f = pmap(
            lambda x: x - lax.psum(x, "i"), axis_name="i", devices=xla_bridge.devices()
        )
        shape = (xla_bridge.device_count(), 4)
        x = onp.arange(prod(shape), dtype=onp.float32).reshape(shape)
        expected = x - onp.sum(x, 0)
    ...
    def testBadAxisSizeError(self):
        ...
        f = pmap(
            lambda x: lax.psum(x, "i"), axis_name="i", devices=xla_bridge.devices()
        )
        with self.assertRaisesRegex(
            ValueError,
    ...
    def testNestedPmapsError(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Nested pmaps with explicit devices argument."
        ):
