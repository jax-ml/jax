...
class SpecToIndicesTest(jtu.JaxTestCase):
  ...
  def testShardsPerAxis(self):
    shape = (4, 8)
    spec = pxla.ShardingSpec(sharding=map(pxla.Sharded, (2, 2)),
                             mesh_mapping=map(pxla.ShardedAxis, (0, 1)))
  ...
  def testUnshardedAxis(self):
    shape = (4, 8)
    spec = pxla.ShardingSpec(sharding=(pxla.Sharded(2), None),
  ...
  def testReplicationPosition2(self):
    shape = (2, 8)
    spec = pxla.ShardingSpec(sharding=(pxla.Unstacked(2), pxla.Sharded(2)),
  ...
  def testMultipleReplications(self):
    shape = (2, 7, 4)
    spec = pxla.ShardingSpec(sharding=(pxla.Unstacked(2), None, pxla.Sharded(2)),
  ...
  @parameterized.named_parameters(
      ...
      for shape, spec in [
          ...
          [(4, 8), pxla.ShardingSpec(sharding=(None, None),
                                     mesh_mapping=())],
          # partitioned, 1 axis
          [(4, 8), pxla.ShardingSpec(sharding=(pxla.Sharded(2), None),
                                     mesh_mapping=(pxla.ShardedAxis(0),))],
          # partitioned, 2 axes
          [(4, 8), pxla.ShardingSpec(sharding=(pxla.Sharded(2), pxla.Sharded(2)),
                                     mesh_mapping=map(pxla.ShardedAxis, (0, 1)))],
          # partitioned + sharding
          [(2, 8), pxla.ShardingSpec(sharding=(pxla.Unstacked(2), pxla.Sharded(2)),
                                     mesh_mapping=map(pxla.ShardedAxis, (0, 1)))],
          ...
          [(1, 8), pxla.ShardingSpec(sharding=(pxla.Unstacked(1), pxla.Sharded(2)),
