...
class LaxTest(jtu.JaxTestCase):
    ...
    def testConvWithGeneralPadding(
        self,
        lhs_shape,
        rhs_shape,
        dtype,
    ...
    @staticmethod
    def _conv_transpose_via_grad(
        data, kernel, strides, padding, dimension_numbers=None
    ):
        ...
        if padding == "VALID":
            o_sdims = [
                in_sdims[i] * strides[i] + max(k_sdims[i] - strides[i], 0)
        ...
        out_spec_inv = [
            x[0] for x in sorted(enumerate(dn.out_spec), key=lambda x: x[1])
        ]
        o_layout = onp.take(onp.array(o_shape), out_spec_inv)
        placeholder = onp.ones(o_layout, data.dtype)
        conv = lambda x: lax.conv_general_dilated(
            x, kernel, strides, padding, one, one, dn
        )
    ...
    def testConvTranspose2DT(
        self, lhs_shape, rhs_shape, dtype, strides, padding, dspec, rng
    ):
        args_maker = lambda: [rng(lhs_shape, dtype), rng(rhs_shape, dtype)]
        ...
        def fun_via_grad(lhs, rhs):
            return self._conv_transpose_via_grad(
                lhs, rhs, strides, padding, dimension_numbers=dspec
            )
    ...
    def testConvTranspose2D(
        ...
        def fun_via_grad(lhs, rhs):
            ...
            return self._conv_transpose_via_grad(
                lhs, rhs_t, strides, padding, dimension_numbers=dspec
            )
    ...
    def testDot(self, lhs_shape, rhs_shape, dtype, precision, rng):
        ...
        self._CompileAndCheck(
            partial(lax.dot, precision=precision), args_maker, check_dtypes=True
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_lhs_shape={}_rhs_shape={}_lhs_contracting={}_rhs_contracting={}".format(
                    jtu.format_shape_dtype_string(lhs_shape, dtype),
                    jtu.format_shape_dtype_string(rhs_shape, dtype),
                    lhs_contracting,
                    rhs_contracting,
    ...
    def testDotGeneralContractOnly(
        self, lhs_shape, rhs_shape, dtype, lhs_contracting, rhs_contracting, rng
    ):
        args_maker = lambda: [rng(lhs_shape, dtype), rng(rhs_shape, dtype)]
    ...
    def testDotGeneralContractAndBatch(
        self, lhs_shape, rhs_shape, dtype, dimension_numbers, rng
    ):
        args_maker = lambda: [rng(lhs_shape, dtype), rng(rhs_shape, dtype)]
    ...
    def testDotGeneralAgainstNumpy(
        self, lhs_shape, rhs_shape, dtype, dimension_numbers, rng
    ):
        args_maker = lambda: [rng(lhs_shape, dtype), rng(rhs_shape, dtype)]
    ...
    def testReverse(self):
        ...
        dimensions = [0]
        self.assertAllClose(
            onp.array([3, 2, 1]), rev(onp.array([1, 2, 3])), check_dtypes=False
        )

        dimensions = [0, 1]
        self.assertAllClose(
            onp.array([[6, 5, 4], [3, 2, 1]]),
    ...
    def testDynamicSliceAgainstNumpy(
        self, shape, dtype, start_indices, size_indices, rng
    ):
        args_maker = lambda: [rng(shape, dtype), onp.array(start_indices)]
    ...
    def testDynamicUpdateSliceAgainstNumpy(
        self, shape, dtype, start_indices, update_shape, rng
    ):
        def args_maker():
            return [
                rng(shape, dtype),
        ...
        self._CheckAgainstNumpy(
            lax.dynamic_update_slice, lax_reference.dynamic_update_slice, args_maker
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_op={}_dtype={}_padding={}".format(
                    op.__name__, onp.dtype(dtype).name, padding
                ),
                "op": op,
                "init_val": init_val,
                "dtype": dtype,
...
LAX_GRAD_OPS = [
    ...
    grad_test_spec(
        lax.floor, nargs=1, order=2, rng=jtu.rand_default(), dtypes=[onp.float64]
    ),
    grad_test_spec(
        lax.ceil, nargs=1, order=2, rng=jtu.rand_default(), dtypes=[onp.float64]
    ),
    grad_test_spec(
        lax.round, nargs=1, order=2, rng=jtu.rand_default(), dtypes=[onp.float64]
    ),
    grad_test_spec(
        lax.exp,
        nargs=1,
    ...
    grad_test_spec(
        lax.atan2, nargs=2, order=2, rng=jtu.rand_default(), dtypes=[onp.float64]
    ),
    grad_test_spec(
        lax.erf, nargs=1, order=2, rng=jtu.rand_small(), dtypes=[onp.float64]
    ),
    grad_test_spec(
        lax.erfc, nargs=1, order=2, rng=jtu.rand_small(), dtypes=[onp.float64]
    ),
    grad_test_spec(
        lax.erf_inv, nargs=1, order=2, rng=jtu.rand_small(), dtypes=[onp.float64]
    ),
    # grad_test_spec(lax.lgamma, nargs=1, order=2, rng=jtu.rand_small(),
    #                dtypes=[onp.float64]),  # TODO(mattjj): enable
    grad_test_spec(
        lax.real, nargs=1, order=2, rng=jtu.rand_default(), dtypes=[onp.complex64]
    ),
    grad_test_spec(
        lax.imag, nargs=1, order=2, rng=jtu.rand_default(), dtypes=[onp.complex64]
    ),
    grad_test_spec(
        lax.complex, nargs=2, order=2, rng=jtu.rand_default(), dtypes=[onp.float32]
    ),
    grad_test_spec(
        lax.conj,
        nargs=1,
    ...
    grad_test_spec(
        ...
        nargs=2,
        order=1,
        rng=jtu.rand_not_small(),
        dtypes=[onp.float64, onp.complex64],
    ),
    grad_test_spec(
        lax.max, nargs=2, order=2, rng=jtu.rand_default(), dtypes=[onp.float64]
    ),
    grad_test_spec(
        lax.min, nargs=2, order=2, rng=jtu.rand_default(), dtypes=[onp.float64]
    ),
...
LAX_GRAD_SPECIAL_VALUE_TESTS = [
    GradSpecialValuesTestSpec(lax.sinh, [0.0]),
    GradSpecialValuesTestSpec(lax.cosh, [0.0]),
    GradSpecialValuesTestSpec(lax.tanh, [0.0, 1000.0]),
    GradSpecialValuesTestSpec(lax.sin, [0.0, onp.pi, onp.pi / 2.0, onp.pi / 4.0]),
    GradSpecialValuesTestSpec(lax.cos, [0.0, onp.pi, onp.pi / 2.0, onp.pi / 4.0]),
    GradSpecialValuesTestSpec(lax.tan, [0.0]),
    GradSpecialValuesTestSpec(lax.asin, [0.0]),
    GradSpecialValuesTestSpec(lax.acos, [0.0]),
    GradSpecialValuesTestSpec(lax.atan, [0.0, 1000.0]),
    GradSpecialValuesTestSpec(lax.erf, [0.0, 10.0]),
    GradSpecialValuesTestSpec(lax.erfc, [0.0, 10.0]),
]
...
class LaxAutodiffTest(jtu.JaxTestCase):
    ...
    def testConvWithGeneralPaddingGrad(
        self, lhs_shape, rhs_shape, dtype, strides, padding, lhs_dil, rhs_dil, rng
    ):
        lhs = rng(lhs_shape, dtype)
        rhs = rng(rhs_shape, dtype)
        conv = partial(
    ...
    def testConvGeneralDilatedGrad(
        self,
        lhs_shape,
        rhs_shape,
        dtype,
        ...
        check_grads_bilinear(
            conv, (lhs, rhs), order=2, modes=["fwd", "rev"], atol=tol, rtol=tol
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            ...
            for lhs_shape in [(2,), (3, 2)]
            for rhs_shape in [(2,), (2, 4)]
            for dtype in float_dtypes
        )
    )
    def testDotGrad(self, lhs_shape, rhs_shape, dtype, rng):
        ...
        check_grads_bilinear(
            dot, (lhs, rhs), order=2, modes=["fwd", "rev"], atol=tol, rtol=tol
        )
    ...
    def testDotGeneralContractAndBatchGrads(
        self, lhs_shape, rhs_shape, dtype, dimension_numbers, rng
    ):
        ...
        check_grads_bilinear(
            dot_general, (lhs, rhs), order=2, modes=["fwd", "rev"], atol=tol, rtol=tol
        )
    ...
    def testPadGrad(self, shape, dtype, pads, rng):
        ...
        operand = rng(shape, dtype)
        padding_value = onp.array(0.0, dtype)
    ...
    def testReverseGrad(self):
        ...
        dimensions = [0]
        check_grads(rev, (onp.array([3.0, 2.0, 1.0]),), 2)

        dimensions = [0, 1]
        check_grads(rev, (onp.array([[6.0, 5.0, 4.0], [3.0, 2.0, 1.0]]),), 2)
    ...
    def testDynamicUpdateSliceGrad(
        self, shape, dtype, start_indices, update_shape, rng
    ):
        tol = 1e-2 if onp.finfo(dtype).bits == 32 else None
        operand = rng(shape, dtype)
    ...
    def testReduceWindowGrad(self, op, init_val, dtype, padding, rng):
        ...
        for shape, dims, strides in all_configs:
            ...
            if op is not lax.add:
                ...
                self.assertEqual(
                    onp.unique(operand).size,
                    operand.size,
                    msg="test requires operand elements to be unique.",
                )
            check_grads(
                fun, (operand,), gradient_order, ["fwd", "rev"], 1e-2, 1e-2, 1e-2
            )
    ...
    def testGatherGrad(self, shape, dtype, idxs, dnums, slice_sizes, rng, rng_idx):
        ...
        gather = lambda x: lax.gather(
            x, idxs, dimension_numbers=dnums, slice_sizes=slice_sizes
        )
        x = rng(shape, dtype)
        check_grads(gather, (x,), 2, ["fwd", "rev"], 1e-2, 1e-2, 1.0)
    ...
    def testScatterAddGrad(
        self, arg_shape, dtype, idxs, update_shape, dnums, rng, rng_idx
    ):
        idxs = rng_idx(idxs.shape, idxs.dtype)
        scatter_add = lambda x, y: lax.scatter_add(x, idxs, y, dimension_numbers=dnums)
        x = rng(arg_shape, dtype)
        y = rng(update_shape, dtype)
        check_grads(scatter_add, (x, y), 2, ["fwd", "rev"], 1e-2, 1e-2, 1.0)
    ...
    def testScatterGrad(
        ...
        x = rng(arg_shape, dtype)
        y = rng(update_shape, dtype)
        check_grads(scatter, (x, y), 2, ["fwd", "rev"], 1e-2, 1e-2, 1.0)
    ...
    def testStopGradient(self):
        ...
        ans = api.grad(lambda x: lax.stop_gradient({"foo": x})["foo"])(3.0)
...
class LaxVmapTest(jtu.JaxTestCase):
    def _CheckBatching(
        self, op, bdim_size, bdims, shapes, dtype, rng, rtol=None, atol=None
    ):
    ...
    def testConvGeneralDilatedBatching(
        ...
        self._CheckBatching(
            conv, 5, (lhs_bdim, rhs_bdim), (lhs_shape, rhs_shape), dtype, rng
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_lhs_shape={}_rhs_shape={}_lhs_contracting={}_rhs_contracting={}_bdims={}".format(
                    jtu.format_shape_dtype_string(lhs_shape, dtype),
                    jtu.format_shape_dtype_string(rhs_shape, dtype),
                    lhs_contracting,
                    rhs_contracting,
    ...
    def testDotGeneralContractOnly(
        self, lhs_shape, rhs_shape, dtype, lhs_contracting, rhs_contracting, bdims, rng
    ):
        dimension_numbers = ((lhs_contracting, rhs_contracting), ([], []))
    ...
    def testDotGeneralContractAndBatch(
        self, lhs_shape, rhs_shape, dtype, dimension_numbers, bdims, rng
    ):
        dot = partial(lax.dot_general, dimension_numbers=dimension_numbers)
        self._CheckBatching(dot, 5, bdims, (lhs_shape, rhs_shape), dtype, rng)
    ...
    def testSelect(self, pred_shape, arg_shape, arg_dtype, bdims, rng):
        ...
        self._CheckBatching(
            op, 5, bdims, (pred_shape, arg_shape, arg_shape), arg_dtype, rng
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_shape={}_bdims={}_fft_ndims={}".format(
                    shape, bdims, fft_ndims
                ),
                "shape": shape,
                "bdims": bdims,
                "fft_ndims": fft_ndims,
