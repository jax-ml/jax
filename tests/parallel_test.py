...
class PapplyTest(jtu.JaxTestCase):
    ...
    def testMap(self):
        ...
        ans = pfun(onp.arange(3.0))
        expected = onp.sin(onp.arange(3.0))
        self.assertAllClose(ans, expected, check_dtypes=False)
    ...
    def testSum(self):
        ...
        arg = onp.arange(15.0).reshape((5, 3))
    ...
    def testLogSoftmax(self):
        ...
        expected_jaxpr = make_jaxpr(
            lambda x: x - np.log(lax.psum(np.exp(x), axis_name))
        )(onp.zeros(5))
        assert repr(jaxpr) == repr(expected_jaxpr)

        ans = soft_pmap(pfun, axis_name)(onp.arange(1.0, 5.0))
        expected = fun(onp.arange(1.0, 5.0))
        self.assertAllClose(ans, expected, check_dtypes=False)
...
class ParallelizeTest(jtu.JaxTestCase):
    ...
    def testTransposeAndAddRank2(self):
        ...
        x = onp.reshape(onp.arange(4.0, dtype=onp.float32), (2, 2))
        expected = fun(x)
        ans = _parallelize(fun)(x)
        self.assertAllClose(ans, expected, check_dtypes=False)
    ...
    def testTransposeAndAddRank3(self):
        ...
        x = onp.reshape(onp.arange(8.0, dtype=onp.float32), (2, 2, 2))
        expected = fun(x)
        ans = _parallelize(fun)(x)
        self.assertAllClose(ans, expected, check_dtypes=False)
    ...
    def testDotGeneral(self, matching, coloring, split):
        ...
        x = onp.reshape(onp.arange(8.0), (2, 2, 2))
        y = onp.reshape(onp.arange(8.0), (2, 2, 2)) + 4.0
