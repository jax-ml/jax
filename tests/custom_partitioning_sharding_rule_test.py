...
from jax._src.custom_partitioning_sharding_rule import ArrayMapping, CompoundFactor, sdy_sharding_rule_to_mlir, str_to_sdy_sharding_rule, SdyShardingRule
...
class SdyShardingRuleTest(jtu.JaxTestCase):
  ...
  def test_value_mapping_str(self):
    v = ArrayMapping("m", CompoundFactor("i", "j"), "k")
    self.assertEqual(str(v), "('m', ('i', 'j'), 'k')")
...
class StrToSdyShardingRuleTest(jtu.JaxTestCase):
  ...
  def test_sharding_rule_factor_infer_k(self):
    rule = str_to_sdy_sharding_rule("_i (j k)-> j foo (m bar_24)", k=10, m=10, bar_24=20)
    self.assertEqual(
        str(rule),
