...
def _uses_cudnn_backend(fn, *args, **kwargs):
...
class NNFunctionsTest(jtu.JaxTestCase):
  ...
  def testDotProductAttention(self, dtype, group_num, use_vmap, impl):
    ...
    if impl == 'cudnn':
      self.assertTrue(_uses_cudnn_backend(sdpa_ans, Q, K, V, bias, mask))
      self.assertTrue(_uses_cudnn_backend(sdpa_vjp_ans, grad))
  ...
  def testDotProductAttentionMask(self, mask_mode):
    ...
    self.assertTrue(_uses_cudnn_backend(sdpa_ans, *args, **kwargs))
    self.assertTrue(_uses_cudnn_backend(sdpa_vjp_ans, grad))
