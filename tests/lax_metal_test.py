...
class LaxBackedNumpyTests(jtu.JaxTestCase):
  ...
  @unittest.skip("MLIR translation rule for primitive 'eigh' not found for platform METAL.")
  @jtu.sample_product(
  ...
  @unittest.skip("Jax-metal fail on testEye2")
  @jtu.sample_product(
    dtype=default_dtypes,
    n=[0, 4],
    m=[None, 0, 1, 3, 4],
  ...
  #unittest.skip("jax-metal fail with empty vshape.")
  @jtu.sample_product(
    ashape=[(15,), (16,), (17,)],
    vshape= [(5,), (5, 5)],#[(), (5,), (5, 5)],
    side=['left', 'right'],
  ...
  @unittest.skip("Jax-metal don't support all dtypes.")
  ...
  #@unittest.skipIf(jtu.device_under_test=="METAL", "Jax-metal fail on empty dim shape.")
  @jtu.sample_product(
    [{'shape': shape, 'axis': axis, 'kth': kth}
     for shape in nonempty_shapes# nonzerodim_shapes
     for axis in range(-len(shape), len(shape))
     for kth in range(-shape[axis], shape[axis])],
    dtype=default_dtypes,
  )
  ...
  #unittest.skip("Jax-metal fail on tanh with np.nan")
  @jtu.sample_product(
    dtype=float_dtypes,
  ...
  def testMathSpecialFloatValues(self, op, dtype):
    ...
    jnp_op = getattr(jnp, op)
    dtype = np.dtype(dtypes.canonicalize_dtype(dtype)).type
    for x in (-np.inf, -100., -2., -1., 0., 1., 2., 100., np.inf,
              jnp.finfo(dtype).max, np.sqrt(jnp.finfo(dtype).max),
              np.sqrt(jnp.finfo(dtype).max) * 2.): #np.nan
      x = dtype(x)
      expected = np_op(x)
      actual = jnp_op(x)
  ...
  @jtu.sample_product(
    [dict(shapes=shapes, dtypes=dtypes)
      ...
      for dtypes in itertools.product(
        *(_valid_dtypes_for_shape(s, complex_dtypes + [None]) for s in shapes))
    ],
  )
  @jax.numpy_rank_promotion('allow')  # This test explicitly exercises implicit rank promotion.
...
#loc2 = loc("jit(gather)/jit(main)/gather[dimension_numbers=GatherDimensionNumbers(offset_dims=(1,), collapsed_slice_dims=(0, 2), start_index_map=(0, 2)) slice_sizes=(1, 2, 1) unique_indices=False indices_are_sorted=False mode=GatherScatterMode.CLIP fill_value=None]"(#loc1))
  ...
  def test_issue_pad(self):
    ir = '''
      module @jit_dummy attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
        func.func public @main(%arg0: tensor<2x2xf32> {mhlo.sharding = "{replicated}"} loc(unknown), %arg1: tensor<3x4xf32> {mhlo.sharding = "{replicated}"} loc(unknown)) -> tensor<4x4xf32> {
          %12 = stablehlo.slice %arg0 [0:1, 1:2] : (tensor<2x2xf32>) -> tensor<1x1xf32>
          %13 = stablehlo.reshape %12 : (tensor<1x1xf32>) -> tensor<f32>
          %14 = stablehlo.pad %arg1, %13, low = [0, 0], high = [1, 0], interior = [0, 0] : (tensor<3x4xf32>, tensor<f32>) -> tensor<4x4xf32>
          return %14 : tensor<4x4xf32>
        }
      }
    '''
    data = np.array([[1, 3], [1, 3]], dtype=np.float32)
    input = np.random.rand(3,4).astype(np.float32)
    res = ReportedIssuesTests.compile_and_exec(ir, [data, input])
    res_ref = ReportedIssuesTests.compile_and_exec(ir, [data, input], run_on_cpu = True)
    jtu.check_eq(res, res_ref)
