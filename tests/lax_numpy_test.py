...
def op_record(
    name, nargs, dtypes, shapes, rng, diff_modes, test_name=None, check_dtypes=True
):
...
JAX_ONE_TO_ONE_OP_RECORDS = [
    ...
    op_record(
        "float_power", 2, inexact_dtypes, all_shapes, jtu.rand_default(), ["rev"]
    ),
    ...
    op_record(
        "not_equal", 2, number_dtypes, all_shapes, jtu.rand_some_equal(), ["rev"]
    ),
    op_record(
    ...
    op_record(
        "tan", 1, number_dtypes, all_shapes, jtu.rand_uniform(-1.5, 1.5), ["rev"]
    ),
...
JAX_COMPOUND_OP_RECORDS = [
    ...
    op_record(
        "isfinite", 1, inexact_dtypes, all_shapes, jtu.rand_some_inf_and_nan(), []
    ),
    ...
    op_record(
        "where", 3, (onp.float32, onp.int64), all_shapes, jtu.rand_some_zero(), []
    ),
...
JAX_BITWISE_OP_RECORDS = [
    op_record(
        "bitwise_and", 2, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_bool(), []
    ),
    op_record(
        "bitwise_not", 1, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_bool(), []
    ),
    op_record(
        "bitwise_or", 2, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_bool(), []
    ),
    op_record(
        "bitwise_xor", 2, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_bool(), []
    ),
]
...
class LaxBackedNumpyTests(jtu.JaxTestCase):
    ...
    def testOp(self, onp_op, lnp_op, rng, shapes, dtypes, check_dtypes):
        ...
        self._CheckAgainstNumpy(
            onp_op, lnp_op, args_maker, check_dtypes=check_dtypes and not py_scalar_arg
        )
    ...
    def testOperatorOverload(self, name, rng, shapes, dtypes):
        ...
        self._CompileAndCheck(
            fun, args_maker, check_dtypes=jtu.PYTHON_SCALAR_SHAPE not in shapes
        )
    ...
    def testPad(
        self, shape, dtype, mode, pad_width_rank, constant_values_rank, rng, irng
    ):
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            ...
            for weights_shape in (
                [None, shape] if axis is None else [None, (shape[axis],), shape]
            )
        ...
        jtu.cases_from_list(
            ...
            for i, arg in enumerate(
                [
                    ...
                    [[1, 2], [3, 4], [5, 6]],
                    [[1, 2.0], [3, 4], [5, 6]],
                    [[3, onp.array(2), 1], onp.arange(3.0)],
                ]
            )
    ...
    def testTake(
        self, shape, dtype, index_shape, index_dtype, axis, mode, rng, rng_indices
    ):
        def args_maker():
            x = rng(shape, dtype)
    ...
    def testNanToNum(self, rng, shape, dtype):
        ...
        args_maker = lambda: [rng(shape, dtype)]
        self._CheckAgainstNumpy(
            onp.nan_to_num, lnp.nan_to_num, args_maker, check_dtypes=True
        )
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            ...
            for a_shape, axis in (((7,), None), ((47, 7), 0), ((4, 101), 1))
    ...
    @parameterized.named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": jtu.format_test_name_suffix(
                    "select", shapes, (onp.bool_,) * n + dtypes
                ),
                "rng": jtu.rand_default(),
                "shapes": shapes,
                "dtypes": dtypes,
    ...
    def testScalarDtypePromotion(self):
        ...
        msg = (
            "jax.numpy differs from numpy in promotion rules for Python scalars."
    ...
    def testLongLong(self):
        self.assertAllClose(
            onp.int64(7), api.jit(lambda x: x)(onp.longlong(7)), check_dtypes=True
        )
    ...
    def testArange(self):
        ...
        self.assertAllClose(
            lnp.arange(4, 21, 9), onp.arange(4, 21, 9), check_dtypes=True
        )
        self.assertAllClose(
            lnp.arange(53, 5, -3), onp.arange(53, 5, -3), check_dtypes=True
        )
        ...
        self.assertAllClose(
            lnp.arange(0, 1, -0.5), onp.arange(0, 1, -0.5), check_dtypes=True
        )
        ...
        self.assertFalse(
            type(lnp.arange(77, dtype=lnp.int32))
            == type(onp.arange(77, dtype=onp.int32))
        )
        self.assertTrue(
            type(lnp.arange(77, dtype=lnp.int32)) == type(lax.iota(onp.int32, 77))
        )
    ...
    def testMathSpecialFloatValues(self, op, dtype):
        ...
        for x in (
            ...
            onp.inf,
            onp.finfo(dtype).max,
            onp.sqrt(onp.finfo(dtype).max),
            onp.sqrt(onp.finfo(dtype).max) * 2.0,
        ):
    ...
    def testVar(self, shape, dtype, out_dtype, axis, ddof, keepdims, rng):
        args_maker = self._GetArgsMaker(rng, [shape], [dtype])
        onp_fun = partial(
            onp.var, dtype=out_dtype, axis=axis, ddof=ddof, keepdims=keepdims
        )
        lnp_fun = partial(
...
GRAD_SPECIAL_VALUE_TEST_RECORDS = [
    GradSpecialValuesTestSpec(lnp.arcsinh, [0.0, 1000.0]),
    GradSpecialValuesTestSpec(lnp.arccosh, [1000.0]),
    GradSpecialValuesTestSpec(lnp.arctanh, [0.0]),
]
