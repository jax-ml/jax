...
def print_and_run(f, *args, **kwargs):
  banner("BEGIN jaxpr")
  print(jax.make_jaxpr(f)(*args, **kwargs))
  banner("END jaxpr")

  banner("BEGIN StableHLO")
  print(jax.jit(f).lower(*args).as_text())
  banner("END StableHLO")

  banner("BEGIN UNOPTIMIZED HLO")
  print(jax.jit(f).lower(*args, **kwargs).as_text(dialect="hlo"))
  banner("END UNOPTIMIZED HLO")

  banner("BEGIN OPTIMIZED HLO")
  print(jax.jit(f).lower(*args, **kwargs).compile().as_text())
  banner("END OPTIMIZED HLO")

  banner("BEGIN Execution")
  print(jax.jit(f)(*args, **kwargs))
  banner("END Execution")
...
class ControlDepsTest(jtu.JaxTestCase):
  ...
  def test_fsdp(self):
    ...
    with jax.set_mesh(self.create_explicit_mesh((n,), ("i",))):
      ...
      def f_fsdp(x, ws):
        ...
        # This is a dummy version of FSDP where x is like a set of activations
  ...
  def test_scan_fsdp(self):
    ...
    with jax.set_mesh(self.create_explicit_mesh((n,), ("i",))):
      ...
      print_and_run(f_scan_fsdp, x, ws)
