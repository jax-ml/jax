...
def run_test(testmodule, gpu_tokens, continue_on_fail):
  ...
  if continue_on_fail:
      cmd = ["python3", "-m", "pytest", '--html={base_dir}/{testfile}_log.html'.format(base_dir, testfile), "--reruns", "3", "-v", testmodule]
  else:
      cmd = ["python3", "-m", "pytest", '--html={base_dir}/{testfile}_log.html'.format(base_dir, testfile), "--reruns", "3", "-x", "-v", testmodule]
