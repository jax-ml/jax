...
Used via `--run_under="python3 ci/utilities/memory_wrapper.py"`. Runs the wrapped
command as a subprocess and records its peak RSS to
`$TEST_UNDECLARED_OUTPUTS_DIR/memory_profile.json`, which Bazel zips into
`bazel-testlogs/**/test.outputs/outputs.zip` for later collection.
"""
