...
if [[ "$JAXCI_RUN_FULL_TPU_TEST_SUITE" == "1" ]]; then
  ...
  JAX_ENABLE_TPU_XDIST=true "$JAXCI_PYTHON" -m pytest -n="$JAXCI_TPU_CORES" -vv --full-trace --log-cli-level=DEBUG --tb=short \
    --deselect=tests/pallas/tpu_pallas_test.py::PallasCallPrintTest --deselect=tests/pallas/tpu_pallas_interpret_test.py::InterpretTest::test_two_cores_along_parallel_dimension_with_race --deselect=tests/pallas/tpu_pallas_interpret_test.py::InterpretTest::test_two_cores_along_parallel_dimension_no_race \
  ...
  # Run single-accelerator tests in parallel
  JAX_ENABLE_TPU_XDIST=true "$JAXCI_PYTHON" -m pytest -n="$JAXCI_TPU_CORES" -vv --full-trace --tb=short \
