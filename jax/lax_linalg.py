...
def cholesky_jvp_rule(primals, tangents):
    ...
    tmp = triangular_solve(
        L, sigma_dot, left_side=False, transpose_a=True, conjugate_a=True, lower=True
    )
...
def _nan_like(c, operand):
    ...
    if onp.issubdtype(dtype, onp.complexfloating):
        nan = c.Constant(onp.array(onp.nan * (1.0 + 1j), dtype=dtype))
    else:
...
def triangular_solve_jvp_rule_a(
    ...
    tmp = triangular_solve(
        a, g_a, left_side, lower, transpose_a, conjugate_a, unit_diagonal
    )
...
def triangular_solve_transpose_rule(
    cotangent, a, b, left_side, lower, transpose_a, conjugate_a, unit_diagonal
):
    ...
    cotangent_b = triangular_solve(
        a, cotangent, left_side, lower, not transpose_a, conjugate_a, unit_diagonal
    )
    return [None, cotangent_b]


def triangular_solve_batching_rule(
    batched_args, batch_dims, left_side, lower, transpose_a, conjugate_a, unit_diagonal
):
    x, y = batched_args
    bx, by = batch_dims
...
triangular_solve_p = standard_primitive(
    triangular_solve_shape_rule, triangular_solve_dtype_rule, "triangular_solve"
)
ad.defjvp2(
    triangular_solve_p,
...
def _triangular_solve_cpu_translation_rule(
    ...
    if len(shape.dimensions()) == 2 and dtype in _cpu_lapack_types:
        ...
        return lapack.jax_trsm(
            c,
            c.Constant(onp.array(1, dtype=dtype)),
            a,
            b,
            left_side,
            lower,
...
def _lu_unblocked(a):
    ...
    def body(k, state):
        ...
        x = a[k, k]
        a = ops.index_update(
            a, ops.index[:, k], np.where(m_idx > k, a[:, k] / x, a[:, k])
        )

        # a[k+1:, k+1:] -= np.outer(a[k+1:, k], a[k, k+1:])
        a = a - np.where(
            (m_idx[:, None] > k) & (n_idx > k),
            np.outer(a[:, k], a[k, :]),
...
def lu_pivots_to_permutation(swaps, m):
  ...
  """
    ...
    result, _ = lax.fori_loop(
        onp.array(0, onp.int32),
        onp.array(k, onp.int32),
        _lu_pivots_body_fn,
        (permutation, swaps),
    )
    return result
...
def _qr_cpu_gpu_translation_rule(geqrf_impl, orgqr_impl, c, operand, full_matrices):
    ...
    if m < n:
        ...
        q = c.Pad(
            r, c.Constant(onp.array(0, dtype=shape.element_type())), padding_config
        )
        q, info_orgqr = orgqr_impl(c, q, tau)

    ok = c.And(
        c.Eq(info_geqrf, c.ConstantS32Scalar(0)),
        c.Eq(info_orgqr, c.ConstantS32Scalar(0)),
    )
    q = _broadcasting_select(
        c, c.Reshape(ok, None, batch_dims + (1, 1)), q, _nan_like(c, q)
    )
    r = _broadcasting_select(
        c, c.Reshape(ok, None, batch_dims + (1, 1)), r, _nan_like(c, r)
    )
    return c.Tuple(q, r)
...
def svd_impl(operand, full_matrices, compute_uv):
    s, u, vt = xla.apply_primitive(
        svd_p, operand, full_matrices=full_matrices, compute_uv=compute_uv
    )
    return s, u, vt
...
def svd_abstract_eval(operand, full_matrices, compute_uv):
    if isinstance(operand, ShapedArray):
        if operand.ndim < 2:
            raise ValueError(
                "Argument to singular value decomposition must have ndims >= 2"
            )
        ...
        n = operand.shape[-1]
        s = ShapedArray(
            batch_dims + (min(m, n),), lax.lax._complex_basetype(operand.dtype)
        )
        u = ShapedArray(
            batch_dims + (m, m if full_matrices else min(m, n)), operand.dtype
        )
        vt = ShapedArray(
            batch_dims + (n if full_matrices else min(m, n), n), operand.dtype
        )
    else:
        raise NotImplementedError
    return s, u, vt
...
def _svd_cpu_gpu_translation_rule(gesvd_impl, c, operand, full_matrices, compute_uv):
    ...
    shape = c.GetShape(operand)
    batch_dims = shape.dimensions()[:-2]
    s, u, vt, info = gesvd_impl(
        c, operand, full_matrices=full_matrices, compute_uv=compute_uv
    )
    ok = c.Eq(info, c.ConstantS32Scalar(0))
    s = _broadcasting_select(
        c, c.Reshape(ok, None, batch_dims + (1,)), s, _nan_like(c, s)
    )
    u = _broadcasting_select(
        c, c.Reshape(ok, None, batch_dims + (1, 1)), u, _nan_like(c, u)
    )
    vt = _broadcasting_select(
        c, c.Reshape(ok, None, batch_dims + (1, 1)), vt, _nan_like(c, vt)
    )
