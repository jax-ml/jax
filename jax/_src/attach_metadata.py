...
mlir.register_lowering(
    xla_metadata_func_p, _xla_metadata_func_lowering_rule, cacheable=False
)
...
def _xla_metadata_value_jvp_rule(primals, tangents, *, xla_metadata):
  ...
  out_primal = xla_metadata_value_p.bind(out_primal, xla_metadata=xla_metadata)
...
def metadata_tagging_hook(cts_out, metadata):
  metadata[ATTACH_TO_VALUE_FLAG_KEY] = True
...
def attach_metadata(*args, **kwargs):
  ...
  Args:
    ...
    **kwargs: Metadata key-value pairs (e.g., op_name="custom_op", key=value).
      ...
      supported to control how metadata is attached: - `attach_to_value`:
      (Default: True) In value-tagging mode, controls whether metadata is
      attached to the forward-pass value. - `attach_to_grad`: (Default: True) In
      value-tagging or function-wrapping modes, controls whether metadata is
      also attached to the gradient during automatic differentiation.
