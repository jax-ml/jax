...
def component_abstract_eval(
  *args, f: Callable[..., Any], component_key: ComponentKey
):
  logging.info("component_abstract_eval: %s", component_key)
  key = aot_util.make_abstract_eval_key(component_key)

  def abstract_eval():
    # TODO(dsuo): How / when to check if we're tracing again by mistake.
    if key not in aot_util.traced_cache:
      aot_util.traced_cache[key] = aot_util.trace(f, *args)
    return tree_util.tree_map(
      lambda x: core.ShapedArray(x.shape, x.dtype),
      aot_util.traced_cache[key].out_info,
    )
...
def component_lowering(
  ctx, *args, f: Callable[..., Any], component_key: ComponentKey
):
  ...
  traced = aot_util.traced_cache[traced_key]
  ...
  call = func_dialect.CallOp(results, ir.FlatSymbolRefAttr.get(fn), args)
