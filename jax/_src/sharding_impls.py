...
def default_pmap_sharding(
  ...
  if sharded_dim is None:
    if devices is None:
      raise ValueError("One of sharded_dim or devices must be set.")
    mesh = mesh_lib.Mesh(np.array(devices), ('_default_pmap_sharding',))
  ...
  mesh = mesh_lib.Mesh(pmap_devices, ('_default_pmap_sharding',))
  spec_list: list[str | None] = [None] * len(shape)
  spec_list[sharded_dim] = '_default_pmap_sharding'
