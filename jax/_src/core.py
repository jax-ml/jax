...
class TracingContext(threading.local):
  ...
  def reset(self):
    ...
    # stackless backward compat with e.g. equinox
    self.eager_constant_folding = False
  ...
  @contextmanager
  def set_eager_constant_folding(self, eager_constant_folding):
    prev = self.eager_constant_folding
    try:
      self.eager_constant_folding = eager_constant_folding
      self.update_thread_local_jit_state()
      yield
    finally:
      self.eager_constant_folding = prev
      self.update_thread_local_jit_state()

  def update_thread_local_jit_state(self):
    ts = self.trace.as_hashable_key() if self.trace is not None else None
    config.update_thread_local_jit_state(
      trace_state=(ts, self.eager_constant_folding),
...
def take_current_trace():
  ...
  with trace_ctx.set_trace(eval_trace):
    yield trace
...
def ensure_compile_time_eval():
  ...
  """
  with trace_ctx.set_eager_constant_folding(True):
    yield
