...
class Jaxpr:
  __slots__ = ['__weakref__', '_all_invars', '_num_consts', '_outvars',
               '_eqns', '_effects', '_debug_info', '_is_high', '_consts']
  ...
  def __init__(self, constvars: Sequence[Var] | Jaxpr,
    ...
    """
    Args:
      constvars: list of variables introduced for constants. Array constants
        are replaced with such variables while scalar constants are kept
        inline.
      invars: list of input variables. Together, `constvars` and `invars` are
        the inputs to the Jaxpr, stored as a single list of binders of which
        the constvars are a prefix.
      ...
      consts: the constant argument values, paired with `constvars`; may be
        omitted, leaving the constants unapplied.
    """
    ...
    assert not self._consts or len(self._consts) == self._num_consts, (
...
def pp_jaxprs(jaxprs: Sequence[Jaxpr],
              context: JaxprPpContext, settings: JaxprPpSettings) -> pp.Doc:
