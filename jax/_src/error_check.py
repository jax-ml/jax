...
def set_error_if(pred: jax.Array, /, msg: str) -> None:
  ...
  under_jit = isinstance(pred, core.Tracer)
  if under_jit:
    out_sharding = core.get_ty(_error_storage.ref).sharding
    in_sharding = core.get_ty(pred).sharding
  else:
    out_sharding = _error_storage.ref.sharding
    in_sharding = pred.sharding

  is_single_device = (
      isinstance(out_sharding, SingleDeviceSharding)  # not under jit
      or out_sharding.mesh.shape_tuple == ()  # under jit
  )
  if is_single_device:  # single-device case.
    pred = pred.any()
  else:  # multi-device case.
    assert isinstance(out_sharding, NamedSharding)
    assert isinstance(in_sharding, NamedSharding)
    has_auto_axes = mesh_lib.AxisTypes.Auto in in_sharding.mesh.axis_types
    if has_auto_axes and under_jit:
      # TODO(ayx): determine the behavior for auto mode under jit.
      raise NotImplementedError(
          "Error checking in auto mode under jit is not supported yet. Please "
          "use explicit mode."
      )
