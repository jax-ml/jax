...
def error_checking_context(mesh: mesh_lib.Mesh | None = None):
  ...
  original state.
  """
  old_error_code_ref = _error_storage.ref

  # If mesh is not provided, get the mesh from the context.
  if mesh is None:
  ...
  try:
    ...
    _error_storage.ref = old_error_code_ref
...
def set_error_if(pred: jax.Array, msg: str) -> None:
  ...
  if isinstance(_error_storage.ref.sharding, SingleDeviceSharding):  # pytype: disable=attribute-error
    pred = pred.any()
  else:
    if _error_storage.ref.sharding.mesh != pred.sharding.mesh:  # pytype: disable=attribute-error
      raise ValueError(
    ...
    pred = shard_map(
        ...
        mesh=_error_storage.ref.sharding.mesh,  # pytype: disable=attribute-error
        in_specs=pred.sharding.spec,  # pytype: disable=attribute-error
        out_specs=_error_storage.ref.sharding.spec,  # pytype: disable=attribute-error
    )(pred)
...
def raise_if_error() -> None:
  ...
  error_code = _error_storage.ref[...].min()  # perform per-device reduction
  ...
  try:
    ...
    _error_storage.ref[...] = jnp.full(
        ...
        jnp.uint32(_NO_ERROR),
        device=_error_storage.ref.sharding,  # pytype: disable=attribute-error
    )
