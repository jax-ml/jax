...
class HiType(core.AbstractValue):
  ...
  def _must_override(self, method: str, needed_for: str) -> NoReturn:
    _must_override(self, method, needed_for)

  # type equality
  def __hash__(self):
    self._must_override("__hash__", "type equality")
  def __eq__(self, other):
    self._must_override("__eq__", "type equality")

  # lowering from hijax type to lojax types
  def lo_ty(self) -> list[core.AbstractValue]:
    self._must_override("lo_ty", "lowering (e.g. under jit)")
  ...
  def lower_val(self, hi_val: HiVal) -> list[LoVal]:  # TODO(mattjj): not lovals
    self._must_override("lower_val", "lowering values (e.g. under jit)")
  def raise_val(self, *lo_vals: LoVal) -> HiVal:
    self._must_override("raise_val", "raising lowered values (e.g. under jit)")

  # autodiff interface
  def to_tangent_aval(self) -> HiType:
    self._must_override("to_tangent_aval", "autodiff")
