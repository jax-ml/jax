...
class ArrayMapping(tuple):
  ...
  Each element is either a string, or a CompoundFactor, examples:
  * ArrayMapping('a')
  * ArrayMapping('b', 'c')
  ...
  def __init__(self, *dim_mappings):
    for d in dim_mappings:
      if not isinstance(d, str) and not isinstance(d, CompoundFactor):
        raise ValueError(
            f"Each element of ArrayMapping must be a str or CompoundFactor, but"
            f" got {type(d)}")
...
def sdy_sharding_rule_to_mlir(
  ...
  ellipsis_rank = None
  for i, value in enumerate(rule.operand_mappings + rule.result_mappings):
    ...
    for j, dim in enumerate(value):
      if isinstance(dim, str):
        add_factor(dim, get_size_for_value_dim(i, j + current_ellipsis_rank))
      else:
        for factor in dim:
