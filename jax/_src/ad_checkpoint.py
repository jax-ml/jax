...
def remat_partial_eval(trace: pe.JaxprTrace, *tracers: core.Tracer,
  ...
  # Make copies of any ref arguments' initial values.
  tracers_ = [trace.new_instantiated_const(t.pval.get_known()[...])
              if isinstance(t.aval, AbstractRef) else t for t in tracers]
...
def _replace_arg_refs_with_vals(jaxpr):
  if any(isinstance(a, AbstractRef) for a in jaxpr.in_avals):
...
def _replace_arg_refs_with_vals_(jaxpr):
  assert not jaxpr.constvars
  in_avals = [a.inner_aval if isinstance(a, AbstractRef) else a
              for a in jaxpr.in_avals]
  def fun(*args):
    args = [core.new_ref(x) if isinstance(a, AbstractRef) else x
            for a, x in zip(jaxpr.in_avals, args)]
    return core.eval_jaxpr(jaxpr, (), *args)
