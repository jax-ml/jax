...
def ragged_all_to_all(
  ...
  arrays between devices. To represent slices of a ragged array to be sent, it
  accepts ``input_offsets`` and ``send_sizes`` representing slices of
  ``operand``. Received results are returned in an array that has the same value
  of the argument ``output``  except with received values written at the slices
  indicated by ``output_offsets`` and `recv_sizes``. Beware, the way
  ``output_offsets`` is encoded may be surprising; see below!
  ...
  by the size of the mapped axis ``axis_name``.
  ...
  ``output_offsets`` does not indicate the slices at which
  its local ``result`` is updated; instead, it indicates where the
  corresponding sent slices are written on their destination instances. To
  compute the local slices at which the received data are written, we must
  perform an ``all_to_all`` on ``output_offsets``.
