...
def _ragged_all_to_all_batched_collective(axis_data, vals_in, dims_in,
  ...
  assert(
      ...
      == input_offsets.shape[input_offsets_dim]
      == send_sizes.shape[send_sizes_dim]
      == output_offsets.shape[output_offsets_dim]
      == recv_sizes.shape[recv_sizes_dim]
  ), "all operands must have the same batch sizes"
  ...
  for i in range(operand.shape[operand_dim]):
    ...
    sliced_result = ragged_all_to_all(
        operand=sliced_operand,
        output=sliced_output,
        input_offsets=sliced_input_offsets,
        send_sizes=sliced_send_sizes,
        output_offsets=sliced_output_offsets,
        recv_sizes=sliced_recv_sizes,
        axis_name=axis_name,
        axis_index_groups=axis_index_groups,
    )
