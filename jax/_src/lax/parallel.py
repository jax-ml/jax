...
def precv(token, expected_dtype_and_shape, axis_name, perm):
  ...
  Args:
    token: a compiler token, either generated by a matching psend or
      ...
      collectives.
    expected_dtype_and_shape: ShapeDtypeStruct(s) containing the dtype and shape
      of the result.
    axis_name: hashable Python object used to name a pmapped axis (see the
      :func:`jax.pmap` documentation for more details).
    perm: list of pairs of ints, representing ``(source_index,
  ...
  Returns:
    Array(s) with the same shape as ``expected_dtype_and_shape``.
  """
  if not isinstance(axis_name, (list, tuple)):
    axis_name = (axis_name,)
  return precv_p.bind(
      token,
      expected_dtype_and_shape=core.ShapedArray(
          expected_dtype_and_shape.shape, expected_dtype_and_shape.dtype
      ),
      axis_name=axis_name,
...
def _precv_lowering(ctx, token, *, expected_dtype_and_shape, axis_name, perm):
  full_perm, other_args = _pcollectives_lowering_common(
      ctx, axis_name=axis_name, perm=perm, op_name="precv"
  )
  recv_op = hlo.RecvOp(
      [mlir.aval_to_ir_type(expected_dtype_and_shape), token.type],
      token,
      source_target_pairs=mlir.dense_int_elements(full_perm),
      **other_args,
  )
...
def _precv_abstract_eval(
    token, *, expected_dtype_and_shape, axis_name, **params
):
  return core.ShapedArray(
      expected_dtype_and_shape.shape, expected_dtype_and_shape.dtype
  ), {*map(core.NamedAxisEffect, axis_name), core.SingleSideCollectiveEffect()}
