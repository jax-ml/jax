...
def broadcast_shapes(
    *shapes: Tuple[Union[int, core.Tracer], ...]
...
def _broadcast_shapes_uncached(*shapes):
  ...
  try:
    return _reduce(_broadcast_ranks, rst, fst)
  except ValueError:
    pass
  ...
  if result_shape is None:
    raise ValueError('Incompatible shapes for broadcasting: {}'.format(
        tuple(shape_list)))
  return result_shape
...
def _broadcast_ranks(s1, s2):
  ...
  if core.symbolic_equal_shape(s1_, s1):
    return s2
  else:
    raise ValueError
...
def _dyn_shape_staging_rule(trace, prim, out_aval, *args, **params):
  ...
  eqn = pe.new_jaxpr_eqn([trace.getvar(x) for x in args],
                         [trace.makevar(out_tracer)], prim, params,
                         core.no_effects, source_info)
...
def round(
    x: Array,
    rounding_method: RoundingMethod = RoundingMethod.AWAY_FROM_ZERO) -> Array:
  r"""Elementwise round.
  ...
  Args:
    ...
    rounding_method: the method to use when rounding halfway values (e.g.,
      `0.5`). See ``lax.RoundingMethod`` for the list of possible values.
...
def _convert_element_type(operand: Array,
                          new_dtype: Optional[DType] = None,
  ...
  if ((old_dtype, old_weak_type) == (new_dtype, new_weak_type) and
      isinstance(operand, (core.Tracer, device_array.DeviceArray))):
    return operand
  else:
...
def dot(lhs: Array,
  ...
  Args:
    ...
    precision: Optional. Either ``None``, which means the default precision for
      ...
      (``Precision.DEFAULT``, ``Precision.HIGH`` or ``Precision.HIGHEST``) or a
      tuple of two :class:`~jax.lax.Precision` enums indicating precision of
      ``lhs``` and ``rhs``.
  ...
  """
  if 1 <= lhs.ndim <= 2 and 1 <= rhs.ndim <= 2 and core.symbolic_equal_dim(
...
def broadcast(operand: Array, sizes: Sequence[int]) -> Array:
  ...
  Args:
    operand: an array
    sizes: a sequence of integers, giving the sizes of new leading dimensions to
  ...
  See Also:
    jax.lax.broadcast_in_dim : add new dimensions at any location in the array
    shape.
  """
...
def reshape(operand: Array,
  ...
  Args:
    ...
    new_sizes: sequence of integers specifying the resulting shape. The size of
      the final array must match the size of the input.
    dimensions: optional sequence of integers specifying the permutation order
      of the input shape. If specified, the length must match ``operand.shape``.
  ...
  if (np.shape(operand) and same_shape and same_dims and
      isinstance(operand, (core.Tracer, device_array.DeviceArray))):
    return operand
  else:
...
def pad(operand: Array, padding_value: Array,
  ...
  """
  return pad_p.bind(
      operand, padding_value, padding_config=tuple(padding_config))
...
def select_n(which: Array, *cases: Array) -> Array:
  ...
  Args:
    which: determines which case should be returned. Must be an array containing
      either a boolean or integer values. May either be a scalar or have shape
      matching ``cases``. For each array element, the value of ``which``
...
def transpose(operand: Array, permutation: Sequence[int]) -> Array:
  ...
  if (permutation == tuple(range(np.ndim(operand))) and
      isinstance(operand, (core.Tracer, device_array.DeviceArray))):
    return operand
  else:
...
def argmin(operand: Array, axis: int,
  ...
  return argmin_p.bind(
      operand, axes=(axis,), index_dtype=dtypes.canonicalize_dtype(index_dtype))


def argmax(operand: Array, axis: int,
           index_dtype: DType) -> Tuple[Array, Array]:
  """Computes the index of the maximum element along ``axis``."""
  return argmax_p.bind(
      operand, axes=(axis,), index_dtype=dtypes.canonicalize_dtype(index_dtype))


def reduce(operands: Any, init_values: Any, computation: Callable[[Any, Any],
                                                                  Any],
  ...
  if monoid_reducer:
    ...
    weak_type = dtypes.is_weakly_typed(
        *flat_operands) and dtypes.is_weakly_typed(*flat_init_values)
    ...
    jaxpr, consts, out_tree = _variadic_reduction_jaxpr(computation,
                                                        tuple(flat_init_avals),
                                                        init_value_tree)
    out = reduce_p.bind(
...
def _get_monoid_reducer(monoid_op: Callable,
  ...
  if (type(aval) is ConcreteArray) and aval.shape == ():
    ...
    if monoid_op is add:
      ...
      return np.equal(aval.val,
                      _get_bitwise_and_identity(dtype)) and _reduce_and
...
def sort(operand: Union[Array, Sequence[Array]],
         dimension: int = -1,
         is_stable: bool = True,
         num_keys: int = 1) -> Union[Array, Tuple[Array, ...]]:
  """Wraps XLA's `Sort

  <https://www.tensorflow.org/xla/operation_semantics#sort>`_ operator.

  For floating point inputs, -0.0 and 0.0 are treated as equivalent, and NaN
  values
  ...
  Args: operand : Array or sequence of arrays dimension : integer dimension
  along which to sort. Default: -1. is_stable : boolean specifying whether to
  use a stable sort. Default: True. num_keys : number of operands to treat as
  sort keys. Default: 1. For num_keys > 1, the sort order will be determined
  lexicographically using the first `num_keys` arrays, with the first key being
  primary. The remaining operands will be returned with the same permutation.
  ...
  if isinstance(operand, Sequence):
    ...
    if not (1 <= num_keys <= len(operand)):
      raise ValueError(
          f'num_keys={num_keys} must be between 1 and len(operand)={len(operand)}'
      )
    ...
    if num_keys != 1:
      raise ValueError(
          f'num_keys={num_keys} must equal 1 for a single operand.')
    dimension = canonicalize_axis(dimension, len(operand.shape))
    return sort_p.bind(
        operand, dimension=dimension, is_stable=is_stable, num_keys=1)[0]


def sort_key_val(keys: Array,
                 values: Array,
                 dimension: int = -1,
  ...
  k, v = sort_p.bind(
      keys, values, dimension=dimension, is_stable=is_stable, num_keys=1)
  return k, v
...
def full(shape: Shape,
         fill_value: Array,
         dtype: Optional[DType] = None) -> Array:
...
def _eye(dtype: DType, shape: Shape, offset: int) -> Array:
  ...
  dtype = dtypes.canonicalize_dtype(dtype)
  bool_eye = eq(
      add(broadcasted_iota(np.int32, shape, 0), np.int32(offset)),
...
def _delta(dtype: DType, shape: Shape, axes: Sequence[int]) -> Array:
  ...
  iotas = [
      broadcasted_iota(np.uint32, base_shape, i)
...
def reduce_precision(operand: Union[float, Array], exponent_bits: int,
  ...
  exponent_bits = core.concrete_or_error(
      operator.index, exponent_bits,
      'exponent_bits argument of lax.reduce_precision')
  mantissa_bits = core.concrete_or_error(
      operator.index, mantissa_bits,
      'mantissa_bits argument of lax.reduce_precision')
  return reduce_precision_p.bind(
...
def batch_matmul(lhs: Array,
  ...
  return dot_general(
      lhs,
      rhs, ((lhs_contract, rhs_contract), (batch, batch)),
      precision=precision)
...
def _upcast_fp16_for_computation(f):
  ...
  def f_wrapped(x):
    ...
    if dtype == np.float16 or dtype == dtypes.bfloat16:
      return convert_element_type(f(convert_element_type(x, np.float32)), dtype)
    return f(x)
...
_fixed_dtype = lambda dtype: lambda *args, **kwargs: dtypes.canonicalize_dtype(
    dtype)
...
def unop(result_dtype, accepted_dtypes, name):
  ...
  prim = standard_primitive(
      _attrgetter('shape'), dtype_rule, name, weak_type_rule=weak_type_rule)
...
def naryop(result_dtype, accepted_dtypes, name):
  ...
  weak_type_rule = partial(_naryop_weak_type_rule, name)
  prim = standard_primitive(
      shape_rule, dtype_rule, name, weak_type_rule=weak_type_rule)
...
def _broadcast_translate(op, ctx, avals_in, avals_out, *args):
  ...
  for aval_in, arg in zip(avals_in, args):
    if aval_out.shape != aval_in.shape:
      bcast_dims = tuple(
          range(len(aval_out.shape) - len(aval_in.shape), len(aval_out.shape)))
...
def _unbroadcast(aval, x):
  ...
  if not aval.shape:
    ...
    dims = [
        i for i, (a, b) in enumerate(zip(x_shape, aval.shape))
        if not core.symbolic_equal_dim(a, b)
    ]
...
def _maybe_broadcast(target_shape, x):
  ...
  if core.symbolic_equal_shape(x_shape, target_shape):
    ...
    dims = [
        i for i, (a, b) in enumerate(zip(x_shape, target_shape))
...
def broadcast_mhlo(aval_out: core.ShapedArray,
...
def _nary_lower_mhlo(op: Callable,
                     ctx,
...
def _substitute_axis_sizes_in_aval(env: Dict[core.Var, ir.Value],
                                   a: core.AbstractValue) -> core.AbstractValue:
...
def _round_to_nearest_even(x):
  ...
  nearest_even_int = sub(round_val, mul(_const(x, 2), floor(mul(half, x))))
  is_odd = eq(nearest_even_int, one)
  return select(
      bitwise_or(gt(fraction, half), bitwise_and(eq(fraction, half), is_odd)),
...
def _round_lower(ctx, x, *, rounding_method):
  if rounding_method is RoundingMethod.AWAY_FROM_ZERO:
    ...
    round_nearest = mlir.cache_lowering(
        mlir.lower_fun(_round_to_nearest_even, multiple_results=False))
...
ad.defjvp2(tanh_p,
           lambda g, ans, x: mul(add(g, mul(g, ans)), sub(_one(x), ans)))
...
if jax._src.lib.mlir_api_version < 31:
  mlir.register_lowering(asin_p,
                         mlir.lower_fun(asin_impl, multiple_results=False))
else:
...
def acos_impl(x):
  if dtypes.issubdtype(_dtype(x), np.complexfloating):
    ...
    rpart = real(result)
    return select(gt(rpart, _const(rpart, 0)), result, neg(result))
  else:
    return select(
        ne(x, _const(x, -1.0)),
        mul(
...
if jax._src.lib.mlir_api_version < 31:
  mlir.register_lowering(atan_p,
                         mlir.lower_fun(atan_impl, multiple_results=False))
else:
...
ad.defjvp(atan2_p, lambda g, x, y: g * (y / (square(x) + square(y))),
...
ad.defjvp(acosh_p, lambda g, x: mul(g, rsqrt((x - _one(x)) * (x + _one(x)))))
...
regularized_incomplete_beta_p = standard_naryop([_float, _float, _float],
...
ad.defjvp(regularized_incomplete_beta_p, betainc_grad_not_implemented,
...
_complex_dtype = lambda dtype, *args: (np.zeros((), dtype) + np.zeros(
    (), np.complex64)).dtype
...
ad.defjvp2(rsqrt_p, lambda g, ans, x: mul(g, mul(_const(x, -0.5), div(ans, x))))
...
def _integer_pow_jvp(g, x, *, y):
  return _zeros(g) if y == 0 else mul(g, mul(
      _const(x, y), integer_pow(x, y - 1)))
...
ad.defjvp(mul_p, lambda xdot, x, y: mul(xdot, y),
...
ad.defjvp(div_p, lambda g, x, y: div(g, y),
...
ad.defjvp(
    rem_p, lambda g, x, y: _maybe_broadcast(
...
def _minmax_complex_lowering(x, y, *, lax_cmp_pick_x):
  ...
  ry = real(y)
  pick_x = select(
      eq(rx, ry), lax_cmp_pick_x(imag(x), imag(y)), lax_cmp_pick_x(rx, ry))
  return select(pick_x, x, y)


max_p: core.Primitive = standard_naryop([_any, _any], 'max')
ad.defjvp2(max_p, lambda g, ans, x, y: mul(g, _balanced_eq(x, ans, y)),
...
ad.defjvp2(min_p, lambda g, ans, x, y: mul(g, _balanced_eq(x, ans, y)),
           lambda g, ans, x, y: mul(g, _balanced_eq(y, ans, x)))
...
mlir.register_lowering(shift_left_p, partial(_nary_lower_mhlo,
                                             mhlo.ShiftLeftOp))

shift_right_arithmetic_p = standard_naryop([_int, _int],
...
def _compare_lower_mhlo(direction: str, ctx, x, y):
  ...
  x, y = broadcast_mhlo(
      aval_out.update(dtype=x_aval.dtype), ctx.avals_in, (x, y))
...
def _convert_element_type_transpose_rule(ct, operand, *, new_dtype, weak_type):
  ...
  if type(ct) is ad_util.Zero:
    ...
    return [
        ad_util.Zero(operand.aval.update(dtype=dtypes.float0, weak_type=False))
    ]
  else:
    return [
        convert_element_type_p.bind(
...
def _convert_element_type_jvp_rule(tangent, operand, *, new_dtype, weak_type):
  if core.primal_dtype_to_tangent_dtype(new_dtype) == dtypes.float0:
    return ad_util.Zero(
        tangent.aval.update(dtype=dtypes.float0, weak_type=False))
  else:
    return convert_element_type_p.bind(
        tangent, new_dtype=new_dtype, weak_type=weak_type)
...
convert_element_type_p.def_impl(
    partial(xla.apply_primitive, convert_element_type_p))
...
def _bitcast_convert_type_dtype_rule(operand, *, new_dtype):
  ...
  if dtypes.issubdtype(old_dtype, np.bool_) or dtypes.issubdtype(
    ...
    if old_dtype != new_dtype:
      raise TypeError(
          f'`bitcast_convert_type` for operand type ({old_dtype}) cannot have different destination type ({new_dtype})'
      )
  if np.dtype(old_dtype).itemsize != np.dtype(new_dtype).itemsize:
    raise TypeError(
        f'`bitcast_convert_type` for operand type ({old_dtype}) must have destination type ({new_dtype}) of same size.'
    )
  return new_dtype
...
def _validate_preferred_element_type(input_dtype, preferred_element_type):
  ...
  if dtypes.issubdtype(input_dtype, np.integer) and dtypes.issubdtype(
    ...
    if any(
        dtypes.issubdtype(input_dtype, t) and
        not dtypes.issubdtype(preferred_element_type, t)
        for t in allowed_types):
      raise TypeError(
          'Input type is incompatible with `preferred_element_type`. The compatible combinations of '
          '(input_type, preferred_element_type) are (integral, integral), (integral, floating), '
          '(floating, floating), (complex, complex.')
    if dtypes.issubdtype(input_dtype,
                         np.signedinteger) and not dtypes.issubdtype(
                             preferred_element_type, np.signedinteger):
      raise TypeError(
...
def _masked(padded_value, logical_shape, dimensions, value=0):
  ...
  masks = [
      broadcasted_iota(np.int32, padded_value.shape, d) < logical_shape[d]
      for d in dimensions
  ]
...
def _dot_general_shape_rule(lhs, rhs, *, dimension_numbers, precision,
  ...
  if not core.symbolic_equal_shape(lhs_contracting_shape,
                                   rhs_contracting_shape):
...
def _dot_general_dtype_rule(lhs, rhs, *, dimension_numbers, precision,
                            preferred_element_type: Optional[DType]):
  input_dtype = naryop_dtype_rule(_input_dtype, [_any, _any], 'dot_general',
                                  lhs, rhs)
...
def _dot_general_transpose_lhs(g,
                               y,
                               *,
                               dimension_numbers,
                               precision,
  ...
  x_contract_sorted_by_y = list(np.take(
      x_contract, np.argsort(y_contract)))  # type: ignore[arg-type]
  ...
  return transpose(
      dot_general(
          ...
          dims,
          precision=precision,
          preferred_element_type=preferred_element_type), tuple(out_axes))
...
def _dot_general_batch_rule(batched_args, batch_dims, *, dimension_numbers,
                            precision, preferred_element_type: Optional[DType]):
  lhs, rhs = batched_args
  new_dimension_numbers, result_batch_dim = _dot_general_batch_dim_nums(
      (lhs.ndim, rhs.ndim), batch_dims, dimension_numbers)
  batched_out = dot_general(
      lhs,
      rhs,
...
def _dot_general_batch_dim_nums(ndims, batch_dims, dimension_numbers):
  ...
  if lbd is not None and rbd is not None:
    ...
    if lbd is not None:
      ...
      other = tuple(
          d for d in range(rhs_ndim)
          if d not in rhs_batch and d not in rhs_contract)
      result_batch_dim = (
          lhs_ndim - len(lhs_contract) + sum(np.less(other, rbd)))
...
def _dot_general_padding_rule(in_avals, out_avals, lhs, rhs, *,
  ...
  padded_axes = [(i, lhs_aval.shape[i].val)
...
def _dot_general_pp_rule(eqn, context, settings):
  ...
  printed_params['dimension_numbers'] = ((list(lhs_cont), list(rhs_cont)),
                                         (list(lhs_batch), list(rhs_batch)))
  lhs = core.pp_vars(eqn.outvars, context, print_shapes=settings.print_shapes)
  rhs = [
      pp.text(eqn.primitive.name),
      core.pp_kv_pairs(sorted(printed_params.items()), context, settings),
...
ad.defbilinear(dot_general_p, _dot_general_transpose_lhs,
...
def _dot_general_lower(ctx, lhs, rhs, *, dimension_numbers, precision,
                       preferred_element_type: Optional[np.dtype]):
...
def _broadcast_in_dim_shape_rule(operand, *, shape, broadcast_dimensions):
  ...
  if len(shape) < operand_ndim:
    msg = (
        'broadcast_in_dim target broadcast shape must have equal or higher '
        'rank to the operand shape; got operand ndim {} and target broadcast '
        'ndim {}.')
  ...
  if not all(
      core.symbolic_equal_one_of_dim(operand.shape[i],
...
def _broadcast_in_dim_typecheck_rule(operand, *dyn_shape, shape,
                                     broadcast_dimensions):
  if not dyn_shape:
    ...
    out_aval = core.DShapedArray(
        tuple(out_shape), operand.aval.dtype, operand.aval.weak_type)
    return [out_aval], core.no_effects


def _broadcast_in_dim_transpose_rule(ct, operand, *dyn_shape, shape,
                                     broadcast_dimensions):
  if type(ct) is ad_util.Zero:
    return [ad_util.Zero(operand.aval)]
  unit_dims = [
      i for i, s in enumerate(operand.aval.shape)
...
def _broadcast_in_dim_staging_rule(trace, x, *dyn, shape, broadcast_dimensions):
...
def _broadcast_in_dim_padding_rule(in_avals, out_avals, x, *dyn_shape, shape,
                                   broadcast_dimensions):
...
def _broadcast_in_dim_jvp_rule(primals, tangents, *, shape,
                               broadcast_dimensions):
  operand, *dyn_shape = primals
  operand_dot, *_ = tangents
  y = broadcast_in_dim_p.bind(
      operand,
      *dyn_shape,
      shape=shape,
      broadcast_dimensions=broadcast_dimensions)
...
def _broadcast_in_dim_partial_eval(trace, operand, *dyn_shape, shape,
                                   broadcast_dimensions):
  if not dyn_shape:
  ...
  eqn = pe.new_eqn_recipe(
      ...
      dict(shape=shape,
           broadcast_dimensions=broadcast_dimensions), core.no_effects,
...
broadcast_in_dim_p = standard_primitive(_broadcast_in_dim_shape_rule,
                                        _input_dtype, 'broadcast_in_dim')
...
pe.custom_partial_eval_rules[
    broadcast_in_dim_p] = _broadcast_in_dim_partial_eval
...
def _clamp_batch_rule(batched_args, batch_dims, **params):
  ...
  size = next(
      x.shape[i] for x, i in zip(batched_args, batch_dims) if i is not None)
...
ad.defjvp(
    clamp_p, lambda g, min, operand, max: select(
        bitwise_and(gt(min, operand), lt(min, max)), g, _zeros(operand)),
    lambda g, min, operand, max: select(
        bitwise_and(gt(operand, min), lt(operand, max)), g, _zeros(operand)),
    lambda g, min, operand, max: select(lt(max, operand), g, _zeros(operand)))
...
if jax._src.lib.mlir_api_version < 30:
  ...
  mlir.register_lowering(clamp_p, partial(_nary_lower_mhlo, mhlo.ClampOp))
...
def _concatenate_shape_rule(*operands, **kwargs):
  ...
  if not 0 <= dimension < operands[0].ndim:
    ...
    raise TypeError(
        msg.format(dimension, ', '.join([str(o.shape) for o in operands])))
  shapes = [
      operand.shape[:dimension] + operand.shape[dimension + 1:]
...
def _concatenate_transpose_rule(t, *operands, dimension):
  operand_shapes = [
      o.aval.shape if ad.is_undefined_primal(o) else o.shape for o in operands
  ]
  if type(t) is ad_util.Zero:
    return [
        ad_util.Zero(o.aval) if ad.is_undefined_primal(o) else None
        for o in operands
    ]
  else:
    ...
    return [
        slicing.slice(t, start, limit) if ad.is_undefined_primal(o) else None
        for o, start, limit in zip(operands, starts, limits)
    ]


def _concatenate_batch_rule(batched_args, batch_dims, *, dimension):
  size = next(op.shape[bdim]
              for op, bdim in zip(batched_args, batch_dims)
              if bdim is not None)
  operands = [
      batching.moveaxis(op, bdim, 0) if bdim is not None else broadcast(
          op, (size,)) for op, bdim in zip(batched_args, batch_dims)
  ]
  return concatenate(operands, dimension + 1), 0


concatenate_p = standard_primitive(_concatenate_shape_rule,
                                   _concatenate_dtype_rule, 'concatenate')
...
def _pad_shape_rule(operand, padding_value, *, padding_config):
  ...
  result = tuple(
      core.sum_dim(l, h, core.dilate_dim(d, i + 1))
...
def _pad_transpose(t, operand, padding_value, *, padding_config):
  if type(t) is ad_util.Zero:
    t_operand = ad_util.Zero(
        operand.aval) if ad.is_undefined_primal(operand) else None
    t_padv = ad_util.Zero(
        padding_value.aval) if ad.is_undefined_primal(padding_value) else None
  else:
    lo, hi, interior = util.unzip3(padding_config)
    total = lambda x: _reduce_sum(x, list(range(t.ndim)))

    def t_op():
      unpad_config = safe_zip(
          np.negative(lo), np.negative(hi), np.zeros_like(interior))
    ...
    t_padv = sub(
        total(t),
        total(t_operand)) if ad.is_undefined_primal(padding_value) else None
...
def _pad_lower(ctx, x, padding_value, *, padding_config):
  ...
  return mhlo.PadOp(x, padding_value, mlir.dense_int_elements(low),
...
def shape_as_value(shape: core.Shape):
  ...
  dims = [
      expand_dims(
          convert_element_type(core.dimension_as_value(d), np.int64), (0,))
      for d in shape
  ]
...
def _reshape_transpose_rule(t, operand, *, new_sizes, dimensions):
  ...
  if dimensions is None:
    ...
    return [
        transpose(
            reshape(t, np.take(operand.aval.shape, dimensions)),
...
def _reshape_staging_rule(trace, x, *dyn, new_sizes, dimensions):
...
transpose_p = standard_primitive(_transpose_shape_rule, _input_dtype,
                                 'transpose')
ad.deflinear2(transpose_p, lambda t, _, permutation:
              [transpose(t, np.argsort(permutation))])  # type: ignore[arg-type]
...
def _select_transpose_rule(t, which, *cases):
  ...
  if type(t) is ad_util.Zero:
    return [None] + [
        ad_util.Zero(c.aval) if ad.is_undefined_primal(c) else None
        for c in cases
    ]
  else:
    zeros = full_like(t, 0)
...
def _select_mhlo_lowering(ctx, which, *cases):
  ...
  def _select(offset, cases):
    ...
    if len(cases) == 1:
      return cases[0]
    mid = len(cases) // 2
    pred = mlir.compare_mhlo(which, mlir.full_like_aval(offset + mid,
                                                        which_aval), lt,
                             compare_type)
...
def _reduce_shape_rule(*avals, computation, jaxpr, consts, dimensions):
  ...
  if any(arg.shape != () for arg in init_val_avals):
    ...
    raise ValueError(
        f'reduce found non-scalar initial value: {init_val_shapes}')
...
def _reduce_dtype_rule(*avals, computation, jaxpr, consts, dimensions):
  ...
  init_val_dtypes = [
      dtypes.canonicalize_dtype(init.dtype) for init in init_val_avals
  ]
...
def _reduce_weak_type_rule(*avals, computation, jaxpr, consts, dimensions):
  operand_avals, init_val_avals = split_list(avals, [len(avals) // 2])
  return [
      op.weak_type and init_val.weak_type
...
def _reduce_batch_rule(batched_args, batch_dims, *, computation, jaxpr, consts,
  ...
  if all(init_value_bdim is batching.not_mapped
    ...
    size = next(x.shape[ax]
                for x, ax in zip(batched_args, batch_dims)
                if ax is not None)
    operands = [
        batching.bdim_at_front(arg, bdim, size)
...
def _reduce_jvp_rule(primals, tangents, *, computation, jaxpr, consts,
                     dimensions):
...
def _reduce_lower(ctx, *values, computation, jaxpr, consts, dimensions):
  assert all(
      isinstance(x, core.ShapedArray) for x in ctx.avals_in), ctx.avals_in
  ...
  with ir.InsertionPoint(reducer):
    ...
    out_nodes, _ = mlir.jaxpr_subcomp(reducer_ctx, jaxpr, mlir.TokenSet(),
                                      consts, *([a] for a in reducer.arguments))
...
def _reduce_number_dtype_rule(name, operand, *args, **kw):
  if not dtypes.issubdtype(operand.dtype, np.number):
    raise TypeError('{} does not accept dtype {}. Accepted dtypes are subtypes '
                    'of number.'.format(name,
                                        np.dtype(operand.dtype).name))
...
def _reduce_op_shape_rule(operand, *, axes, input_shape=None):
  ...
  if not all(0 <= a < operand.ndim for a in axes):
    raise ValueError(
        f'reduction axes {axes} contains out-of-bounds indices for {operand}.')
  axes = frozenset(axes)
...
def _argminmax_dtype_rule(operand, *, axes, index_dtype):
  if not dtypes.issubdtype(index_dtype, np.integer):
    raise TypeError('index_dtype must be an integer type, but got {}'.format(
        np.dtype(index_dtype).name))
  return index_dtype


def _compute_argminmax(value_comparator, get_identity, operand, *, index_dtype,
                       axes):
  ...
  def reducer_fn(op_val_index, acc_val_index):
    ...
    pick_op_val = bitwise_or(
        value_comparator(op_val, acc_val), ne(op_val, op_val))
    # If x and y are not NaN and x = y, then pick the first
    pick_op_index = bitwise_or(
        pick_op_val, bitwise_and(eq(op_val, acc_val), lt(op_index, acc_index)))
    return (select(pick_op_val, op_val,
                   acc_val), select(pick_op_index, op_index, acc_index))

  res = reduce([operand, indices],
               [get_identity(operand.dtype),
                np.array(0, index_dtype)], reducer_fn, axes)
  return res[1]


argmin_p = standard_primitive(
    _argminmax_shape_rule,
    _argminmax_dtype_rule,
    'argmin',
    weak_type_rule=_strip_weak_type)
batching.defreducer(argmin_p)
ad.defjvp_zero(argmin_p)

argmax_p = standard_primitive(
    _argminmax_shape_rule,
    _argminmax_dtype_rule,
    'argmax',
    weak_type_rule=_strip_weak_type)
...
def _reduce_logical_shape_rule(operand, *, axes):
  if operand.dtype != np.bool_ and not np.issubdtype(operand.dtype, np.integer):
    raise TypeError(
        f'logical reduction requires operand dtype bool or int, got {operand.dtype}.'
    )
  return tuple(np.delete(operand.shape, axes))
...
mlir.register_lowering(
    reduce_sum_p,
    partial(_unary_reduce_lower, mhlo.AddOp, lambda dtype: np.array(0, dtype)))
mlir.register_lowering(
    reduce_prod_p,
    partial(_unary_reduce_lower, mhlo.MulOp, lambda dtype: np.array(1, dtype)))
mlir.register_lowering(
    reduce_or_p,
    partial(_unary_reduce_lower, mhlo.OrOp, _get_bitwise_or_identity))
mlir.register_lowering(
    reduce_and_p,
    partial(_unary_reduce_lower, mhlo.AndOp, _get_bitwise_and_identity))
mlir.register_lowering(
    reduce_xor_p,
    partial(_unary_reduce_lower, mhlo.XorOp, _get_bitwise_or_identity))
mlir.register_lowering(
    reduce_min_p, partial(_unary_reduce_lower, mlir.min_mhlo,
                          _get_min_identity))
mlir.register_lowering(
    reduce_max_p, partial(_unary_reduce_lower, mlir.max_mhlo,
                          _get_max_identity))
...
def _reduce_precision_shape_rule(operand, *, exponent_bits, mantissa_bits):
  ...
  if exponent_bits < 1:
    raise ValueError(
        f'reduce_precision: exponent_bits must be positive; got {exponent_bits}'
    )
  if mantissa_bits < 0:
    raise ValueError(
        f'reduce_precision: mantissa_bits must be non-negative; got {mantissa_bits}'
    )
  return operand.shape
...
def _float_to_int_for_sort(x):
  ...
  flipped = bitcast_convert_type(
      sub(unsigned_dtype.type(np.iinfo(signed_dtype).max), unsigned),
      signed_dtype)
...
def _sort_lt_comparator(*operands, num_keys=1):
  ...
  for xk, yk in zip(x_keys[::-1], y_keys[::-1]):
    p = (
        bitwise_or(lt(xk, yk), bitwise_and(eq(xk, yk), p))
        if p is not None else lt(xk, yk))
  return p
...
def _sort_le_comparator(*operands, num_keys=1):
  ...
  for xk, yk in zip(x_keys[::-1], y_keys[::-1]):
    p = (
        bitwise_or(lt(xk, yk), bitwise_and(eq(xk, yk), p))
        if p is not None else le(xk, yk))
  return p
...
def _operands_to_keys(*operands, num_keys=1):
  ...
  for x, y in zip(operands[:2 * num_keys:2], operands[1:2 * num_keys:2]):
    ...
    if dtypes.issubdtype(x.dtype, np.complexfloating):
      x_keys.extend(
          [_float_to_int_for_sort(real(x)),
           _float_to_int_for_sort(imag(x))])
      y_keys.extend(
          [_float_to_int_for_sort(real(y)),
           _float_to_int_for_sort(imag(y))])
...
def _sort_jvp(primals, tangents, *, dimension, is_stable, num_keys):
  ...
  primals = sort_p.bind(
      *(primals + (iotas[dimension],)),
      dimension=dimension,
      is_stable=is_stable,
      num_keys=num_keys)
  idx = tuple(
      primals[-1] if i == dimension else iotas[i] for i in range(len(shape)))
  tangents_out = tuple(
      t if type(t) is ad_util.Zero else t[idx] for t in tangents)
...
def _sort_lower(ctx, *operands, dimension, is_stable, num_keys):
  ...
  with ir.InsertionPoint(comparator):
    lower_comparator = mlir.lower_fun(
        partial(_sort_lt_comparator), multiple_results=False)
    sub_ctx = ctx.replace(
        primitive=None,
...
def _top_k_abstract_eval(operand, *, k):
  ...
  if len(operand.shape) == 0:
    raise TypeError('top_k operand must have >= 1 dimension, got {}'.format(
        operand.shape))
  ...
  return (operand.update(
      shape=shape, dtype=operand.dtype, weak_type=operand.weak_type),
...
def _top_k_batch_rule(batched_args, batch_dims, *, k):
  ...
  if bdim == operand.ndim - 1:
    ...
    return (transpose(top_k_v, perm), transpose(top_k_i, perm)), (bdim, bdim)
  else:
...
def _create_token_lowering(ctx, *operands):
  aval_out, = ctx.avals_out
  return chlo.CreateTokenOp(mlir.aval_to_ir_type(aval_out)).results
...
def infeed(token, shape=None, partitions=None):
  """Consumes an infeed value of `shape` from the host.
  ...
  xs_and_token = infeed_p.bind(
      token, shapes=tuple(flat_shapes), partitions=partitions)
...
def rng_uniform(a, b, shape):
  ...
  Experimental and its use is discouraged.
...
def _rng_uniform_abstract_eval(a, b, *, shape):
  ...
  return a.update(
      shape=shape, dtype=a.dtype, weak_type=(a.weak_type and b.weak_type))
...
def _rng_uniform_lowering(ctx, a, b, *, shape):
  aval_out, = ctx.avals_out
  shape, = mlir.ir_constants(
      np.array(aval_out.shape, np.int64), canonicalize_types=False)
...
def _rng_bit_generator_lowering(ctx, key, *, shape, dtype, algorithm):
  ...
  out_key, out_vals = mhlo.RngBitGeneratorOp(
      key.type, ir.RankedTensorType.get(shape, rbg_etype), algorithm_attr,
      key).results
  if key_etype == u32_type:
  ...
  if rbg_etype != etype:
    out_vals = mhlo.ConvertOp(
        ir.RankedTensorType.get(
            ir.RankedTensorType(out_vals.type).shape, etype), out_vals).result
...
rng_bit_generator_p.def_impl(partial(xla.apply_primitive, rng_bit_generator_p))
...
mlir.register_lowering(rng_bit_generator_p, _rng_bit_generator_lowering)
...
def _iota_lower(ctx, *dyn_shape, dtype, shape, dimension):
  ...
  if dyn_shape:
    ...
    return mhlo.IotaOp(
        mlir.aval_to_ir_type(aval_out), mlir.i64_attr(dimension)).results
...
def _iota_padding_rule(in_avals, out_avals, *dyn_shape, dtype, shape,
                       dimension):
  out_aval, = out_avals
  new_shape = []
  new_dyn_shape = []
...
def padtype_to_pads(in_shape, window_shape, window_strides, padding):
  ...
  if padding == PaddingType.SAME:
    ...
    pad_sizes = np.maximum(0, (out_shape - 1) * window_strides + window_shape -
                           in_shape)
...
def _check_same_dtypes(name, ignore_fp_precision, *ttypes):
  ...
  if ignore_fp_precision:
    types = [
        np.floating
        if dtypes.issubdtype(dtype, np.floating) else np.complexfloating
        if dtypes.issubdtype(dtype, np.complexfloating) else dtype
        for dtype in types
    ]
  if len({dtypes.canonicalize_dtype(t) for t in types}) != 1:
    if ignore_fp_precision:
      msg = (
          'lax.{} requires arguments to have same dtypes up to floating point '
          'precision, got {}.')
    else:
...
def _check_shapelike(fun_name, arg_name, obj, non_zero_shape=False):
  ...
  try:
    ...
    raise TypeError(msg.format(fun_name, arg_name, tuple(map(type,
                                                             obj)))) from err
  lower_bound, bound_error = ((1, 'strictly positive') if non_zero_shape else
                              (0, 'nonnegative'))
...
def canonicalize_precision(
    precision: PrecisionLike) -> Optional[Tuple[PrecisionType, PrecisionType]]:
  ...
  if precision is None:
    ...
    return type_cast(Tuple[PrecisionType, PrecisionType],
                     (precision, precision))
  elif (isinstance(precision, (list, tuple)) and len(precision) == 2 and all(
      isinstance(p, xla_client.PrecisionConfig.Precision) for p in precision)):
    ...
    s1, s2 = precision
    p1 = type_cast(Tuple[PrecisionType, PrecisionType],
                   canonicalize_precision(s1))[0]
    p2 = type_cast(Tuple[PrecisionType, PrecisionType],
                   canonicalize_precision(s2))[0]
    return (p1, p2)
  else:
    raise ValueError(
