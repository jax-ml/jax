...
@overload
def sort(operand: Sequence[Array],
         dimension: int = -1,
         is_stable: bool = True,
...
@overload
def sort(operand: Array,
         dimension: int = -1,
         is_stable: bool = True,
         num_keys: int = 1) -> Array:
  ...
...
def broadcast_xhlo(aval_out: core.ShapedArray,
                   avals: Sequence[core.ShapedArray],
                   args: Sequence[ir.Value]) -> Sequence[ir.Value]:
  """Broadcasts StableHLO values with broadcast-compatible shapes to the same shape.
  """
  out = []
...
def _nary_lower_xhlo(op: Callable,
                     ctx,
                     *args: Union[ir.Value, Sequence[ir.Value]],
                     explicit_type=False,
                     **params):
  """Lowers an elementwise operator to its StableHLO/CHLO equivalent.

  Args:
    explicit_type: does the StableHLO/CHLO operator require its output type to
      be provided?
  """
  del params
...
mlir.register_lowering(neg_p, partial(_nary_lower_xhlo, xhlo.NegOp))

sign_p = standard_unop(_num, 'sign')
ad.defjvp_zero(sign_p)


def _sign_lower_xhlo(ctx, x):
  x_aval, = ctx.avals_in
  if dtypes.issubdtype(x_aval.dtype, np.unsignedinteger):
    return xhlo.SelectOp(
        mlir.compare_xhlo(x, mlir.full_like_aval(ctx, 0, x_aval),
                          'EQ', 'UNSIGNED').result,
        mlir.full_like_aval(ctx, 0, x_aval),
        mlir.full_like_aval(ctx, 1, x_aval)).results
  return xhlo.SignOp(x).results


mlir.register_lowering(sign_p, _sign_lower_xhlo)

nextafter_p = standard_naryop([_float, _float], 'nextafter')
mlir.register_lowering(nextafter_p, partial(_nary_lower_xhlo, chlo.NextAfterOp))

floor_p = standard_unop(_float, 'floor')
ad.defjvp_zero(floor_p)
mlir.register_lowering(floor_p, partial(_nary_lower_xhlo, xhlo.FloorOp))

ceil_p = standard_unop(_float, 'ceil')
ad.defjvp_zero(ceil_p)
mlir.register_lowering(ceil_p, partial(_nary_lower_xhlo, xhlo.CeilOp))
...
def _round_lower(ctx, x, *, rounding_method):
  if rounding_method is RoundingMethod.AWAY_FROM_ZERO:
    return xhlo.RoundOp(x).results
  else:
    assert rounding_method is RoundingMethod.TO_NEAREST_EVEN
    return xhlo.RoundNearestEvenOp(x).results
...
mlir.register_lowering(is_finite_p, partial(_nary_lower_xhlo, xhlo.IsFiniteOp))
...
mlir.register_lowering(exp_p, partial(_nary_lower_xhlo, xhlo.ExpOp))

log_p = standard_unop(_float | _complex, 'log')
ad.defjvp(log_p, lambda g, x: div(g, x))
mlir.register_lowering(log_p, partial(_nary_lower_xhlo, xhlo.LogOp))

expm1_p = standard_unop(_float | _complex, 'expm1')
ad.defjvp2(expm1_p, lambda g, ans, x: mul(g, add(ans, _one(ans))))
mlir.register_lowering(expm1_p, partial(_nary_lower_xhlo, xhlo.Expm1Op))

log1p_p = standard_unop(_float | _complex, 'log1p')
ad.defjvp(log1p_p, lambda g, x: div(g, add(x, _one(x))))
mlir.register_lowering(log1p_p, partial(_nary_lower_xhlo, xhlo.Log1pOp))

tanh_p = standard_unop(_float | _complex, 'tanh')
ad.defjvp2(tanh_p, lambda g, ans, x: mul(add(g, mul(g, ans)),
                                         sub(_one(x), ans)))
mlir.register_lowering(tanh_p, partial(_nary_lower_xhlo, xhlo.TanhOp))

logistic_p = standard_unop(_float | _complex, 'logistic')
ad.defjvp2(logistic_p, lambda g, ans, x: mul(g, mul(ans, sub(_one(ans), ans))))
# TODO(phawkins): switch to stablehlo.logistic lowering; debug numerical problems.
# mlir.register_lowering(logistic_p, partial(_nary_lower_xhlo, xhlo.LogisticOp))
...
mlir.register_lowering(sin_p, partial(_nary_lower_xhlo, xhlo.SineOp))

cos_p = standard_unop(_float | _complex, 'cos')
ad.defjvp(cos_p, lambda g, x: neg(mul(g, sin(x))))
mlir.register_lowering(cos_p, partial(_nary_lower_xhlo, xhlo.CosineOp))
...
mlir.register_lowering(tan_p, partial(_nary_lower_xhlo, chlo.TanOp))
...
mlir.register_lowering(asin_p, partial(_nary_lower_xhlo, chlo.AsinOp))
...
mlir.register_lowering(atan_p, partial(_nary_lower_xhlo, chlo.AtanOp))
...
mlir.register_lowering(lgamma_p, partial(_nary_lower_xhlo, chlo.LgammaOp))

digamma_p = standard_unop(_float, 'digamma')
mlir.register_lowering(digamma_p, partial(_nary_lower_xhlo, chlo.DigammaOp))
...
mlir.register_lowering(bessel_i1e_p, partial(_nary_lower_xhlo,
                                             chlo.BesselI1eOp))
...
mlir.register_lowering(erf_p, partial(_nary_lower_xhlo, chlo.ErfOp))

erfc_p = standard_unop(_float, 'erfc')
ad.defjvp(erfc_p, lambda g, x: mul(_const(x, -2. / np.sqrt(np.pi)),
                                   mul(g, exp(neg(square(x))))))
mlir.register_lowering(erfc_p, partial(_nary_lower_xhlo, chlo.ErfcOp))
...
mlir.register_lowering(real_p, partial(_nary_lower_xhlo, xhlo.RealOp))

imag_p = unop(_complex_basetype, _complex, 'imag')
ad.deflinear2(imag_p, lambda t, _: [complex(np.zeros((), _dtype(t)), neg(t))])
mlir.register_lowering(imag_p, partial(_nary_lower_xhlo, xhlo.ImagOp))
...
mlir.register_lowering(complex_p, partial(_nary_lower_xhlo, xhlo.ComplexOp))
...
mlir.register_lowering(abs_p, partial(_nary_lower_xhlo, xhlo.AbsOp))
...
mlir.register_lowering(sqrt_p, partial(_nary_lower_xhlo, xhlo.SqrtOp))

rsqrt_p = standard_unop(_float | _complex, 'rsqrt')
ad.defjvp2(rsqrt_p,
           lambda g, ans, x:
           mul(g, mul(_const(x, -0.5), div(ans, x))))
mlir.register_lowering(rsqrt_p, partial(_nary_lower_xhlo, xhlo.RsqrtOp))

cbrt_p = standard_unop(_float, 'cbrt')
ad.defjvp2(cbrt_p,
           lambda g, ans, x: mul(g, mul(_const(x, 1/3), integer_pow(ans, -2))))
mlir.register_lowering(cbrt_p, partial(_nary_lower_xhlo, xhlo.CbrtOp))
...
mlir.register_lowering(pow_p, partial(_nary_lower_xhlo, xhlo.PowOp))
...
def _integer_pow_lowering(ctx, x, *, y):
  ...
  # expansion when the StableHLO is lowered to HLO, because the StableHLO
  # lowering clones the callee. Consider unconditionally caching when the
  # StableHLO->HLO lowering doesn't expand the program.
  if y >= 4:
...
mlir.register_lowering(not_p, partial(_nary_lower_xhlo, xhlo.NotOp))

and_p = standard_naryop([_bool_or_int, _bool_or_int], 'and')
ad.defjvp_zero(and_p)
mlir.register_lowering(and_p, partial(_nary_lower_xhlo, xhlo.AndOp))

or_p = standard_naryop([_bool_or_int, _bool_or_int], 'or')
ad.defjvp_zero(or_p)
mlir.register_lowering(or_p, partial(_nary_lower_xhlo, xhlo.OrOp))

xor_p = standard_naryop([_bool_or_int, _bool_or_int], 'xor')
ad.defjvp_zero(xor_p)
mlir.register_lowering(xor_p, partial(_nary_lower_xhlo, xhlo.XorOp))

population_count_p = standard_unop(_int, 'population_count')
mlir.register_lowering(population_count_p,
                       partial(_nary_lower_xhlo, xhlo.PopulationCountOp))

clz_p = standard_unop(_int, 'clz')
mlir.register_lowering(clz_p, partial(_nary_lower_xhlo, xhlo.ClzOp))
...
mlir.register_lowering(add_p, partial(_nary_lower_xhlo, xhlo.AddOp))
...
mlir.register_lowering(sub_p, partial(_nary_lower_xhlo, xhlo.SubtractOp))
...
mlir.register_lowering(mul_p, partial(_nary_lower_xhlo, xhlo.MulOp))
...
mlir.register_lowering(div_p, partial(_nary_lower_xhlo, xhlo.DivOp))
...
mlir.register_lowering(rem_p, partial(_nary_lower_xhlo, xhlo.RemOp))
...
mlir.register_lowering(max_p, partial(_nary_lower_xhlo, mlir.max_xhlo))

min_p: core.Primitive = standard_naryop([_any, _any], 'min')
ad.defjvp2(min_p,
           lambda g, ans, x, y: mul(g, _balanced_eq(x, ans, y)),
           lambda g, ans, x, y: mul(g, _balanced_eq(y, ans, x)))
mlir.register_lowering(min_p, partial(_nary_lower_xhlo, mlir.min_xhlo))

shift_left_p = standard_naryop([_int, _int], 'shift_left')
ad.defjvp_zero(shift_left_p)
mlir.register_lowering(shift_left_p, partial(_nary_lower_xhlo,
                                             xhlo.ShiftLeftOp))
...
mlir.register_lowering(shift_right_arithmetic_p,
                       partial(_nary_lower_xhlo, xhlo.ShiftRightArithmeticOp))
...
mlir.register_lowering(shift_right_logical_p,
                       partial(_nary_lower_xhlo, xhlo.ShiftRightLogicalOp))


def _compare_lower_xhlo(direction: str, ctx, x, y):
  avals_in, (aval_out,) = ctx.avals_in, ctx.avals_out
  ...
  return mlir.compare_xhlo(x, y, direction, compare_type).results

eq_p = naryop(_fixed_dtype(np.bool_), [_any, _any], 'eq')
ad.defjvp_zero(eq_p)
mlir.register_lowering(eq_p, partial(_compare_lower_xhlo, 'EQ'))

ne_p = naryop(_fixed_dtype(np.bool_), [_any, _any], 'ne')
ad.defjvp_zero(ne_p)
mlir.register_lowering(ne_p, partial(_compare_lower_xhlo, 'NE'))

ge_p = naryop(_fixed_dtype(np.bool_), [_ordered, _ordered], 'ge')
ad.defjvp_zero(ge_p)
mlir.register_lowering(ge_p, partial(_compare_lower_xhlo, 'GE'))

gt_p = naryop(_fixed_dtype(np.bool_), [_ordered, _ordered], 'gt')
ad.defjvp_zero(gt_p)
mlir.register_lowering(gt_p, partial(_compare_lower_xhlo, 'GT'))

le_p = naryop(_fixed_dtype(np.bool_), [_ordered, _ordered], 'le')
ad.defjvp_zero(le_p)
mlir.register_lowering(le_p, partial(_compare_lower_xhlo, 'LE'))

lt_p = naryop(_fixed_dtype(np.bool_), [_ordered, _ordered], 'lt')
ad.defjvp_zero(lt_p)
mlir.register_lowering(lt_p, partial(_compare_lower_xhlo, 'LT'))
...
def _convert_element_type_lower(ctx, operand, *, new_dtype, weak_type):
  ...
  if (dtypes.issubdtype(aval_in.dtype, np.complexfloating) and
      not dtypes.issubdtype(new_dtype, np.complexfloating)):
    operand = xhlo.RealOp(operand).result
    aval_in = aval_in.update(dtype=_real_dtype(aval_in.dtype))
  return [mlir.convert_xhlo(ctx, operand, aval_in, aval_out)]
...
def _bitcast_convert_type_lower(ctx, operand, *, new_dtype):
  aval_out, = ctx.avals_out
  return xhlo.BitcastConvertOp(mlir.aval_to_ir_type(aval_out), operand).results
...
def precision_attr(precision: PrecisionType) -> ir.ArrayAttr:
  ...
  return ir.ArrayAttr.get(
      [xhlo.PrecisionAttr.get(str(p)) for p in full_precision])
...
def _dot_general_lower(ctx, lhs, rhs, *, dimension_numbers,
  ...
  if ctx.module_context.platform == "cpu":
    if lhs_aval.dtype == np.float16:
      ...
      lhs = xhlo.ConvertOp(ir.RankedTensorType.get(lhs_aval.shape, f32),
                           lhs).result
    if rhs_aval.dtype == np.float16:
      f32 = mlir.dtype_to_ir_type(np.dtype(np.float32))
      rhs = xhlo.ConvertOp(ir.RankedTensorType.get(rhs_aval.shape, f32),
                           rhs).result
  dot_dnums = xhlo.DotDimensionNumbers.get(
  ...
  if xc.mlir_api_version < 40:
    ...
    # TODO(b/253644255): Improve type inference of DotGeneralOp to take
    # preferred_element_type into account.
    results = [mlir.aval_to_ir_type(aval_out)]
    operands = [lhs, rhs]
    attributes = {
        'dot_dimension_numbers': dot_dnums,
        'precision_config': precision_attr(precision)
    }
    return [
        xhlo.DotGeneralOp.build_generic(
...
mlir.register_lowering(clamp_p, partial(_nary_lower_xhlo, xhlo.ClampOp))
...
def _concatenate_lower(ctx, *xs, dimension):
  return xhlo.ConcatenateOp(xs, mlir.i64_attr(dimension)).results
...
def _pad_lower(ctx, x, padding_value, *, padding_config):
  ...
  return xhlo.PadOp(x, padding_value, mlir.dense_int_elements(low),
...
def _reshape_lower(ctx, x, *dyn_shape, new_sizes, dimensions):
  aval_out, = ctx.avals_out
  if dimensions is not None:
    x = xhlo.TransposeOp(x, mlir.dense_int_elements(dimensions)).result
  if dyn_shape:
...
def _rev_lower(ctx, x, *, dimensions):
  return xhlo.ReverseOp(x, mlir.dense_int_elements(dimensions)).results
...
def _transpose_lower(ctx, x, *, permutation):
  ...
  return xhlo.TransposeOp(x, mlir.dense_int_elements(permutation)).results
...
def _select_xhlo_lowering(ctx, which, *cases):
  which_aval = ctx.avals_in[0]
  if which_aval.dtype == np.dtype(np.bool_):
    assert len(cases) <= 2
    if len(cases) == 1: return cases
    return xhlo.SelectOp(which, cases[1], cases[0]).results
  ...
  def _select(offset, cases):
    ...
    if len(cases) == 1:
      return cases[0]
    mid = len(cases) // 2
    pred = mlir.compare_xhlo(which,
                             mlir.full_like_aval(ctx, offset + mid, which_aval),
                             lt, compare_type)
    return xhlo.SelectOp(pred, _select(offset, cases[:mid]),
...
mlir.register_lowering(select_n_p, _select_xhlo_lowering)
...
def _reduce_lower(ctx, *values, computation, jaxpr, consts, dimensions):
  ...
  op = xhlo.ReduceOp([mlir.aval_to_ir_type(aval) for aval in ctx.avals_out],
  ...
  with ir.InsertionPoint(reducer):
    ...
    xhlo.ReturnOp(util.flatten(out_nodes))
  return op.results
...
def _unary_reduce_lower(reducer, unit_factory, ctx, x, *, axes):
  ...
  op = xhlo.ReduceOp([mlir.aval_to_ir_type(aval_out)], [x],
  ...
  with ir.InsertionPoint(reducer_region):
    ...
    xhlo.ReturnOp(add.results)
  return op.results


mlir.register_lowering(
    reduce_sum_p, partial(_unary_reduce_lower, xhlo.AddOp, _get_sum_identity))
mlir.register_lowering(
    reduce_prod_p, partial(_unary_reduce_lower, xhlo.MulOp, _get_prod_identity))
mlir.register_lowering(
    reduce_or_p,
    partial(_unary_reduce_lower, xhlo.OrOp, _get_bitwise_or_identity))
mlir.register_lowering(
    reduce_and_p,
    partial(_unary_reduce_lower, xhlo.AndOp, _get_bitwise_and_identity))
mlir.register_lowering(
    reduce_xor_p,
    partial(_unary_reduce_lower, xhlo.XorOp, _get_bitwise_or_identity))
mlir.register_lowering(
    reduce_min_p, partial(_unary_reduce_lower, mlir.min_xhlo,
                          _get_min_identity))
mlir.register_lowering(
    reduce_max_p, partial(_unary_reduce_lower, mlir.max_xhlo,
                          _get_max_identity))
...
def _reduce_precision_lower(ctx, operand, *, exponent_bits, mantissa_bits):
  aval_out, = ctx.avals_out
  return xhlo.ReducePrecisionOp(operand, mlir.i32_attr(exponent_bits),
...
def _sort_lower(ctx, *operands, dimension, is_stable, num_keys):
  assert all(isinstance(x, core.ShapedArray) for x in ctx.avals_in), ctx.avals_in
  sort = xhlo.SortOp([mlir.aval_to_ir_type(aval) for aval in ctx.avals_out],
  ...
  with ir.InsertionPoint(comparator):
    ...
    out = lower_comparator(sub_ctx, *[[a] for a in comparator.arguments],
                           num_keys=num_keys)
    xhlo.ReturnOp(util.flatten(out))
  return sort.results
...
def _create_token_lowering(ctx, *operands):
  aval_out, = ctx.avals_out
  if xc.mlir_api_version < 40:
    return xhlo.CreateTokenOp(mlir.aval_to_ir_type(aval_out)).results
  else:
    return xhlo.CreateTokenOp().results
...
def _after_all_lowering(ctx, *operands):
  aval_out, = ctx.avals_out
  if xc.mlir_api_version < 40:
    return xhlo.AfterAllOp(mlir.aval_to_ir_type(aval_out), operands).results
  else:
    return xhlo.AfterAllOp(operands).results
...
def _infeed_lowering(ctx, token, *, shapes, partitions):
  ...
  infeed = xhlo.InfeedOp(
      flat_output_types + [xhlo.TokenType.get()],
      token,
...
def _rng_uniform_lowering(ctx, a, b, *, shape):
  ...
  return xhlo.RngOp(a, b, shape,
                    xhlo.RngDistributionAttr.get('UNIFORM')).results
...
def _rng_algorithm(algorithm: RandomAlgorithm):
  if algorithm == RandomAlgorithm.RNG_THREE_FRY:
    return xhlo.RngAlgorithmAttr.get('THREE_FRY')
  elif algorithm == RandomAlgorithm.RNG_PHILOX:
    return xhlo.RngAlgorithmAttr.get('PHILOX')
  elif algorithm == RandomAlgorithm.RNG_DEFAULT:
    return xhlo.RngAlgorithmAttr.get('DEFAULT')
  else:
    assert False
