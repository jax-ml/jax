...
import jax
from jax._src import ad_checkpoint, ad_util, api, config, core, dispatch, dtypes, effects
from jax._src import linear_util as lu
from jax._src import source_info_util, state, util
...
from jax._src.lax import lax, slicing, windowed_reductions
from jax._src.lax.control_flow.common import (
    _abstractify,
    _avals_short,
    _check_tree_and_avals,
    _initial_style_jaxpr,
    _initial_style_jaxpr_attrs,
    _make_closed_jaxpr_attrs,
    _prune_zeros,
    _typecheck_param,
)
...
from jax._src.util import merge_lists, partition_list, safe_map, safe_zip, split_list, unzip2, weakref_lru_cache
from jax.tree_util import keystr, tree_flatten, tree_flatten_with_path, tree_map, tree_unflatten, treedef_is_leaf
...
def scan(
    ...
    if length is not None:
        ...
        if not all(length == l for l in lengths):
            msg = "scan got `length` argument of {} which disagrees with " "leading axis sizes {}."
    ...
    if disallowed_effects:
        raise NotImplementedError(f"Effects not supported in `scan`: {disallowed_effects}")
...
def _check_scan_carry_type(body_fun, in_carry, out_carry_tree, out_avals):
    ...
    if carry_name:
        component = lambda p: (
            f"the input carry component {carry_name}{keystr(p)}" if p else f"the input carry {carry_name}"
        )
    else:
        component = lambda p: (f"the input carry at path {keystr(p)}" if p else "the input carry")
    ...
    if in_carry_tree != out_carry_tree:
        ...
        if out_carry is None:
            ...
            differences = "\n".join(
                ...
                for path, thing1, thing2, explanation in equality_errors(in_carry, out_carry)
            )
        raise TypeError(
...
def _aval_mismatch_extra(a1: core.AbstractValue, a2: core.AbstractValue) -> str:
    ...
    if isinstance(a1, core.ShapedArray) and isinstance(a2, core.ShapedArray):
        ...
        return (
            ", so " * (dtype_mismatch or shape_mismatch)
...
def _scan_impl_unrolled(*args, reverse, length, num_consts, num_carry, linear, f_impl, x_avals, y_avals):
...
def _scan_impl_loop(*args, reverse, length, num_consts, num_carry, linear, f_impl, x_avals, y_avals):
    consts, init, xs = split_list(args, [num_consts, num_carry])
...
def _scan_impl_block_unrolled(
    *args, reverse, length, num_consts, num_carry, linear, block_length, f_impl, x_avals, y_avals
):
    consts, init, xs = split_list(args, [num_consts, num_carry])
    ...
    outs = _scan_impl_loop(
        ...
        *xs_block,
        ...
        linear=linear,
        f_impl=f_impl_block,
        x_avals=x_block_avals,
        y_avals=y_block_avals,
    )
...
def _scan_impl(*args, reverse, length, num_consts, num_carry, jaxpr, linear, unroll, _split_transpose):
...
def _split_leading_dim(i, aval, x):
    assert x.ndim >= 1
    return (slicing.slice_in_dim(x, 0, i), slicing.slice_in_dim(x, i, x.shape[0]))
...
def _scan_abstract_eval(*args, reverse, length, num_consts, num_carry, jaxpr, linear, unroll, _split_transpose):
    carry_avals, y_avals = split_list(jaxpr.out_avals, [num_carry])
    ys_avals = _map(partial(_prepend_dim_to_aval, length), y_avals)
    return carry_avals + ys_avals, jaxpr.effects


def _scan_jvp(primals, tangents, reverse, length, jaxpr, num_consts, num_carry, linear, unroll, _split_transpose):
    ...
    for _ in range(1 + len(carry_nz)):
        ...
        jaxpr_jvp, nonzeros_out = ad.jvp_jaxpr(jaxpr, nonzeros, instantiate=carry_nz + [False] * num_ys)
    ...
    tangents = [ad.instantiate_zeros(t) if nz else t for t, nz in zip(tangents, nonzeros)]
    ...
    tangents_out = [
        next(tangents_out_iter) if nz else ad_util.Zero.from_value(p) for p, nz in zip(primals_out, nonzeros_out)
    ]
    return primals_out, tangents_out


def _scan_partial_eval(
    trace, *tracers, reverse, length, num_consts, num_carry, jaxpr, linear, unroll, _split_transpose
):
    num_ys = len(jaxpr.out_avals) - num_carry
    ...
    tracers = [trace.instantiate_const(t) if uk else t for t, uk in zip(tracers, unknowns)]
    ...
    jaxpr_unknown = pe.move_binders_to_back(jaxpr_unknown, [True] * num_res + [False] * sum(unknowns))
    ...
    const_pvals = [pe.PartialVal.known(t.pval.get_known()) for t in tracers[:num_consts] if t.pval.is_known()]
    other_pvals = [pe.PartialVal.unknown(aval) for aval in jaxpr_known.in_avals[len(const_pvals) :]]
    ...
    jaxpr_unknown = pe.move_binders_to_front(
        jaxpr_unknown, [False] * sum(unknowns) + [pval.is_known() for pval in res_pvals]
    )
    ...
    fwds_known = [
        (
            in_idx
            if out_idx >= num_carry - sum(carry_uk)
            and in_idx is not None
            and in_idx >= len(jaxpr_known_consts) + num_carry - sum(carry_uk)
            else None
        )
        for out_idx, in_idx in enumerate(fwds_known)
    ]
    ...
    jaxpr_known_ = jaxpr_known_.replace(outvars=[x for x, i in zip(jaxpr_known_.outvars, fwds_known) if i is None])
    ...
    known_inputs = list(jaxpr_known_consts) + [t.pval.get_known() for t in tracers[num_consts:] if t.pval.is_known()]
    if not jaxpr_known.out_avals and not jaxpr_known.effects:
        ...
        out_known = scan_p.bind(
            *known_inputs,
            ...
            length=length,
            jaxpr=jaxpr_known,
            num_consts=len(jaxpr_known_consts),
    ...
    out_known = [next(out_known_iter) if f is None else _maybe_put(known_inputs[f]) for f in fwds_known]
    ...
    ys_avals = [core.unmapped_aval(length, core.no_axis_name, 0, y_aval) for y_aval in y_avals]
    out_tracers = [
        pe.JaxprTracer(trace, pe.PartialVal.unknown(a), None) for a in itertools.chain(carry_avals, ys_avals)
    ]
    del carry_avals, y_avals
    # Create equation.
    linear_unknown = tuple(
        [False] * len(intensive_res) + [l for l, uk in zip(linear, unknowns) if uk] + [False] * len(extensive_res)
    )
    name_stack = source_info_util.current_name_stack()[len(trace.name_stack) :]
    source = source_info_util.current().replace(name_stack=name_stack)
    assert len(out_tracers) == len(jaxpr_unknown.out_avals)
    eqn = pe.new_eqn_recipe(
        [*intensive_res, *unknown_inputs, *extensive_res],
        out_tracers,
        scan_p,
        dict(
            reverse=reverse,
...
def _scan_transpose(cts, *args, reverse, length, num_consts, num_carry, jaxpr, linear, unroll, _split_transpose):
    ...
    jaxpr_trans, attrs_tracked = _transpose_scan_jaxpr(jaxpr, num_ires, num_consts - num_ires, num_eres, ct_ys_is_zeros)
    linear_trans = (
        [False] * num_ires
        + [False] * len(attrs_tracked)
    ...
    if not _split_transpose:
        ...
        inst_mask = [False] * transpose_num_out_carry + [True] * (len(jaxpr_trans.out_avals) - transpose_num_out_carry)

        unknowns_mask = [False] * (len(transpose_inputs) - len(eres)) + [True] * len(eres)
        ...
        jaxpr_known_body, jaxpr_unknown_body, outs_mask, res_avals = pe.partial_eval_jaxpr_nounits(
            jaxpr_trans, unknowns_mask, inst_mask
        )

        num_knowns = len(outs_mask) - sum(outs_mask)

        linear_list = list(linear_trans)
        known_linear = [l for mask, l in zip(unknowns_mask, linear_list) if not mask]
        unknown_linear = [l for mask, l in zip(unknowns_mask, linear_list) if mask]
        unknown_linear = [False] * len(res_avals) + unknown_linear

        known_args = [arg for mask, arg in zip(unknowns_mask, transpose_inputs) if not mask]
        unknown_args = [arg for mask, arg in zip(unknowns_mask, transpose_inputs) if mask]
        ...
        outs = [next(known_results_iter) if not mask else next(unknown_results_iter) for mask in outs_mask]
...
@weakref_lru_cache
def _transpose_scan_jaxpr(jaxpr, num_res1, num_c, num_res2, ct_ys_is_zeros):
    ...
    res1_avals, c_avals, a_avals, res2_avals = split_list(jaxpr.in_avals, [num_res1, num_c, num_a])
    ...
    b_ys_avals_stripped = [aval for aval, is_zero in zip(b_ys_avals, ct_ys_is_zeros) if not is_zero]

    @lu.wrap_init
    def transposed(*res1_cbar_bbar_res2):
        res1, c_bar, b_bar, ys_bar_stripped, res2 = split_list(
            res1_cbar_bbar_res2, [num_res1, num_c, num_b, len(b_ys_avals_stripped)]
        )
        ...
        primals = (
            res1
            + [ad.UndefinedPrimal(aval) for aval in c_avals]
            + [ad.UndefinedPrimal(aval) for aval in a_avals]
            + res2
        )
        cbar_abar = ad.backward_pass(jaxpr.jaxpr, False, jaxpr.consts, primals, b_bar + ys_bar)
    ...
    return _make_closed_jaxpr_attrs(
        transposed, tuple(res1_avals + c_avals + b_carry_avals + b_ys_avals_stripped + res2_avals)
    )


def _scan_batching_rule(
    spmd_axis_name,
    axis_size,
    axis_name,
    main_type,
    ...
    new_init = [
        (
            batching.broadcast(x, axis_size, 0)
            if now_batched and not was_batched
            else batching.moveaxis(x, d, 0) if now_batched else x
        )
        for x, d, was_batched, now_batched in zip(init, init_bdims, init_batched, carry_batched)
    ]
    new_xs = [
        batching.moveaxis(x, d, 1) if d is not batching.not_mapped and d != 1 else x for x, d in zip(xs, xs_bdims)
    ]
...
def _scan_dce_rule(used_outputs: list[bool], eqn: core.JaxprEqn) -> tuple[list[bool], core.JaxprEqn]:
    ...
    for i in range(1 + num_carry):
        ...
        jaxpr_dce, used_inputs = pe.dce_jaxpr(
            jaxpr.jaxpr, used_outputs, instantiate=[False] * num_consts + used_carry_out + [False] * num_xs
        )
        used_consts, used_carry_in, used_extensive_in = split_list(used_inputs, [num_consts, num_carry])
    ...
    _, new_effects = eqn.primitive.abstract_eval(*[v.aval for v in new_invars], **new_params)
    new_eqn = pe.new_jaxpr_eqn(new_invars, new_outvars, eqn.primitive, new_params, new_effects, eqn.source_info)
...
def _scan_partial_eval_custom(saveable, unks_in, inst_in, eqn):
    ...
    for _ in range(1 + len(carry_uk)):
        ...
        jaxpr_known_, jaxpr_staged_, unks_out, inst_out, num_res = pe.partial_eval_jaxpr_custom(
            jaxpr.jaxpr,
            in_unknowns=unks_in,
            in_inst=True,
            ensure_out_unknowns=carry_uk + [False] * num_ys,
            ensure_out_inst=True,
            saveable=saveable,
        )
    ...
    jaxpr_staged = pe.move_binders_to_back(jaxpr_staged, [True] * num_res + [False] * len(jaxpr.in_avals))
    ...
    new_inst = [x for x, inst in zip(eqn.invars, inst_in) if type(x) is core.Var and not inst]
    ...
    jaxpr_known_hoist, jaxpr_known_loop, loop_dep, consts_known_lp_avals = pe.partial_eval_jaxpr_nounits(
        jaxpr_known,
    ...
    jaxpr_staged = pe.move_binders_to_front(jaxpr_staged, [False] * sum(inst_in) + _map(operator.not_, loop_dep_res))
    ...
    ext_avals = [core.unmapped_aval(eqn.params["length"], core.no_axis_name, 0, a) for a in ext_avals_mapped]
    newvar = core.gensym()
    ...
    call_jaxpr_, _, call_jaxpr_consts, () = pe.trace_to_jaxpr_dynamic(known, [v.aval for v in ins_known])
    call_jaxpr = core.ClosedJaxpr(call_jaxpr_, call_jaxpr_consts)
    eqn_known = pe.new_jaxpr_eqn(
        ins_known,
        [*intensive_res, *out_binders_known, *extensive_res],
        core.closed_call_p,
    ...
    linear_staged = [False] * len(intensive_res) + list(eqn.params["linear"]) + [False] * len(extensive_res)
    params_staged = dict(
        eqn.params,
        jaxpr=jaxpr_staged,
    ...
    eqn_staged = pe.new_jaxpr_eqn(
        [*intensive_res, *eqn.invars, *extensive_res],
        out_binders_staged,
        eqn.primitive,
        params_staged,
...
def _scan_typecheck(
    bind_time, *in_atoms, reverse, length, num_consts, num_carry, jaxpr, linear, unroll, _split_transpose
):
    del _split_transpose
    if not bind_time:
        _, *in_atoms = in_atoms
    ...
    tc(num_consts, "num_consts", "non-negative int", type(num_consts) is int and num_consts >= 0)
    tc(num_carry, "num_carry", "non-negative int", type(num_carry) is int and num_carry >= 0)
    tc(jaxpr, "jaxpr", "ClosedJaxpr", type(jaxpr) is core.ClosedJaxpr)
    tc(linear, "linear", "tuple of bool", type(linear) is tuple and all(type(x) is bool for x in linear))
    ...
    if len(linear) != len(avals):
        raise core.JaxprTypeError(f"scan param linear has length {len(linear)} for {len(avals)} operands")

    const_avals, init_avals, x_avals = split_list(avals, [num_consts, num_carry])
    const_avals_jaxpr, init_avals_jaxpr, x_avals_jaxpr = split_list(jaxpr.in_avals, [num_consts, num_carry])
    ...
    y_avals = [core.unmapped_aval(length, core.no_axis_name, 0, a) for a in y_avals_mapped]
...
def _scan_state_discharge_rule(
    in_avals, out_avals, *args, jaxpr, num_consts, num_carry, linear, unroll, reverse, length, _split_transpose
):
    jaxpr, consts = jaxpr.jaxpr, jaxpr.consts
    if consts:
        raise NotImplementedError
    consts, carry, xs = split_list(args, [num_consts, num_carry])
    consts_linear, carry_linear, xs_linear = split_list(linear, [num_consts, num_carry])
    consts_avals, carry_avals, xs_avals = split_list(in_avals, [num_consts, num_carry])
    ...
    if discharged_consts:
        raise NotImplementedError(
            "Discharged jaxpr has consts. If you see this, "
    ...
    def wrapped(*refs_and_args):
        consts, refs, carry, xs = split_list(refs_and_args, [num_remaining_consts, num_refs, num_carry])
        consts_with_refs = merge_lists(is_ref, consts, refs)
        outs_and_refs = core.eval_jaxpr(discharged_jaxpr, (), *consts_with_refs, *carry, *xs)
        carry, ys, out_refs = split_list(outs_and_refs, [num_carry, num_extensive_out])
        assert len(out_refs) == num_refs
        return [*out_refs, *carry, *ys]

    new_in_avals = [
        *remaining_const_avals,
        *[a.inner_aval for a in in_ref_avals],
        *carry_avals,
        *[core.mapped_aval(length, 0, a) for a in xs_avals],
    ]
    new_jaxpr, _, (), () = pe.trace_to_jaxpr_dynamic(lu.wrap_init(wrapped), new_in_avals)
    new_linear = (*remaining_consts_linear, *in_refs_linear, *carry_linear, *xs_linear)
    all_out = scan_p.bind(
        *remaining_consts,
        *in_refs,
        *carry,
    ...
    new_invals = [
        *merge_lists(is_ref, [None] * num_remaining_consts, refs_out),
        *[None] * num_carry,
        *[None] * num_extensive_in,
    ]
...
mlir.register_lowering(scan_p, mlir.lower_fun(_scan_impl, multiple_results=True))
...
def while_loop(cond_fun: Callable[[T], BooleanNumeric], body_fun: Callable[[T], T], init_val: T) -> T:
    ...
    def _create_jaxpr(init_val):
        ...
        if not isinstance(pred_aval, ShapedArray) or pred_aval.strip_weak_type().strip_named_shape() != ShapedArray(
            (), np.bool_
        ):
    ...
    _check_tree_and_avals("body_fun output and input", body_tree, body_jaxpr.out_avals, in_tree_children[0], init_avals)
    ...
    if disallowed_effects:
        raise NotImplementedError(f"Effects not supported in `while`: {disallowed_effects}")
    outs = while_p.bind(
        *cond_consts,
        *body_consts,
        *init_vals,
...
def _join_while_effects(body_jaxpr, cond_jaxpr, body_nconsts, cond_nconsts) -> effects.Effects:
    joined_effects = set()
...
def _while_loop_abstract_eval(*avals, cond_jaxpr, body_jaxpr, body_nconsts, cond_nconsts):
    del avals
    joined_effects = _join_while_effects(body_jaxpr, cond_jaxpr, body_nconsts, cond_nconsts)
    disallowed_effects = effects.control_flow_allowed_effects.filter_not_in(joined_effects)
    if disallowed_effects:
        raise NotImplementedError(f"Effects not supported in `while`: {disallowed_effects}")
...
def _while_loop_batching_rule(
    spmd_axis_name, axis_size, axis_name, main_type, args, dims, cond_nconsts, cond_jaxpr, body_nconsts, body_jaxpr
):
    ...
    for _ in range(1 + len(carry_bat)):
        _, carry_bat_out = batching.batch_jaxpr(
            body_jaxpr,
            axis_size,
            bconst_bat + carry_bat,
            instantiate=carry_bat,
            axis_name=axis_name,
            spmd_axis_name=spmd_axis_name,
            main_type=main_type,
        )
    ...
    if pred_bat:
        ...
        if any(_IOEffect in fn.effects for fn in [body_jaxpr, cond_jaxpr]):
            raise Exception("Unordered IO effects not supported in while_loop " "with batched predicate")
        carry_bat = [True] * len(carry_bat)
        carry_dims = [0] * len(carry_bat)
        body_jaxpr_batched, _ = batching.batch_jaxpr_axes(
            body_jaxpr,
            axis_size,
            bconst_dims + carry_dims,
            carry_dims,
            axis_name=axis_name,
            spmd_axis_name=spmd_axis_name,
            main_type=main_type,
...
def _while_loop_jvp(primals, tangents, cond_nconsts, cond_jaxpr, body_nconsts, body_jaxpr):
    nonzeros = [type(t) is not ad_util.Zero for t in tangents]
    ...
    for _ in range(1 + len(carry_nz)):
        ...
        body_jvp, nonzeros_out = ad.jvp_jaxpr(body_jaxpr, body_nonzeros, instantiate=carry_nz)
    ...
    body_jvp_rearranged = ad.rearrange_binders(
        body_jvp, [body_nconsts, num_carry], [len(bconst_dot), len(init_dot)], [num_carry], [len(init_dot)]
    )

    newvar = core.gensym()
    invars_aug = cond_jaxpr.jaxpr.invars + [newvar(core.get_aval(x)) for x in init_dot]
    cond_jaxpr_augmented = core.Jaxpr(
        cond_jaxpr.jaxpr.constvars,
        invars_aug,
    ...
    out_tangents = [
        next(out_tangents_iter) if nz else ad_util.Zero.from_value(p) for p, nz in zip(out_carry, nonzeros_out)
    ]
    return out_carry, out_tangents


def _while_partial_eval(
    trace: pe.JaxprTrace,
    *tracers: pe.Tracer,
    cond_nconsts: int,
    cond_jaxpr: pe.ClosedJaxpr,
    body_nconsts: int,
    body_jaxpr: pe.ClosedJaxpr,
) -> Sequence[pe.Tracer]:
    ...
    in_consts = [t.pval.get_known() for uk, t in zip(cond_consts_uk + body_consts_uk + carry_uk, tracers) if not uk]
    ...
    body_jaxpr_known = body_jaxpr_known.replace(
        jaxpr=body_jaxpr_known.jaxpr.replace(outvars=body_jaxpr_known.jaxpr.outvars[:num_known_outs])
    )
    out_known = while_p.bind(
        *in_consts,
        cond_nconsts=cond_nconsts_known,
        cond_jaxpr=cond_jaxpr_known,
        body_nconsts=body_nconsts_known,
...
def _while_partial_eval_custom(saveable, unks_in, inst_in, eqn):
    ...
    cond_consts_uk, body_consts_uk, carry_init_uk = split_list(unks_in, [cond_nconsts, body_nconsts])
    ...
    for _ in range(1 + len(carry_uk)):
        ...
        jaxpr_known_, _, carry_uk_out, _, num_res = pe.partial_eval_jaxpr_custom(
            body_jaxpr.jaxpr,
            in_unknowns=body_unks_in,
            in_inst=True,
            ensure_out_unknowns=carry_uk,
            ensure_out_inst=True,
    ...
    cond_jaxpr_known_, _, [cond_uk], _, _ = pe.partial_eval_jaxpr_custom(
        cond_jaxpr.jaxpr,
        cond_unks_in,
        in_inst=True,
    ...
    effects_known = core.join_effects(cond_jaxpr_known.effects, body_jaxpr_known.effects)
    eqn_known = pe.new_jaxpr_eqn(ins_known, out_binders_known, while_p, params_known, effects_known, eqn.source_info)
...
def _while_transpose_error(*_, **kwargs):
    raise ValueError(
        "Reverse-mode differentiation does not work for "
...
#   token, pred = cond(token, x)
# ```
def _while_lowering(ctx, *args, cond_jaxpr, body_jaxpr, cond_nconsts, body_nconsts):
    pred_aval = cond_jaxpr.out_avals[0]
    ...
    with ir.InsertionPoint(cond_block):
        flat_cond_args = [cond_block.arguments[i] for i in range(len(flat_loop_carry_types))]
        ...
        cond_consts = [mlir.ir_constants(xla.canonicalize_dtype(x)) for x in cond_jaxpr.consts]
        ...
        if batched:
            ...
            (pred,) = lax._unary_reduce_lower(
                hlo.OrOp, lambda dtype: np.array(False, dtype), pred_ctx, pred, axes=tuple(range(len(pred_aval.shape)))
            )
        hlo.return_([pred])
    ...
    with ir.InsertionPoint(body_block):
        flat_body_args = [body_block.arguments[i] for i in range(len(flat_loop_carry_types))]
        ...
        body_consts = [mlir.ir_constants(xla.canonicalize_dtype(x)) for x in body_jaxpr.consts]
        ...
        if batched:
            ...
            ((body_pred,),), _ = mlir.jaxpr_subcomp(
                ctx.module_context,
                cond_jaxpr.jaxpr,
                body_pred_name_stack,
                mlir.TokenSet(),
                cond_consts,
                *(x + z),
                dim_var_values=ctx.dim_var_values,
            )
            new_z = _map(partial(_pred_bcast_select_hlo, ctx, pred_aval, body_pred), new_z, z, body_jaxpr.out_avals)

        hlo.return_([*util.flatten(out_tokens), *util.flatten(x), *util.flatten(y), *util.flatten(new_z)])
...
def _while_typecheck(_, *in_atoms, cond_jaxpr, body_jaxpr, cond_nconsts, body_nconsts):
...
def _while_discharge_rule(in_avals, out_avals, *args, cond_jaxpr, body_jaxpr, cond_nconsts, body_nconsts):
    ...
    cond_consts_avals, body_consts_avals, carry_avals = split_list(in_avals, [cond_nconsts, body_nconsts])
    ...
    remaining_body_const_avals, ref_avals = partition_list(is_ref, body_consts_avals)
    num_refs = sum(is_ref)
    ...
    if body_jaxpr_consts:
        raise NotImplementedError(
            "Body jaxpr has consts. If you see this error, "
            "please open an issue at "
            "https://github.com/google/jax/issues"
        )
    ...
    discharged_body_jaxpr, discharged_consts = state_discharge.discharge_state(body_jaxpr, ())
    if discharged_consts:
        raise NotImplementedError

    def new_body(*consts_refs_carry):
        consts, refs, carry = split_list(consts_refs_carry, [num_remaining_consts, num_refs])
        consts_and_refs = merge_lists(is_ref, consts, refs)
        carry_refs = core.eval_jaxpr(discharged_body_jaxpr, (), *consts_and_refs, *carry)
    ...
    new_body_jaxpr, _, new_body_consts, () = pe.trace_to_jaxpr_dynamic(
        lu.wrap_init(new_body), [*remaining_body_const_avals, *[a.inner_aval for a in ref_avals], *carry_avals]
    )
    if new_body_consts:
        raise NotImplementedError
    ...
    def new_cond(*consts_refs_carry):
        consts, refs, carry = split_list(consts_refs_carry, [cond_nconsts, num_refs])
    ...
    new_cond_jaxpr, _, new_cond_consts, () = pe.trace_to_jaxpr_dynamic(
        lu.wrap_init(new_cond), [*cond_consts_avals, *[a.inner_aval for a in ref_avals], *carry_avals]
    )
    if new_cond_consts:
        raise NotImplementedError
    ...
    updated_body_consts = merge_lists(is_ref, [None] * num_remaining_consts, refs_out)
    invals_out = [*[None] * cond_nconsts, *updated_body_consts, *[None] * num_carry]
...
def _pred_bcast_select_hlo(
    ctx,
    pred_aval: core.ShapedArray,
    pred: ir.Value,
    xs: Sequence[ir.Value],
    ys: Sequence[ir.Value],
    x_y_aval: core.AbstractValue,
) -> Sequence[ir.Value]:
    if x_y_aval is core.abstract_token:
        ...
        assert pred_aval.shape == x_y_aval.shape[: len(pred_aval.shape)], (pred_aval.shape, x_y_aval)
        x_y_aval = core.physical_aval(x_y_aval)
        bcast_pred = mlir.broadcast_in_dim(
            ctx,
            pred,
            core.DShapedArray(x_y_aval.shape, np.dtype(np.bool_)),
...
def fori_loop(lower, upper, body_fun, init_val, *, unroll: int | bool | None = None):
    ...
    if lower_dtype == upper_dtype:
        ...
        # scalar.
        dtype = None
        if np.issubdtype(lower_dtype, np.signedinteger) and np.issubdtype(upper_dtype, np.signedinteger):
        ...
        if dtype is None:
            raise TypeError(
                "lower and upper arguments to fori_loop must have equal "
    ...
    if isinstance(core.get_aval(lower), ConcreteArray) and isinstance(core.get_aval(upper), ConcreteArray):
        try:
            lower_ = int(lower)
            upper_ = int(upper)
        except TypeError:
    ...
    if unroll is not None:
        raise ValueError("Can only use `unroll` in `fori_loop` if the loop bounds " "are statically known.")
    ...
    _, _, result = while_loop(_fori_cond_fun, _fori_body_fun(body_fun), (lower, upper, init_val))
    return result
...
def _rng_bit_generator_batching_rule(batched_args, batch_dims, *, shape, dtype, algorithm):
    ...
    if bd is batching.not_mapped:
        return lax.rng_bit_generator_p.bind(keys, shape=shape, dtype=dtype, algorithm=algorithm), (None, None)
    keys = batching.moveaxis(keys, bd, 0)
    batch_size = keys.shape[0]
    key = keys[0]
    new_key, bits = lax.rng_bit_generator_p.bind(key, shape=(batch_size, *shape), dtype=dtype, algorithm=algorithm)
...
def associative_scan(fn: Callable, elems, reverse: bool = False, axis: int = 0):
    ...
    def _scan(elems):
        ...
        if num_elems % 2 == 0:
            ...
            even_elems = combine(odd_elems, [slicing.slice_in_dim(e, 2, None, stride=2, axis=axis) for e in elems])

        # The first element of a scan is the same as the first element
        # of the original `elems`.
        even_elems = [
            lax.concatenate([slicing.slice_in_dim(elem, 0, 1, axis=axis), result], dimension=axis)
...
def cumred_gpu_impl(window_reduce: Callable, reduce_fn: Callable, x, *, axis: int, reverse: bool):
    ...
    if not core.is_constant_dim(x.shape[axis]):
        raise NotImplementedError(
            "associative scan reductions not implemented with shape polymorphism " "and native serialization on GPU"
        )
    if x.shape[axis] > 32:
...
def _cumred_batch_rule(prim, batched_args, batch_dims, *, axis: int, reverse: bool):
...
def _cumred_dtype_rule(name, operand, *args, **kw):
    if not dtypes.issubdtype(operand.dtype, np.number):
        raise TypeError(
            "{} does not accept dtype {}. Accepted dtypes are subtypes "
...
def _cumulative_reduction_primitive(name, reduce_fn, reduce_window_fn):
    reducer_p = lax.standard_primitive(_cumred_shape_rule, partial(_cumred_dtype_rule, name), name)
    batching.primitive_batchers[reducer_p] = partial(_cumred_batch_rule, reducer_p)

    def register_lowering(fn, platform=None):
        mlir.register_lowering(
            reducer_p, mlir.cache_lowering(mlir.lower_fun(fn, multiple_results=False)), platform=platform
        )
    ...
    for platform in ["cuda", "rocm"]:
        register_lowering(partial(cumred_gpu_impl, reduce_window_fn, reduce_fn), platform)
...
def _cumulative_jvp_rule(primals, tangents, *, axis: int, reverse: bool, combine_fn: Callable):
    # Irrespective of backend, we always use the parallel prefix scan
    # implementation when differentiating because reduce_window is not
    # arbitrarily differentiable.
    return api.jvp(partial(associative_scan, combine_fn, axis=axis, reverse=reverse), primals, tangents)
