...
def _scan_state_partial_discharge_rule(should_discharge, in_avals, out_avals, *args, jaxpr, num_consts,
  ...
  # Get the inner value for the consts that we need to discharge. Leave the rest as-is.
  deref_consts = tuple([c.inner_aval for c in avals_for_wrapped[n_val_consts + n_carry:n_consts + n_carry]])
  avals_for_wrapped_no_refs = avals_for_wrapped[:n_val_consts + n_carry] + deref_consts + avals_for_wrapped[n_consts + n_carry:]

  val_consts, carry_in, ref_consts_in, val_xs, ref_xs_in = split_list_checked(avals_for_wrapped_no_refs,
  ...
  carry_out, ys, ref_consts_out, ref_xs_out = split_list_checked(discharged_jaxpr.outvars,
    [n_carry, n_ys, n_ref_consts, n_ref_xs])
  new_jaxpr = core.Jaxpr(constvars=(), invars=[*consts, *carry_in, *xs], outvars=[*carry_out, *ref_consts_out, *ys, *ref_xs_out], eqns=discharged_jaxpr.eqns)
