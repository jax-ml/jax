...
def switch(index, branches: Sequence[Callable], *operands,
  ...
  for i, (out_tree, jaxpr) in enumerate(zip(out_trees[1:], jaxprs[1:])):
    _check_branch_outputs("branch 0", f"branch{i+1}", branches[0], branches[i+1],
...
def _cond(pred, true_fun: Callable, false_fun: Callable, *operands,
  ...
  _check_branch_outputs(
      'true_fun', 'false_fun', true_fun, false_fun, out_tree, false_out_tree,
      true_jaxpr.out_avals, false_jaxpr.out_avals)
...
def _check_branch_outputs(
    name1, name2, f1, f2, out_tree1, out_tree2, out_avals1, out_avals2) -> None:
  ...
  if out_tree1 != out_tree2:
    ...
    raise TypeError(
        'cond branch outputs must have the same pytree structure, but they '
        'differ:\n\n'
        f'{name1} is {info1}\n' + f'{name2} is {info2}\n\n'
        f'{differences}\n'
