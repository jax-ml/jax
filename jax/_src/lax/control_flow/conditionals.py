...
def _cond_linearize(is_vjp, nzs, *primals_in, branches, **params):
  ...
  branch_sres_avals = FlatTree.pack(tuple(branch_sres_avals))
...
def _cond_partial_eval(trace, *tracers, branches, **params):
  ...
  dummy = FlatTree.flatten([[]] * len(branches_known))
...
def _cond_partial_eval_custom(saveable, unks_in, inst_in, eqn):
  ...
  dummy = FlatTree.flatten([[]] * len(branches_known_))
  branches_known = _join_cond_outputs(
...
def _join_cond_outputs(
  ...
  def augment_jaxpr(jaxpr, ures_indices, idx):
    def f_aug(*args):
      ...
      aug_ures = map(ad_util.zeros_like_aval, merged_ures_avals)
      aug_ures = util.subvals(aug_ures, zip(ures_indices, ures))
      aug_sres = sres_avals.map(ad_util.zeros_like_aval).unpack()
...
def _join_cond_pe_staged_jaxpr_inputs(
  ...
  def augment_jaxpr(jaxpr, ures_indices, idx):
    ...
    return core.ClosedJaxpr(jaxpr.jaxpr.replace(invars=invars), jaxpr.consts)
