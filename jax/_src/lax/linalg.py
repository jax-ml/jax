...
def linalg_batching_rule(prim, batched_args, batch_dims, **params):
  batched_args = [batching.moveaxis(x, bd, 0)
                  for x, bd in zip(batched_args, batch_dims)]
  out = prim.bind(*batched_args, **params)
  return (out, (0,) * len(out)) if prim.multiple_results else (out, 0)

def register_linalg_batching(prim):
  batching.primitive_batchers[prim] = partial(linalg_batching_rule, prim)
