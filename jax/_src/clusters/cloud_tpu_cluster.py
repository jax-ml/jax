...
class BaseTpuCluster(clusters.ClusterEnv):
  ...
  def get_coordinator_address(cls) -> str:
    ...
    logger.debug(
        "TPU Cluster using coordinator address: %s", coordinator_address)
  ...
  def get_process_count(cls) -> int:
    ...
    logger.debug(
        'Total process count of %s = %s processes per slice and %s slices',
        total_process_count, processes_per_slice, num_slices)
  ...
  def get_process_id(cls) -> int:
    ...
    logger.debug(
        "Process ID of %s generated by within-slice id %s and slice id %s",
        process_id, process_id_in_slice, slice_id)
    return process_id
...
class GceTpuCluster(BaseTpuCluster):
  ...
  def is_env_present(cls) -> bool:
    ...
    if metadata_code == metadata_response_code_success:
      ...
      logger.debug(
          'Did not detect Gce Tpu Cluster since agent-worker-number is not set'
          ' in metadata')
...
class GkeTpuCluster(BaseTpuCluster):
  ...
  @classmethod
  def is_env_present(cls) -> bool:
    if (running_in_cloud_tpu_vm and
      ...
      if not running_in_cloud_tpu_vm:
        ...
        logger.debug(
            'Did not detect TPU GKE cluster since TPU_WORKER_HOSTNAMES is'
            ' not set')
      return False
