...
def _norm_jvp(ord, axis, keepdims, primals, tangents):
  ...
  out = norm(x, ord=ord, axis=axis, keepdims=keepdims)
  if ord is None or ord in ("f", "fro", 2):
    denom = out
    if not keepdims and axis is not None:
      denom = jnp.expand_dims(denom, axis)
    out_grad = reductions.sum(
        ufuncs.real(dx * ufuncs.conj(x)) / jnp.where(denom != 0, denom, 1),
...
def _vector_norm_jvp(axis, keepdims, ord, primals, tangents):
  ...
  if ord is None or ord == 2:
    ...
    return jax.jvp(partial(__vector_norm, axis=axis, keepdims=keepdims, ord=ord),
                   primals, tangents)
