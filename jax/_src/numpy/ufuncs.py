...
def _replace_inf(x):
  return _where(isposinf(real(x)), lax._zeros(x), x)
...
def _comparison_op(numpy_fn, lax_fn):
  ...
  def fn(x1, x2):
    ...
    if dtypes.issubdtype(dtypes.dtype(x1), np.complexfloating):
      rx = lax.real(x1)
      ry = lax.real(x2)
      return _where(lax.eq(rx, ry), lax_fn(lax.imag(x1), lax.imag(x2)),
                        lax_fn(rx, ry))
    return lax_fn(x1, x2)
  return _wraps(numpy_fn)(fn)
...
def _float_divmod(x1, x2):
  ...
  mod = _where(ind, mod + x2, mod)
  div = _where(ind, div - _constant_like(div, 1), div)
...
def logaddexp(x1, x2):
  ...
  if dtypes.issubdtype(x1.dtype, np.floating):
    delta = lax.sub(x1, x2)
    return _where(lax._isnan(delta),
...
def _wrap_between(x, _a):
  ...
  rem = _where(lax.lt(rem, zero), lax.add(rem, two_a), rem)
...
def remainder(x1, x2):
  ...
  return _where(do_plus, lax.add(trunc_mod, x2), trunc_mod)
