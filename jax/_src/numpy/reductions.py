...
def check_mean(name: str, mean: ArrayLike | None) -> Array | None:
  if mean is None:
    return None
  mean = ensure_arraylike(name, mean)
  if not dtypes.issubdtype(mean.dtype, np.inexact):
    raise ValueError(f"jnp.{name}: mean must have a floating-point or complex dtype; got {mean.dtype=}.")
  return mean
...
def var(a: ArrayLike, axis: Axis = None, dtype: DTypeLike | None = None,
        ...
        where: ArrayLike | None = None, correction: int | float | None = None,
        mean: ArrayLike | None = None) -> Array:
  ...
  Args:
    ...
    mean: optional, mean of the input array, computed along the given axis.
      ...
      computing it from the input array.
    out: Unused by JAX.
