...
from jax._src.util import safe_zip, safe_map, set_module, canonicalize_axis
...
@export
def size(a: ArrayLike | SupportsSize | SupportsShape, axis: int | tuple[int, ...] | None = None) -> int:
  ...
  Args:
    ...
    axis: optional integer or tuple of integers indicating which axis or axes to count
  ...
  if axis is None:
    if hasattr(a, "size"):
      ...
      return np.size(a)  # type: ignore[arg-type]
  else:
    if hasattr(a, "shape"):
      _shape = a.shape
    else:
      _shape = np.shape(a)  # type: ignore[arg-type]

    if isinstance(axis, int):
      return _shape[axis]
    elif isinstance(axis, tuple):
      ndim = len(_shape)
      axis = tuple(canonicalize_axis(ax, ndim) for ax in axis)
      if len(axis) != len(set(axis)):
        raise ValueError(f"Duplicate axis in numpy.size: {axis=}")
      return math.prod(_shape[ax] for ax in axis)
    else:
      raise TypeError(f"Invalid axis in numpy.size: {axis=}")
