...
def size(a: ArrayLike | SupportsSize | SupportsShape, axis: int | Sequence[int] | None = None) -> int:
  ...
  if axis is None:
    ...
    ndim = len(_shape)
    axis = [canonicalize_axis(i, ndim) for i in axis]
    if len(axis) != len(set(axis)):
        raise ValueError(f"Duplicate axis passed to numpy.size: {axis=}")
    return math.prod(_shape[i] for i in axis)

  else:
    raise TypeError(f"Invalid axis passed to numpy.size: {axis=}")
