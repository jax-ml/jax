...
from jax._src.numpy.lax_numpy import (
    ...
    diag, dot, finfo, full as jnp_full, ones, outer, roll, trim_zeros,
...
def roots(p: ArrayLike, *, strip_zeros: bool = True) -> Array:
  ...
  """
  p_arr = ensure_arraylike("roots", p)
  p_arr = atleast_1d(promote_dtypes_inexact(p_arr)[0])
  del p
  if p_arr.ndim != 1:
...
def polyfit(x: ArrayLike, y: ArrayLike, deg: int, rcond: float | None = None,
  ...
  if full:
    ...
    return c, resids, rank, s, jnp_full((), rcond)
  elif cov:
