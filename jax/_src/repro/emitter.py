...
class EmitGlobalContext:
  ...
  def emit_function_def(self, fun_def: FunctionDef,
    ...
    if res is None:
      ...
      res = emit_ctx.emit_function_def(fun_def)
      self.emitted_functions[fun_def.id] = res
    return res
...
class EmitFunctionDefContext:
  ...
  def __init__(self, global_ctx: EmitGlobalContext,
    ...
    self.externals_nested: dict[int, Any] = {}  # direct externals, and from nested
    self.indent: int = 0
  ...
  def emit_operand_user_func(self, f: Func) -> None:
    ...
    self.externals[id(f)] = f
    self.externals_nested[id(f)] = f
    for ae in f_emitted.all_externals.values():
      self.externals_nested[id(ae)] = ae

  def emit_function_def(self, fun_def: FunctionDef) -> EmittedFunction:
    ...
    return EmittedFunction(self.lines, self.externals, self.externals_nested)
...
"""
    ...
    if func_ctx.externals_nested:
      msg = ("Got undefined symbols: " +
             ", ".join(f"{e} = {global_ctx.var_name_for_val(e)}"
                       for e in func_ctx.externals_nested.values()))
      assert False  # TODO
...
def load(repro_source: str, repro_path: pathlib.Path) -> Callable[[], Any]:
  ...
  if (main_repro_fun := custom_namespace.get("main_repro")) is not None:  # type: ignore
    return main_repro_fun  # type: ignore
  return custom_namespace["main"]  # type: ignore
