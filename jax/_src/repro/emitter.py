...
def initialize_operand_emitter():
  ...
  _operand_emitter_by_type[sharding.AxisType] = emit_enum("sharding.AxisType")
  ...
  def __call__(self):
    ...
    with tracker.new_tracking_state():
      ...
      with tracker.flags_override(
        ...
        try:
          return Func(self.func, is_jax=False)()
        finally:
          self.last_error_repro = tracker.last_error_repro()


  def to_source(self, *, extra_message: str = "",
...
def save(repro_source: str,
  ...
  out_dir: str = path.make_jax_dump_dir(dump_to)  # type: ignore
...
def eval_repro(repro_path: pathlib.Path, repro_source: str) -> Any:
  postamble = """
