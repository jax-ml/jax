...
def initialize_operand_emitter():
  ...
  from jax._src.export.shape_poly import _DimExpr, _DimTerm, _DimFactor  # type: ignore
  ...
  @partial(register_emitter_by_type, export.SymbolicScope)
  def emit_SymbolicScope(ctx: "EmitFunctionDefContext", v: export.SymbolicScope):
    constraints = [c.debug_str for c in v._explicit_constraints]
    if constraints:
      return ctx.named_value(f"export.SymbolicScope({tuple(constraints)!r})", prefix="scope")
    else:
      return ctx.named_value("export.SymbolicScope()", prefix="scope")
...
def initialize_operand_emitter_pallas_tpu():
  ...
  def emit_TensorCoreMesh(ctx: "EmitFunctionDefContext", v: TensorCoreMesh) -> str:
    res = f"pltpu.TensorCoreMesh(axis_name={v.axis_name!r}, num_cores={v.num_cores})"
    return ctx.named_value(res, prefix="mesh")
...
def initialize_operand_emitter_hijax():
  ...
  def emit_hitype(ctx: "EmitFunctionDefContext", v: hijax.HiType) -> str:
    ...
    if isinstance(v, repro_api.ReproHiType):
      to_tangent_aval = v._tangent_aval
      name = v.name
    ...
    lo_ty_str = ctx.traverse_value(lo_ty) if lo_ty is not None else "None"
    return f"ReproHiType(\"{name}\", {unique_id}, tangent_aval={to_tangent_aval_str}, lo_ty={lo_ty_str})"
...
class EmitFunctionDefContext:
  ...
  def emit_context_for_statement(self, stmt: Statement):
    ...
    trace_context_keys = config.config_ext.trace_context_names()
    # TODO: It seems that the axis_env is in the trace_context, at the end,
    # but not in names?
    assert len(stmt.state_context.trace_context) == len(trace_context_keys), (
  ...
  def abstract_mesh_for_top_statement(self, stmt: Statement):
    ...
    if am is None:  # Make a mesh
      ...
      # meshes inside!!
      from jax._src import xla_bridge  # type: ignore
      dev0 = xla_bridge.local_devices()[0]
...
"""
  ...
  default_device = xla_bridge.local_devices()[0]
