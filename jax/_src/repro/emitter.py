...
def initialize_operand_emitter():
  ...
  #   @partial(register_emitter_by_type, partition_spec.UnconstrainedSingleton)
  #   def emit_UnconstrainedSingleton(ctx: "EmitFunctionDefContext", v):
  #     return "PartitionSpec._get_default_unconstrained()"
  # else:
  #   from jax._src import lib as jaxlib
  #   @partial(register_emitter_by_type, jaxlib._jax.UnconstrainedSingleton)
  #   def emit_UnconstrainedSingleton(ctx: "EmitFunctionDefContext", v):
  #     return "PartitionSpec._get_default_unconstrained()"
  ...
  def emit_TransformedRef(ctx: "EmitFunctionDefContext", v: types.TransformedRef) -> str:
    ...
    for t in v.transforms:
      if isinstance(t, indexing.NDIndexer):
        ...
        res += f".at[{indices}]"
      elif isinstance(t, types.BitcastTransform):
        res += f".bitcast({ctx.traverse_value_atom(t.dtype)})"
      elif isinstance(t, types.ReshapeTransform):
        res += f".reshape({ctx.traverse_value(t.shape)})"
      else:
        raise NotImplementedError(f"emit_TransformedRef: {t}")
...
def check_repro_emit(call: Statement):
  ...
  c._statement = call
  _ = c.to_source(extra_comment="check repro emit",
...
class EmitFunctionDefContext:
  ...
  def emit_function_def(self, fun_def: FunctionDef) -> None:
    ...
    self.emit_line(f"return {result_str if fun_def.level > 0 else last_result_str}" +
                   ("  # uncaught exception" if fun_def.uncaught_exception else ""))
...
class collector:
  ...
  def __call__(self):
    ...
    if tracker._thread_local_state.call_stack:
      raise ValueError(
          "collect_repro can only be used outside any JAX function. "
          f"call_stack: {tracker._thread_local_state.call_stack}")
...
def collect(fn: Callable, repro_name_prefix: str | None = None) -> Callable:
  # TODO: document
  if repro_name_prefix is None:
    repro_name_prefix = "gemini_jax_repro"
  else:
  ...
  def _fn_with_repro_collection(*args, **kwargs):
    ...
    try:
      ...
      if col._statement is not None:
        # We managed to collect something.
        repro_source = col.to_source()
        save(repro_source, repro_name_prefix[:256])
