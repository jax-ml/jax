...
def initialize_operand_emitter():
  ...
  def emit_ShapedArray(ctx: "EmitFunctionDefContext", v: core.ShapedArray) -> str:
    ...
    return (f"core.ShapedArray({ctx.traverse_value(v.shape)}, dtype={dtype}, "
  ...
  def emit_TransformedRef(ctx: "EmitFunctionDefContext", v: types.TransformedRef) -> str:
    ...
    for t in v.transforms:
      if isinstance(t, indexing.NDIndexer):
        ...
        raise NotImplemented(f"emit_TransformedRef: {t}")
...
class collector:
  ...
  def __call__(self):
    ...
    # TODO: try to recover this check. Problem when trying to use collected
    # somewhere inside a JAX program.
...
def collect(fn: Callable, repro_name: str | None) -> Callable:
  # TODO: document
  if repro_name is None:
    repro_prefix = 'gemini_jax_repro'
  else:
    repro_prefix = re.sub(r'[^\w\s-]', '_', repro_name.lower())
    repro_prefix = re.sub(r'_+', '_', repro_prefix).strip('_')
  ...
  def _fn_with_repro_collection(*args, **kwargs):
    ...
    try:
      ...
      save(repro_source, repro_prefix[:256])
