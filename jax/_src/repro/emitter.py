...
def initialize_operand_emitter():
  ...
  def emit_ShapeDtypeStruct(ctx: "EmitFuncContext", v: core.ShapeDtypeStruct) -> str:
    dtype = ctx.emit_operand_atom(v.dtype)
    sharding = ctx.emit_operand_atom(v.sharding)
  ...
  def emit_ShapedArray(ctx: "EmitFuncContext", v: core.ShapedArray) -> str:
    dtype = ctx.emit_operand(v.dtype)
    vma = ctx.emit_operand(v.vma)
    memory_space = ctx.emit_operand_atom(v.memory_space)
    return f"core.ShapedArray({ctx.emit_operand(v.shape)}, dtype={dtype}, vma={vma}, memory_space={memory_space})"
  ...
  def emit_AbstractMesh(ctx: "EmitFuncContext", v: sharding.AbstractMesh) -> str:
    res = (f"sharding.AbstractMesh({v.axis_sizes}, {ctx.emit_operand(v.axis_names)}, "
  ...
  def emit_PRNGKeyArray(ctx: "EmitFuncContext", v: PRNGKeyArray) -> str:
    res = f"prng.PRNGKeyArray({emit_PRNGImpl(ctx, v._impl)}, {ctx.emit_operand_atom(v._base_array)})"
  ...
  def emit_Slice(ctx: "EmitFuncContext", v: indexing.Slice) -> str:
    start = ctx.emit_operand_atom(v.start)
    size = ctx.emit_operand_atom(v.size)
    stride = ctx.emit_operand_atom(v.stride)
  ...
  def emit_GridSpec(ctx: "EmitFuncContext", v: pallas_core.GridSpec) -> str:
    grid = ctx.emit_operand(v.grid)
    in_specs = ctx.emit_operand(v.in_specs)
    out_specs = ctx.emit_operand(v.out_specs)
    scratch_shapes = ctx.emit_operand(v.scratch_shapes)
  ...
  def emit_BlockSpec(ctx: "EmitFuncContext", v: pallas_core.BlockSpec) -> str:
    index_map = ctx.emit_operand_atom(v.index_map)
  ...
  def emit_IndexMapFunc(ctx: "EmitFuncContext", v: pallas_core._IndexMapFunc) -> str:
    ...
    return ctx.emit_operand_atom(v.index_map)

  from jax.experimental.pallas import tpu as pltpu
  @partial(register_emitter, pltpu.CompilerParams)
  def emit_CompilerParams(ctx: "EmitFuncContext", v: pltpu.CompilerParams) -> str:
    dimension_semantics = ctx.emit_operand(v.dimension_semantics)
    allow_input_fusion = ctx.emit_operand(v.allow_input_fusion)
    flags = ctx.emit_operand(v.flags)
    kernel_type = ctx.emit_operand_atom(v.kernel_type)
  ...
  def emit_MemoryRef(ctx: "EmitFuncContext", v: pallas_core.MemoryRef) -> str:
    inner_aval = ctx.emit_operand_atom(v.inner_aval)
    memory_space = ctx.emit_operand_atom(v.memory_space)
...
class EmitFuncContext:
  ...
  def flatten_custom_pytree(t) -> Any:
    ...
    # internally.
    from jax._src.state import indexing
    return maybe_singleton(tree_util.tree_leaves(t, is_leaf=lambda l: isinstance(l, (Func, indexing.Slice))))

  @staticmethod
  def flatten_custom_pytree_statics(args, kwargs,
  ...
  @staticmethod
  def jax_jit_call_dyn_args_kwargs(jitted_fun: Func,
    ...
    dyn_args, dyn_kwargs = EmitFuncContext.flatten_custom_pytree_statics(
        args, kwargs,
  ...
  def preprocess_args_for_call(self, call: Call) -> tuple[tuple[Any, ...], dict[str, Any]]:
    ...
    if call.func.api_name == "jax_jit_call":
      ...
      dyn_args, dyn_kwargs, new_jit_call_kwargs = EmitFuncContext.jax_jit_call_dyn_args_kwargs(
          jitted_fun, jit_kwargs, rest_args, call.kwargs)
  ...
  def preprocess_args_for_body(call: Call) -> tuple[Sequence[Any], dict[str, Any]]:
    ...
    if hasattr(call, "jit_kwargs"):
      dyn_args, dyn_kwargs, _ = EmitFuncContext.jax_jit_call_dyn_args_kwargs(
          call.func, call.jit_kwargs, call.args, call.kwargs
      )
      return dyn_args, dyn_kwargs
  ...
  def emit_body(self) -> EmittedFunction:
    ...
    arg_names = [self.new_local_name() for a in inv_args]
    args_str = ", ".join(arg_names)
    if inv_kwargs:
      args_str += ", *, " + ", ".join(inv_kwargs.keys())
    self.emit_line(f"def {self.global_ctx.var_for_val(invocation.func)}({args_str}):")
    self.indent += 2
    for a_path, a in tree_util.tree_leaves_with_path(
        (inv_args, inv_kwargs),
        is_leaf=lambda n: isinstance(n, FuncPartial)):
      if a_path[0].idx == 0:  # among *args
        assert len(a_path) >= 2 and a_path[1].idx < len(arg_names), (a_path, arg_names)
        a_path_str = f"{arg_names[a_path[1].idx]}{keystr_no_flat_index(a_path[2:])}"
        self.define_value(a, a_path_str)

      else:  # among **kwargs
        assert len(path) >= 2 and path[1].key in inv_kwargs, (path, inv_kwargs)
        path_str = f"{path[1].key}{keystr_no_flat_index(path[2:])}"
        self.define_value(a, path_str)
  ...
  def emit_call(self, c: Call) -> str:
    ...
    results_with_paths: list[tuple[Any, tree_util.KeyPath]]
    results_with_paths = [(r, path)
                          for path, r in tree_util.tree_leaves_with_path(
                             result,
                             is_leaf=lambda n: (isinstance(n, FuncPartial)))]
    pre_res = _thread_local_state.emit_call_preprocessor(
        self, func, args, kwargs, results_with_paths)
    if pre_res is None: return "_"
    func, args, kwargs, results_with_paths = pre_res

    callee_name_str = self.emit_operand_atom(func)
    ...
    if not results_with_paths:
      # TODO: this seems to arise for the set primitive, without results
      # How can we tell if we should drop this or not?
      self.emit_line(f"_ = {callee_name_str}({call_args_str})  # {c}")
      return "_"
    overall_res_name: str | None = None  # Emit lazily the call
    for r, path_or_lit in results_with_paths:
      if isinstance(path_or_lit, EmitLiterally):
        r_n = self.new_local_name()
        self.emit_line(f"{r_n} = {path_or_lit.literal}")
        self.define_value(r, r_n)
        continue
      if overall_res_name is None:
        overall_res_name = self.new_local_name()
        self.emit_line(f"{overall_res_name} = {callee_name_str}({call_args_str})  # {c}")
      path_str = f"{overall_res_name}{keystr_no_flat_index(path_or_lit)}"
      self.define_value(r, path_str)
...
tracker._emit_repro = _emit_repro


def collect_repro(func: Callable, *args, **kwargs) -> Any:
  assert config.repro_dir.value, "You must set JAX_REPRO_DIR=something"
  _thread_local_state.emitted_repros = []
  with flag_override("collect_repro_on_success", True):
    res = func(*args, **kwargs)

  repro_stack_entry = _thread_local_state.call_stack[-1]
  assert repro_stack_entry.func.is_main
  _emit_repro(repro_stack_entry, "repro_test.collect_repro")
  return res
