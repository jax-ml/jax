...
def state_context(**kwargs) -> contextlib.AbstractContextManager[Any]:
  ...
  def get_state_context_manager(k: str, new_val: Any) -> contextlib.AbstractContextManager[Any]:
    ...
    if k == "user_context":
      old_val = config.jax_jit_cpp_cache_obj._obj
    else:
    ...
    return _LocalContextManager(old_val, new_val)
...
def export_call(fun: Callable, jit_kwargs: dict[str, Any],
  ...
  return repro_bypass_wrapper(_export.export)(fun_jit, **exp_kwargs)(*args, **kwargs)
...
try:
  ...
  class ReproHiType(hijax.HiType):
    ...
    def lo_ty(self):
      if self._lo_ty is not None:
        return list(self._lo_ty)
      raise NotImplementedError(f"lo_ty: {self}")

    def lower_val(self, hi_val: "ReproHiVal"):
      assert isinstance(hi_val, ReproHiVal), hi_val
      return list(hi_val.lo_vals)
    ...
    def inc_rank(self, size: int | None, spec: hijax.MappingSpec):
      return self
...
def jax_vjphiprimitive_call(hi_prim_name: str, *args, expand: Callable,
  ...
  base_class = type(prim) if prim is not None else hijax.HiPrim

  class ReproVJPHiPrimitive(base_class):  # pyrefly: ignore[invalid-inheritance]
    def __init__(self):
      ...
      if prim is not None and hasattr(prim, "effects"):
        self.effects = prim.effects
      hijax.HiPrim.__init__(self)
