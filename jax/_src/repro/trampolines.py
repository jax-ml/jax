...
# Dict indexed by repro_api_name. Used by the api_boundary decorator to look
# up the trampoline, and invoke it on the real_api_fun.
...
def redirect_trampoline(api_name: str, api_fun: Callable) -> Callable:
  """
  Redirects an API call to `api_fun` to a function `repro_api.api_name`.
  Use when the API call returns arrays, and you need to specify
  map_user_func_args, or we cannot use directly the JAX implementation.
  ...
  and returns arrays, then `redirect_trampoline("api_name", api_fun)` will call

    repro_api.api_name(*args, **kwargs)
...
for repro_api_name, api_name in [
  ("jax.lax.cond", "cond"),
  ...
  ("jax.vjp", "vjp"),
  ("pallas.tpu.emit_pipeline_with_allocations", "pallas_tpu_emit_pipeline_with_allocations"),
  ("fuser.make_scalar_prefetch_handler", "fuser_make_scalar_prefetch_handler"),
  ("fuser.get_fusion_values", "fuser_get_fusion_values"),
]:
  api_trampolines[repro_api_name] = partial(redirect_trampoline, api_name)


def redirect_decorator_trampoline(api_name: str, api_fun: Callable) -> Callable:
  """
  A trampoline to use for an API function that acts as a decorator factory for
  a function that returns arrays.
  If `api_fun` is used like:

    api_fun(*api_args, **api_kwargs)(fun)

  and returns arrays, then `redirect_decorator_trampoline("api_name", api_fun)`
  when used in lieu of `api_fun` will call:

    repro_api.api_name(fun, *api_args, **api_kwargs)
  ...
  def trampoline(*api_args, **api_kwargs):
    def decorator(fun):
      return getattr(repro_api, api_name)(fun, *api_args, **api_kwargs)
    return decorator
  trampoline.__name__ = f"{api_name}_trampoline"
  trampoline.__qualname__ = trampoline.__name__
  trampoline.real_api_fun = api_fun  # pyrefly: ignore[missing-attribute]
...
def uncurry_trampoline(api_name: str, api_fun: Callable) -> Callable:
  ...
  available and then call:
    ...
    repro_api.api_name_call(fun, api_args, api_kwargs, *args, **kwargs)

  """
  api_call_name = f"{api_name}_call"
  from jax._src.repro import repro_api

  def trampoline(fun, *api_args, **api_kwargs):
    def call_trampoline(*args, **kwargs):
      return getattr(repro_api, api_call_name)(
        fun, api_args, api_kwargs, *args, **kwargs)
    call_trampoline.__name__ = f"{api_call_name}_trampoline"
...
for repro_api_name, transform_name in [
  ...
  ("pallas.run_state", "pallas_run_state"),
  ("pallas.mosaic_gpu.kernel", "pallas_gpu_kernel"),
  ("pallas.mpmd_map", "pallas_mpmd_map"),
  ("jax.custom_gradient", "custom_gradient"),
  ("flax.core.axes_scan.scan", "flax_axes_scan"),
  ("pallas.mosaic_gpu.emit_pipeline", "pallas_gpu_emit_pipeline"),
  ("pallas.mosaic_gpu.emit_pipeline_warp_specialized", "pallas_gpu_emit_pipeline_warp_specialized"),
  ("pallas.tpu.emit_pipeline", "pallas_tpu_emit_pipeline"),
...
def uncurry_maybe_decorator_trampoline(api_name: str, api_fun: Callable) -> Callable:
  """
  A trampoline to use for an API function that can act as a decorator factory
  for a function that returns a single first-order function.
  The API is called first with configuration args/kwargs, and returns a
  decorator that takes a single function (fun).
  If `api_fun` is used like:

    api_fun(fun, **api_kwargs)(*args, *kwargs)

  or as a decorator

    api_fun(None, **api_kwargs)(fun)(*args, **kwargs)

  then `uncurry_decorator_trampoline("api_name", api_fun)` when used
  in lieu of `api_fun` will defer all processing until `fun` and the args
  are available and then call:

    repro_api.api_name_call(fun, api_kwargs, *args, *kwargs)

  """
  api_call_name = f"{api_name}_call"
  from jax._src.repro import repro_api

  def trampoline(fun: Callable | None = None, **api_kwargs):
    def actual_decorator(fun: Callable):
      def call_trampoline(*args, **kwargs):
        return getattr(repro_api, api_call_name)(
            fun, api_kwargs, *args, **kwargs)
      call_trampoline.__name__ = f"{api_call_name}_trampoline"
      call_trampoline.__qualname__ = call_trampoline.__name__
      return call_trampoline

    if fun is None:  # Called with some api_kwargs
...
def jit_trampoline(is_pjit: bool, real_jit: Callable) -> Callable:
  ...
  def jit_trampoline_wrapper(fun: Callable | None = None, /, **jit_kwargs):
    ...
    if (getattr(fun, "_apply_primitive", None) or
        getattr(fun, "__module__", "") == "jax._src.pallas.mosaic.interpret.thread_map"):
...
@api_trampoline("pallas.core_map")
...
def pallas_kernel_trampoline(real_api_fun: Callable):
  from jax._src import api as _api  # type: ignore
  from jax._src.repro.repro_api import pallas_kernel_call  # type: ignore
...
api_trampolines["pallas.tpu_sc.parallel_loop"] = partial(
  redirect_decorator_trampoline, "pallas_parallel_loop")


for repro_api_name, api_name in [
  ("fuser.fuse", "fuser_fuse"),
  ("fuser.evaluate", "fuser_evaluate"),
]:
  api_trampolines[repro_api_name] = \
      partial(uncurry_maybe_decorator_trampoline, api_name)


@api_trampoline("fuser.fusible")
def fuser_fusible_trampoline(real_api_fun: Callable) -> Callable:
  from jax._src.repro.repro_api import fuser_fusible_call

  def jax_fuser_fusible_trampoline(fun: Callable | None = None, **trans_kwargs):
    def actual_decorator(fun: Callable):
      # We pass the function twice because it is traced twice
      return partial(fuser_fusible_call, fun, fun, trans_kwargs)
    if fun is None:
      return actual_decorator
    return actual_decorator(fun)
...
api_trampolines["pltpu.async_copy"] = partial(redirect_trampoline, "pallas_async_copy")
api_trampolines["pltpu.make_async_copy"] = partial(redirect_trampoline, "pallas_make_async_copy")
api_trampolines["pltpu.make_async_remote_copy"] = partial(redirect_trampoline, "pallas_make_async_remote_copy")


def unsupported_callback_trampoline(repro_api_name: str, real_api_fun: Callable) -> Callable:
  def trampoline(*args, **kwargs):
    raise NotImplementedError(f"{repro_api_name} is not supported in repro mode")
  trampoline.__name__ = f"{repro_api_name.replace('.', '_')}_trampoline"
  trampoline.__qualname__ = trampoline.__name__
  trampoline.real_api_fun = real_api_fun  # pyrefly: ignore[missing-attribute]
  return trampoline

for repro_api_name in [
    "jax.pure_callback",
    "jax.experimental.io_callback",
    "jax.debug.callback",
]:
  api_trampolines[repro_api_name] = partial(unsupported_callback_trampoline, repro_api_name)
...
def vjphiprimitive_call_trampoline(real_api_fun: Callable):
  def hijax_vjphiprimitive_call_trampoline(*args, **kwargs):
    ...
    hi_prim: hijax.HiPrim
    hi_prim, *rest_args = args

    # Unpack the user's HiPrim into a bunch of functions.
    if hasattr(hi_prim, "_expand"):  # It is a repro_api.ReproVJPHiPrimitive
      # TODO: comment why we need this
      hi_prim_name = hi_prim.name
      hi_prim_expand = hi_prim._expand.fun if hasattr(hi_prim._expand, "map_user_func_args") else hi_prim._expand
      hi_prim_jvp = hi_prim._jvp.fun if hasattr(hi_prim._jvp, "map_user_func_args") else hi_prim._jvp
      hi_prim_batch = hi_prim._batch.fun if hasattr(hi_prim._batch, "map_user_func_args") else hi_prim._batch
      hi_prim_vjp_fwd = hi_prim._fwd.fun if hasattr(hi_prim._fwd, "map_user_func_args") else hi_prim._fwd
      hi_prim_bwd_retval = hi_prim._bwd_retval.fun if hasattr(hi_prim._bwd_retval, "map_user_func_args") else hi_prim._bwd_retval
    else:
      hi_prim_name = str(hi_prim.__class__)
      ...
      if type(hi_prim).vjp_bwd is not hijax.HiPrim.vjp_bwd:
        raise NotImplementedError("vjp_bwd")  # We support only vj_bwd_retval for now

    for t in it.chain(hi_prim.in_avals_flat, hi_prim.out_avals_flat):
      if isinstance(t, hijax.HiType):
        repro_api.register_repro_hitype(t)

    return repro_api.jax_vjphiprimitive_call(hi_prim_name,
      *rest_args, expand=hi_prim_expand, jvp=hi_prim_jvp,
      batch=hi_prim_batch, fwd=hi_prim_vjp_fwd, bwd_retval=hi_prim_bwd_retval,
      in_avals=hi_prim.in_avals, out_aval=hi_prim.out_aval,
