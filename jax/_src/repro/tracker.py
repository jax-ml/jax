...
def last_error_repro() -> tuple[pathlib.Path, str] | None:
  """The last repro (path and source) that was saved due to uncaught exception."""
  return _thread_local_state.last_error_repro
  ...
  def end_call(*, res, exc: Exception | None):
    ...
    try:
      ...
      if exc is None:
        ...
        wrapped_res = res
        if call.func.is_jax:  # TODO: ideally we want only api_name functions to return functions
          ...
          wrapped_res = tuple(results) if isinstance(res, tuple) else results[0]

        call.result = Call.copy_args_results(wrapped_res)
        if _thread_local_state.flags.log_calls:
          res_str = "res=Ok"
      else:
        wrapped_res = None
...
def true_bind_wrapper(actual_true_bind: Callable):
  def wrapper(prim, *args, **params):
    if not repro_enabled():
      return actual_true_bind(prim, *args, **params)
...
def uncaught_exception_handler(main_invocation: Call, traceback_str: str):
  ...
  repro_source = c.to_source(extra_message="uncaught exception " + traceback_str)
  repro_path = emitter.save(repro_source, repro_name_prefix="jax_error_repro")
  _thread_local_state.last_error_repro = (repro_path, repro_source)
...
def repro_boundary(boundary_fun: Callable, *,
  ...
  def repro_boundary_wrapper(*args, **kwargs):
    ...
    the_func = func or Func(boundary_fun, is_jax=is_jax, api_name=api_name,
    ...
    if api_name in ["jax.lax.scan", "jax.lax.while_loop", "jax.lax.fori_loop",
      ...
      # called from JAX-internal functions.
      has_user_func = False
      def find_user_func(a):
        ...
        return has_user_func
      the_func.map_user_funcs(find_user_func, *args, **kwargs)
      if not has_user_func:
        if _thread_local_state.flags.log_calls:
          logging.info(f"Ignoring call to {api_name} from within JAX source")
        return boundary_fun(*args, **kwargs)

    if api_name == "jax_jit_call" and _pallas_call_wrapped_re.search(str(args[0])):
      if _thread_local_state.flags.log_calls:
        logging.info(f"Ignoring call to {api_name} from within JAX source")
      return boundary_fun(*args, **kwargs)
    ...
    try:
      wrapped_args, wrapped_kwargs = Call.start_call(args, kwargs, func=the_func)
...
# We skip the jax.jit on the pallas_call.wrapped function because
# it is made from JAX internals.
_pallas_call_wrapped_re = re.compile(r"_pallas_call\.<locals>\.wrapped")
