...
class ReproFlags:
  ...
  # the collect_repro_enabled is True
  collect_repro_enabled = True
...
def flag_override(flag_name: str, flag_value):
  """Indicates that the current context is an explicit device_put*() call."""
  prev = getattr(_thread_local_state.flags, flag_name)
...
class _ThreadLocalState(threading.local):
  def __init__(self):
    ...
    self.undefined_value_handler: Callable[["EmitFuncContext", Any], str] | None = None  # type: ignore
...
class Func:
  ...
  def __init__(self, fun: Callable, *, is_jax: bool,
    ...
    self.map_user_funcs = map_user_funcs  # See docstring for repro_boundary
...
class Call:
  ...
  def __init__(self, level: int, parent,
    ...
    self.emitted_function: Union["EmittedFunction", None] = None  # type: ignore

    not _thread_local_state.flags.enable_checks or check_traceback(self.raw_traceback.frames)
    ...
    try:
      ...
      if caller.func.is_main:
        ...
        if (all(not isinstance(l, (FuncPartial, Func)) for l in returned_leaves) and
          ...
          if _thread_local_state.flags.enable_checks:
            logging.info(f"Popping the last call from main: {call}")
          caller.body.pop()
...
def true_bind_primitive(prim: Primitive, args, params):
  ...
  if (_thread_local_state.flags.enable_checks and
    ...
    _thread_local_state.warn_or_error(
        ...
        "annotated with core.repro_boundary")
  try:
    with flag_override("collect_repro_enabled", False):
...
def repro_enabled():
  return config.repro_dir.value and _thread_local_state.flags.collect_repro_enabled
...
def repro_boundary(boundary_fun: Callable, *,
  ...
  if func is None:
    if map_user_funcs is None:
      def map_user_funcs(to_apply, *args, **_):
        return tuple(to_apply(a) if callable(a) and not isinstance(a, FuncPartial) else a
                     for a in args)
  ...
  def wrapper(*args, **kwargs):
    ...
    if api_name in ["jax.lax.scan", "jax.lax.while_loop", "jax.lax.fori_loop",
      ...
      map_user_funcs(find_user_func, *args, **kwargs)
...
def bypass_repro_wrapper(f: Callable) -> Callable:
  ...
  i.e., the one without the repro_boundary wrapper.
  """
...
boundary_trampolines["jax.jit"] = jit_trampoline
...
# TODO: merge this with the fusible_trampoline, maybe even make it part
# of the generic_trampoline. The new thing here is that this can be used
# as a decorator without using "partial".
    ...
    if f.function_name == "_get_result_paths_thunk":
      if (frames[i - 1].function_name != "Func.__call__" and
        ...
        msg = (
            ...
            f" `{frames[i - 1]}`. This typically means a "
...
What are we missing:
* caching is changed (mostly folied)
* named_call is not handled (a nop)
