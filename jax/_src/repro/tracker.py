...
class _ThreadLocalState(threading.local):
  ...
  def initialize_state_if_needed(self):
    ...
    # Hook repros into core.Primitive.bind
    ...
    if not hasattr(ufuncs.remainder, "is_pjit") or not hasattr(ufuncs.remainder, "fun"):
      raise RuntimeError(
...
def bind_wrapper(actual_bind: Callable) -> Callable:
  def bind(*prim_and_args, **params):
    ...
    if config.enable_checks.value:
      if (func_src_info := is_higher_order_primitive(prim, args, params)):
        _thread_local_state.warn_or_error(
            f"Encountered primitive {prim} containing Jaxprs or functions, "
  ...
  bind.real_boundary_fun = actual_bind
  return bind

def check_call_to_tracing():
  if config.enable_checks.value and is_enabled():
...
def trace_to_jaxpr_wrapper(actual_trace_to_jaxpr: Callable) -> Callable:
  ...
  trace_to_jaxpr.real_boundary_fun = actual_trace_to_jaxpr
...
def trace_to_jaxpr_dynamic_wrapper(actual_trace_to_jaxpr_dynamic: Callable) -> Callable:
  ...
  trace_to_jaxpr_dynamic.real_boundary_fun = actual_trace_to_jaxpr_dynamic
...
def generic_trampoline(trans: str, real_transform: Callable) -> Callable:
  ...
  This generic_trampoline works when there is one transformed function, passed
  as the first positional argument, and the transformation returns the
  transformed function (which itself is first-order).
...
def jit_trampoline(is_pjit: bool, real_jit: Callable) -> Callable:
  ...
  def jax_jit_trampoline(fun: Callable | None = None, /, **jit_kwargs):
    ...
    jax_jit_call_trampoline.__wrapped__ = fun
