...
def _monkey_patch_jax():
  ...
  if "trace_to_jaxpr_wrapper" not in str(old_trace_to_jaxpr):
    pe.trace_to_jaxpr = trace_to_jaxpr_wrapper(old_trace_to_jaxpr)  # pyrefly: ignore[bad-assignment]
...
def call_boundary(func: Union[Func, "Primitive"],
  ...
  if (not is_user_func(func) and
    ...
    if _thread_local_state.flags.log_calls:
      ...
      func_desc = func_api_name(func) or getattr(func, "name", str(func))
      logging.info(f"{'  ' * indent} Ignoring call to {func_desc} from within JAX source (){detail}")
    return boundary_fun(*args, **kwargs)
