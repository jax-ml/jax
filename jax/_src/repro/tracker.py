...
class Func:
  def __init__(self, fun: Callable, *, is_user: bool,
    ...
    self.filter_statics: Callable[[tuple[Any, ...], dict[str, Any]], tuple[tuple[Any, ...], dict[str, Any]]] = lambda args, kwargs: (args, kwargs)
...
class NormalizerContext:
  ...
  The goal is to: replace core.Tracer to avoid leaked tracers;
  to handle large arrays to reduce memory usage;
  to simplify the values by eliminating user-defined PyTrees, which are not
  needed for repro generation; however, we keep some PyTrees for which we have
  ...
  def normalize_value(self, v: Any, is_use: bool):
    ...
    for t in type(v).__mro__:
      if False and (normalizer := _value_normalization_by_type.get(t)) is not None:
...
class Call:
  ...
  @staticmethod
  def start_call(func: Union["Func", "Primitive"], args, kwargs) -> tuple[tuple[Any, ...], dict[str, Any]]:
  ...
  def set_args(self, args, kwargs):  # Statement
    if isinstance(self.func, Func):
      if self.func.api_name in ("jax_jit_call", "jax_jit_aot_trace_call",
        ...
        jitted_fun.filter_statics = lambda args, kwargs: jax_jit_call_filter_statics(
        ...
        if custom_vjp_kwargs["nondiff_argnums"]:
          # TODO: make filter_args standard
          fun.filter_statics = lambda args, kwargs: filter_statics(args, kwargs,
                                                                   static_argnums=custom_vjp_kwargs["nondiff_argnums"])
          fwd.filter_statics = lambda args, kwargs: filter_statics(args, kwargs,
                                                                   static_argnums=custom_vjp_kwargs["nondiff_argnums"])
          bwd.filter_statics = lambda args, kwargs: filter_statics(args, kwargs,
        ...
        if cjvp_kwargs["nondiff_argnums"]:
          ...
          for fun_jvp in fun_jvps:
            fun_jvp.filter_statics = lambda args, kwargs: filter_statics(args, kwargs,
...
class FunctionDef(Call):
  ...
  def set_args(self, args, kwargs):  # FunctionDef
    # if hasattr(self.func, "jit_kwargs"):
    #   args, kwargs, _ = jax_jit_call_filter_statics(
    #       self.func, self.func.jit_kwargs, args, kwargs)
    args, kwargs = self.func.filter_statics(args, kwargs)
    self.args: tuple[Any, ...] = self.normalizer_ctx.normalize_value(args, False)  # type: ignore
    self.kwargs: dict[str, Any] = self.normalizer_ctx.normalize_value(kwargs, False)  # type: ignore
...
# def jax_repro_collect_trampoline(real_boundary_fun):
#   #from jax._src.repro.repro_api import jax_repro_collect
#   def repro_collect_trampoline(fun):
#     if _thread_local_state.call_stack:
#       stop = 1
#     else:
#       return real_boundary_fun(fun)
#     pass
#   repro_collect_trampoline.real_boundary_fun = real_boundary_fun
#   return repro_collect_trampoline
