...
class Repro(ddmin.Repro[ddmin.Candidate]):
  ...
  def progress_msg(self, state: ddmin.State[ddmin.Candidate]) -> str:  # pyrefly: ignore[bad-override]
  ...
  def get_reduced_repro(self, reduction_candidates: list[ddmin.Candidate]) -> Union["Repro", None]:  # pyrefly: ignore[bad-override]
  ...
  def check_repro(repro_source: str,
                  ...
                  test_repro_fun: TestReproFun,
                  strategy_factory: ReductionStrategyFactory, *,
                  raise_on_failure: bool = True) -> str | tuple[emitter.collector, list[Any]]:
