...
# This is an example pre-processor that helps with one particular large repro
def emit_call_preprocessor(ctx: EmitFuncContext,
                           func, args, kwargs, results_with_paths):

  if not isinstance(func, Func):
    # Drop primitive calls, except those with side-effects or those that error
    # out. Those do not have results.
    if results_with_paths:
      return None

  else:
    # Drop the JAX functions, if they have a single non-error result.
    if (len(results_with_paths) == 1 and
...
def undefined_value_handler(ctx: EmitFuncContext, v: Any) -> str:
  ...
  def fake_array(a) -> str:
    if isinstance(a.dtype, prng.KeyTy):
      return f"fake_prng_key({ctx.traverse_value_atom(a.dtype._impl)}, {a.shape})"
    else:
      return f"np.ones({a.shape}, dtype={ctx.traverse_value_atom(a.dtype)})"
