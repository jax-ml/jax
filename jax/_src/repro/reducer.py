...
class Repro(ddmin.Repro[ddmin.Candidate]):
  ...
  def get_all_candidates(self) -> list[ddmin.Candidate]:
  ...
  def progress_msg(self, state: ddmin.State[ddmin.Candidate]) -> str:
    ...
    return f" --start_file={self.repro_path} --strategy={name} --granularity={state.granularity} --start_index={state.start_index}"
