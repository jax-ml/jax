...
def custom_vmap_partial_eval(primitive, trace, *tracers, **params):
  if all(trace.to_jaxpr_tracer(t).is_known() for t in tracers):
    return trace.default_process_primitive(primitive, tracers, params)
  raise NotImplementedError(
      "custom_vmap functions do not support reverse-mode autodiff. custom_vjp "
      "can be used to support customizing both vmap and reverse-mode autodiff."
  )
...
pe.custom_partial_eval_rules[custom_vmap_p] = functools.partial(
    custom_vmap_partial_eval, custom_vmap_p
)
