...
def check_read(device_id, clock, buffer_key, rnge, source_info=None):
  ...
  for i in range(num_writes):
    ...
    if not ranges_overlap(rnge, write_range):
      continue
    print('RACE DETECTED\n'
          f'  read of {buffer_key}[{rnge}] from {device_id}, {user_frame} at {clock}\n'
          f'  write of {buffer_key}[{write_range}] from {write_device_id}, {write_frame} at {write_clock}')
    with races.lock:
...
def check_write(device_id, clock, buffer_key, rnge, source_info=None):
  ...
  for i in range(num_writes):
    ...
    if not ranges_overlap(rnge, write_range):
      continue
    print('RACE DETECTED\n'
          f'  write of {buffer_key}[{rnge}] from {device_id}, {user_frame} at {clock}\n'
          f'  write of {buffer_key}[{write_range}] from {write_device_id}, {write_frame} at {write_clock}')
    with races.lock:
      races.races_found = True
    break
  ...
  for i in range(num_reads):
    ...
    print('RACE DETECTED\n'
          f'  write of {buffer_key}[{rnge}] from {device_id}, {user_frame} at {clock}\n'
          f'  read of {buffer_key}[{read_range}] from {read_device_id}, {read_frame} at {read_clock}')
    with races.lock:
      races.races_found = True
    return
