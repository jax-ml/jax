...
def _allocate_semaphore(device_id, shape):
  ...
  # NOTE: For now, we use a relatively uncommon datatypen (int16) for
...
def interpret_pallas_call(
  ...
  for i, var in enumerate(jaxpr.invars[grid_mapping.num_index_operands:]):
    output_idx = i - grid_mapping.num_index_operands - grid_mapping.num_inputs
    ...
    if var.aval.memory_space == mosaic_core.TPUMemorySpace.SEMAPHORE:
      kernel_buffer_ids.append(callback.io_callback(
          ...
          var.aval.shape,
          ordered=True))
    elif is_output and var.aval.memory_space == mosaic_core.TPUMemorySpace.ANY:
  ...
  for buffer_id, var in zip(kernel_buffer_ids, jaxpr.invars):
    # TODO(jburnim): Filter out HBM outputs.
    if var.aval.memory_space == mosaic_core.TPUMemorySpace.SEMAPHORE:
      pass
    else:
      callback.io_callback(
