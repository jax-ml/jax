...
def join_bits(
  ...
  if two_bits is not None:
    ...
    # Zero-extend 2b -> 4b so bits [3:2] are 00. Use bitcast to int4 to
    # prevent MLIR from folding extsi(extui(i2 -> i4), i8) into extui(i2 -> i8).
    two_b_u4 = two_bits.astype(jnp.uint2).astype(jnp.uint4)
    two_b_i4 = plm_primitives.bitcast(two_b_u4, ty=jnp.int4)

  if four_bits is None:
    assert two_b_i4 is not None
    return two_b_i4.astype(jnp.int8)

  if jax.dtypes.itemsize_bits(four_bits.dtype) != 4:
    raise ValueError(f"Expected 4-bit type, got {four_bits.dtype}")
  if two_b_i4 is None:
    return four_bits.astype(jnp.uint4).astype(jnp.int8)

  low_8b = four_bits.astype(jnp.int4).astype(jnp.int8)
  high_8b = two_b_i4.astype(jnp.int8)
...
def split_bits(
  ...
  for bw in bitwidths:
    ...
    if shift == 0:
      ...
      shifted = jax.lax.shift_right_logical(x_i8, jnp.int8(shift))
