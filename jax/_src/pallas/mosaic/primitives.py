...
def _validate_dma_args(
    src_tref_aval, dst_tref_aval, dst_sem_tref_aval, src_sem_tref_aval,
):
  if not all(
      isinstance(x, (state.AbstractRef, state.TransformedRef))
      for x in [src_tref_aval, dst_tref_aval, dst_sem_tref_aval]
  ):
  ...
  if src_sem_tref_aval is not None:
    if not isinstance(src_sem_tref_aval, (state.AbstractRef, state.TransformedRef)):
      raise ValueError(
          f"Expected the source semaphore to be a Ref, but got: {src_sem_tref_aval}"
      )
  ...
  if src_tref_aval.dtype != dst_tref_aval.dtype:
    raise ValueError(
        ...
        f"but got {src_tref_aval.dtype} and {dst_tref_aval.dtype}."
    )

  def _is_static(d):
    return isinstance(d, (int, np.int32, np.int64, np.ndarray))

  if not all(
      ds == dd for ds, dd in zip(src_tref_aval.shape, dst_tref_aval.shape)
      if _is_static(ds) and _is_static(dd)
  ):
      raise ValueError(
...
def _dma_start_abstract_eval(*args, tree, device_id_type, priority, add):
  ...
  _validate_dma_args(src_ref_aval, dst_ref_aval, dst_sem_aval, src_sem_aval)
