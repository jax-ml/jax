...
from jax._src.pallas import primitives as pl_prims
...
def dma_start_partial_discharge_rule(should_discharge, in_avals, out_avals,
  ...
  def new_dst_sem(dst_sem=dst_sem):
  ...
  def new_src_sem(src_sem=src_sem):
  ...
  new_vals += (new_dst() if dst_discharge else None,)  # dst_val
  new_vals += (None,) * num_dst_transform_vals
  new_vals += (new_dst_sem() if dst_sem_discharge else None,)  # dst_sem
  new_vals += (None,) * num_dst_sem_transforms
  if is_remote:
    new_vals += (new_src_sem() if src_sem_discharge else None,) # src_sem
  ...
  if not dst_discharge:
    sp.ref_swap(dst_ref, None, new_dst(dst_ref=dst_ref[...]))
  if not dst_sem_discharge:
    sp.ref_set(dst_sem, None, new_dst_sem(dst_sem=dst_sem[...])[...])
  if is_remote and not src_sem_discharge:
    sp.ref_set(src_sem, None, new_src_sem(src_sem=src_sem[...])[...])
