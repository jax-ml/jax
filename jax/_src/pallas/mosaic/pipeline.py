...
BlockDim: TypeAlias = pl.Blocked | pl.Element | pl.BoundedSlice | int | None

def _make_block_slice(
    block_index: jax.Array, block_size: BlockDim, size: int, tiling: int
) -> pl.Slice | slice:
...
class BufferedRef:
  ...
  def compute_slice(self, grid_indices):
    ...
    return jax.tree.map(_make_block_ds, indices, block_shape)
