...
def emit_pipeline_warp_specialized(
  ...
  def pipeline(*gmem_refs: state.AbstractRef):
    ...
    flat_gmem_refs, _ = jax.tree.flatten(gmem_refs)
    flat_in_gmem_refs = flat_gmem_refs[: len(flat_in_specs)]
    flat_out_gmem_refs = flat_gmem_refs[len(flat_in_specs) :]

    if len(flat_in_gmem_refs) != len(flat_in_gmem_refs):
      raise ValueError(
          "Number of input refs does not match number of input specs."
      )

    if len(flat_out_gmem_refs) != len(flat_out_specs):
      raise ValueError(
    ...
    for has_seq_dim in in_spec_has_seq_axis:
      ...
      flat_in_smem_barriers.append(
          gpu_core.Barrier(num_arrivals=1, num_barriers=num_barriers)
      )
  ...
  def scoped_pipeline(
    ...
    def compute_block():
      ...
      # This is true if any of the outputs need to be transferred inside the loop.
      copies_out_in_loop = not all(
          bref.is_index_invariant for bref in flat_out_brefs
      )
      ...
      def compute_loop_body(step, carry):
        ...
        if manual_consumed_barriers:
          barriers = jax.tree.unflatten(
              in_specs_treedef,
              [barrier.at[slot] for barrier in flat_consumed_barrier_refs],
          )
