...
def _block_spec_from_block_mapping(
  ...
  index_map_jaxpr = functools.partial(
      jax.core.eval_jaxpr,
  ...
  def index_map(*indices):
    ...
    return index_map_jaxpr(*new_indices)
...
def lower_jaxpr_to_module(
  ...
  dimension_semantics = params.pop("dimension_semantics", None)
  ...
  def pipeline_fn(*refs, scratch_refs=()):
    if len(scratch_refs) != grid_mapping.num_scratch_operands:
      return primitives.run_scoped(
          functools.partial(pipeline_fn, *refs),
          scratch_refs=[
              ref_for_aval(v.aval)
