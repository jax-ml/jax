...
class ModuleContext:
  ...
  auto_barriers: bool
  optimized_load_store_only: bool
  ...
  def body(launch_ctx: mgpu.LaunchContext, *buffers: ir.Value):
    *buffers_gmem, (
    ...
    module_ctx = ModuleContext(
        ...
        optimized_load_store_only=params.optimized_load_store_only,
    )
...
def _get_lowering_rule(ctx: LoweringRuleContext, x_ref, *leaves, tree):
  ...
  optimized = ctx.module_ctx.optimized_load_store_only
  match transforms:
...
def _swap_lowering_rule(
  ...
  match transforms:
    case (
      ...
      value.store_tiled(x_smem, swizzle=swizzle, optimized=optimized)
    case ():
      match value.layout:
        case mgpu.TiledLayout():
          ...
          value.store_untiled(x_smem, optimized=optimized)
        case _:
