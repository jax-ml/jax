...
def _swap_lowering_rule(
  ...
  match transforms:
    case (
        gpu_core.UnswizzleRef(swizzle_elems),
        gpu_core.UntileRef(tiling),
        *permutation,
    ):
      match permutation:
        case (gpu_core.TransposeRef(permutation),):
          assert len(tiling) == len(permutation)
          x_smem = mgpu.memref_transpose(
              x_smem,
              permutation + tuple(p + len(permutation) for p in permutation),
          )
        case ():
          ...
        case _:
          raise NotImplementedError(f"Unsupported smem transforms {transforms}")
      if tiling != (64, swizzle_elems):
