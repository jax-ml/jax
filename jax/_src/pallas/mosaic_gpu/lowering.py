...
class ModuleContext:
  ...
  lowering_semantics: mgpu.LoweringSemantics
  user_thread_semantics: mgpu.UserThreadSemantics
  ...
  def body(launch_ctx: mgpu.LaunchContext, *buffers: ir.Value):
    *buffers_gmem, (runtime_smem, runtime_barriers, runtime_tmem) = buffers
    ...
    module_ctx = ModuleContext(
        ...
        user_thread_semantics=mgpu.UserThreadSemantics.Warpgroup,
    )
...
mosaic_lowering_rules = {
    ...
    (mgpu.LoweringSemantics.Lane, mgpu.UserThreadSemantics.Warpgroup): {} ,
    # Lowering rules when using Mosaic GPU warpgroup semantics.
    (mgpu.LoweringSemantics.Warpgroup, mgpu.UserThreadSemantics.Warpgroup): {},
}
...
def register_lowering_rule(
    ...
    lowering_semantics: mgpu.LoweringSemantics,
    user_thread_semantics: mgpu.UserThreadSemantics = mgpu.UserThreadSemantics.Warpgroup,
):
  def deco(fn):
    mosaic_lowering_rules[
        (lowering_semantics, user_thread_semantics)][primitive] = fn
    return fn
  ...
  for eqn in jaxpr.eqns:
    ...
    with source_info_util.user_context(eqn.source_info.traceback), loc:
      if eqn.primitive not in mosaic_lowering_rules[
          (module_ctx.lowering_semantics, module_ctx.user_thread_semantics)]:
        raise NotImplementedError(
            ...
            f"{module_ctx.user_thread_semantics}. "
...
mosaic_lowering_rules[mgpu.LANExWG_SEMANTICS].update({
    lax.neg_p: lambda ctx, x: -x,
    lax.not_p: lambda ctx, x: ~x,
})

mosaic_lowering_rules[mgpu.WGxWG_SEMANTICS].update({
...
for op, si_impl, ui_impl, f_impl in [
  ...
  mosaic_lowering_rules[mgpu.WGxWG_SEMANTICS][op] = partial(
...
mosaic_lowering_rules[mgpu.LANExWG_SEMANTICS][lax.logistic_p] = _lower_fun(
    _logistic, multiple_results=False
)
mosaic_lowering_rules[mgpu.WGxWG_SEMANTICS][lax.logistic_p] = (
