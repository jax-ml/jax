...
def arrive_expect_tx(barrier, *refs):
...
def _arrive_expect_tx_abstract_eval(barrier, *, shapes):
  del shapes  # Unused
  _check_ref(barrier, "barrier", gpu_core.SMEM)
...
def _arrive_expect_tx_lowering(ctx, barrier, shapes, warpgroup_sync=True):
  bytes = 0
  for sh in shapes:
    bits = pallas_utils.dtype_bitwidth(sh.dtype) * math.prod(sh.shape)
    if bits % 8:
      raise ValueError(
          f"Can only transfer integer bytes (shape={sh.shape},"
          f" dtype={sh.dtype})"
      )
    bytes += bits // 8
  if bytes == 0:
...
def copy_gmem_to_smem(
    ...
    *,
    ...
    emit_expect_arrivals: bool = True,
) -> None:
  ...
  if emit_expect_arrivals:
    arrive_expect_tx(barrier, src)
