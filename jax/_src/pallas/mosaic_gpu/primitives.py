...
def barrier_test(
  ...
  Args:
    barrier: The barrier to test.
    predicate: Predicate specifying whether or not to test the barrier. If
      ``None``, the barrier will be tested.

  Returns:
    A boolean array indicating whether or not the barrier is complete. If
    ``predicate`` is ``False``, returns ``False``.
  """
  barrier, transforms = state_primitives.get_ref_and_transforms(
