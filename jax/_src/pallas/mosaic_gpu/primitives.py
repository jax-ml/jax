...
async_store_p = jax_core.Primitive("async_store")
async_store_p.multiple_results = True


@async_store_p.def_effectful_abstract_eval
def _async_store_abstract_eval(
    src,
    ref,
    barrier,
    cluster_idx,
  ...
  if math.prod(transformed_barrier.shape) != 1:
    raise TypeError(
        "Expected a single barrier, got a barrier reference with shape"
        f" {transformed_barrier.shape}"
    )

  effs = {gpu_core._memory_effect, state.WriteEffect(1), state.WriteEffect(2)}
  return (), effs


@lowering.register_lowering_rule(async_store_p, mgpu.LoweringSemantics.Lane)
@lowering.register_lowering_rule(async_store_p, *gpu_core.LANExWARP_SEMANTICS)
def _async_store_lowering(
    ctx: lowering.LoweringRuleContext,
    src,
    ref,
    barrier,
  ...
  match remaining_ref_transforms:
    case (gpu_core.UnswizzleRef(swizzle), gpu_core.UntilingTransform(tiling)):
      pass
    case _:
      raise NotImplementedError("async_store requires a tiled and swizzled ref")
...
def async_store(
  ...
  Args:
    ...
    cluster_idx: The index of the target cluster block.
