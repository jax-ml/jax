...
def _tcgen05_mma_abstract_eval(acc, a, b, barrier, accumulate,
                               *transforms_leaves,
                               ...
                               collective_axis):
  del (accumulate, transforms_leaves, acc_transforms_tree, a_transforms_tree,
       b_transforms_tree, barrier_transforms_tree)
...
def _tcgen05_mma_lowering(
  ...
  (acc_transforms_leaves, a_transforms_leaves, b_transforms_leaves,
   barrier_transforms_leaves, _) = (
      util.split_list(
          transforms_leaves,
  ...
  if acc_transforms_tree is not None:
    ...
    acc, acc_transforms = lowering._handle_transforms(
        ctx, acc, acc_transforms, handle_transposes=False, handle_reshapes=False
    )
    if acc_transforms:
      raise NotImplementedError(f"Unsupported transforms: {acc_transforms}.")
    ...
    match a_transforms:
      ...
      case ():
        if isinstance(a_ref, tcgen05.TMEMRef):
          lhs_tiling = None  # Avoid undefined variable below.
        else:
          raise NotImplementedError(f"Unsupported transforms: {a_transforms}.")
      case _:
        raise NotImplementedError(f"Unsupported transforms: {a_transforms}.")
