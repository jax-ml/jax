  ...
  def inner(f):
    def wrapper(*args):
      ...
      for a, t in zip(flat_args, flat_arg_types):
        ...
        # Unwrap from the tracer
        aval = a
        if isinstance(a, jax_core.Tracer):
          aval = a.aval

        if isinstance(t, (ParameterizedLayout, Layout)) and isinstance(aval, jax_core.ShapedArray):
          raw_refs_flat_args.append(a)
        elif isinstance(t, RefType) and isinstance(aval, state_types.TransformedRef):
          tmp_ref = dataclasses.replace(aval, transforms=())
          tmp_ref = functools.reduce(lambda r, t: t.undo(r), reversed(t.transforms), tmp_ref)
          if tmp_ref != a:
            raise ValueError(f"Transform mismatch: {tmp_ref.transforms} != {t.transforms}")
          raw_refs_flat_args.append(tmp_ref.ref)
        elif isinstance(t, RefType) and isinstance(aval, pallas_core.AbstractMemoryRef) and not t.transforms:
          raw_refs_flat_args.append(a)
        else:
          raise ValueError(f"Mismatched type: {aval, t}")
  ...
  match (ty, v):
    ...
    case (GPUShapeDtypeStruct(), mgpu.FragmentedArray()):
      ...
      if v.mlir_dtype != mlir_dtype or ty.shape != v.shape or v.layout != ty.layout.to_mgpu():
        raise ValueError(
            f"Array type mismatch at {v.mlir_dtype, v.shape, v.layout} !="
            f" {mlir_dtype, ty.shape, ty.layout.to_mgpu()}."
        )
