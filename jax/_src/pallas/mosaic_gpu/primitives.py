...
def semaphore_signal_parallel(
    semaphores: tuple[_Ref, ...],
    incs: int | jax.Array | tuple[int | jax.Array, ...] = 1,
    *,
    device_ids: pallas_primitives.DeviceId | tuple[pallas_primitives.DeviceId, ...] = None,
):
  ...
  """
  num_signals = len(semaphores)
  refs, transforms = util.unzip2(
      map(pallas_primitives._get_ref_and_transforms, semaphores)
  )
  if not isinstance(incs, tuple):
    incs = (incs,) * num_signals
  incs = [jnp.asarray(i, dtype=jnp.int32) for i in incs]
  if not isinstance(device_ids, tuple):
    device_ids = (device_ids,) * num_signals
