...
def planar_snake(
  ...
  """
  tile_width_is_power_of_2 = (tile_width & (tile_width - 1)) == 0
  major_size_is_power_of_2 = (
  ...
  is_full_tile = lin_idx < num_full_tiles * tile_size

  # If `tile_width` is a power of 2, we branch, as we expect the first branch
  # to be significantly faster. Otherwise, branchless is better.
  if tile_width_is_power_of_2:
    tile_idx, lin_idx_within_tile = lax_divmod(lin_idx, tile_size)
    major, minor_within_tile = lax.cond(
        is_full_tile,
    ...
    def full():
      # We express the calculation differently here so if `major_size` is a
      # power of two, at least one of the divmods will be cheap.
      line_idx, minor = lax_divmod(lin_idx, tile_width)
      tile_idx, major = lax_divmod(line_idx, major_size)
      return tile_idx, major, minor

    def last():
      lin_idx_within_tile = lin_idx - num_full_tiles * tile_size
      return num_full_tiles, *lax_divmod(lin_idx_within_tile, last_tile_width)

    if major_size_is_power_of_2:
      tile_idx, major, minor_within_tile = lax.cond(is_full_tile, full, last)
    else:
      sel = functools.partial(lax.select, is_full_tile)
      tile_idx, major, minor_within_tile = jax.tree.map(sel, full(), last())

  is_even_tile = lax.rem(tile_idx, jnp.int32(2)) == 0
  major = lax.select(is_even_tile, major, major_size - 1 - major)
