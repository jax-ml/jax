...
class JaxprInterpreter:
  ...
  def _interpret_copy_smem_to_gmem_p(
    ...
    return callback.io_callback(
        ...
        thread=self.thread,
        src_allocation_key_as_array=src,
        raw_src_transforms=src_transforms,
        dst_allocation_key_as_array=dst,
        raw_dst_transforms=dst_transforms,
        predicate=predicate,
    ), []
  ...
  def _interpret_wgmma_ref_p(
    ...
    return callback.io_callback(
        ...
        raw_acc_transforms=acc_transforms,
        a_allocation_key_as_array=a,
        raw_a_transforms=a_transforms,
        b_allocation_key_as_array=b,
        raw_b_transforms=b_transforms,
    ), []
  ...
  def _interpret_tcgen05_mma_p(self, eqn, token: jax.Array, invals):
    ...
    return callback.io_callback(
        ...
        raw_a_scale_transforms=a_scale_transforms,
        b_scale_allocation_key_as_array=b_scale,
        raw_b_scale_transforms=b_scale_transforms,
        a_sparse_metadata_allocation_key_as_array=a_sparse_metadata,
        raw_a_sparse_metadata_transforms=a_sparse_metadata_transforms,
    ), []
  ...
  def _interpret_async_copy_smem_to_tmem_p(
    ...
    return (
        callback.io_callback(
            ...
            raw_smem_transforms=smem_transforms,
            tmem_allocation_key_as_array=tmem,
            raw_tmem_transforms=tmem_transforms,
        ),
        [],
    )
  ...
  def _interpret_async_load_tmem_p(self, eqn, token: jax.Array, invals):
    ...
    token, out = callback.io_callback(
        ...
        raw_src_transforms=jax.tree.unflatten(eqn.params["tree"], invals[1:]),
    )
    return token, [out]
  ...
  def _interpret_async_store_tmem_p(
    ...
    return callback.io_callback(
        ...
        raw_dst_transforms=jax.tree.unflatten(tree, dst_transform_vals),
        vals=value,
    ), []
