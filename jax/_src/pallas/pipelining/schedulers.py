...
def static_rolled_scheduler(
    nd_loop: internal.NDLoopStruct,
    args: Sequence[Any],
    initial_state: PipelineState | None = None,
    eval_fn: EvalStageFunc | None = None,
):
  """A pipeline scheduler that rolls the pipeline into a single loop.

  This scheduler rolls the prologue, main body, and epilogue into a single loop.
  This significantly reduces the program size at the cost of introducing more
  dynamic control flow. The scoreboard is still tracked statically.
  """
  if eval_fn is None:
    eval_fn = eval_stage
  ...
  def compute_offsets(scoreboard: Scoreboard) -> Sequence[int]:
    while any(scoreboard.stage_counters[i] < 1 for i in range(num_stages)):
      new_scoreboard = scoreboard.copy()
      for stage_idx, stage in enumerate(stages):
        current_stage_counter = scoreboard.stage_counters[stage_idx]
  ...
  offsets = compute_offsets(scoreboard)
  max_offset = max(offsets)
  scoreboard = dataclasses.replace(scoreboard, stage_counters=offsets)
  ...
  def loop_body(itr: jax.Array, carry: tuple[PipelineState, GridCarry]):
    ...
    while any(stages_left):
      ...
      for stage_idx, stage in enumerate(stages_left):
        ...
        if can_run:
          stage_offset = offsets[stage_idx]
          stage_itr = itr - max_offset + stage_offset
          in_bounds = jnp.logical_and(stage_itr >= 0, stage_itr < num_itrs)
          ctx = make_ctx(
                      stage,
                      stage_idx,
                      Scoreboard.create(stages),
                      pipeline_state,
                      grid_carry=grid_carry,
                      offset=stage_itr,
                  )
          pipeline_state: PipelineState = lax.cond(in_bounds,
                            lambda ref_args: eval_fn(ctx, stage, ref_args),  # pytype: disable=cell-var-from-loop
                            lambda ref_args: pipeline_state,
                            args
                            )
          new_scoreboard.increment_stage_counter(stage_idx)
          stages_left[stage_idx] = None
  ...
  num_loop_itrs = num_itrs + max_offset
  grid_carry = GridCarry.init(
      offsets=offsets, grid=nd_loop.grid, dynamic=True)
  init_carry = (initial_state, grid_carry)
  jax.lax.fori_loop(0, num_loop_itrs, loop_body, init_carry)
