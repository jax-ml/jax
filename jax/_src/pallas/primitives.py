...
def device_id() -> jax.Array:
  return device_id_p.bind()

@device_id_p.def_abstract_eval
def device_id_abstract_eval(**_):
  return jax_core.ShapedArray((), jnp.int32)
...
def check_sem_avals(
  ...
  # Uncomment check below when semaphores on GPUs get allocated in scratch spaces
  # if not any(
...
def dma_start_discharge_rule(in_avals, out_avals,
  ...
  recv_size = jnp.minimum(updates.size, pallas_core.SEMAPHORE_MAX_VALUE)
  recv_size = jnp.array(recv_size, dtype=pallas_core.SEMAPHORE_INTERPRET_DTYPE)
  ...
  if is_remote:
    send_size = jnp.minimum(local_src.size, pallas_core.SEMAPHORE_MAX_VALUE)
    send_size = jnp.array(send_size, dtype=pallas_core.SEMAPHORE_INTERPRET_DTYPE)
...
def dma_wait_discharge_rule(in_avals, out_avals,
  ...
  copy_size = jnp.minimum(updates.size, pallas_core.SEMAPHORE_MAX_VALUE)
  copy_size = jnp.array(copy_size, dtype=pallas_core.SEMAPHORE_INTERPRET_DTYPE)
