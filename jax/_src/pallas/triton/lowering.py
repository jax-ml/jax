...
def _masked_load_lowering_rule(
  ...
  *_, mask_aval, other_aval = args_tree.unflatten(ctx.avals_in)
  ...
  offsets32 = _ir_cast(offsets, ir.IntegerType.get_signless(32), signed=False)

  if block_info.full_shape_dtype.dtype in (jnp.int4, jnp.uint4):
    offsets = _floordiv(offsets, _full(offsets.type, 2), signed=False)

  shape = idx.get_indexer_shape()
  ptr = _add(_bcast_to(ptr, shape), offsets)
  if mask is not None:
  ...
  in_lsb = _mod(offsets32, _full(offsets32.type, 2), signed=False)
