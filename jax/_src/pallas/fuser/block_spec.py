...
def _concatenate_push_rule(
    ctx: PushRuleContext,
    *input_block_specs: pallas_core.BlockSpec,
    dimension: int,
):
  if not input_block_specs:
    raise ValueError("concatenate requires at least one input")

  out_concat_size = 0
  for bs, aval_in in zip(input_block_specs, ctx.avals_in):
    assert isinstance(aval_in, core.ShapedArray)
    block_dim = bs.block_shape[dimension]
    if block_dim is None or isinstance(block_dim, pallas_core.Squeezed):
      block_dim = 1
    b_size = _block_size(block_dim)
    if (b_size is not None and b_size != aval_in.shape[dimension]) or (
        b_size is None and aval_in.shape[dimension] != 1
    ):
      raise ValueError(
          "Only support fully-contiguous inputs along the concatenated axis"
      )
    out_concat_size += aval_in.shape[dimension]

  ref_bs = input_block_specs[0]
  ref_block_shape = tuple(ref_bs.block_shape)
  new_block_shape = util.tuple_update(
      ref_block_shape, dimension, out_concat_size
  )
  for bs in input_block_specs[1:]:
    bs_block_shape = tuple(bs.block_shape)
    for i in range(len(new_block_shape)):
      if i == dimension:
        continue
      if bs_block_shape[i] != ref_block_shape[i]:
        raise ValueError("Block shapes must match for non-concatenated axes")

  def new_index_map(*args):
    idx = ref_bs.index_map(*args)
    return util.tuple_update(idx, dimension, 0)
