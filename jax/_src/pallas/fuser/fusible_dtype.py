...
def physicalize(f):
  ...
  def wrapper(*args, **kwargs):
    ...
    args_avals = jax.tree.map(core.typeof, args)
...
def physicalize_closed_jaxpr(jaxpr: core.ClosedJaxpr) -> core.ClosedJaxpr:
  ...
  in_avals_ft = tree_util.FlatTree.flatten((tuple(in_avals), {}))
  ...
  assert len(closed_jaxpr.jaxpr.constvars) == len(closed_jaxpr.consts), "Mismatched consts"
  return closed_jaxpr
...
def physicalize_jaxpr(jaxpr: core.Jaxpr) -> core.Jaxpr:
  ...
  in_avals_ft = tree_util.FlatTree.flatten(((tuple(const_avals), tuple(in_avals)), {}))
