...
def _unbatch(fusion: fusion_lib.Fusion, axis: int | None):
  ...
  axis_size = tree_util.tree_leaves(fusion.out_type)[0].shape[axis]

  if args:
    arg_specs = tuple(map(get_spec, args))
    spec = block_spec.push_block_spec(fusion, *arg_specs)(*args)
  else:
    spec = tree_util.tree_map(get_spec, fusion.out_type)

  spec_puller = block_spec.pull_block_spec(fn, spec, grid=(axis_size,))
...
def _fusible_batching_rule(
    args, in_axes, *, func, output_fusion_prefix, in_tree, **_
):
  def new_func(*fusions):
    *in_fusions, out_fusions = fusions

    # For each fusion, use `get_fusion_values` and `pull_block_spec` to create
    # a new "un-vmapped" fusion. Call `func` with the new fusions and their
    # values, then vmap over the result
    in_fns, in_values, in_value_axes = zip(*map(_unbatch, in_fusions, in_axes))

    if out_fusions is None:
      out_fns = out_values = out_value_axes = None
    else:
      unbatch_out = functools.partial(_unbatch, axis=0)
      out_fusion_tree = tree_util.tree_structure(out_fusions)
      out_fns, out_values, out_value_axes = tree_util.tree_transpose(
          out_fusion_tree, None, tree_util.tree_map(unbatch_out, out_fusions)
      )

    axis_name = object()

    def unbatched(in_values, out_values):
      batch_idx = jax.lax.axis_index(axis_name)

      def get_fusion(fn, values, *arg_types):
        fn_closed = lambda *args: fn((batch_idx,), (), values, *args)
        in_type = (arg_types, {})
...
batching.primitive_batchers[fusible_p] = _fusible_batching_rule
