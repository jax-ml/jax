...
def fuse_jaxpr(
  ...
  def fusible_with_fusions():
    return fusible.func(*in_fusions, output_fusions)

  _, in_tree = tree_util.tree_flatten(())
  debug_info = api_util.debug_info(
      "fusible_with_fusions", fusible_with_fusions, (), {}
  )
  flat_fun, out_tree_thunk = api_util.flatten_fun_nokwargs(
      lu.wrap_init(fusible_with_fusions, debug_info=debug_info), in_tree
  )
  traced_jaxpr, _, traced_consts = pe.trace_to_jaxpr_dynamic(flat_fun, [])
  if traced_jaxpr.effects:
    raise ValueError(
        "Effects are not supported in fusible with infer_types. "
        f"Tracing revealed effects: {traced_jaxpr.effects}."
    )
  out_flat = jax_core.eval_jaxpr(traced_jaxpr, traced_consts)
