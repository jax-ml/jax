...
class ShardMapTrace(core.Trace):
  ...
  def process_shard_map(self, prim, fun, args, mesh, in_specs,
    ...
    return out_vals.map2(
        out_mats, lambda val, vma: ShardMapTracer(self, vma, val)
    )
...
def _add_reshapes(which: Sequence[bool],
  ...
  jaxpr_known_closed, _ = pe.trace_to_jaxpr_internal(
  ...
  avals_in = ft.flatten((avals_in, {}))
  jaxpr_staged_closed, _ = pe.trace_to_jaxpr_internal(
