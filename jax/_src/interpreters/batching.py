...
def callback_batching_rule(
  ...
  (axis_size,) = {
      a.shape[d] for a, d in zip(args, dims) if d is not not_mapped
  }
  new_args = [
      arg if dim is not_mapped else moveaxis(arg, dim, 0)
      for arg, dim in zip(args, dims)
  ]
  batched_result_avals = tuple(
      core.unmapped_aval(axis_size, 0, aval)
  ...
  if vmap_method == "legacy_vectorized":
    ...
    bcast_args = [
        jax.lax.broadcast(x, (size,)) if d is not_mapped else x
    ...
    is_batched = [d is not not_mapped for d in dims]
