...
def _partial_eval_jaxpr_custom_cached(
    ...
    saveable: Callable[..., RematCases | bool],
  ) -> tuple[Jaxpr, Jaxpr, list[bool], list[bool], int, int]:
  ...
  for eqn in jaxpr.eqns:
    ...
    if rule:
      ...
      policy = ensure_enum(saveable(eqn.primitive, *[x.aval for x in eqn.invars],
                                    **eqn.params))

      if eqn.effects or policy is Saveable:
        map(partial(write, False, False), eqn.outvars)
      elif type(policy) == Offloadable:
        from jax._src.dispatch import device_put_p, TransferToMemoryKind
...
RematCases = Recompute | Saveable | Offloadable
