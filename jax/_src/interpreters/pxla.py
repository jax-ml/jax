...
def _cached_lowering_to_hlo(closed_jaxpr, api_name, fun_name, backend,
  ...
  if len(device_assignment) > 1:
    ...
    unsupported_effects = effects.shardable_ordered_effects.filter_not_in(unsupported_effects)
    if len(unsupported_effects) > 0:
      raise ValueError(
        "The following ordered effects are not supported for more than 1 device: " +
        str(unsupported_effects))
