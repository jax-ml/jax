...
from jax._src.tree_util import (tree_map, tree_flatten, tree_unflatten,
                                tree_leaves, register_pytree_node, PyTreeDef,
                                FlatTree)
...
def linearize_subtrace(_f: Callable, _store: lu.Store, _is_vjp: bool,
  ...
  fwd_out_ty = FlatTree.flatten((out_primals, res, structured_residuals)).map(typeof)
...
def linearize_jaxpr(
    ...
    *,
    is_vjp: bool,
) -> tuple[core.ClosedJaxpr, FlatTree, Sequence[bool], Sequence[int | None], core.ClosedJaxpr]:
...
def _linearize_jaxpr(
  ...
  fwd_jaxpr = core.ClosedJaxpr(fwd_jaxpr, fwd_consts)

  fwd_out_ty = FlatTree.flatten((out_primals, tangent_consts, structured_residuals)).map(typeof)
...
def direct_linearize(traceable, primals, *, has_aux, is_vjp):
  ...
  if has_aux:
    return out_primals, out_zeros, jaxpr, consts, structured_residuals, aux
  else:
    return out_primals, out_zeros, jaxpr, consts, structured_residuals
