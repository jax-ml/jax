...
class LoweringParameters:
  ...
  # During lowering hoist the core.Literal constants as args for the main
  # function.
  hoist_constants_as_args: bool = True
...
class LoweringCacheValue:
  ...
  hoisted_consts: list[ArrayLike]  # The hoisted constants expected by `func`
...
class LoweringRuleContext:
  ...
  # The values that should be used for the Literal constants, by id of the const.
...
def lower_jaxpr_to_module(
  ...
  runtime.
  The inputs already account for the hoisted constants.
  """
  ...
  with ctx.context, ir.Location.unknown(ctx.context):
    ...
    lower_jaxpr_to_fun(
        ...
        num_hoisted_consts=num_hoisted_consts,
        main_function=True,
...
def _hoist_const_args(lowering_params: LoweringParameters,
                      main_function: bool) -> bool:
  return (lowering_params.hoist_constants_as_args or not main_function)
...
def lower_jaxpr_to_fun(
  ...
  nr_args = num_hoisted_consts + len(jaxpr.in_avals)

  try:
    assert nr_args == len(in_avals), (nr_args, in_avals)
  except:
    stop = 1
  assert replicated_args is None or nr_args == len(replicated_args), (nr_args, replicated_args)
  assert arg_shardings is None or nr_args == len(arg_shardings), (nr_args, arg_shardings)
  assert arg_layouts is None or nr_args == len(arg_layouts), (nr_args, arg_layouts)
  assert arg_memory_kinds is None or nr_args == len(arg_memory_kinds), (nr_args, arg_memory_kinds)
  assert arg_names is None or nr_args == len(arg_names), (nr_args, arg_names)

  # Function inputs: *dim_var_values, *tokens, *hoisted_consts, *actual_inputs
  ...
  # Order of arguments: dim vars, tokens, hoisted_consts, array inputs
  ...
  if (
    ...
    if num_hoisted_consts > 0:
      const_arg_attrs = arg_attrs[num_dim_vars + num_tokens :
                                  num_dim_vars + num_tokens + num_hoisted_consts]
  ...
  with ir.InsertionPoint(entry_block):
    ...
    dim_var_values, _, hoisted_const_values, _ = util.split_list(
        flat_args, [num_dim_vars, num_tokens, num_hoisted_consts])
    ...
    if num_hoisted_consts == 0:
      ...
      hoisted_const_values = [ir_constant(xla.canonicalize_dtype(c))
                              for c in  hoisted_consts]
    const_lowering = {id(c): c_arg
                      for c, c_arg in zip(hoisted_consts,
...
def jaxpr_subcomp(ctx: ModuleContext, jaxpr: core.Jaxpr,
  ...
  def read(v: core.Atom) -> IrValues:
    if type(v) is core.Literal:
      ...
      if c is None:
        c = _lower_constant(v.val, const_lowering)
  ...
  assert all(_is_ir_values(v) for v in consts_for_constvars), consts_for_constvars
  assert isinstance(name_stack, source_info_util.NameStack), type(name_stack)
  assert len(args) == len(jaxpr.invars), (jaxpr, args)
  assert len(consts_for_constvars) == len(jaxpr.constvars), (jaxpr, consts_for_constvars)
...
def _lower_constant(
    const: ArrayLike,
    const_lowering: dict[int, IrValues]) -> IrValues:
  if np.shape(const) and (c_val := const_lowering.get(id(const))) is not None:
    return c_val
  # TODO(necula): some constants arise not from the original Jaxpr but from
  #  the lowering itself, e.g., for PRNG. We could lift them also, but we
  #  can only do that after lowering.
  return ir_constant(xla.canonicalize_dtype(const))
...
def _cached_lowering(ctx: ModuleContext, eqn: core.JaxprEqn,
  ...
  hoisted_const_args = tuple(_lower_constant(c, const_lowering)
                             for c in cache_entry.hoisted_consts)
  args = flatten_ir_values(dim_var_values + tokens_in_args + hoisted_const_args + args)
...
def _emit_lowering_rule_as_fun(
  ...
  hoisted_consts =  core.eqn_params_hoisted_consts(params)
  hoisted_const_avals = tuple(core.abstractify(c) for c in hoisted_consts)

  input_types = map(aval_to_ir_type, hoisted_const_avals + avals_in)  # type: ignore
  ...
  with ir.InsertionPoint(entry_block):
    ...
    dim_var_values, token_args, hoisted_const_args, unflattened_args = \
      util.split_list(unflattened_args,
                      [num_dim_vars, len(ordered_effects), len(hoisted_consts)])
    const_lowering = {id(c): c_arg
                      for c, c_arg in zip(hoisted_consts, hoisted_const_args)}
  ...
  return LoweringCacheValue(func_op, output_types, hoisted_consts, inline)
  ...
  def f_lowered(ctx: LoweringRuleContext, *args, **params):
    ...
    with manager:
      if config.dynamic_shapes.value:
        ...
        jaxpr, _, consts_for_constvars = pe.trace_to_jaxpr_dynamic(wrapped_fun, ctx.avals_in)
...
def _lower_jaxpr_to_fun_cached(ctx: ModuleContext,
  ...
  if not call_jaxpr.consts and arg_names is result_names is None:
    ...
    func_op = lower_jaxpr_to_fun(
        ctx, fn_name, call_jaxpr, effects,
        num_hoisted_consts=num_hoisted_consts, in_avals=in_avals,
...
def call_lowering(fn_name, call_jaxpr: core.ClosedJaxpr | core.Jaxpr, backend,
  ...
  hoisted_consts = core.jaxpr_hoisted_consts(call_jaxpr)
  hoisted_const_values = [_lower_constant(c, const_lowering)
                          for c in hoisted_consts]
  args = tuple(hoisted_const_values) + args
  if arg_names is not None:
    arg_names = [""] * len(hoisted_consts) + arg_names
  in_avals = tuple(core.abstractify(c) for c in hoisted_consts) + tuple(in_avals)

  func_op, output_types, effects = lower_called_computation(
      fn_name, call_jaxpr, ctx, len(hoisted_consts), in_avals, out_avals,
      tokens_in,
...
def build_mlir_module_helper(
  ...
  assert (not config.use_simplified_jaxpr_constants.value or
          not core.jaxpr_hoisted_consts(closed_jaxpr.jaxpr))
  lowering_result = lower_jaxpr_to_module(name, closed_jaxpr,
                                          num_hoisted_consts=0,
