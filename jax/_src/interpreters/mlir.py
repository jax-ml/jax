...
def _emit_lowering_rule_as_fun(
  ...
  hoisted_const_avals = tuple(core.shaped_abstractify(c) for c in hoisted_consts)
...
def call_lowering(fn_name, call_jaxpr: core.ClosedJaxpr | core.Jaxpr, backend,
  ...
  if isinstance(call_jaxpr, core.ClosedJaxpr):
    hoisted_consts = core.jaxpr_hoisted_consts(call_jaxpr.jaxpr)
  else:
  ...
  in_avals = tuple(core.shaped_abstractify(c) for c in hoisted_consts) + tuple(in_avals)
