...
def logpdf(x: ArrayLike,
  ...
  return jnp.where(ok, log_pdf, jnp.nan)


def pdf(x: ArrayLike, loc: ArrayLike = 0, scale: ArrayLike = 1) -> ArrayLike:
  r"""
...
def logcdf(x: ArrayLike,
  ...
  log_cdf = lax.log1p(lax.neg(lax.exp(neg_exp_z)))
  return jnp.where(ok, log_cdf, jnp.nan)


def cdf(x: ArrayLike, loc: ArrayLike = 0, scale: ArrayLike = 1) -> ArrayLike:
  r"""
...
def ppf(p: ArrayLike, loc: ArrayLike = 0, scale: ArrayLike = 1) -> ArrayLike:
  r"""
  ...
  return jnp.where(ok, quantile, jnp.nan)


def sf(x: ArrayLike, loc: ArrayLike = 0, scale: ArrayLike = 1) -> ArrayLike:
  r"""
...
def logsf(x: ArrayLike, loc: ArrayLike = 0, scale: ArrayLike = 1) -> ArrayLike:
  r"""
  ...
  return jnp.where(ok, log_sf, jnp.nan)
