...
class StatefulPRNG:
  ...
  def split(self, num: int | Sequence[int]) -> StatefulPRNG:
    ...
    See also:
      - :meth:`jax.random.StatefulPRNG.spawn`: This is similar to ``split``, but
  ...
  def spawn(self, n_children: int) -> list['StatefulPRNG']:
    ...
    See also:
      - :meth:`jax.random.StatefulPRNG.split`: this is similar to spawn, but returns
        a single mapped :class:`jax.random.StatefulPRNG`` which can be passed to
        :func:`jax.vmap`.
    """
...
def stateful_rng(seed: typing.ArrayLike | None = None, *,
  ...
  Returns:
    A :class:`~jax.random.StatefulPRNG` object, with methods for generating
    random values.

  Notes:
    The :class:`~jax.random.StatefulPRNG` object created by this method uses
  ...
  if seed is None:
    if not core.trace_ctx.is_top_level():
      raise TypeError(
        "When used within transformed code, jax.random.stateful_rng()"
