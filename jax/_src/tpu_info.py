...
class TpuInfo:
  ...
  def get_sublane_tiling(self, dtype: dtypes.DTypeLike) -> int:
...
def get_tpu_generation() -> int:
  """Returns the TPU generation number (e.g.

  2, 3, 4, 5, 6, 7, 8) or 0 if not on TPU.
  """
  try:
    return get_tpu_info().generation
  except Exception:
    return 0
...
def get_tpu_info_for_chip(
  ...
  if (
    ...
    raise ValueError(
        ...
        " support Megacore must have num_tensor_cores_per_logical_device=1,"
        f" but got {num_tensor_cores_per_logical_device}."
    )
...
def get_device_kind() -> str:
  ...
  try:
    ...
    return devices[0].device_kind if devices else ""
  except Exception:
    return ""
...
def get_num_device_cores() -> int:
  ...
  try:
    devices = xla_bridge.devices()
    return getattr(devices[0], "num_cores", 1) if devices else 1
  except Exception:
    return 1
