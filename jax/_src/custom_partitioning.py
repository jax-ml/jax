...
class custom_partitioning:
  ...
  * ``sharding_rule``: an SdyShardingRule object, an Einsum-like notation string
  ...
  def def_partition(self, partition, infer_sharding_from_operands,
    ...
    self.sharding_rule = (
      sharding_rule
      if sharding_rule is None or isinstance(sharding_rule, Callable) or isinstance(sharding_rule, SdyShardingRule)
...
def _custom_partitioning_lowering_rule(ctx: mlir.LoweringRuleContext, *values,
  ...
  if sharding_rule is not None:
    ...
    if not isinstance(sharding_rule, SdyShardingRule):
      ...
      if isinstance(sharding_rule, str):
        ...
        if not isinstance(sharding_rule, SdyShardingRule):
          raise ValueError("sharding_rule callable must produce either an SdyShardingRule object or an Einsum-like notation string.")
