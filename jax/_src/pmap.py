...
from jax._src.tree_util import (
    broadcast_flattened_prefix_with_treedef, broadcast_prefix, prefix_errors,
    tree_flatten, tree_map, tree_unflatten
)
...
def pmap(f, axis_name=None, *, in_axes=0, out_axes=0,
  ...
  if devices is not None:
    ...
    try:
      ...
      raise TypeError("'devices' argument to pmap must be a sequence of Devices, or None.")
  ...
  def infer_params(*args, **kwargs):
    ...
    dyn_f, dyn_argnums, dyn_args = get_dyn_args(wrapped_f, static_broadcasted_tuple, args)
    ...
    in_axes_flat = get_in_axes_flat(in_axes, dyn_argnums, dyn_args, kwargs, len(dyn_args_flat), dyn_args_tree)
    local_axis_size = axis_size or mapped_axis_size(dyn_args_flat, in_axes_flat)
    donated_invars = get_donated_invars(donate_tuple, dyn_args_tree, len(dyn_args_flat))
    mesh_devices = get_mesh_devices(devices, backend, local_axis_size, trace_state_clean)
    cached = _cached_shard_map(dyn_f, dyn_args_tree, in_axes_flat, out_axes_flat,
        out_axes_tree, donated_invars, mesh_devices, axis_name)
    if process_count > 1:
      dyn_args_flat = host_local_array_to_global_array(
          dyn_args_flat, cached, trace_state_clean, donated_invars)
    return cached, dyn_args_flat, dyn_args_tree, donate_tuple, process_count, trace_state_clean

  @util.wraps(f)
  def wrapped(*args, **kwargs):
    cached, dyn_args_flat, _, _, process_count, trace_state_clean = infer_params(*args, **kwargs)
  ...
  def lower(*args, **kwargs):
    cached, args_flat, in_tree, donated_tuple, _, _ = infer_params(*args, **kwargs)
    ...
    return stages.Lowered(lowered._lowering, args_info, lowered.out_tree,  # pylint: disable=protected-access
...
@lu.cache
def _cached_shard_map(fun, in_tree, in_axes_flat, out_axes_flat,
    out_axes_tree, donated_invars, mesh_devices, axis_name):
  ...
  out_specs = tree_map(partial(_axes_to_pspec, axis_name), out_axes, is_leaf=lambda x: x is None)

  def _fun(*flat_args):
    args = tree_map(lambda x, ax: x if ax is None else lax.squeeze(x, [ax]),
        flat_args, in_axes_flat)
  ...
  out_global_shardings = tree_map(
      ...
      out_specs, is_leaf=lambda x: x is None)
...
class CachedShardMap(NamedTuple):
  ...
  in_specs_flat: tuple[Any, ...]  # Flattened in_specs for batched C++ call
  ...
  mesh: Mesh
  out_specs: Any  # out_specs prefix (pytree of PartitionSpecs)
  out_local_shardings_thunk: Callable[..., Any]
  donate_argnums: list[int]
  out_global_shardings: Any  # pytree of shardings
...
def get_mesh_devices(
  ...
  if devices is not None:
    ...
    mesh_devices = tuple(
        d for process_index in range(process_count)
...
def _local_to_global_aval(shape, dtype, sharding):
  ...
  return pxla.mesh_local_to_global(
      sharding.mesh, sharding_impls.get_array_mapping(pspec_prepared), local_aval)
...
def _global_to_local_aval(shape, dtype, sharding):
  ...
  return pxla.mesh_global_to_local(
      sharding.mesh, sharding_impls.get_array_mapping(pspec_prepared), global_aval)
...
def host_local_array_to_global_array(dyn_args_flat, cached, trace_state_clean,
                                     donated_invars):
  ...
  Args:
    ...
    trace_state_clean: True if in execution mode (not tracing).
    donated_invars: Tuple of bools indicating which args are donated.
      For donated args that require the slow path, we delete the original
      to free memory.
  ...
  if not trace_state_clean:
    ...
    return list(mhu.host_local_array_to_global_array(
  ...
  if dyn_args_flat and isinstance(dyn_args_flat[0], (core.Tracer, core.AbstractValue)):
  ...
  for i, arr in enumerate(dyn_args_flat):
    ...
    if (typ == array.ArrayImpl and
        _is_sharding_equivalent(arr.sharding, local_sharding, len(arr.shape))):
      ...
      arrays = [arr[idx] for idx in _local_device_indices(local_sharding, shape)]
      dyn_args_flat[i] = pxla.batched_device_put(
          global_aval, global_sharding,
          arrays, list(local_sharding._device_assignment))  # pylint: disable=protected-access
  ...
  for i, arr in enumerate(out_flat):
    ...
    if typ == array.ArrayImpl:
      ...
      out_flat[i] = pxla.batched_device_put(
          ...
          list(local_sharding._device_assignment))  # pylint: disable=protected-access
    if prng_impl is not None:
