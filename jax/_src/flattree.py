...
from jax._src.util import (Either, safe_map, safe_zip, unzip2)
...
class FlatTree(Generic[T]):
  ...
  def _iter_update(self, xs_iter: Any) -> FlatTree[T]:
    raise NotImplementedError

  def unpack(self): raise TypeError(f"Not a FlatTree tuple: {self}")
  def unpack_aux(self) -> Any:
    raise NotImplementedError
...
class FTTuple(FlatTree):
  ...
  @cached_property
  def treedef(self):
    return tree_util.treedef_tuple(t.treedef for t in self.trees)  # pyrefly: ignore[bad-override]
...
class FTArgsAndKwargs(FlatTree):
  args: tuple[Either[Any, FlatTree[T]], ...]  # pyrefly: ignore[bad-specialization]
  kwarg_keys : tuple[Any, ...]
  kwarg_vals: tuple[Either[Any, FlatTree[T]], ...]  # pyrefly: ignore[bad-specialization]
  ...
  def __eq__(self, other):
    if not isinstance(other, FTArgsAndKwargs):
      return False
    if self.kwarg_keys != other.kwarg_keys:
      return False
    if len(self.args) != len(other.args) or len(self.kwarg_vals) != len(
        other.kwarg_vals
    ):
      return False
    for x, y in it.chain(
        zip(self.args, other.args), zip(self.kwarg_vals, other.kwarg_vals)
    ):
      if x.is_left != y.is_left:
        return False
      if x.is_left:
        if (
            type(x.from_left()) is not type(y.from_left())
            or x.from_left() != y.from_left()
        ):
          return False
      else:
        if x.from_right() != y.from_right():
          return False
    return True
  ...
  @cached_property
  def tree(self):
    return self.tree_without_statics
