...
from jax._src.sharding_impls import (
    ...
    prepare_axis_resources, parse_flatten_op_sharding)
  ...
  if any_auto:
    in_shardings: Tuple[MeshShardingMinusUnspecified, ...] = cast(  # type:ignore[no-redef]
        Tuple[MeshShardingMinusUnspecified, ...], tuple(
            NamedSharding._from_xla_op_sharding(i._op_sharding, mesh)  # type: ignore
            if isinstance(i, GSPMDSharding) else i for i in in_shardings))
    out_shardings: Tuple[MeshSharding, ...] = cast(  # type: ignore[no-redef]
        Tuple[MeshSharding, ...], tuple(
            NamedSharding._from_xla_op_sharding(o._op_sharding, mesh)  # type: ignore
...
def _sharding_constraint_hlo_lowering(ctx, x_node, *, sharding,
  ...
  if isinstance(axis_ctx, sharding_impls.SPMDAxisContext):
    mesh = resource_env.physical_mesh
    mps = NamedSharding._from_xla_op_sharding(sharding._op_sharding, mesh)
