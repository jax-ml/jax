...
def _batch_indexer(
  ...
  continuous, but become non-continuous in the new indexer due to the insertion
...
def _get_vmap(batched_args, batched_dims, *, tree):
  ...
  new_indexer = _batch_indexer(indexer, dims, axis_size, ref.shape, ref_dim, idx_is_batched)
  ...
  is_new_int_indexing, _, _ = indexing.unpack_ndindexer(new_indexer)
  all_new_int_indexers_adjacent = bool(
      np.all(np.diff(np.where(is_new_int_indexing)[0]) == 1)
  )

  out = get_p.bind(ref, *flat_indexers, tree=tree)
  if all_int_indexers_adjacent:  # not moved
    if all_new_int_indexers_adjacent:  # not moved
      out_bdim = is_new_int_indexing.index(True) + 0  # batched dim is at the front of the int indexer
      return out, out_bdim
    else:  # previously not moved, now moved, because there is a batched dim inserted at `ref_dim`
      # e.g.
      # original:        S S S A A S   (array indexers are continuous)
      # new:             S S A S A A S (first a is inserted at `ref_dim`)
      # indexing result: A S S S S     (no longer continuous, moved to the front)
      # we should
      # 1. split into [B, A_rest] S S S S
      # 2. move A_rest back to its original position
      #    e.g. B S S S A_rest S
      # 3. return 0 since the batched dim is at the front
      ...
      array_indexer_shape = new_indexer.int_indexer_shape
      array_indexer_len = len(array_indexer_shape)
      simple_indexer_len = len(out.shape) - array_indexer_len

      transpose_order = list(range(len(out.shape)))
      transpose_order = (
          transpose_order[0],
          *transpose_order[-simple_indexer_len:(-simple_indexer_len+original_pos) or None],
          *transpose_order[1:-simple_indexer_len],
          *(() if (-simple_indexer_len+original_pos == 0) else transpose_order[-simple_indexer_len+original_pos:]),
      )
...
def _swap_vmap(batched_args, batched_dims, *, tree):
  ...
  if not all_new_int_indexers_adjacent:  # expected to be moved
  ...
  # original:          S S S A A S
  # batched:           A S S S A A S
  # indexing result:   A S S S S

  # but value expects: S S S A S = S S S [B A_rest] S

  # ==============

  # original             S S B_A
  # batched              B_A S S B_A
  # indexing result:     B_A S S
  # but value expects:   B S S A

  transpose_order_inversed = None

  if all_int_indexers_adjacent and not all_new_int_indexers_adjacent:  # should not be moved, but moved
    original_pos = is_int_indexing.index(True)
  ...
  # Value should not be transposed, but we need to transpose it to accommodate
  # the swap_p result. As a result, the swap_p result is also transposed. We
