...
def _strip_mosaic_debug_info(m: ir.Module) -> None:
  ...
  def _strip_kernel(op: ir.Operation) -> ir.WalkResult:
    call_target_name = op.attributes.get("call_target_name")
    if (
        op.name != "stablehlo.custom_call"
        or not call_target_name
        or ir.StringAttr(call_target_name).value != "tpu_custom_call"
    ):
      return ir.WalkResult.ADVANCE
    backend_config_attr = op.attributes.get("backend_config")
    if not backend_config_attr:
      return ir.WalkResult.ADVANCE
    try:
      bc = json.loads(ir.StringAttr(backend_config_attr).value)
    except (json.JSONDecodeError, ValueError):
      return ir.WalkResult.ADVANCE
    custom_call_config = bc.get("custom_call_config")
    if not custom_call_config:
      return ir.WalkResult.ADVANCE
    ...
    body = custom_call_config.get("body")
    if not body:
      return ir.WalkResult.ADVANCE
    ctx = m.context
