...
class CacheKey:
  ...
  @staticmethod
  def append_extra_flag_prefixes(prefixes: list[str]):
    """Append the supplied list to extra flag prefixes. This will be
    used in get(), so any calls to append_extra_flag_prefixes() must
    precede calls to get().
    """
    CacheKey._extra_flag_prefixes += prefixes

  @staticmethod
  def get_extra_flag_prefixes() -> list[str]:
    """Return the extra flag prefixes set by calls to
    append_extra_flag_prefixes(). Used in get().
    """
  ...
  def get(
    ...
    get() takes in the MLIR module and compile_options of a program
...
def _hash_xla_flags(hash_obj):
  ...
  for arg in sys.argv:
    if arg.startswith("--xla") or any(
        arg.startswith(p) for p in CacheKey.get_extra_flag_prefixes()
    ):
