...
def _mapped_axis_size(fn, tree, vals, dims, name):
  ...
  def _get_axis_size(name: str, aval: core.AbstractValue, axis: core.AxisSpec,
                     ) -> core.AxisSize:
    return aval.axis_size(axis)

  sizes = core.dedup_referents(_get_axis_size(name, typeof(x), d)
  ...
  all_sizes = [_get_axis_size(name, typeof(x), d) if d is not None else None
