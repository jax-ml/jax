...
def input_saved_vjp(f: Callable, which: Sequence[bool], *primals,
  ...
  dbg = debug_info("input_saved_vjp", f, primals, {})
  ...
  res_spec = [RSpec(id_map[id(r)], True) if id(r) in id_map else
              ...
              for r in residuals]
  f_vjp = Partial(_input_saved_vjpfun, res_spec, filt_tree, in_tree, out_tree(),
...
def _input_saved_vjpfun(res_spec, filtered_tree, in_tree, out_tree, jaxpr,
  ...
  if filtered_tree != filtered_tree_:
    raise ValueError(
        ...
        "  _, f_vjp = input_saved_vjp(f, which, *inputs, ...)\n"
