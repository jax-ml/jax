...
def check_layout(
    ...
    if q_dtype not in [
        ...
        raise NotImplementedError(
            f"Q must be fp16/bf16/fp8_e4m3fn/fp8_e5m2, got {q_dtype}"
        )
    ...
    def check_seqlen_offsets(tensor, name):
        if tensor is not None:
            ...
            if rank != expected_rank:
                raise ValueError(
                    f"{name} must have a rank of {expected_rank}, got {rank}"
                )
            b = tensor.shape[0]
            if b != qB:
...
def check_is_flash_attention(
    ...
    if is_fp8:
        ...
        if not (
            (is_training and qH == 128 and T % 128 == 0 and S % 128 == 0)
            or (not is_training and qH <= 256 and qH % 16 == 0)
        ):
            raise NotImplementedError(
        ...
        if is_training and has_bias and (T % 2 != 0 or S % 2 != 0):
            raise NotImplementedError(f"Unsupported sequence length Q {T}, KV {S}.")
...
def _dot_product_attention_fwd(
    ...
    check_is_flash_attention(
        ...
        check_is_paged_attention(page_table_k),
    )
    outputs = _dot_product_attention_fwd_p_wrapper.bind(
        ...
        is_training=False or return_residual,
    )
    if return_residual:
...
def _fix_seqlen_offsets(q_seqlen, kv_seqlen, q_offsets, kv_offsets, query, key):
    ...
    def _cu_offset(offsets, max_seq):
        ...
        offsets = jnp.where(
            offsets >= 0,
            offsets
            + (jnp.arange(batch, dtype=offsets.dtype) * max_seq)[..., np.newaxis],
            offsets,
        )
        return offsets
...
def _dot_product_attention_fwd_impl(
    ...
    q_seqlen, kv_seqlen, q_offsets, kv_offsets = _fix_seqlen_offsets(
        q_seqlen, kv_seqlen, q_offsets, kv_offsets, query, key
    )
...
def _attention_out_aval(in_aval, shape=None, dtype=None):
    ...
    return core.ShapedArray(
        shape,
        dtype,
        sharding=NamedSharding(in_aval.sharding.mesh, PartitionSpec()),
...
def _dot_product_attention_fwd_abstract(
    ...
    *,
    ...
    if is_training:
        ...
        return (_attention_out_aval(query, output_shape),)  # output
...
def _dot_product_attention_fwd_cuda_lowering(
    ...
    is_paged_attention = check_is_paged_attention(
        ir.RankedTensorType(page_table_k.type)
    )
    ...
    if is_training:
        ...
        result_layouts = [output_layout] + default_layouts(
            softmax_stat_shape, workspace_shape
        )
    else:
        result_types = [
            ir.RankedTensorType.get(output_shape, query_type.element_type),
...
def _dot_product_attention_bwd_cuda_lowering(
    ...
    if has_dbias:
        # cuDNN supports bias for this case
        result_types.append(ir.RankedTensorType.get(bias_shape, bias_type.element_type))
    ...
    dqkv = (
        hlo.transpose(out.results[0], grad_transpose_perm),
...
def _dot_product_attention_bwd_batcher(
    ...
    *,
    ...
    # Determine the effective batch dimension. In jax.jacobian, grad_output is
    # batched (batch_dims[12] = 0) while residual args like query may not be
    # (batch_dims[0] = None). We need to broadcast unbatched args to match.
    grad_output_bdim = batch_dims[12]
    query_bdim = batch_dims[0]
    has_bias, has_dbias = variadic_args

    if query_bdim is None and grad_output_bdim is not None:
        # grad_output is batched but residuals are not (e.g., jax.jacobian).
        # Broadcast all unbatched residual args to have the vmap batch dimension.
        vmap_size = grad_output.shape[grad_output_bdim]
        query = jnp.broadcast_to(jnp.expand_dims(query, 0), (vmap_size,) + query.shape)
        key = jnp.broadcast_to(jnp.expand_dims(key, 0), (vmap_size,) + key.shape)
        value = jnp.broadcast_to(jnp.expand_dims(value, 0), (vmap_size,) + value.shape)
        activation = jnp.broadcast_to(
            jnp.expand_dims(activation, 0), (vmap_size,) + activation.shape
        )
        fwd_output = jnp.broadcast_to(
            jnp.expand_dims(fwd_output, 0), (vmap_size,) + fwd_output.shape
        )
        if has_padding(mask_type):
            q_seqlen = jnp.broadcast_to(
                jnp.expand_dims(q_seqlen, 0), (vmap_size,) + q_seqlen.shape
            )
            kv_seqlen = jnp.broadcast_to(
                jnp.expand_dims(kv_seqlen, 0), (vmap_size,) + kv_seqlen.shape
            )
        if has_bias and batch_dims[3] is None:
            bias = jnp.broadcast_to(jnp.expand_dims(bias, 0), (vmap_size,) + bias.shape)
        out_bdims = 0, 0, 0
    else:
    ...
    value = jnp.reshape(value, (B,) + key.shape[-3:])
    if has_bias and (
        batch_dims[3] is not None
        or (query_bdim is None and grad_output_bdim is not None)
    ):
        bias = jnp.reshape(bias, (B, N, T, S))
    if has_padding(mask_type):
        q_seqlen = jnp.reshape(q_seqlen, (B,))
...
def _check_qkv_bias_mask_spec(query_spec, key_spec, value_spec, bias_spec, layout):
    # check qkv spec
    ...
    # check bias spec
    if bias_spec:
        *bias_batch_spec, bias_num_head_spec, bias_q_seq_spec, bias_kv_seq_spec = (
            bias_spec
        )
        if (
            any(bias_batch_spec)
            and bias_batch_spec != batch_spec
            or bias_num_head_spec is not None
            and bias_num_head_spec != num_head_spec
        ):
            raise ValueError(
...
def _infer_fwd_output_sharding(mesh, arg_shapes, variadic_args, is_training, layout):
    ...
    _check_qkv_bias_mask_spec(query_spec, key_spec, value_spec, bias_spec, layout)
...
def _fwd_shardy_rule(value_types, result_types, layout, is_training, is_fp8):
    ...
    if is_training:
        ...
        if result_types[-1].shape[0] == value_types[0].shape[0]:
            ...
            output_sharding += (
                ArrayMapping(CompoundFactor("batch", "n"), "nhead", "qseq"),
            )
...
def _dot_product_attention_fwd_infer_sharding_from_operands(
    ...
    return _infer_fwd_output_sharding(
...
def _dot_product_attention_fwd_shardy_rule(
    ...
    return _fwd_shardy_rule(
        value_types, result_types, layout, is_training, is_fp8=False
    )
...
batching.primitive_batchers[_dot_product_attention_fwd_p_wrapper] = (
    _dot_product_attention_fwd_batcher
)
batching.primitive_batchers[_dot_product_attention_bwd_p_wrapper] = (
...
@functools.partial(
    custom_derivatives.custom_vjp, nondiff_argnums=(10, 11, 12, 13, 14, 15, 16, 17, 18)
)
...
fp8_params_keys = [
    ...
    "amax_dK",
    "amax_dV",
    "amax_dP",  # place holder for bwd output
    "descale_q",
    "descale_k",
    "descale_v",
    "descale_s",
...
def check_fp8_params(params):
    ...
    if missing_keys:
        raise ValueError(
            f"The following keys are missing from fp8_params: {', '.join(missing_keys)}"
        )


check_is_flash_attention_fp8 = functools.partial(
    check_is_flash_attention, has_bias=False, is_fp8=True
)


def _dot_product_attention_fp8_fwd(
    query, key, value, fp8_params_fwd, scale, use_causal_mask, layout, cudnn_version
):
...
def _dot_product_attention_fp8_fwd_rule(
    query, key, value, fp8_params, scale, use_causal_mask, layout, cudnn_version
):
    check_is_flash_attention_fp8(
    ...
    outputs = _dot_product_attention_fp8_fwd_p_wrapper.bind(
        ...
        key,
        value,
        *params_from_keys(fp8_params, fp8_params_keys_fwd),
        scale=scale,
        use_causal_mask=use_causal_mask,
        layout=layout,
        is_training=True,
    ...
    res = (
        ...
        outputs[3],
        outputs[0],
        params_from_keys(fp8_params, fp8_params_keys_bwd),
    )
...
def _dot_product_attention_fp8_fwd_cuda_lowering(
    ...
    backend_config = create_dot_product_attention_fp8_backend_config(
        ...
        T,
        S,
        ir.BF16Type.get(),  # query_type.element_type,
        scale,
        mask_type,
        layout,
        is_bwd=False,
    ...
    if is_training:
        ...
        result_layouts = [output_layout] + default_layouts(
            amax_shape, amax_shape, softmax_stat_shape, workspace_shape
        )
    else:
        result_types = [
            ir.RankedTensorType.get(output_shape, query_type.element_type),
        ...
        result_layouts = [output_layout] + default_layouts(
            amax_shape, amax_shape, workspace_shape
        )

    operand_shapes = [
        ir.RankedTensorType(operand.type).shape for operand in operands[:3]
    ]
    ...
    if is_training:
        return [
            hlo.transpose(out.results[0], output_transpose_perm),
            out.results[1],
            out.results[2],
            out.results[3],
        ]
...
def _dot_product_attention_fp8_bwd_cuda_lowering(
    ...
    result_layouts = [grad_layout, grad_layout, grad_layout] + default_layouts(
        amax_shape, amax_shape, amax_shape, amax_shape
    )
...
def _dot_product_attention_fp8_fwd_infer_sharding_from_operands(
    scale, use_causal_mask, layout, is_training, mesh, arg_shapes, result_shape
):
...
def _dot_product_attention_fp8_fwd_partition(
    ...
    impl = functools.partial(
        _dot_product_attention_fp8_fwd_impl,
        scale=scale,
        use_causal_mask=use_causal_mask,
        layout=layout,
        is_training=is_training,
...
def _dot_product_attention_fp8_fwd_shardy_rule(
    scale, use_causal_mask, layout, is_training, mesh, value_types, result_types
):
...
def _infer_fp8_bwd_output_sharding(mesh, arg_shapes, layout):
    ...
    output_shardings = _infer_bwd_output_sharding(
        mesh, arg_shapes, layout, variadic_args
    )

    # Prepare amax_sharding
    amax_sharding = NamedSharding(
        mesh, PartitionSpec()
    )  # Use a default spec or adjust as needed
...
def _dot_product_attention_fp8_bwd_infer_sharding_from_operands(
    scale, use_causal_mask, layout, mesh, arg_shapes, result_shape
):
...
def _dot_product_attention_fp8_bwd_partition(
    ...
    arg_shardings = tuple(arg_i.sharding for arg_i in arg_shapes)
    impl = functools.partial(
        _dot_product_attention_fp8_bwd_impl,
        scale=scale,
        use_causal_mask=use_causal_mask,
        layout=layout,
    )
...
batching.primitive_batchers[_dot_product_attention_fp8_fwd_p_wrapper] = (
...
dispatch.prim_requires_devices_during_lowering.add(_dot_product_attention_fp8_fwd_p)
dispatch.prim_requires_devices_during_lowering.add(
    _dot_product_attention_fp8_fwd_p_wrapper
)
dispatch.prim_requires_devices_during_lowering.add(_dot_product_attention_fp8_bwd_p)
dispatch.prim_requires_devices_during_lowering.add(
    _dot_product_attention_fp8_bwd_p_wrapper
)
...
def _dot_product_attention_fp8(
    query: Array,
    key: Array,
    value: Array,
...
_dot_product_attention_fp8.defvjp(
    _dot_product_attention_fp8_fwd_rule, _dot_product_attention_fp8_bwd_rule
)
...
def paged_attention(
    ...
    *,
    ...
    has_dbias = has_bias and should_export_dbias(bias.shape, query.shape, layout)
