...
def _export_lowered(
  ...
  if xla_extension.sdy.get_mesh(mlir.module_to_bytecode(mlir_module)):
    shardy_enabled = True
...
def _module_to_bytecode(module: ir.Module, shardy_enabled: bool) -> bytes:
  if xla_extension_version >= 314 and shardy_enabled:
...
def _call_exported_lowering(ctx: mlir.LoweringRuleContext, *args,
  ...
  shardy_enabled = False
  mesh = None
  if sdy_mesh_axes := xla_extension.sdy.get_mesh(mlir.module_to_bytecode(submodule)):
    mesh = mesh_lib.AbstractMesh(tuple(sdy_mesh_axes))
    shardy_enabled = True
  ...
  if shardy_enabled:
    args = tuple(
        wrap_with_sharding(
            ctx, x, x_aval,
            _hlo_sharding_to_named_sharding(x_sharding, mesh))
...
def wrap_with_sharding(
    ...
    x_sharding: sharding_impls.NamedSharding | HloSharding,
) -> ir.Value:
