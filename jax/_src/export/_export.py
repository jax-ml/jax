...
def _export_lowered(
  ...
  shardy_enabled = has_sdy_mesh(mlir_module)
...
def has_sdy_mesh(submodule: ir.Module) -> bool:
  symtab = ir.SymbolTable(submodule.operation)
  if any(s in symtab for s in ["mesh", "empty_mesh", "maximal_mesh_0"]):
    return True
...
def _call_exported_lowering(ctx: mlir.LoweringRuleContext, *args,
  ...
  shardy_enabled = has_sdy_mesh(submodule)
  if shardy_enabled:
