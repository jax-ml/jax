...
# Version 6, December 5th, 2025, adds serialization for sharding as
...
def _serialize_exported(
  ...
  # TODO(necula): For 30 days after 12/5/2025 we must serialize the HLO
...
def _serialize_sharding(
  ...
  if is_unspecified:
    ...
    # 12/5/2025 because old deserializers check the kind and abort if they
  ...
  # 12/5/2025 because old deserializers can only deserialize this sharding.
...
def _deserialize_sharding(s: ser_flatbuf.Sharding) -> _export.HloSharding | named_sharding.NamedSharding | None:
  ...
  # TODO(necula): We must keep reading the HloSharding for 6 months after 12/5/2025.
