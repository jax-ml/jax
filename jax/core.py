...
class Primitive(object):
    ...
    def bind(self, *args, **kwargs):
        assert skip_checks or all(
            isinstance(arg, Tracer) or valid_jaxtype(arg) for arg in args
        ), args
    ...
    def impl(self, *args, **kwargs):
        raise NotImplementedError(
            "Evaluation rule for '{}' not implemented".format(self.name)
        )

    def abstract_eval(self, *args, **kwargs):
        raise NotImplementedError(
            "Abstract evaluation for '{}' not implemented".format(self.name)
        )
...
def find_top_trace(xs):
    try:
        top_trace = max(
            (x.trace for x in xs if isinstance(x, Tracer)), key=attrgetter("level")
        )
    except ValueError:
        return None
    else:
    ...
    def full_raise(self, val):
        ...
        if val.trace.master is self.master:
            if val.trace.sublevel == sublevel:
                ...
                raise Exception(
                    "Can't lift sublevels {} to {}".format(val.trace.sublevel, sublevel)
                )
        elif val.trace.level < level:
            if val.trace.sublevel > sublevel:
                raise Exception(
                    "Incompatible sublevel: {}, {}".format(val.trace, (level, sublevel))
                )
...
def call_bind(primitive, f, *args, **params):
    top_trace = find_top_trace(args)
    level = (
        trace_state.trace_stack.next_level(True)
        if top_trace is None
        else top_trace.level
    )
        ...
        if eqn.bound_subjaxprs:
            for subjaxpr, const_vars, bound_vars in eqn.bound_subjaxprs:
                pp_subexpr = pp_subexpr + (
                    ...
                    >> pp(
                        " [ {} ; {} ]".format(
                            print_vars(const_vars), print_vars(bound_vars)
                        )
                    )
                )
        return (
            ...
            >> pp_kv_pairs(eqn.params.items())
            >> pp(" ")
            >> pp(print_vars(eqn.invars))
        ) + pp_subexpr

    return pp(
        "{{ lambda {} ; {} ; {}.".format(
            print_vars(jaxpr.constvars),
            print_vars(jaxpr.freevars),
            print_vars(jaxpr.invars),
        )
    ) + (
        (pp("let ") >> vcat(map(pp_eqn, jaxpr.eqns)))
        + pp("in {} }}".format(jaxpr.outvars))
    ).indent(
        2
    )
