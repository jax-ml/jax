...
def numpy_close(a, b, atol=ATOL, rtol=RTOL, equal_nan=False):
    ...
    return onp.allclose(
        a, b, atol=atol * a.size, rtol=rtol * b.size, equal_nan=equal_nan or testing_tpu
    )
...
def rand_not_small():
    post = lambda x: x + onp.where(x > 0, 10.0, -10.0)
    randn = npr.RandomState(0).randn
    return partial(_rand_dtype, randn, scale=3.0, post=post)
...
def rand_some_equal():
    ...
    return partial(_rand_dtype, randn, scale=100.0, post=post)
...
class JaxTestCase(parameterized.TestCase):
    ...
    def assertArraysAllClose(self, x, y, check_dtypes, atol=None, rtol=None):
        ...
        if not onp.allclose(x, y, atol=atol, rtol=rtol, equal_nan=True):
            msg = (
                "Arguments x and y not equal to tolerance atol={}, rtol={}:\n"
                "x:\n{}\n"
                "y:\n{}\n"
    ...
    def _CheckAgainstNumpy(
        self, numpy_reference_op, lax_op, args_maker, check_dtypes=False, tol=1e-5
    ):
        args = args_maker()
        numpy_ans = numpy_reference_op(*args)
        lax_ans = lax_op(*args)
        self.assertAllClose(
            lax_ans, numpy_ans, check_dtypes=check_dtypes, atol=tol, rtol=tol
        )
