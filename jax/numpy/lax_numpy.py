...
from .. import lax
from .. import ops
from ..util import partial, unzip2, prod as _prod, subvals, safe_zip
from ..tree_util import tree_leaves, tree_flatten

FLAGS = flags.FLAGS
flags.DEFINE_enum(
    "jax_numpy_rank_promotion",
    os.getenv("JAX_NUMPY_RANK_PROMOTION", "allow"),
...
class ndarray(np.ndarray, metaclass=_ArrayMeta):
    ...
    def __init__(shape, dtype=None, buffer=None, offset=0, strides=None, order=None):
        raise TypeError(
            "jax.numpy.ndarray() should not be instantiated explicitly."
...
def _promote_shapes(fun_name, *args):
    ...
    if len(args) < 2:
        ...
        if not nonscalar_ranks or len(set(nonscalar_ranks)) == 1:
            ...
            return [
                broadcast_to(arg, (1,) * (result_rank - len(shp)) + shp)
...
def _rank_promotion_warning_or_error(fun_name, shapes):
    if FLAGS.jax_numpy_rank_promotion == "warn":
        msg = (
            "Following NumPy automatic rank promotion for {} on shapes {}. "
        ...
        msg = (
            "Operands could not be broadcast together for {} on shapes {} "
...
def _check_arraylike(fun_name, *args):
    ...
    if _any(not _arraylike(arg) for arg in args):
        pos, arg = next((i, arg) for i, arg in enumerate(args) if not _arraylike(arg))
...
def _comparison_op(numpy_fn, lax_fn):
    def fn(x1, x2):
        ...
        if issubdtype(_dtype(x1), complexfloating):
            ...
            ry = lax.real(x2)
            return lax.select(
                lax.eq(rx, ry), lax_fn(lax.imag(x1), lax.imag(x2)), lax_fn(rx, ry)
            )
        return lax_fn(x1, x2)
...
def _logical_op(np_op, bitwise_op):
    ...
    def op(*args):
        ...
        args = (x if issubdtype(_dtype(x), bool_) else lax.ne(x, zero(x)) for x in args)
...
@_wraps(np.copysign)
def copysign(x1, x2):
    if issubdtype(_dtype(x1), complexfloating) or issubdtype(
        _dtype(x2), complexfloating
    ):
...
def _logaddexp_jvp(primals, tangents):
    ...
    tangent_out = t1 * exp(_replace_inf(x1) - _replace_inf(primal_out)) + t2 * exp(
        _replace_inf(x2) - _replace_inf(primal_out)
    )
...
def logaddexp2(x1, x2):
    ...
    return lax.select(
        ...
        lax.add(
            amax,
            lax.div(lax.log1p(exp2(-lax.abs(delta))), _constant_like(x1, np.log(2))),
        ),
    )
...
def _logaddexp2_jvp(primals, tangents):
    ...
    tangent_out = t1 * 2 ** (_replace_inf(x1) - _replace_inf(primal_out)) + t2 * 2 ** (
        _replace_inf(x2) - _replace_inf(primal_out)
    )
    return primal_out, tangent_out
...
def signbit(x):
    ...
    if issubdtype(dtype, integer):
        ...
        raise ValueError("jax.numpy.signbit is not well defined for %s" % dtype)
...
def _conv(x, y, mode, op, precision):
    ...
    if len(x) == 0 or len(y) == 0:
        raise ValueError(
            f"{op}: inputs cannot be empty, got shapes {x.shape} and {y.shape}."
        )
    ...
    result = lax.conv_general_dilated(
        x[None, None, :], y[None, None, :], (1,), padding, precision=precision
    )
...
def _normalize_float(x):
    ...
    x2 = where(
        cond, full_like(x, -info.nmant, dtype=np.int32), zeros_like(x, dtype=np.int32)
    )
...
def cbrt(x):
    (x,) = _promote_dtypes_inexact(x)
    return lax.sign(x) * power(lax.abs(x), _constant_like(x, 1.0 / 3.0))
...
def sinc(x):
    ...
    return where(eq_zero, lax._const(x, 1), lax.div(lax.sin(pi_x), pi_x))
...
def _gradient(a, varargs, axis):
    ...
    if _min([s for i, s in enumerate(a.shape) if i in axis]) < 2:
        raise ValueError(
            "Shape of array too small to calculate "
...
def moveaxis(a, source, destination):
    ...
    if len(source) != len(destination):
        raise ValueError(
            "Inconsistent number of elements: {} vs {}".format(
...
def _where(condition, x=None, y=None):
    if x is None or y is None:
        raise ValueError(
            "Either both or neither of the x and y arguments should "
            "be provided to jax.numpy.where, got {} and {}.".format(x, y)
        )
...
def bincount(x, weights=None, minlength=0, *, length=None):
    ...
    return ops.index_add(
        zeros((length,), _dtype(weights)), ops.index[clip(x, 0)], weights
    )
...
def broadcast_arrays(*args):
    ...
    if len(set(shapes)) == 1:
        return [
            arg if isinstance(arg, ndarray) or isscalar(arg) else array(arg)
            for arg in args
        ]
...
def split(ary, indices_or_sections, axis=0):
    ...
    if isinstance(indices_or_sections, (tuple, list) + _arraylike_types):
        indices_or_sections = [
            core.concrete_or_error(int, i_s, "in jax.numpy.split argument 1")
        ...
        indices_or_sections = core.concrete_or_error(
            int, indices_or_sections, "in jax.numpy.split argument 1"
        )
...
def _round_to_nearest_even(x):
    ...
    return lax.select(
        lax.bitwise_or(
            lax.gt(fraction, half), lax.bitwise_and(lax.eq(fraction, half), is_odd)
        ),
        lax.add(round_val, one),
        round_val,
    )
...
def isinf(x):
    ...
    if issubdtype(dtype, floating):
        ...
        return lax.bitwise_or(
            lax.eq(lax.abs(re), _constant_like(re, inf)),
...
def _make_reduction(
    np_fun, op, init_val, preproc=None, bool_op=None, upcast_f16_for_computation=False
):
    ...
    def reduction(a, axis=None, dtype=None, out=None, keepdims=False):
        ...
        result = lax.reduce(
            a,
            _reduction_init_val(a, init_val),
            op if computation_dtype != np.bool_ else bool_op,
            dims,
        )
        if keepdims:
...
sum = _make_reduction(
    np.sum, lax.add, 0, upcast_f16_for_computation=True, bool_op=lax.bitwise_or
)
product = prod = _make_reduction(
    np.prod, lax.mul, 1, bool_op=lax.bitwise_and, upcast_f16_for_computation=True
)
...
def _var_promote_types(a_dtype, dtype):
    if dtype:
        if not issubdtype(dtype, complexfloating) and issubdtype(
            a_dtype, complexfloating
        ):
            msg = (
...
def _make_nan_reduction(np_reduction, jnp_reduction, init_val, nan_if_all_nan):
    ...
    def nan_reduction(a, axis=None, out=None, keepdims=False, **kwargs):
        out = jnp_reduction(
            where(isnan(a), _reduction_init_val(a, init_val), a),
            axis=axis,
            out=out,
            keepdims=keepdims,
            **kwargs,
        ...
        if nan_if_all_nan:
            return where(
                all(isnan(a), axis=axis, keepdims=keepdims), _constant_like(a, nan), out
            )
        else:
            return out
...
nancumsum = _make_cumulative_reduction(
    np.nancumsum, lax.cumsum, fill_nan=True, fill_value=0
)
nancumprod = _make_cumulative_reduction(
    np.nancumprod, lax.cumprod, fill_nan=True, fill_value=1
)
...
def _pad_symmetric_or_reflect(array, pad_width, mode):
    ...
    for i in range(ndim(array)):
        ...
        def build_padding(padding, forward):
            ...
            if padding > 0:
                x = lax.slice_in_dim(
                    array if forward else rarray, offset, padding + offset, axis=i
                )
                xs.append(x)
            return xs
...
def concatenate(arrays, axis=0):
    ...
    if len(arrays) == 1:
        ...
        while len(arrays) > 1:
            arrays = [
                lax.concatenate(arrays[i : i + k], axis)
...
def _block(xs):
    if isinstance(xs, tuple):
        raise ValueError("jax.numpy.block does not allow tuples, got {}".format(xs))
...
def _can_call_numpy_array(x):
    return _all(not isinstance(l, (core.Tracer, DeviceValue)) for l in tree_leaves(x))
...
def linspace(start, stop, num=50, endpoint=True, retstep=False, dtype=None, axis=0):
    ...
    if num > 1:
        ...
        if issubdtype(dtype, integer):
            ...
            out = reshape(broadcast_start, bounds_shape) + reshape(
                lax.iota(dt, num), iota_shape
            ...
            out = (
                reshape(broadcast_start, bounds_shape) * (1 - step)
...
def logspace(start, stop, num=50, endpoint=True, base=10.0, dtype=None, axis=0):
    ...
    lin = linspace(
        start, stop, num, endpoint=endpoint, retstep=False, dtype=None, axis=axis
    )
...
def meshgrid(*args, **kwargs):
    ...
    if indexing == "xy":
        ...
        raise ValueError(
            "Valid values for indexing are 'xy' and 'ij', got {}".format(indexing)
        )
...
def _repeat_scalar(a, repeats, axis=None):
    ...
    if axis < 0 or axis >= num_dims:
        raise ValueError(
            "axis {} is out of bounds for array of dimension {}".format(axis, num_dims)
        )

    # Broadcasts to [..., X, repeats, ...] and reshapes to [..., X * repeats, ...]
    broadcast_shape = list(a_shape)
    broadcast_shape.insert(axis + 1, repeats)
    broadcast_dims = np.concatenate(
        (np.arange(0, axis + 1), np.arange(axis + 2, num_dims + 1))
    )
...
def repeat(a, repeats, axis=None):
    ...
    if repeats_raveled.size != a.shape[axis]:
        raise ValueError(
            f"repeats shape {repeats_raveled.shape} does not match "
...
def diagonal(a, offset=0, axis1=0, axis2=1):
    ...
    diag_size = _max(
        0, _min(a_shape[axis1] + _min(offset, 0), a_shape[axis2] - _max(offset, 0))
    )
...
def tensordot(a, b, axes=2, *, precision=None):
    ...
    if type(axes) is int:
        ...
        if type(ax1) == type(ax2) == int:
            ...
            contracting_dims = (
                tuple(_canonicalize_axis(i, a_ndim) for i in ax1),
        ...
        msg = (
            "tensordot axes argument must be an int, a pair of ints, or a pair "
            "of lists/tuples of ints."
        )
        raise TypeError(msg)
    return lax.dot_general(a, b, (contracting_dims, ((), ())), precision=precision)
...
def _einsum(
    ...
    for operand_indices, contracted_names_set, einstr in contractions:
        ...
        if len(operand_indices) == 1:
            ...
            batch_names = sorted(
                (set(lhs_names) & set(rhs_names)) - set(contracted_names)
            )
            ...
            if lhs_batch != rhs_batch or set(lhs_batch) != set(batch_dims):
                ...
                batch_names_str = "".join(
                    lhs_names[i] for i in range(len(lhs_names)) if i in batch_dims
                )
            ...
            names = (
                batch_names_str
                + _removechars(lhs_names, deleted_names)
...
def sort_complex(a, axis=-1, kind="quicksort", order=None):
    if kind != "quicksort":
        warnings.warn("'kind' argument to argsort is ignored.")
    if order is not None:
        raise ValueError("'order' argument to argsort is not supported.")
    if axis is None:
        return sort_complex(a.ravel(), 0)
    else:
        axis = _canonicalize_axis(axis, ndim(a))
        if np.isreal(a).any():
...
def _roll(a, shift, axis):
    ...
    for x, i in zip(broadcast_to(shift, b_shape), np.broadcast_to(axis, b_shape)):
...
def rollaxis(a, axis, start=0):
    ...
    if not (-a_ndim <= axis < a_ndim):
        raise ValueError(
            f"axis={axis} is out of bounds for array of dimension {a_ndim}"
        )
...
def take(a, indices, axis=None, out=None, mode=None):
    ...
    return lax.gather(
        a, indices[..., None], dimension_numbers=dnums, slice_sizes=tuple(slice_sizes)
    )
...
def unique(
    ...
    raise NotImplementedError("np.unique is not implemented for the axis argument")
    ...
    for idx_pos, i in enumerate(idx):
        ...
        if advanced_indexes is not None and (
            ...
            gather_indices = lax.broadcast_in_dim(
                gather_indices,
                np.insert(gather_indices.shape, -1, shape),
...
def _expand_bool_indices(idx):
    ...
    for i in idx:
        try:
            ...
            abstract_i = None
        if (
            isinstance(abstract_i, ShapedArray)
            and issubdtype(abstract_i.dtype, bool_)
            or isinstance(i, list)
            and _all(not _shape(e) and issubdtype(_dtype(e), bool_) for e in i)
        ):
            ...
            if not type(abstract_i) is ConcreteArray:
                msg = (
                    "Array boolean indices must be static (e.g. no dependence on an "
...
def gcd(x1, x2):
    ...
    gcd, _ = lax.while_loop(_gcd_cond_fn, _gcd_body_fn, (lax.abs(x1), lax.abs(x2)))
    return gcd
...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None, aweights=None):
    msg = (
        "jax.numpy.cov not implemented for nontrivial {}. "
...
def _view(arr, dtype=None, type=None):
    ...
    if nbits_in < nbits_out:
        ...
        arr_bytes = arr_bytes.reshape(
            arr.shape[:-1] + (-1, nbits_out // nbits_in)
        ).astype(dt_out)
        arr_bytes = (arr_bytes << shifts).sum(-1).astype(dt_out)
    else:
        shifts = arange(0, nbits_in, nbits_out, dtype=dt_in)
        arr_bytes = ((arr_bytes[..., newaxis] >> shifts) & iinfo(dt_out).max).astype(
            dt_out
        )
...
def _unimplemented_setitem(self, i, x):
    msg = (
        "'{}' object does not support item assignment. JAX arrays are "
