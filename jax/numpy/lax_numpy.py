...
def _promote_shapes(fun_name, *args):
    ...
    if len(args) < 2:
        ...
        if not nonscalar_ranks or len(set(nonscalar_ranks)) == 1:
            ...
            return [
                lax.reshape(arg, (1,) * (result_rank - len(shp)) + shp)
                if shp and len(shp) != result_rank
                else arg
...
def logaddexp(x1, x2):
    x1, x2 = _promote_shapes(
        "logaddexp", *_promote_to_result_dtype(onp.logaddexp, x1, x2)
    )
    amax = lax.max(x1, x2)
...
def logaddexp2(x1, x2):
    x1, x2 = _promote_shapes(
        "logaddexp2", *_promote_to_result_dtype(onp.logaddexp2, x1, x2)
    )
    amax = lax.max(x1, x2)
    return lax.add(
...
def sinc(x):
    ...
    return where(
        lax.eq(x, lax._const(x, 0)), lax._const(x, 1), lax.div(lax.sin(pi_x), pi_x)
    )
...
def mean(a, axis=None, dtype=None, out=None, keepdims=False):
    ...
    if dtype is None:
        if onp.issubdtype(_dtype(a), onp.bool_) or onp.issubdtype(
            _dtype(a), onp.integer
        ):
...
def _make_cumulative_reduction(
    onp_reduction, window_reduce, init_val, squash_nan=False
):
...
def concatenate(arrays, axis=0):
    ...
    while len(arrays) > 1:
        arrays = [
            lax.concatenate(arrays[i : i + k], axis) for i in range(0, len(arrays), k)
        ]
    return arrays[0]
...
def tri(N, M=None, k=0, dtype=None):
    ...
    mask = lax.ge(
        (lax.broadcast_in_dim(x, shape=(N, M), broadcast_dimensions=(0,)) + int32(k)),
        lax.broadcast(y, [N]),
    )
...
def matmul(a, b):  # pylint: disable=missing-docstring
    ...
    result = lax.dot_general(
        a, b, (((ndim(a) - 1,), (ndim(b) - 2,)), (batch_dims, batch_dims))
    )
    ...
    for operand_indices, contracted_names, einstr in contractions:
        ...
        if len(operand_indices) == 1:
            ...
            if contracted_names:
                ...
                names = (
                    batch_names
                    + removechars(lhs_names, deleted_names)
                ...
                rhs_shape = (
                    rhs.shape[:nbatch] + (1,) * (lhs.ndim - nbatch) + rhs.shape[nbatch:]
                )
...
def _gather(arr, treedef, static_idx, dynamic_idx):
    ...
    y = lax.gather(
        arr, indexer.gather_indices, indexer.dnums, indexer.gather_slice_shape
    )
...
def _expand_bool_indices(idx):
    ...
    for i in idx:
        ...
        if (
            ...
            and onp.issubdtype(abstract_i.dtype, onp.bool_)
...
def median(a, axis=None, out=None, overwrite_input=False, keepdims=False):
    q = 0.5
    return quantile(
        a, q, axis=axis, out=out, overwrite_input=overwrite_input, keepdims=keepdims
    )
