...
def slogdet(a):
    ...
    logdet = np.where(
        is_zero, np.array(-np.inf, dtype=dtype), np.sum(np.log(np.abs(diag)), axis=-1)
    )
...
def inv(a):
    if np.ndim(a) < 2 or a.shape[-1] != a.shape[-2]:
        raise ValueError(
            "Argument to inv must have shape [..., n, n], got {}.".format(np.shape(a))
        )
    return solve(
...
def _norm(x, ord, axis, keepdims):
    ...
    if num_axes == 1:
        if ord is None or ord == 2:
            return np.sqrt(
                np.sum(np.real(x * np.conj(x)), axis=axis, keepdims=keepdims)
            )
        elif ord == np.inf:
            ...
            return np.sum(
                x != 0, dtype=np.finfo(lax.dtype(x)).dtype, axis=axis, keepdims=keepdims
            )
        elif ord == 1:
            ...
            return np.power(
                np.sum(np.abs(x) ** ord, axis=axis, keepdims=keepdims), 1.0 / ord
            )
        ...
        if ord is None or ord in ("f", "fro"):
            ...
            if not keepdims and col_axis > row_axis:
                col_axis -= 1
            return np.amax(
                np.sum(np.abs(x), axis=row_axis, keepdims=keepdims),
                axis=col_axis,
                keepdims=keepdims,
            )
        elif ord == -1:
            ...
            if not keepdims and row_axis > col_axis:
                row_axis -= 1
            return np.amax(
                np.sum(np.abs(x), axis=col_axis, keepdims=keepdims),
                axis=row_axis,
                keepdims=keepdims,
            )
        elif ord == -np.inf:
