...
class BatchTrace(Trace):
    ...
    def process_map(self, map_primitive, f, tracers, params):
        ...
        if all(dim is not_mapped for dim in dims):
            ...
            vals = [
                moveaxis(x, d, 1) if d is not not_mapped and d != 1 else x
...
def batch_jaxpr(jaxpr, size, batched, instantiate):
    ...
    avals_in = [
        _promote_aval_rank(size, a) if b else a for a, b in zip(jaxpr.in_avals, batched)
    ]
...
def batched_traceable(size, batched, instantiate, *vals):
    ...
    out_vals = [
        moveaxis(x, d, 0)
        if d is not not_mapped and d != 0
        else broadcast(x, size, 0)
        if d is not_mapped and inst
        else x
        for x, d, inst in zip(out_vals, out_dims, instantiate)
    ]
    out_batched = [
        d is not not_mapped or inst for d, inst in zip(out_dims, instantiate)
    ]
