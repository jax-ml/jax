...
class JaxprTrace(Trace):
    ...
    def process_primitive(self, primitive, tracers, params):
        if primitive in custom_partial_eval_rules:
            ...
            if primitive.multiple_results:
                out_tracers = [
                    JaxprTracer(self, PartialVal((aval, unit)), None)
                ...
                out_tracer.recipe = new_jaxpr_eqn(
                    tracers, [out_tracer], primitive, (), params
                )
                return out_tracer
    ...
    def process_call(self, call_primitive, f, tracers, params):
        ...
        out_pv_consts, consts = split_list(
            out_flat, [len(out_flat) - len(jaxpr.constvars)]
        )
        ...
        eqn = new_jaxpr_eqn(
            tracers, out_tracers, call_primitive, (bound_subjaxpr,), params
        )
        for t in out_tracers:
            t.recipe = eqn
        return out_tracers
    ...
    def process_map(self, map_primitive, f, tracers, params):
        ...
        out_pvs = [
            None if pv is None else _unmapped_aval(params["axis_size"], pv)
        ...
        eqn = new_jaxpr_eqn(
            tuple(it.chain(const_tracers, tracers)),
            out_tracers,
            map_primitive,
            (bound_subjaxpr,),
            params,
    ...
    def post_process_call(self, call_primitive, out_tracers, params):
        ...
        def todo(x):
            ...
            eqn = new_jaxpr_eqn(
                [], out_tracers, call_primitive, (bound_subjaxpr,), params
            )
            for t in out_tracers:
                t.recipe = eqn
            return out_tracers
    ...
    def post_process_map(self, map_primitive, out_tracers, params):
        ...
        def todo(x):
            ...
            eqn = new_jaxpr_eqn(
                const_tracers, out_tracers, map_primitive, (bound_subjaxpr,), params
            )
            for t in out_tracers:
                t.recipe = eqn
            return out_tracers
...
def abstract_eval_fun(fun, *avals, **params):
    ...
    _, pvals_out, _ = trace_to_jaxpr(
        lu.wrap_init(fun, params), pvals_in, instantiate=True
    )
...
def partial_eval_jaxpr(jaxpr, unknowns, instantiate):
    ...
    def fun(*vals):
        ...
        jaxpr_2, out_pvals_2, consts_2 = trace_to_jaxpr(
            f, pvals, instantiate=instantiate
        )
    ...
    jaxpr_1, out_pvals, consts_1 = trace_to_jaxpr(
