...
def shard_args(backend, device_ordinals, assignments, axis_size, tuple_args, args):
  ...
  """
        ...
        if type(arg) is ShardedDeviceArray:
            if nrep == len(arg.device_buffers):
                for r, buf in enumerate(arg.device_buffers):
                    buffers[r][a] = (
                        buf
                        if buf.device_ordinal() == device_ordinals[r]
                        else buf.copy_to_device(device_ordinals[r])
                    )
            else:
                for r, buf in enumerate(arg.device_buffers):
                    buffers[r][a] = xla.device_put(
                        x[assignments[r]], ordinals[r], backend=backend
                    )
        else:
            bufs = shard_arg_handlers[type(arg)](
                arg, device_ordinals, assignments, backend=backend
            )
    ...
    if tuple_args:
        buffers = [
            [xla.make_tuple(bufs, device_ordinals[r], backend)]
...
shard_arg_handlers = {}
shard_arg_handlers[core.Unit] = lambda x, ordinals, _, backend=None: [
    xla.device_put(core.unit, d, backend=backend) for d in ordinals
]
...
def _shard_array(x, ordinals, assignments, backend=None):
    nrep = len(ordinals)
    return (
        xla.device_put(x[assignments[r]], ordinals[r], backend=backend)
        for r in range(nrep)
    )
...
def _shard_device_array(x, ordinals, assignments, backend=None):
    ...
    xs = x._unstack()
    return (
        xla.device_put(xs[assignments[r]], ordinals[r], backend=backend)
        for r in range(nrep)
    )
...
def compile_replicated(
    ...
    *abstract_args
    ...
    logging.vlog(
        1,
        "compile_replicated: axis_size=%d global_axis_size=%d jaxpr_replicas=%d"
    ...
    if devices is None:
        ...
        if num_replicas != len(devices):
            raise ValueError(
                "compiling computation that requires %s replicas, "
    ...
    built_c = xla.jaxpr_computation(
        jaxpr, backend, axis_env, consts, (), arg_shapes, tuple_args=tuple_args
    )
...
def axis_index(axis_name):
    ...
    return axis_index_p.bind(
        dummy_arg,
        hard_size=frame.hard_size,
        soft_size=frame.soft_size,
        axis_name=axis_name,
    )
  ...
  """
    ...
    def _value(self):
        if self._npy_value is None:
            ...
            self._npy_value = self._collect(
                [self.device_buffers[i].to_py() for i in ids]
            )
...
def _shard_sharded_device_array(x, ordinals, assignments):
    ...
    if n == len(x.device_buffers):
        return (
            b if b.device_ordinal() == ordinals[r] else b.copy_to_device(ordinals[r])
...
def xla_pmap_impl(fun, *args, **params):
    ...
    compiled_fun = parallel_callable(
        fun, backend, axis_name, axis_size, devices, *abstract_args
    )
...
def parallel_callable(fun, backend, axis_name, axis_size, devices, *avals):
    ...
    with core.new_master(JaxprTrace, True) as master:
        jaxpr, (out_pvals, consts, env) = trace_to_subjaxpr(
            dynamic_fun, master, False
        ).call_wrapped([pval] + pvals)
    ...
    if all(pv is None for pv in out_pvs):
        ...
        handlers = [
            _pval_to_result_handler(axis_size, None, pval) for pval in out_pvals
        ]
        ...
        compiled, nrep = compile_replicated(
            jaxpr,
            backend,
            axis_name,
            axis_size,
        ...
        handle_args = partial(
            shard_args, backend, device_ordinals, assignments, axis_size, tuple_args
        )
        handle_outs = _pvals_to_results_handler(axis_size, nrep, out_pvals)
        return partial(
            execute_replicated, compiled, backend, nrep, handle_args, handle_outs
        )
...
def execute_replicated(compiled, backend, nrep, in_handler, out_handler, *args):
    if nrep > xb.device_count(backend):
        msg = (
            "executing pmap computation that requires {} replicas, but only {} "
...
def _pmap_translation_rule(
    ...
    sharded_outs = xla.jaxpr_subcomp(
        c, jaxpr, backend, new_env, const_nodes, freevar_nodes, *in_nodes_sharded
    )
    outs = [
        _xla_unshard(c, xla.axis_groups(new_env, axis_name), r) for r in sharded_outs
    ]
    return c.Tuple(*outs)
...
def _xla_shard(c, x):
    ...
    if xla_shape.is_tuple():
        ...
        return c.Reshape(
            c.DynamicSlice(x, start_indices, [1] + dims[1:]), None, dims[1:]
        )
...
def _xla_unshard(c, replica_groups, x):
    ...
    if xla_shape.is_tuple():
        ...
        padded = c.Broadcast(
            c.Constant(onp.array(0, xla_shape.numpy_dtype())), [axis_size] + dims
        )
        padded = c.DynamicUpdateSlice(
            padded, c.Reshape(x, None, [1] + dims), start_indices
        )
...
class SplitAxisTrace(core.Trace):
            ...
            if primitive in xla.parallel_translations:
                ...
                if name == params["axis_name"]:
                    ...
                    try:
                        ...
                        msg = (
                            "split_axis for {} not implemented. Open a feature request!"
                        )
    ...
    def process_call(self, call_primitive, f, tracers, params):
        ...
        if call_primitive in pe.map_primitives:
            ...
            if all(name is not_mapped for name in names):
                ...
                return [
                    SplitAxisTracer(self, a, x) for a, x in zip(names_out(), vals_out)
                ]
    ...
    def process_map(self, map_primitive, f, tracers, params):
        ...
        if all(name is not_mapped for name in names):
            ...
            vals_trans = [
                batching.moveaxis(x, 0, 1) if d is not not_mapped else x
