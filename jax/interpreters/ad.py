...
def jvp_subtrace(master, primals, tangents):
    ...
    in_tracers = [
        JVPTracer(trace, x, t) if t is not zero else x
        for x, t in zip(primals, tangents)
    ]
    ans = yield in_tracers, {}
    out_tracers = map(trace.full_raise, ans)
    yield unzip2(
        [(out_tracer.primal, out_tracer.tangent) for out_tracer in out_tracers]
    )
...
def linearize(traceable, *primals, **kwargs):
    ...
    in_pvals = tuple(pe.PartialVal((None, p)) for p in primals) + tuple(
        pe.PartialVal((get_aval(p).at_least_vspace(), core.unit)) for p in primals
    )
...
def vjp(traceable, primals, has_aux=False):
    if not has_aux:
        ...
        out_primals, pvals, jaxpr, consts, aux = linearize(
            traceable, *primals, has_aux=True
        )
...
class JVPTrace(Trace):
    ...
    def process_primitive(self, primitive, tracers, params):
        ...
        try:
            ...
            raise NotImplementedError(
                "Forward-mode differentiation rule for '{}' not implemented".format(
                    primitive
                )
            )
    ...
    def process_call(self, call_primitive, f, tracers, params):
        ...
        f_jvp, out_tree_def = traceable(
            jvp_subtrace(f, self.master), len(primals), in_tree_def
        )
...
def defvjp_all(prim, custom_vjp):
    ...
    def fun_jvp(xs, ts, **params):
        ...
        primals, tangents = split_list(
            primals_and_tangents, [len(primals_and_tangents) // 2]
        )
    ...
    def fun_jvp_partial_eval(trace, *tracers, **params):
        ...
        tangents_out = fun_lin_p.bind(
            *it.chain(res, tangents),
            trans_jaxpr=jaxpr,
            num_res=len(res),
            out_avals=out_avals
        )
...
def defvjp(prim, *vjps):
    def vjpmaker(*primals):
        ...
        vjpfun = lambda ct: [
            vjp(ct, *primals) if vjp else zeros_like_jaxval(x)
...
def defvjp2(prim, *vjps):
    def vjpmaker(*primals):
        ans = prim.bind(*primals)
        vjpfun = lambda ct: [
            vjp(ct, ans, *primals) if vjp else zeros_like_jaxval(x)
            for x, vjp in zip(primals, vjps)
        ]
        return ans, vjpfun
...
def jvp_jaxpr(jaxpr, nonzeros, instantiate):
    ...
    jaxpr_out, pvals_out, literals_out = pe.trace_to_jaxpr(
        f_jvp, pvals, instantiate=True
    )
...
def rearrange_binders(jaxpr, primals_in, tangents_in, primals_out, tangents_out):
    ...
    new_typed_jaxpr = core.TypedJaxpr(
        new_jaxpr, jaxpr.literals, new_in_avals, new_out_avals
    )
