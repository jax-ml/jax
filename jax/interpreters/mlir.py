...
# Lowering and execution path that converts jaxprs into the MLIR StableHLO/CHLO
# dialects.
...
def shape_tensor(sizes: Sequence[Union[int, ir.RankedTensorType]]
  ...
  def lower_dim(d):
    if type(d) is int:
      ...
      return xhlo.ReshapeOp(
          int1d,
          xhlo.ConvertOp(aval_to_ir_type(core.ShapedArray((), np.int32)), d))
  d, *ds = map(lower_dim, sizes)
  if not ds:
    return d
  else:
    return xhlo.ConcatenateOp([d, *ds], i64_attr(0)).result
...
ir_type_handlers[core.AbstractToken] = lambda _: [xhlo.TokenType.get()]
...
def _numpy_array_constant(x: np.ndarray, canonicalize_types
  ...
  return (xhlo.ConstantOp(attr).result,)
...
def _ndarray_constant_handler(val: np.ndarray, canonicalize_types
  ...
  if dtypes.result_type(val) == dtypes.float0:
    ...
    out = xhlo.BroadcastInDimOp(
        ir.RankedTensorType.get(val.shape,
                                dtype_to_ir_type(collapsed_val.dtype)),
...
def _token_constant_handler(val, canonicalize_types):
  if xc.mlir_api_version < 40:
    return [xhlo.CreateTokenOp(xhlo.TokenType.get()).result]
  else:
    return [xhlo.CreateTokenOp().result]
...
class DimPolyEvaluator:
  ...
  def __add__(self, other: Union[np.int32, DimPolyEvaluator]):
    ...
    return DimPolyEvaluator(xhlo.AddOp(self.value, other.value).result)

  def __radd__(self, other: np.int32):
    return DimPolyEvaluator(xhlo.AddOp(ir_constant(other), self.value).result)

  def __mul__(self, other: Union[np.int32, DimPolyEvaluator]):
    if not isinstance(other, DimPolyEvaluator):
      other = DimPolyEvaluator(ir_constant(other))
    return DimPolyEvaluator(xhlo.MulOp(self.value, other.value).result)

  def __rmul__(self, other: np.int32):
    return DimPolyEvaluator(xhlo.MulOp(ir_constant(other), self.value).result)
...
def lower_jaxpr_to_module(
  ...
  """Lowers a top-level jaxpr to a StableHLO module.
  ...
  # StableHLO channels need to start at 1
...
def token_type() -> Sequence[ir.Type]:
  return [xhlo.TokenType.get()]

def create_token() -> Token:
  if xc.mlir_api_version < 40:
    return wrap_singleton_ir_values(
        xhlo.CreateTokenOp(xhlo.TokenType.get()).result)
  else:
    return wrap_singleton_ir_values(xhlo.CreateTokenOp().result)

class TokenSet:
  """An immutable container of tokens to be used to lower effectful jaxprs. When lowering
  effectful jaxprs, we need to thread StableHLO tokens to sequence them. Each effect
  will need its own token that will be threaded in and out of the effectful
  primitives. A `TokenSet` encapsulates a set of StableHLO tokens that will be
...
def lower_jaxpr_to_fun(
  ...
  Args:
    ...
    create_tokens: if true, the StableHLO will create tokens and ignore dummy
      input tokens.
    ...
    use_sharding_annotations: if True, use mhlo.sharding annotations on
      ...
      xhlo.custom_call operators with sharding annotations.
      TODO(b/228598865): remove this option when mhlo.sharding annotations
      are propagated on non-entry functions during StableHLO->HLO conversion.
  ...
  with ir.InsertionPoint(entry_block):
    ...
    for aval, arg in zip(jaxpr.in_avals, unflattened_args):
      if replace_tokens_with_dummy and aval is core.abstract_token:
        if xc.mlir_api_version < 40:
          args.append(xhlo.CreateTokenOp(xhlo.TokenType.get()).results)
        else:
          args.append(xhlo.CreateTokenOp().results)
      else:
        args.append(arg)
...
def jaxpr_subcomp(ctx: ModuleContext, jaxpr: core.Jaxpr,
  ...
  """Lowers a jaxpr into StableHLO, inlined into an existing function.
...
def broadcast_in_dim(ctx: LoweringRuleContext, op, aval_out: core.AbstractValue, *,
  ...
  if not core.is_constant_shape(aval_out.shape):  # type: ignore
    ...
    return xhlo.DynamicBroadcastInDimOp(
...
def reshape(ctx: LoweringRuleContext, op, aval_out: core.AbstractValue) -> ir.Value:
  ...
  if not core.is_constant_shape(aval_out.shape):  # type: ignore
    shape = eval_dynamic_shape(ctx, aval_out.shape)  # type: ignore
    return xhlo.DynamicReshapeOp(
        aval_to_ir_type(aval_out),
        op,
        shape_tensor(shape),
    ).result
  else:
    return xhlo.ReshapeOp(aval_to_ir_type(aval_out), op).result
...
def slice_op(ctx: LoweringRuleContext, x, aval_out, *,
  ...
  if any(not core.is_constant_shape(s) for s in (start_indices, limit_indices, strides)):
    ...
    return xhlo.RealDynamicSliceOp(
        aval_to_ir_type(aval_out), x, shape_tensor(start_indices),
        shape_tensor(limit_indices), shape_tensor(strides)).result
  else:
    return xhlo.SliceOp(x, dense_int_elements(start_indices),
...
def dynamic_slice(ctx: LoweringRuleContext, aval_out, x, *,
  ...
  if not core.is_constant_shape(slice_sizes):
    ...
    return xhlo.RealDynamicSliceOp(
        aval_to_ir_type(aval_out), x, shape_tensor(start_indices),
        shape_tensor(slice_sizes), shape_tensor([1] * len(slice_sizes))).result
  else:
    return xhlo.DynamicSliceOp(x, start_indices,
...
def add_jaxvals_lowering(ctx, x, y):
  return xhlo.AddOp(x, y).results
register_lowering(ad_util.add_jaxvals_p, add_jaxvals_lowering)

register_lowering(ad_util.stop_gradient_p, lambda ctx, x: [x])


def compare_xhlo(x, y, direction: str, comparison_type: Optional[str] = None):
  """Creates xhlo.CompareOp."""
  ...
  return xhlo.CompareOp(
      x,
      y,
      xhlo.ComparisonDirectionAttr.get(direction),
      compare_type=xhlo.ComparisonTypeAttr.get(comparison_type))


def _minmax_xhlo(op, cmp, x, y):
  ...
  if ir.ComplexType.isinstance(tensor_type.element_type):
    rx = xhlo.RealOp(x).result
    ry = xhlo.RealOp(y).result
    real_eq = compare_xhlo(rx, ry, "EQ", "FLOAT")
    real_cmp = compare_xhlo(rx, ry, cmp, "FLOAT")
    imag_cmp = compare_xhlo(
        xhlo.ImagOp(x).result,
        xhlo.ImagOp(y).result, cmp, "FLOAT")
    which = xhlo.SelectOp(real_eq, imag_cmp, real_cmp).result
    return xhlo.SelectOp(which, x, y)
  else:
    return op(x, y)


min_xhlo = partial(_minmax_xhlo, xhlo.MinOp, "LT")
max_xhlo = partial(_minmax_xhlo, xhlo.MaxOp, "GT")


def convert_xhlo(ctx: LoweringRuleContext, x, aval_in, aval_out):
  ...
  if (not core.is_opaque_dtype(aval_out.dtype) and
    ...
    return compare_xhlo(x, full_like_aval(ctx, 0, aval_in), "NE",
                        compare_type).result
  return xhlo.ConvertOp(aval_to_ir_type(aval_out), x).result
...
def _wrap_with_spmd_op(name: str,
  ...
  if unspecified_dims:
    ...
    backend_config = ""
  op = xhlo.CustomCallOp([result_type], [x],
...
def cache_lowering(f):
  ...
  def cached_lowering(ctx, *args, **params):
    ...
    try:
      ...
      # computations have been ported to xhlo.
...
def xla_computation_to_mlir_module(
    xla_computation: xc.XlaComputation) -> ir.Module:
  module_str = xc._xla.mlir.xla_computation_to_mlir_module(xla_computation)
  if xc.mlir_api_version < 40:
    module_str = xc._xla.mlir.stablehlo_to_mhlo(module_str)
  return ir.Module.parse(module_str)


def merge_mlir_modules(dst_module: ir.Module, sym_name: str,
...
def xla_fallback_lowering(prim: core.Primitive):
  ...
  def fallback(ctx: LoweringRuleContext, *args, **params):
    ...
    callee_name = merge_mlir_modules(module_ctx.module,
                                     f"xla_fallback_{prim.name}", xla_module)
    output_types = map(aval_to_ir_types, ctx.avals_out)
    flat_output_types = util.flatten(output_types)
    ...
    if not prim.multiple_results:
      return [call]
    flat_results = [
        xhlo.GetTupleElementOp(call, i32_attr(i)).result
...
def send_to_host(channel: int,
                 token: xhlo.TokenType,
                 operand: Any,
                 aval: core.ShapedArray,
                 name: str,
                 *,
                 sharding: Optional[xc.OpSharding] = None) -> ir.Value:
  channel_handle = xhlo.ChannelHandle.get(channel, SEND_TO_HOST_TYPE)
  if xc.mlir_api_version < 40:
    send_op = xhlo.SendOp(
        xhlo.TokenType.get(), [operand],
        token,
        channel_handle,
        is_host_transfer=ir.BoolAttr.get(True))
  else:
    send_op = xhlo.SendOp([operand],
                          token,
                          channel_handle,
                          is_host_transfer=ir.BoolAttr.get(True))
...
def receive_from_host(channel: int,
                      token: xhlo.TokenType,
                      out_aval: core.ShapedArray,
                      name: str,
                      *,
                      sharding: Optional[xc.OpSharding] = None) -> ir.Value:
  channel_handle = xhlo.ChannelHandle.get(channel, RECV_FROM_HOST_TYPE)
  recv_op = xhlo.RecvOp([aval_to_ir_type(out_aval),
                         xhlo.TokenType.get()],
                        token,
                        channel_handle,
                        is_host_transfer=ir.BoolAttr.get(True))
...
def _emit_tpu_python_callback(
  ...
  if xc.mlir_api_version < 40:
    token = token or xhlo.CreateTokenOp(xhlo.TokenType.get()).result
  else:
    token = token or xhlo.CreateTokenOp().result
...
def emit_python_callback(
  ...
  """Emits StableHLO that calls back to a provided Python function."""
  ...
  result = xhlo.CustomCallOp(
      ...
      operand_layouts=(None if operand_mlir_layouts is None else
                       ir.ArrayAttr.get(operand_mlir_layouts)),
      result_layouts=(None if result_mlir_layouts is None else
                      ir.ArrayAttr.get(result_mlir_layouts)))
  if sharding is not None:
    set_sharding(result, sharding)
  results = [
      xhlo.GetTupleElementOp(result, i32_attr(i)).result
