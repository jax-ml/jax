...
def mask_subtrace(master, in_vals, shape_exprs):
    ...
    in_tracers = [
        MaskTracer(trace, x, s).full_lower() for x, s in zip(in_vals, shape_exprs)
    ]
...
class Poly(Counter):  # type Poly = Map Mon Int -- monomials to coeffs
    ...
    def __str__(self):
        return " + ".join(
            "{} {}".format(v, k) if v != 1 else str(k) for k, v in sorted(self.items())
        ).strip()
...
class Mon(Counter):  # type Mon = Map Id Int -- ids to degrees
    ...
    def __str__(self):
        return " ".join(
            "{}**{}".format(k, v) if v != 1 else str(k) for k, v in sorted(self.items())
        )
...
def eval_dim_expr(env, poly):
    terms = [
        mul(coeff, prod([pow(env[id], deg) for id, deg in mon.items()]))
...
class MaskTrace(Trace):
    ...
    def process_primitive(self, primitive, tracers, params):
        ...
        if primitive in shape_parameterized_primitive_rules:
            ...
            logical_shapes = map(
                partial(eval_shape_expr, shape_envs.logical), shape_exprs
            )
