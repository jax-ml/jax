...
device_put_handlers = {}
device_put_handlers[core.Unit] = lambda _, n, backend=None: xc.Buffer.from_pyval(
    (), n, backend=xb.get_backend(backend)
)
...
def xla_primitive_callable(prim, *abstract_args, **params):
    ...
    compiled = built_c.Compile(
        compile_options=xb.get_compile_options(), backend=xb.get_backend(backend)
    )
...
def primitive_computation(prim, *xla_shapes, **params):
    ...
    if prim in backend_specific_translations[platform]:
        ...
        rule(
            c, *xla_args, backend=backend, **new_params
        )  # return val set as a side-effect on c
...
def compile_jaxpr(
    jaxpr, device, backend, axis_env, const_vals, tuple_args, *abstract_args
):
    ...
    built_c = jaxpr_computation(
        jaxpr, backend, axis_env, const_vals, (), arg_shapes, tuple_args=tuple_args
    )
    device_assignment = (device.id,) if device else None
    compile_opts = xb.get_compile_options(
        num_replicas=axis_env.nreps, device_assignment=device_assignment
    )
    return built_c.Compile(
        compile_options=compile_opts, backend=xb.get_backend(backend)
    )


def build_jaxpr(jaxpr, backend, axis_env, const_vals, tuple_args, *abstract_args):
    arg_shapes = map(aval_to_xla_shape, abstract_args)
    return jaxpr_computation(
        jaxpr, backend, axis_env, const_vals, (), arg_shapes, tuple_args=tuple_args
    )
...
def jaxpr_computation(
    jaxpr, backend, axis_env, const_vals, freevar_shapes, arg_shapes, tuple_args=False
):
...
def jaxpr_subcomp(c, jaxpr, backend, axis_env, consts, freevars, *args):
    ...
    for eqn in jaxpr.eqns:
        ...
        if eqn.primitive in backend_specific_translations[platform]:
            ...
            ans = reduction_translations[eqn.primitive](
                c, *in_nodes, backend=backend, **new_params
            )
...
def _xla_callable(fun, device, backend, *abstract_args):
    ...
    with core.new_master(pe.JaxprTrace, True) as master:
        ...
        compiled = compile_jaxpr(
            jaxpr, device, backend, axis_env, consts, tuple_args, *abstract_args
        )
...
def _execute_replicated(compiled, backend, handlers, tuple_args, *args):
    input_bufs = [
        [device_put(x, device_num, backend=backend) for x in args]
        for device_num in compiled.DeviceOrdinals()
    ]
    if tuple_args:
        input_bufs = [
            [make_tuple(bufs, device_num)]
...
def _xla_call_translation_rule(
    c, jaxpr, axis_env, const_nodes, freevar_nodes, in_nodes, device=None, backend=None
):
    del device  # Ignored.
...
def _device_put_device_array(x, device_num, backend):
    ...
    if (
        not hasattr(x.device_buffer, "platform")
...
def _instantiate_device_constant(const, device_num=0, backend=None, cutoff=1e6):
    ...
    if const.size > cutoff and device_num == 0:
        ...
        return xc.Buffer.from_pyval(
            onp.asarray(const), device_num, backend=xb.get_backend(backend)
        )
