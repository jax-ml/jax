...
def _prime_factors(x: int) -> list[int]:
  # Finding prime factors of the global shape sizes helps efficiently dividing
  # the global shape into chunks of a target size.
  factors = []
  while x % 2 == 0:
    factors.append(2)
    x //= 2
...
def _compute_chunk_shape(
  ...
  This helps the tensorstore kvstore driver to limit the largest file size on
  disk to below the ``file_size_target``. We compute a chunk with a byte size at
  most 110% of the ``file_size_target``.
  """
  local_shape = list(local_shape)
  if len(local_shape) == 0 or _prod(local_shape) == 0:
    # a zero size array needs a non-zero chunk passed to tensorstore for compat.
    return [max(z, 1) for z in local_shape]
  total_size = _prod(local_shape) * jnp.dtype(dtype).itemsize
