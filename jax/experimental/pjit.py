...
def pjit(fun: Callable,
  ...
  def lower(*args, _global_avals=False, **kwargs):
    ...
    lowering = _pjit_lower(
        ...
        params['out_shardings'], params['donated_invars'], params['name'],
        in_is_global)
...
def _pjit_call_impl(*args, jaxpr,
                    in_shardings, out_shardings, donated_invars, name,
  ...
  compiled = _pjit_lower(
      jaxpr, in_shardings, out_shardings, donated_invars, name,
      in_is_global).compile(
...
def _pjit_lower(
  ...
  # interface too.
  mesh = pxla._get_mesh_from_sharding(in_shardings)
  return pxla.lower_mesh_computation(
      fun, 'pjit', name, mesh, in_axes, out_axes, donated_invars,
      True, jaxpr.in_avals, tiling_method=None, in_is_global=in_is_global)


def _pjit_abstract_eval(*args, jaxpr, out_shardings,
...
def _pjit_batcher(insert_axis,
                  ...
                  donated_invars, name, in_positional_semantics,
                  out_positional_semantics):
...
def _resource_typing_pjit(avals, params, source_info, resource_env, named_axis_resources):
  ...
  for aval, s in zip(jaxpr.in_avals, params['in_shardings']):
    if resource_env.physical_mesh != s.mesh:
...
def with_sharding_constraint(x, axis_resources):
  ...
  in_sharding_flat = tree_map(
      lambda x: sharding.MeshPspecSharding._from_parsed_pspec(mesh, x),
      axis_resources_flat)

  for xf, i in safe_zip(x_flat, in_sharding_flat):
    pjit_check_aval_sharding(i, xf, "with_sharding_constraint arguments",
                             allow_uneven_sharding=True)

  outs = [sharding_constraint_p.bind(xf, in_sharding=i.normalize())
          for xf, i in safe_zip(x_flat, in_sharding_flat)]
  return tree_unflatten(tree, outs)

def _sharding_constraint_impl(x, in_sharding):
...
ad.deflinear2(sharding_constraint_p,
              lambda ct, _, in_sharding: (
                  sharding_constraint_p.bind(
                      ct, in_sharding=in_sharding.normalize()),))

def _sharding_constraint_mhlo_lowering(ctx, x_node, *, in_sharding):
  aval, = ctx.avals_in
  return [
      mlir.wrap_with_sharding_op(
          x_node,
          in_sharding._to_xla_op_sharding(aval.ndim,
                                          ctx.module_context.axis_context),
          unspecified_dims=get_unconstrained_dims(in_sharding))
  ]
...
def _sharding_constraint_batcher(insert_axis, axis_size, axis_name, main_type,
  ...
  y = sharding_constraint_p.bind(
      x,
      in_sharding=_pjit_batcher_for_sharding(in_sharding, d, new_parts))
  return y, d
...
def _resource_typing_sharding_constraint(avals, params, source_info,
                                         _, named_axis_resources):
  aval, = avals
  _check_resources_against_named_axes(
    "with_sharding_constraint input", aval,
    params['in_sharding']._parsed_pspec, named_axis_resources)
...
def get_unconstrained_dims(in_sharding: sharding.MeshPspecSharding):
  return {i for i, axes in enumerate(in_sharding._parsed_pspec)
          if axes is None}
...
def _check_array_device_assignment(pjit_mesh, in_shardings):
  ...
  for i in in_shardings:
    ...
    if not pjit_mesh.empty and mesh_devices != arr_device_assignment:
    ...
    if pjit_mesh.empty and first_device_assignment != arr_device_assignment:
