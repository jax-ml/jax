...
[getting started Colab](JAX2TF_getting_started.ipynb). For details of
...
dimensions that are unknown at JAX tracing time. In the example above, the
`in_shapes` specification does not convey more information than the partial `tf.TensorSpec`,
...
JAX tracing mechanism does shape checking using the same strict rules as for
when the shapes are fully known. For example, given the `in_shapes=["(b, d, d)"])`
...
For any function `f_jax` and input signature `abs_sig` and concrete input `x` whose
shape matches `abs_sig`:
 * If the conversion to TensorFlow succeeds: `f_tf = tf.function(jax2tf.convert(f_jax, in_shapes))(abs_sig)`
 * and the TensorFlow execution succeeds with result `y`: `f_tf(x) = y`
...
shape-polymorphic TF graph, but which will give an error when converting with jax2tf.
  ...
  * Otherwise, the specifier must be a comma-separated string of the form `(dim_1, ..., dim_n)`, denoting
    ...
    corresponding dimensions from the shape specifier and the `TensorSpec` are matched:
       ...
       * a dimension specifier can also be a lowercase identifier, denoting a dimension-size
         ...
         constant.
       * a dimension specifier can also be a **linear** polynomial expression involving dimension
...
in the shape parameters of the `broadcast_in_dim` JAX primitive.
...
print(jax.make_jaxpr(image_mask_jas)(np.ones((1024, 28, 28)), np.ones((28, 28)))
>> { lambda  ; a b.
...
When tracing and converting with abstract shapes this parameter will contain polynomials
...
I think it should be possible to mentally distinguish JAX code that computes shapes from
...
so the output of the conversion could be:
...
mapping the shape variables in the `in_shapes` specification to TF expressions
