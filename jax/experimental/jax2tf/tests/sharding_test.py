...
class ShardedJitHloTest(tf_test_util.JaxToTfTestCase):
  ...
  def _log_sharding_annotations(self,
                                  f_jax,
                                  args: Sequence[Any],
                                  *,
                                  num_partitions=2,
    ...
    Ideally this would check the sharding of intermedia results in JAX and
  ...
  def test_pjit_basic1D(self):
    ...
    self._log_sharding_annotations(
        jax_func, [x, x],
        num_partitions=2)
...
class PjitTest(tf_test_util.JaxToTfTestCase):
  ...
  def test_xmap_basic(self):
    ...
    def f(a, b):
      return a * 2, b * 4
    devices = np.array(local_devices[:1]).reshape((1, 2))
