...
from lingvo.flax import base_layer as jax_base_layer
from lingvo.flax.layers.linears import Linear as jax_linear
import numpy as np
...
class Jax2TfTest(tf_test_util.JaxToTfTestCase):
  ...
  def test_convert_of_jaxlayer_fprop_outside_context(self):
    ...
    params_init = jax_layer.init(init_key)
    ...
    output_sol = np.array(test_input) @ np.array(params_init['params']['w'])

    # Convert jax_layer.fprop() to TF function
    with jax_context:
      tf_layer = jax2tf.convert(jax_layer.apply)

    # Outside of context
    output = tf_layer(params_init, test_input)

    self.assertAllClose(output, output_sol, rtol=1e-1, atol=0)
