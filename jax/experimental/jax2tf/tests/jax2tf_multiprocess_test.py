...
class Jax2TfMultiProcessTest(tf_test_util.JaxToTfTestCase,
  ...
  def test_multi_process_pjit_export(self):
    ...
    with jtu.create_mesh((4, 2), ("x", "y")):
      pjit_matmul = pjit(jnp.matmul, in_shardings=(P("x", "y"), None))
      jax_result = multihost_utils.process_allgather(pjit_matmul(w, x))
