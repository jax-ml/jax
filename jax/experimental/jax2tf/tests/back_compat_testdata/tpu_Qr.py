    ...
    expected_outputs=(
        array(
            [
                ...
                [-0.8944271, -0.18257444, 0.4082482],
            ],
            dtype=float32,
        ),
        array(
            [
                [-6.7082043, -8.049844, -9.391484],
                [0.0, 1.0954441, 2.1908882],
                [0.0, 0.0, 0.0],
            ],
            dtype=float32,
        ),
...
""",
    mlir_module_serialized=(
        ...
        b' shape=(3, 3)'
        b' dimension=0]\x00broadcast_dimensions\x00jit(<lambda>)/jit(main)/jit(triu)/add\x00jit(<lambda>)/jit(main)/jit(triu)/iota[dtype=int32'
        b' shape=(3, 3)'
        b' dimension=1]\x00compare_type\x00comparison_direction\x00jit(<lambda>)/jit(main)/jit(triu)/ge\x00jit(<lambda>)/jit(main)/jit(triu)/broadcast_in_dim[shape=(3,'
        b' 3) broadcast_dimensions=()]\x00jit(<lambda>)/jit(main)/jit(triu)/select_n\x00custom-call.3\x00custom-call.2\x00get-tuple-element.3\x00get-tuple-element.4\x00tuple.5\x00jit(<lambda>)/jit(main)/iota[dtype=float32'
        b' shape=(9,)'
        b' dimension=0]\x00jit(<lambda>)/jit(main)/reshape[new_sizes=(3, 3)'
        b' dimensions=None]\x00jit(<lambda>)/jit(main)/qr[full_matrices=True]\x00edge_padding_high\x00edge_padding_low\x00interior_padding\x00jit(<lambda>)/jit(main)/pad[padding_config=((0,'
        b' 0, 0), (0, 0,'
        b' 0))]\x00private\x00jax.result_info\x00geqrf\x00xla_fallback_geqrf\x00householder_product\x00xla_fallback_householder_product\x00triu\x00\x00(f32[3,3]{1,0},'
        b' f32[3]{0})\x00[0]\x00[1]\x00main\x00public\x00ProductOfElementaryHouseholderReflectors\x00Qr\x00'
    ),
