...
""",
    mlir_module_serialized=(
        ...
        b' keep_unused=False'
        b' inline=False]\x00jit(<lambda>)/jit(main)/eigh_jacobi[lower=True'
        b' sort_eigenvalues=True]\x00api_version\x00backend_config\x00call_target_name\x00called_computations\x00has_side_effect\x00operand_layouts\x00output_operand_aliases\x00result_layouts\x00custom-call.2\x00get-tuple-element.4\x00get-tuple-element.3\x00tuple.5\x00jit(<lambda>)/jit(main)/jit(tril)/iota[dtype=int32'
        b' shape=(8, 8)'
        b' dimension=0]\x00broadcast_dimensions\x00jit(<lambda>)/jit(main)/jit(tril)/add\x00jit(<lambda>)/jit(main)/jit(tril)/iota[dtype=int32'
        b' shape=(8, 8)'
        b' dimension=1]\x00compare_type\x00comparison_direction\x00jit(<lambda>)/jit(main)/jit(tril)/ge\x00jit(<lambda>)/jit(main)/jit(tril)/broadcast_in_dim[shape=(8,'
        ...
        b' 0)]\x00jit(<lambda>)/jit(main)/add\x00jit(<lambda>)/jit(main)/div\x00private\x00jax.result_info\x00tril\x00eigh_jacobi\x00xla_fallback_eigh_jacobi\x00[0]\x00[1]\x00main\x00public\x001,1,15,0.000010\x00Eigh\x00(f32[8,8]{1,0},'
        b' f32[8]{0})\x00(f32[8]{0}, f32[8,8]{1,0})\x00'
    ),
