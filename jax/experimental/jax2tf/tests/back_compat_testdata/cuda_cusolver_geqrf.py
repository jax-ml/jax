...
data_2023_03_18['unbatched'] = dict(
    ...
    expected_outputs=(
        array(
            [
                [0.0, 0.9128705, 0.40824863],
                [-0.44721356, 0.36514878, -0.8164964],
                [-0.8944271, -0.18257457, 0.40824813],
            ],
            dtype=float32,
        ),
        array(
            [
                [-6.7082043e00, -8.0498438e00, -9.3914843e00],
                [0.0000000e00, 1.0954436e00, 2.1908882e00],
                [0.0000000e00, 0.0000000e00, 5.6703755e-08],
            ],
            dtype=float32,
        ),
    ),
...
""",
    mlir_module_serialized=(
        ...
        b' dimensions=None]\x00jit(<lambda>)/jit(main)/geqrf\x00jit(<lambda>)/jit(main)/qr[full_matrices=True]\x00edge_padding_high\x00edge_padding_low\x00interior_padding\x00jit(<lambda>)/jit(main)/pad[padding_config=((0,'
        b' 0, 0), (0, 0,'
        b' 0))]\x00jit(<lambda>)/jit(main)/householder_product\x00callee\x00jax.result_info\x00triu\x00[0]\x00[1]\x00main\x00public\x00private\x00\x00\x00\x00\x00\x01\x00\x00\x00\x03\x00\x00\x00\x03\x00\x00\x00\x00\x00\x03\x00\x00cusolver_geqrf\x00\x00\x00\x00\x00\x01\x00\x00\x00\x03\x00\x00\x00\x03\x00\x00\x00\x03\x00\x00\x00'
        b' \x81\x00\x00\x00cusolver_orgqr\x00'
    ),
...
data_2023_03_18['batched'] = dict(
    ...
    expected_outputs=(
        ...
        array(
            [
                [
                    [-6.7082038e00, -8.0498447e00, -9.3914852e00],
                    [0.0000000e00, 1.0954450e00, 2.1908898e00],
                    [0.0000000e00, 0.0000000e00, 4.8374091e-08],
                ],
                [
                    [-2.1213203e01, -2.2910259e01, -2.4607319e01],
                    [0.0000000e00, 3.4641042e-01, 6.9282258e-01],
                    [0.0000000e00, 0.0000000e00, 1.4548683e-06],
                ],
            ],
            dtype=float32,
        ),
...
""",
    mlir_module_serialized=(
        ...
        b' dimension=0]\x00jit(<lambda>)/jit(main)/jit(triu)/add\x00jit(<lambda>)/jit(main)/jit(triu)/iota[dtype=int32'
        b' shape=(3, 3)'
        b' dimension=1]\x00jit(<lambda>)/jit(main)/jit(triu)/ge\x00jit(<lambda>)/jit(main)/jit(triu)/broadcast_in_dim[shape=(2,'
        b' 3, 3) broadcast_dimensions=(1,'
        b' 2)]\x00jit(<lambda>)/jit(main)/jit(triu)/broadcast_in_dim[shape=(2,'
        b' 3, 3)'
        b' broadcast_dimensions=()]\x00jit(<lambda>)/jit(main)/jit(triu)/select_n\x00jit(<lambda>)/jit(main)/iota[dtype=float32'
        b' shape=(18,)'
        b' dimension=0]\x00jit(<lambda>)/jit(main)/reshape[new_sizes=(2, 3, 3)'
        b' dimensions=None]\x00jit(<lambda>)/jit(main)/geqrf\x00jit(<lambda>)/jit(main)/qr[full_matrices=True]\x00edge_padding_high\x00edge_padding_low\x00interior_padding\x00jit(<lambda>)/jit(main)/pad[padding_config=((0,'
        b' 0, 0), (0, 0, 0), (0, 0,'
        b' 0))]\x00jit(<lambda>)/jit(main)/householder_product\x00callee\x00jax.result_info\x00triu\x00[0]\x00[1]\x00main\x00public\x00private\x00\x00\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x03\x00\x00\x00\x00cublas_geqrf_batched\x00\x00\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x03\x00\x00\x00\x03\x00\x00\x00'
        b' \x81\x00\x00\x00cusolver_orgqr\x00'
    ),
    xla_call_module_version=4,
    function_list=[],
