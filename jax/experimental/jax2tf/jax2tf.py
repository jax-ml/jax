...
def _pjit(*args: TfVal,
  ...
  for s in in_shardings:
    if len(s.device_set) != len(s.addressable_devices):
      raise NotImplementedError(
          "jax2tf translation for pjit over multi-process "
...
def _pjit_sharding_constraint(arg: TfVal, *,
                              in_sharding: sharding.MeshPspecSharding,
                              _in_avals: Sequence[core.ShapedArray],
                              _out_aval: core.ShapedArray,
                              **kwargs) -> TfVal:
  return _shard_value(arg, _in_avals[0], in_sharding)
