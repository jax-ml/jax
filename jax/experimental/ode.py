...
alpha = np.array([1 / 5, 3 / 10, 4 / 5, 8 / 9, 1.0, 1.0, 0])
beta = np.array(
    [
        [1 / 5, 0, 0, 0, 0, 0, 0],
        [3 / 40, 9 / 40, 0, 0, 0, 0, 0],
...
def fit_4th_order_polynomial(y0, y1, y_mid, dy0, dy1, dt):
  ...
  """
    ...
    a = np.dot(np.hstack([-2.0 * dt, 2.0 * dt, np.array([-8.0, -8.0, 16.0])]), v)
    b = np.dot(np.hstack([5.0 * dt, -3.0 * dt, np.array([18.0, 14.0, -32.0])]), v)
    c = np.dot(np.hstack([-4.0 * dt, dt, np.array([-11.0, -5.0, 16.0])]), v)
    d = dt * dy0
    e = y0
    return a, b, c, d, e
...
def initial_step_size(fun, t0, y0, order, rtol, atol, f0):
  ...
  """
    ...
    h1 = np.where(
        np.all(np.asarray([d1 <= 1e-15, d2 < 1e-15])),
        np.maximum(1e-6, h0 * 1e-3),
        (0.01 / np.max(d1 + d2)) ** order_pow,
    )

    return np.minimum(100.0 * h0, h1)
...
def vjp_odeint(ofunc, y0, t, *args, **kwargs):
  ...
  """
    ...
    def _fori_body_fun(i, val):
        ...
        aug_ans = odeint(
            rev_aug_dynamics, aug_y0, this_tarray, flat_args, rtol=rtol, atol=atol
        )
...
def benchmark_odeint(fun, y0, tspace, *args):
    ...
    for k in range(n_trials):
        ...
        print(
            "scipy odeint elapsed time ({} of {}): {}".format(
    ...
    for k in range(n_trials):
        ...
        print(
            "JAX odeint elapsed time ({} of {}): {}".format(
                k + 1, n_trials, end - start
            )
        )
    print(
        "norm(scipy result-jax result): {}".format(
...
def test_odeint_grad():
    ...
    # check swoop
    for cond in ((y0, ts, 0.1, 0.2), (big_y0, ts, 0.1, 0.3)):
...
def test_defvjp_all():
    ...
    for k in range(n_trials):
        ...
        print(
            "JAX jacrev elapsed time ({} of {}): {}".format(
                k + 1, n_trials, end - start
            )
        )
