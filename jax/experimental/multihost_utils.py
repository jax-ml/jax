...
def alive_devices(devices: list[xla_client.Device]) -> list[xla_client.Device]:
  """Returns the subset of the provided devices that are alive and healthy.

  `alive_devices` is a low-level fault tolerance primitive that can be used to
  implement fault tolerant multi-process JAX programs.

  Barrier Semantics

  It's important that every process agrees on which devices are alive to avoid
  ...
  To ensure that every process agrees on the set of alive devices, the
  `alive_devices` function has barrier-like semantics. Consider an invocation
  `alive_devices(devices)` where `devices` includes devices across a set of
  processes P. The invocation acts as a barrier, waiting for every process in P
  to call `alive_devices(devices)`. Afterwards, `alive_devices` returns the same
  set of alive devices `A` to all the processes in P. This ensures that every
  process agrees on the set of alive devices.

  Note that `alive_devices` does not actually act as a barrir for *every*
  process in P because some processes in P might have failed. Instead, the
  `alive_devices` function waits only for the processes with a device in the
  returned set of alive devices A.
  ...
  `alive_devices(jax.devices())`. The invocation returns devices 1, 2, 3, 4, 5,
  and 6. Because these devices are hosted by processes A, B, and C, the call to
  `alive_devices` acts as a barrier across processes A, B, and C. Process D,
  ...
  Returns:
    The subset of the provided devices that are alive and healthy.
  ...
  if xla_bridge.process_index() not in process_ids:
    # A process can only participate in an alive_devices call if it hosts some
  ...
  if len(process_ids) == 1:
    ...
    # devices are all alive.
    return devices

  alive_process_ids = client.get_alive_nodes(list(process_ids))
  return [d for d in devices if d.process_index in alive_process_ids]
