...
def attention(q, k, v, config: TuningConfig, save_residuals: bool = False):
  ...
  def kernel(q_ref, k_ref, v_ref, out_ref, lse_ref, scoped):
    ...
    def _compute_wg():
      ...
      if lse_ref is not None:
        ...
        lse_smem = lse_smem2[0].at[wg_idx]
        lse_smem[...] = lse.astype(jnp.float32)
      plgpu.commit_smem()
  ...
  def entry(q_ref, k_ref, v_ref, out_ref, lse_ref):
    ...
    scratch = (qo_scratch, k_scratch, v_scratch)
    if save_residuals:
      scratch += (
          plgpu.SMEM((compute_wgs, block_q), jnp.float32),
      )
    pl.run_scoped(
...
def attention_with_pipeline_emitter(q, k, v, config: TuningConfig, save_residuals=False):
  ...
  out, lse = run_function(q, k, v,
                          jnp.full_like(q, jnp.inf),  # out
                          lse)
