...
def _attention_forward(q, k, v, config: TuningConfig, save_residuals: bool = False):
  ...
  def kernel(q_ref, k_ref, v_ref, out_ref, lse_ref, scoped):
    ...
    if config.causal:
      total_q = (lax.axis_index("q_seq") + 1) * (2 * block_q)
      total_kv_steps = pl.cdiv(total_q, jnp.array(block_kv, jnp.int32))
    else:
      total_kv_steps = kv_seq_len // block_kv
    ...
    def _compute_wg():
      ...
      def kv_loop(kv_step, carry, causal: bool = False):
        acc, m_i, l_i = carry
        slot = jnp.remainder(kv_step, max_concurrent_steps)
        ...
        def compute_pv(acc_ref):
          ...
          wait_step = kv_step + 1
          wait_slot = jnp.remainder(wait_step, max_concurrent_steps)
      ...
      if config.causal:
        ...
        lax.fori_loop(kv_steps, total_kv_steps, epilogue_kv_loop, None)
      else:
        acc, m_i, l_i = lax.fori_loop(0, total_kv_steps, kv_loop, (acc, m_i, l_i))
    ...
    def _memory_wg():
      ...
      def prologue_kv_loop(kv_step, _):
        s = (batch, pl.ds(kv_step * block_kv, block_kv), kv_head)
        plgpu.copy_gmem_to_smem(k_ref.at[s], k_smem.at[kv_step], k_barriers.at[kv_step])
        plgpu.copy_gmem_to_smem(v_ref.at[s], v_smem.at[kv_step], v_barriers.at[kv_step])
      lax.fori_loop(0, max_concurrent_steps, prologue_kv_loop, None, unroll=True)

      def kv_loop(kv_step, _):
        tma_step = kv_step + max_concurrent_steps
        tma_slot = jnp.remainder(kv_step, max_concurrent_steps)
      ...
      lax.fori_loop(0, total_kv_steps - max_concurrent_steps, kv_loop, None)
...
def attention_reference(q, k, v, causal=False, save_residuals=False):
  ...
  if causal:
    ...
    logits = jnp.where(mask, logits, float(jnp.finfo(jnp.float32).min))
