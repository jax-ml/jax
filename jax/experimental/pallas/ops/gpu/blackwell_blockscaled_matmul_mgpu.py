            ...
            def _loop_body(ki, _):
              ...
              with jax.named_scope("copy_scales_to_tmem"):
                plgpu.async_copy_scales_to_tmem(
                    a_scale_smem.at[slot], a_scale_tmem, collective=collective
                )
                plgpu.async_copy_scales_to_tmem(
                    b_scale_smem.at[slot], b_scale_tmem, collective=collective
                )
...
def main(_) -> None:
  ...
  for m, n, k in problem_shapes:
    ...
    tuning_it = itertools.product(
        ...
        (2, 3, 4, 6, 8),  # max_concurrent_steps
    ...
    for (
      ...
      try:
        ...
        if (
          ...
          continue
        raise
      np.testing.assert_allclose(out, expected, rtol=5e-3)
