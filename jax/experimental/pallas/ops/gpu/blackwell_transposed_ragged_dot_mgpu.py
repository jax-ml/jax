...
def ragged_dot_kernel(a, b, group_sizes, config: TuningConfig):
  ...
  def kernel(a_gmem, b_gmem, group_sizes_gmem, out_gmem):
    ...
    @functools.partial(pl.run_scoped,
        a_smem=plgpu.SMEM(
            (max_concurrent_steps, tile_k, block_tile_m),
...
def ragged_dot_reference(a, b, g):
  return lax.ragged_dot(jnp.transpose(a, (1, 0)),
                        b, g, preferred_element_type=jnp.float16)
...
def main(_) -> None:
  ...
  a = jax.random.uniform(jax.random.key(1), (K, M), jnp.float16)
