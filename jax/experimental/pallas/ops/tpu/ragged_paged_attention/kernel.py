...
def ragged_paged_attention_kernel(
  ...
  def strided_load_kv(ref, start, step):
    ...
    # NOTE: bf16 is handled differently from other dtypes because the current
    # implementation is faster in v5e.
    if ref.dtype == jnp.bfloat16:
      ...
      if packing == 2:
        mask = jnp.uint32(0x0000FFFF)
        bitcast_dst_dtype = jnp.uint16
      elif packing == 4:
        mask = jnp.uint32(0x000000FF)
        bitcast_dst_dtype = jnp.uint8
      else:  # packing == 8:
        mask = jnp.uint32(0x0000000F)
        bitcast_dst_dtype = jnp.uint4

      bitwidth = 32 // packing
      for i in range(0, packing, 2):
        bk = (b >> i * bitwidth) & mask
        k = pltpu.bitcast(bk.astype(bitcast_dst_dtype), ref.dtype)
        k_list.append(k)
        bv = (b >> ((i + 1) * bitwidth)) & mask
