...
def _mgpu_async_load_op_lowering_rule(
  ...
  with utils.single_thread(predicate=ctx.single_thread_per_block_predicate):
...
def InstantiateSingleThreadPredicate(
    module: ir.Module
) -> LoweringContext:
  predicate = None
  ...
  if predicate is None:
    raise ValueError(
        "No suitable function found to instantiate the single thread predicate."
    )
...
def lower_mgpu_dialect(
  ...
  ctx = LoweringContext(
      ...
      else InstantiateSingleThreadPredicate(module),
  )
