...
def _mgpu_slice_smem_op_lowering_rule(
  ...
  transformed_ty = transform_type(ir.MemRefType(op.result.type), transforms)
  memory_space = utils.WORKGROUP_NVPTX_ADDRESS_SPACE
  ptr = utils.memref_ptr(sliced_ref, memory_space=memory_space)
  transformed_ref = utils.ptr_as_memref(
      ptr, transformed_ty, ptr_memory_space=memory_space
  )
