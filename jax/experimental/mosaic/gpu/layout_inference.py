...
def _in_layout_for_operand(
  ...
  return inference_utils.in_layouts(op)[operand_number]
...
def _value_layout(value: ir.Value) -> ir.Attribute | None:
  ...
  if isinstance(owner, ir.Operation):
    if not inference_utils.has_out_layouts_set(owner):
      return None
    value_result_number = [
        r for r in owner.results if ir.VectorType.isinstance(r.type)
    ].index(value)
    return inference_utils.out_layouts(owner)[value_result_number]
  ...
  if isinstance(owner, ir.Block):
    ...
    if not inference_utils.has_in_layouts_set(owner_op):
      return None
    value_arg_number = [
        r for r in block.arguments if ir.VectorType.isinstance(r.type)
    ].index(value)
    return inference_utils.in_layouts(owner_op)[value_arg_number]
...
def _earliest_use(
    regions: list[ir.Region], uses: Sequence[ir.OpOperand]
) -> ir.OpView:
  ...
  for region in regions:
    for block in region:
      for op in block:
        if op in owners:
          return op
  raise ValueError(f"None of uses are in the given block")
...
def _insert_memref_layout_cast(layout: ir.Attribute, view_op: memref.ViewOp):
  ...
  uses = [use for use in view_op.result.uses]
