...
def find_assignments_for(
  ...
  while True:
    ...
    # Remove the hints that pertain to variables that already have an
    # assignment.
    hints = [h for h in hints if h.variable not in equation_system.assignments]
...
def producer_variable(variable: Variable) -> Variable:
  ...
  raise ValueError(f"Producer {producer} is not an operation nor a block.")


def consumer_variables(variable: Variable) -> list[Variable]:
...
def equation_system_and_hints_for_op(
  ...
  for variable in all_variables:
    ...
    #
    # Even though lookup in a list is linear, the performance impact may be
    # positive (since the list is expected to be small), and we care about
    # having a deterministic iteration order.
    ...
    for v in filter(lambda v: v.is_result, union):
