...
def speculatively_reduce_least_replicated_hint_expression(
    expressions: tuple[eqns.Expression, ...],
) -> eqns.Expression | None:
  assert len(expressions) >= 1
  choices: list[eqns.ConstantExpression] = []
  for e in expressions:
    if (red := speculatively_reduce_expression(e)) is not None:
      choices.append(red)

  if not choices:
    return None

  maybe_choice = eqns.simplify_expression(
...
def speculatively_reduce_most_replicated_hint_expression(
    expressions: tuple[eqns.Expression, ...],
) -> eqns.Expression | None:
  assert len(expressions) >= 1
  choices: list[eqns.ConstantExpression] = []
...
def speculatively_reduce_expression(
    e: eqns.Expression
) -> eqns.Expression | None:
  """Attempts to reduce an expression to a possible `ConstantExpression`.

  The mechanism used here is simple: we recursively gather all the
  `ConstantExpression`s that we have within the expression, and attempt to
  reduce the expression to a single `ConstantExpression` by ignoring all the
  unknown variables.

  If the result of the reduction is `Unsatisfiable`, we return `None`.

  This is not a safe reduction function. It is only intended for
  use in guiding the search for possible assignments.
  """
  match e:
    case eqns.ConstantExpression():
      return e
    case eqns.LeastReplicatedExpression():
      return speculatively_reduce_least_replicated_hint_expression(e.expressions)
    case eqns.MostReplicatedExpression():
      return speculatively_reduce_most_replicated_hint_expression(e.expressions)
    case eqns.Variable():
      return None
    case _:
...
def extract_variable_assignment_from_hint(
  ...
  red = speculatively_reduce_expression(hint.expression)
...
def _optimization_barrier_equation_system(
  ...
  for i, operand in enumerate(op.operands):
    if not is_vector(operand):
      continue
    variable = eqns.Variable(VariableKey(op, VariableType.OPERAND, i))
    keys_for_variable[variable] = [
        VariableKey(op, VariableType.OPERAND, i),
        VariableKey(op, VariableType.RESULT, i)
    ]

  return eqns.EquationSystem(), keys_for_variable
