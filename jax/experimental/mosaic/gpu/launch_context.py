...
class LaunchContext:
    ...
    if (tma_desc := self.tma_descriptors.get(tma_desc_key, None)) is None:
      ...
      def init_tma_desc(host_ptr):
        ...
        # We treat narrow floats as integers
          ...
          tma_dtype = 1
        elif isinstance(ref_ty.element_type, ir.Float8E8M0FNUType):
          tma_dtype = 1
        elif isinstance(ref_ty.element_type, ir.Float4E2M1FNType):
          tma_dtype = 0
        else:
  ...
  def async_copy(
    ...
    if reduction_op is not None:
      ...
      if not any(
          isinstance(gmem_ref_ty.element_type, t)
    ...
    if any(is_gathered_dim):
      ...
      if not isinstance(idx_dtype, ir.IntegerType) or utils.bitwidth(idx_dtype) > 32:
    ...
    if implementation == AsyncCopyImplementation.CP_ASYNC:
      ...
      if element_bitwidth < 8:
        gep_type = i8
      elif isinstance(element_type, ir.FloatType) and ir.FloatType(element_type).width == 8:
