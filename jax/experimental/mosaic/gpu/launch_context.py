...
def _is_tma_reduction_op_supported(
    reduction_op: TMAReductionOp, dtype: ir.Type
) -> bool:
  ...
  with a few small deviations:
  - we allow add for s64 types and treat them as u64.
  - we allow dec and inc for s32 types and treat them as u32.
  - we don't allow and, or, or xor for floating point types.
  """
...
def _tma_dma_type(
    element_type: ir.Type, reduction_op: TMAReductionOp | None
) -> int:
  """Returns the TMA DMA type for the given element type and reduction op."""
  if isinstance(element_type, ir.IntegerType):
    ...
    if bitwidth == 2:
      ...
      if reduction_op == "smax" or reduction_op == "smin":
        tma_dtype = 9
      else:
        tma_dtype = 3
    elif bitwidth == 64:
