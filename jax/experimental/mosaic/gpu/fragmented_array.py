...
class Tiling:
  ...
  def untile_shape(self, shape: tuple[int, ...]) -> tuple[int, ...]:
    ...
    def fail():
      raise ValueError(
          f"shape {shape} is not a valid result of applying the given tiling"
          f" {self}"
      )
  ...
  def transfer_tiled2(
    ...
    * the current address for load/store instructions
    ...
    if ref_tiling.untile_shape(tuple(ref_ty.shape)) != shape:
      del ref_ty
      # We will try to fill as many tiles as we can.
      if len(ref_tiling_shape) != len(shape):
        raise ValueError

      prefix_slice, suffix_slice = zip(*[
          (slice(math.ceil(d / td)), slice(d % td or td))
          for td, d in zip(ref_tiling_shape, shape)
      ])
      ref = utils.memref_slice(ref, prefix_slice + suffix_slice)
      ref_ty = ir.MemRefType(ref.type)
      ref_tiling_shape = tuple(ref_ty.shape[ref_ty.rank // 2:])
      ref_tiling = Tiling((ref_tiling_shape,))
      assert ref_tiling.untile_shape(tuple(ref_ty.shape)) == shape
