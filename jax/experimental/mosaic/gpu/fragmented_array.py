...
from typing import Iterable, Protocol, Sequence, TypeVar, Iterator
...
class TiledLayout:
  ...
  def thread_idxs(self, shape) -> Iterator[tuple[ir.Value, ...]]:
    # We first find the linear index and then divide by the shape to
    # get the index.
    i32 = ir.IntegerType.get_signless(32)
    index = ir.IndexType.get()
    strides = utils.get_contiguous_strides(shape)
    tile_strides = self.tiling.tile_strides(strides)
    dyn_strides = [c(s, i32) for s in tile_strides]
    warp_offset = utils.dyn_dot(self.warp_indices(), dyn_strides)
    lane_offset = utils.dyn_dot(self.lane_indices(), dyn_strides)
    ...
    for tile_idx in np.ndindex(register_shape):
      ...
      idx = []
      for dim in strides:
        idx.append(arith.index_castui(index, arith.divui(dyn_lin_idx, c(dim, i32))))
        dyn_lin_idx = arith.remui(dyn_lin_idx, c(dim, i32))
      yield tuple(idx)
...
class FragmentedArray:
  ...
  def reduce(self, op: str | Callable[[ir.Value, ir.Value], ir.Value], axis):
    ...
    index = ir.IndexType.get()
    i32 = ir.IntegerType.get_signless(32)
    # TODO(cperivol): This leaks information about the WGMMA_LAYOUT
    # tiled layout. Find a better way.
  ...
  def _store_untiled_tiled(self, ref: ir.Value):
    ...
    memory_space = None
    if ref_space:
      assert ref_space == smem_space, ref_space
      memory_space = 3
