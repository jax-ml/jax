...
def tmem_load(tmem_addr, shape, num):
  ...
  if num * num_out_regs >= 256:
    raise ValueError("Loading too much TMEM at once")
  num_out_regs *= num
...
class TMEMLayout:
  ...
  def __post_init__(self):
    assert 32 <= self.elements_in_tile[0] <= 128
    assert self.elements_in_tile[0].bit_count() == 1
...
class TMEMRef:
  ...
  def slice(self, *idxs):
    ...
    # TODO: DO NOT SUBMIT: Make sure this does not slice below tiling!!
    return TMEMRef(
