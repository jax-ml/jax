...
class TMEMRef:
  ...
  def load(self, layout: fa.TiledLayout | None = None, is_signed: bool | None = None):
    ...
    if layout == LAYOUT and lane_tiling == 1:
    ...
    elif layout == fa.WGMMA_LAYOUT and lane_tiling == 2:
      regs_shape = layout.registers_shape(self.shape)
      # Load half the columns, since they are folded over lanes.
      raw_registers = _load_32xcols(
          self.address, self.shape[1] // lane_tiling, self.dtype, packing
      )
    ...
    elif layout == TMEM_NATIVE_LAYOUT and lane_tiling == 1:
      regs_shape = layout.registers_shape(self.shape)
