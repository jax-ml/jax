...
def _infer_vector_load_store_transforms(
  ...
  value_transforms = inference_utils.value_transforms(op.base)

  if layout == fa.WGMMA_LAYOUT:
    layout_transforms = infer_transforms_for_wgmma_ref(
        ir.MemRefType(op.base.type)
    )
  elif isinstance(layout, fa.TiledLayout):
    raise NotImplementedError(
        f"Non-WGMMA tiled layouts are not supported yet, but got {layout}"
    )
  else:
    layout_transforms = None

  if value_transforms is not None and layout_transforms is not None:
    if value_transforms != layout_transforms:
      raise ValueError(
          f"Conflicting transforms for {op.base} in {op}: "
          f"{value_transforms} != {layout_transforms}."
      )
...
def _infer_memref_view_transforms(op: memref.ViewOp) -> OptionalTransforms:
  ...
  # TODO(bchetioui): handle offsets, etc...
