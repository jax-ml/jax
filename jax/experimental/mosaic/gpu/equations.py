...
class EquationSystem:
  """An equation system contains a set of equations, unknowns, and assignments.

  Assignments assign constant values to variables in the system. Unknowns denote
  variables whose value is unknown. Equations describe relationships between
  variables, and can be used to determine assignments for unknown variables.
  """
  ...
  def __post_init__(self):
    ...
    self.equations = {reduce_equation(e, self.assignments) for e in self.equations}

  def __or__(self, other: EquationSystem) -> EquationSystem | Unsatisfiable:
...
def _simplify_system_once(
  ...
  for equation in equation_system.equations:
    simplified_equation = reduce_equation(equation, equation_system.assignments)
