...
def simplify_replicated_expression(
    ...
    reducer: Callable[[fa.FragmentedLayout, fa.FragmentedLayout], fa.FragmentedLayout]
  ...
  for expr in expr.expressions:
    ...
    if isinstance(simplified_expr, Unsatisfiable):
  ...
  match expr:
    ...
    case MostReplicatedExpression():
      return simplify_replicated_expression(
          expr, assignments, MostReplicatedExpression, layouts_lib.join_layouts
      )
    case LeastReplicatedExpression():
      return simplify_replicated_expression(
          expr, assignments, LeastReplicatedExpression, layouts_lib.meet_layouts
      )
    case _:
      assert_never(expr)
