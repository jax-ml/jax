...
def merge_tiled_constraints(
  ...
  for constraint in constraints:
    match constraint:
      case Tiled(expr=expr, num_tiled_dims=num_tiled_dims) if isinstance(
          expr, Variable
      ):
        prev = var_to_const.get(expr)
        if prev is None:
          var_to_const[expr] = constraint
        else:
          if prev.num_tiled_dims != num_tiled_dims:
            return Unsatisfiable()
          var_to_const[expr] = Tiled(expr, num_tiled_dims)
      case _:
        result.append(constraint)
