...
def uniform(key, shape=(), dtype=onp.float64, minval=0.0, maxval=1.0):
...
def _uniform(key, shape, dtype, minval, maxval):
    ...
    float_bits = lax.bitwise_or(
        ...
        onp.array(1.0, dtype).view(onp.uint32 if nbits == 32 else onp.uint64),
    )
    floats = lax.bitcast_convert_type(float_bits, dtype) - onp.array(1.0, dtype)
...
def _normal(key, shape, dtype):
    ...
    lo = onp.nextafter(onp.array(-1.0, dtype), 0.0, dtype=dtype)
    hi = onp.array(1.0, dtype)
...
def _cauchy(key, shape, dtype):
    ...
    u = uniform(key, shape, dtype, minval=onp.finfo(dtype).eps, maxval=1.0)
...
def _gamma_one(key, alpha):
    ...
    one_over_three = _constant_like(alpha, 1.0 / 3.0)
    ...
    _, _, V, _ = lax.while_loop(
        _cond_fn, _body_fn, (key, zero, one, _constant_like(alpha, 2))
    )
...
def _gamma_grad_one(z, alpha):
    ...
    def _case4(zagf):
        ...
        for i in range(8):
            c.append(
                _bivariate_coef[0][i]
                + u * (_bivariate_coef[1][i] + u * _bivariate_coef[2][i])
            )
    ...
    _, _, grad, flag = lax.while_loop(
        lambda zagf: (~zagf[3]) & (zagf[0] < 0.8), _case1, (z, alpha, 0.0, False)
    )
    _, _, grad, flag = lax.while_loop(_cond2, _case2, (z, alpha, grad, flag))
    _, _, grad, flag = lax.while_loop(_cond3, _case3, (z, alpha, grad, flag))
    _, _, grad, flag = lax.while_loop(
        lambda zagf: ~zagf[3], _case4, (z, alpha, grad, flag)
    )
    return grad
...
def _gumbel(key, shape, dtype):
    ...
    return -np.log(
        -np.log(uniform(key, shape, dtype, minval=onp.finfo(dtype).eps, maxval=1.0))
    )
...
def _laplace(key, shape, dtype):
    ...
    u = uniform(key, shape, dtype, minval=-1.0 + np.finfo(dtype).epsneg, maxval=1.0)
