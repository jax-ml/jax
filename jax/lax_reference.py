...
def div(lhs, rhs):
    if onp.issubdtype(onp.result_type(lhs), onp.integer):
        ...
        select = onp.logical_and(
            onp.sign(lhs) != onp.sign(rhs), onp.remainder(lhs, rhs) != 0
        )
...
def conv_with_general_padding(
    ...
    return _conv(
        _dilate(lhs, lhs_dilation), _dilate(rhs, rhs_dilation), window_strides, padding
    )
...
def broadcast_in_dim(operand, shape, broadcast_dimensions):
    inshape = tuple(
        1 if i not in broadcast_dimensions else d for i, d in enumerate(shape)
    )
...
def pad(operand, padding_value, padding_config):
    ...
    rhs_slices = tuple(
        _slice(l if l < 0 else 0, -h if h < 0 else None) for l, h in zip(lo, hi)
    )
...
def rev(operand, dimensions):
    ...
    indexer = (
        _slice(None, None, -1) if d in dimensions else _slice(None)
...
def slice(
    operand, start_indices, limit_indices, strides=None
...
def dynamic_slice(operand, start_indices, slice_sizes):
    ...
    idx = tuple(
        _slice(start, start + size) for start, size in zip(start_indices, slice_sizes)
    )
    section = operand[idx]
...
def reduce(
    operand, init_value, computation, dimensions
):  # pylint: disable=redefined-builtin
...
def reduce_window(
    ...
    view = _conv_view(
        op.reshape((1, 1) + op.shape),
        (1, 1) + dims,
        strides,
        pads,
        pad_value=init_value,
    )[0]
...
def _conv(lhs, rhs, window_strides, pads):
    view, view_axes, rhs_axes, out_axes = _conv_view(
        lhs, rhs.shape, window_strides, pads, 0.0
    )
...
def _conv_view(lhs, rhs_shape, window_strides, pads, pad_value):
    ...
    out_shape = (
        onp.floor_divide(onp.subtract(in_shape, filter_shape), window_strides) + 1
    )
...
def _pad(arr, pads, pad_value):
    out = onp.pad(
        arr, onp.maximum(0, pads), mode="constant", constant_values=pad_value
    ).astype(arr.dtype)
    slices = tuple(
...
def _conv_general_permutations(dimension_numbers):
    ...
    rhs_perm = (rhs_spec.index("O"), rhs_spec.index("I")) + tuple(
        i for i, c in enumerate(rhs_spec) if c not in {"O", "I"}
    )
...
_monoids = {
    ...
    "logical_and": MonoidRecord(
        onp.logical_and.reduce, _identity_getter(onp.logical_and)
    ),
...
def _reducer_from_pyfunc(py_binop, init_val):
    def reducer(operand, axis=0):
        ...
        result = onp.full(
            onp.delete(onp.shape(operand), axis),
            init_val,
