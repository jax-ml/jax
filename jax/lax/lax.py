...
def broadcast_shapes(*shapes):
  ...
  if not onp.all((shapes == result_shape) | (shapes == 1)):
    raise ValueError("Incompatible shapes for broadcasting: {}".format(tuple(map(tuple, shapes))))
...
def _canonicalize_shape(shape):
  ...
  try:
    ...
    pass
  msg = ("Shapes must be 1D sequences of concrete values of integer type, " "got {}")
...
def convert_element_type(operand, new_dtype):
  ...
  if old_dtype != new_dtype:
    if (onp.issubdtype(old_dtype, onp.complexfloating)
        and not onp.issubdtype(new_dtype, onp.complexfloating)):
      msg = "Casting complex values to real discards the imaginary part"
      warnings.warn(msg, onp.ComplexWarning, stacklevel=2)
      operand = real(operand)
      old_dtype = _dtype(operand)
    return convert_element_type_p.bind(operand, new_dtype=new_dtype, old_dtype=old_dtype)
  else:
    return operand
...
def conv_general_dilated(lhs, rhs, window_strides, padding, lhs_dilation=None, rhs_dilation=None,
                         dimension_numbers=None, feature_group_count=1, precision=None):
  ...
  if type(dimension_numbers) is not ConvDimensionNumbers:
    dimension_numbers = conv_dimension_numbers(lhs.shape, rhs.shape, dimension_numbers)
  ...
  return conv_general_dilated_p.bind(
      ...
      lhs_shape=lhs.shape, rhs_shape=rhs.shape, precision=_canonicalize_precision(precision))
...
def dot(lhs, rhs, precision=None):
  ...
  if 1 <= lhs.ndim <= 2 and 1 <= rhs.ndim <= 2 and lhs.shape[-1] == rhs.shape[0]:
    return dot_general(lhs, rhs, (((lhs.ndim - 1,), (0,)), ((), ())), precision=precision)
  else:
    raise TypeError("Incompatible shapes for dot: got {} and {}.".format(lhs.shape, rhs.shape))
...
def dot_general(lhs, rhs, dimension_numbers, precision=None):
  ...
  if onp.issubdtype(lhs.dtype, onp.integer):
    ...
    lhs_noncontract_dims = tuple(
        sorted(set(range(onp.ndim(lhs))) - set(lhs_batch_dims) - set(lhs_contract_dims)))
    rhs_noncontract_dims = tuple(
        sorted(set(range(onp.ndim(rhs))) - set(rhs_batch_dims) - set(rhs_contract_dims)))
    lhs = transpose(lhs, lhs_batch_dims + lhs_noncontract_dims + lhs_contract_dims)
    rhs = transpose(rhs, rhs_batch_dims + rhs_noncontract_dims + rhs_contract_dims)
    new_lhs_shape = onp.insert(
        onp.shape(lhs),
        len(lhs_batch_dims) + len(lhs_noncontract_dims), (1,) * len(rhs_noncontract_dims))
    new_rhs_shape = onp.insert(
        onp.shape(rhs), len(lhs_batch_dims), (1,) * len(lhs_noncontract_dims))
    lhs = reshape(lhs, new_lhs_shape)
    rhs = reshape(rhs, new_rhs_shape)
    out_ndim = (len(lhs_batch_dims) + len(lhs_noncontract_dims) + len(rhs_noncontract_dims))
    return reduce(
        mul(lhs, rhs), _zero(lhs), add, tuple(range(out_ndim, out_ndim + len(lhs_contract_dims))))

  return dot_general_p.bind(lhs, rhs, dimension_numbers=(contract_dims, batch_dims),
...
def broadcast_in_dim(operand, shape, broadcast_dimensions):
  ...
  if any(x < 0 or x >= len(shape) for x in broadcast_dimensions):
    msg = ("broadcast dimensions must be >= 0 and < ndim(shape), got {} for " "shape {}")
    raise ValueError(msg.format(broadcast_dimensions, shape))
  return broadcast_in_dim_p.bind(operand, shape=tuple(shape),
...
def slice(operand, start_indices, limit_indices, strides=None):
  ...
  operator.
  """
  if (onp.all(onp.equal(start_indices, 0)) and onp.all(onp.equal(limit_indices, operand.shape))
      and strides is None):
    return operand
  else:
...
def dynamic_slice(operand, start_indices, slice_sizes):
  ...
  return dynamic_slice_p.bind(operand, *start_indices, slice_sizes=tuple(slice_sizes),
                              operand_shape=operand.shape)
...
def dynamic_update_slice(operand, update, start_indices):
  ...
  """
  start_indices = _dynamic_slice_indices(operand, start_indices)
  return dynamic_update_slice_p.bind(operand, update, *start_indices, update_shape=update.shape)
...
def scatter_add(operand, scatter_indices, updates, dimension_numbers):
  ...
  return scatter_add_p.bind(operand, scatter_indices, updates, update_jaxpr=jaxpr,
...
def scatter_min(operand, scatter_indices, updates, dimension_numbers):
  ...
  return scatter_min_p.bind(operand, scatter_indices, updates, update_jaxpr=jaxpr,
                            update_consts=consts, dimension_numbers=dimension_numbers,
                            updates_shape=updates.shape)
...
def scatter_max(operand, scatter_indices, updates, dimension_numbers):
  ...
  return scatter_max_p.bind(operand, scatter_indices, updates, update_jaxpr=jaxpr,
                            update_consts=consts, dimension_numbers=dimension_numbers,
                            updates_shape=updates.shape)
...
def scatter(operand, scatter_indices, updates, dimension_numbers):
  ...
  Returns:
    An array containing the sum of `operand` and the scattered updates.
  """
  jaxpr, consts = _reduction_jaxpr(_scatter_reduction_computation, _abstractify(_const(operand, 0)))
  return scatter_p.bind(operand, scatter_indices, updates, update_jaxpr=jaxpr, update_consts=consts,
                        dimension_numbers=dimension_numbers, updates_shape=updates.shape)
...
def index_take(src, idxs, axes):
  ...
  dnums = GatherDimensionNumbers(offset_dims=offset_dims, collapsed_slice_dims=axes,
...
def reduce(operand, init_value, computation, dimensions):
  ...
  if monoid_reducer:
    ...
    return reduce_p.bind(operand, init_value, computation=computation, jaxpr=jaxpr, consts=consts,
...
def _reduce_sum(operand, axes):
  return reduce_sum_p.bind(operand, axes=tuple(axes), input_shape=onp.shape(operand))
...
def reduce_window(operand, init_value, computation, window_dimensions, window_strides, padding):
  ...
  if monoid_reducer:
    ...
    jaxpr, consts = _reduction_jaxpr(computation, _abstractify(init_value))
    return reduce_window_p.bind(operand, init_value, jaxpr=jaxpr, consts=consts,
...
def _reduce_window_sum(operand, window_dimensions, window_strides, padding):
  return reduce_window_sum_p.bind(operand, window_dimensions=tuple(window_dimensions),
                                  window_strides=tuple(window_strides), padding=padding,
...
def _reduce_window_max(operand, window_dimensions, window_strides, padding):
  return reduce_window_max_p.bind(operand, window_dimensions=tuple(window_dimensions),
                                  window_strides=tuple(window_strides), padding=padding)

def _reduce_window_min(operand, window_dimensions, window_strides, padding):
  return reduce_window_min_p.bind(operand, window_dimensions=tuple(window_dimensions),
                                  window_strides=tuple(window_strides), padding=padding)

def _select_and_scatter(operand, select, window_dimensions, window_strides, padding, source,
                        init_value, scatter):
  ...
  return select_and_scatter_p.bind(operand, source, init_value, select_jaxpr=select_jaxpr,
                                   select_consts=select_consts, scatter_jaxpr=scatter_jaxpr,
                                   scatter_consts=scatter_consts,
                                   window_dimensions=tuple(window_dimensions),
                                   window_strides=tuple(window_strides), padding=padding)

def _select_and_scatter_add(source, operand, select_prim, window_dimensions, window_strides,
                            padding):
  return select_and_scatter_add_p.bind(source, operand, select_prim=select_prim,
                                       window_dimensions=tuple(window_dimensions),
                                       window_strides=tuple(window_strides), padding=padding)

def _select_and_gather_add(tangents, operand, select_prim, window_dimensions, window_strides,
                           padding):
  return select_and_gather_add_p.bind(tangents, operand, select_prim=select_prim,
                                      window_dimensions=tuple(window_dimensions),
                                      window_strides=tuple(window_strides), padding=padding)
...
def conv(lhs, rhs, window_strides, padding, precision=None):
  ...
  return conv_general_dilated(lhs, rhs, window_strides, padding, precision=precision)
...
def conv_with_general_padding(lhs, rhs, window_strides, padding, lhs_dilation, rhs_dilation,
  ...
  Returns:
    An array containing the convolution result.
  """
  return conv_general_dilated(lhs, rhs, window_strides, padding, lhs_dilation=lhs_dilation,
...
def conv_transpose(lhs, rhs, strides, padding, dimension_numbers=None, transpose_kernel=False,
                   precision=None):
  ...
  if padding in {'SAME', 'VALID'}:
    pads = [_conv_transpose_padding(k, s, padding) for k, s in zip(k_sdims.tolist(), strides)]
  else:
    pads = padding
  if transpose_kernel:
  ...
  return conv_general_dilated(lhs, rhs, one, pads, strides, one, dn, precision=precision)
...
def batch_matmul(lhs, rhs):
  ...
  if _min(lhs.ndim, rhs.ndim) < 2:
    raise ValueError('Arguments to batch_matmul must be at least 2D, got {}, {}'.format(
        lhs.ndim, rhs.ndim))
  if lhs.ndim != rhs.ndim:
    raise ValueError('Arguments to batch_matmul must have same ndim, got {}, {}'.format(
        lhs.ndim, rhs.ndim))
...
def asin(x):
  ...
  return mul(_const(x, 2), atan2(x, add(_const(x, 1), sqrt(sub(_const(x, 1), square(x))))))
...
def acos(x):
  ...
  return select(
      ne(x, _const(x, -1.0)),
      mul(_const(x, 2), atan2(sqrt(sub(_const(x, 1), square(x))), add(_const(x, 1), x))),
      full_like(x, onp.pi))
...
def _upcast_fp16_for_computation(f):
  ...
  def f_wrapped(x):
    ...
    if dtype == onp.float16:
      return convert_element_type(f(convert_element_type(x, onp.float32)), dtype)
    return f(x)
...
def standard_abstract_eval(shape_rule, dtype_rule, *args, **kwargs):
  ...
  least_specialized = _max(map(type, args), key=operator.attrgetter('array_abstraction_level'))
...
def binop(result_dtype, accepted_dtypes, name, translation_rule=None):
  ...
  prim = standard_primitive(shape_rule, dtype_rule, name, translation_rule=translation_rule)
...
ad.defjvp(atan2_p, lambda g, x, y: _brcast(g, y) * (y / (square(x) + square(y))),
...
ad.defjvp(erf_p, lambda g, x: mul(_const(x, 2. / onp.sqrt(onp.pi)), mul(g, exp(neg(square(x))))))

erfc_p = standard_unop(_float, 'erfc')
ad.defjvp(erfc_p,
          lambda g, x: mul(_const(x, 2. / onp.sqrt(onp.pi)), mul(neg(g), exp(neg(square(x))))))

erf_inv_p = standard_unop(_float, 'erf_inv')
ad.defjvp2(erf_inv_p,
           lambda g, ans, x: mul(_const(x,
                                        onp.sqrt(onp.pi) / 2.), mul(g, exp(square(ans)))))
...
complex_p = binop(_complex_dtype, [_complex_elem_types, _complex_elem_types], 'complex')
...
def _abs_jvp_rule(g, ans, x):
  if _iscomplex(x):
    return _maybe_real(
        mul(g, div(_maybe_conj(x), _replace_zero(convert_element_type(ans, _dtype(x))))))
  else:
...
ad.defjvp2(rsqrt_p, lambda g, ans, x: _safe_mul(g, mul(_const(x, -0.5), pow(x, _const(x, -1.5)))))
...
ad.defjvp(sub_p, lambda g, x, y: _brcast(g, y), lambda g, x, y: _brcast(neg(g), x))
...
def _safe_mul_translation_rule(c, x, y):
  ...
  out_shape = broadcast_shapes(c.GetShape(x).dimensions(), c.GetShape(y).dimensions())
  return c.Select(c.Or(c.Eq(x, zero), c.Eq(y, zero)), c.Broadcast(zero, out_shape), c.Mul(x, y))

safe_mul_p = standard_binop([_num, _num], 'safe_mul', translation_rule=_safe_mul_translation_rule)
...
def _broadcasting_select(c, which, x, y):
  ...
  which_shape, x_shape, y_shape = (c.GetShape(t).dimensions() for t in (which, x, y))
  out_shape = broadcast_shapes(which_shape, x_shape, y_shape)
  bcast_dims = lambda shape: tuple(range(len(out_shape) - len(shape), len(out_shape)))
...
def _minmax_translation_rule(c, x, y, minmax=None, cmp=None):
  ...
  if onp.issubdtype(dtype, onp.complexfloating):
    ...
    return _broadcasting_select(
        c, c.Select(c.Eq(rx, ry), comparator(c.Imag(x), c.Imag(y)), comparator(rx, ry)), x, y)
  return minmax(c)(x, y)

max_p = standard_binop([_any, _any], 'max',
                       translation_rule=partial(_minmax_translation_rule, minmax=lambda c: c.Max,
                                                cmp=lambda c: c.Gt))
ad.defjvp2(max_p, lambda g, ans, x, y: mul(_brcast(g, y), _balanced_eq(x, ans, y)),
           lambda g, ans, x, y: mul(_brcast(g, x), _balanced_eq(y, ans, x)))

min_p = standard_binop([_any, _any], 'min',
                       translation_rule=partial(_minmax_translation_rule, minmax=lambda c: c.Min,
                                                cmp=lambda c: c.Lt))
ad.defjvp2(min_p, lambda g, ans, x, y: mul(_brcast(g, y), _balanced_eq(x, ans, y)),
           lambda g, ans, x, y: mul(_brcast(g, x), _balanced_eq(y, ans, x)))
...
convert_element_type_p = standard_primitive(_convert_element_type_shape_rule,
                                            _convert_element_type_dtype_rule,
                                            'convert_element_type',
                                            _convert_element_type_translation_rule)
ad.deflinear(convert_element_type_p,
...
bitcast_convert_type_p = standard_primitive(_bitcast_convert_type_shape_rule,
                                            _bitcast_convert_type_dtype_rule,
                                            'bitcast_convert_type',
                                            _bitcast_convert_type_translation_rule)
...
def _conv_general_dilated_shape_rule(lhs, rhs, window_strides, padding, lhs_dilation, rhs_dilation,
                                     dimension_numbers, feature_group_count, **unused_kwargs):
  assert type(dimension_numbers) is ConvDimensionNumbers
  if not feature_group_count > 0:
    msg = ("conv_general_dilated feature_group_count " "must be a positive integer, got {}.")
  ...
  if rhs.shape[dimension_numbers.rhs_spec[0]] % feature_group_count:
    ...
    raise ValueError(msg.format(rhs.shape[dimension_numbers.rhs_spec[0]], feature_group_count))
...
def _conv_general_dilated_dtype_rule(lhs, rhs, window_strides, padding, lhs_dilation, rhs_dilation,
                                     dimension_numbers, **unused_kwargs):
  return binop_dtype_rule(_input_dtype, [_float, _float], 'conv_general_dilated', lhs, rhs)

_conv_spec_transpose = lambda spec: (spec[1], spec[0]) + spec[2:]
_conv_sdims = lambda spec: spec[2:]

def _conv_general_dilated_transpose_lhs(g, rhs, window_strides, padding, lhs_dilation, rhs_dilation,
  ...
  padding = _conv_general_vjp_lhs_padding(
      onp.take(lhs_shape, lhs_sdims), onp.take(rhs_shape, rhs_sdims), window_strides,
      onp.take(g.shape, out_sdims), padding, lhs_dilation, rhs_dilation)
  revd_weights = rev(rhs, rhs_sdims)
  return conv_general_dilated(g, revd_weights, window_strides=lhs_dilation, padding=padding,
                              lhs_dilation=window_strides, rhs_dilation=rhs_dilation,
                              dimension_numbers=trans_dimension_numbers,
                              feature_group_count=feature_group_count, precision=precision)

def _conv_general_dilated_transpose_rhs(g, lhs, window_strides, padding, lhs_dilation, rhs_dilation,
                                        dimension_numbers, feature_group_count, lhs_shape,
                                        rhs_shape, precision):
  assert type(dimension_numbers) is ConvDimensionNumbers
  ...
  padding = _conv_general_vjp_rhs_padding(
      onp.take(lhs_shape, lhs_sdims), onp.take(rhs_shape, rhs_sdims), window_strides,
      onp.take(g.shape, out_sdims), padding, lhs_dilation, rhs_dilation)
  return conv_general_dilated(lhs, g, window_strides=rhs_dilation, padding=padding,
                              lhs_dilation=lhs_dilation, rhs_dilation=window_strides,
                              dimension_numbers=trans_dimension_numbers,
                              feature_group_count=feature_group_count, precision=precision)

def _conv_general_dilated_translation_rule(c, lhs, rhs, window_strides, padding, lhs_dilation,
                                           rhs_dilation, dimension_numbers, feature_group_count,
                                           precision, **unused_kwargs):
  assert type(dimension_numbers) is ConvDimensionNumbers
  dimension_numbers = _conv_general_proto(dimension_numbers)
  return c.ConvGeneralDilated(lhs, rhs, window_strides, padding, lhs_dilation, rhs_dilation,
                              dimension_numbers, feature_group_count,
                              precision_config=_precision_config(precision))

def _conv_general_dilated_batch_rule(batched_args, batch_dims, window_strides, padding,
  ...
  if lhs_bdim is not None and rhs_bdim is not None:
    ...
    out = conv_general_dilated(new_lhs, new_rhs, window_strides, padding, lhs_dilation,
    ...
    out = conv_general_dilated(new_lhs, rhs, window_strides, padding, lhs_dilation, rhs_dilation,
                               dimension_numbers, feature_group_count, precision=precision)
    ...
    if feature_group_count == 1:
      new_rhs = _reshape_axis_into(rhs_bdim, rhs_spec[0], rhs)
      out = conv_general_dilated(lhs, new_rhs, window_strides, padding, lhs_dilation, rhs_dilation,
                                 dimension_numbers, feature_group_count, precision=precision)
      ...
      new_rhs = _reshape_axis_into(rhs_bdim + int(rhs_spec[0] < rhs_bdim), rhs_spec[0] + 1, new_rhs)
...
conv_general_dilated_p = standard_primitive(_conv_general_dilated_shape_rule,
...
def _dot_general_shape_rule(lhs, rhs, dimension_numbers, precision):
  ...
  if not onp.all(onp.equal(lhs_batch, rhs_batch)):
    msg = ("dot_general requires same lhs and rhs batch dimension numbers, " "got {} and {}.")
    raise TypeError(msg.format(lhs_batch, rhs_batch))
  ...
  if not onp.all(onp.equal(lhs_contracting_shape, rhs_contracting_shape)):
    msg = ("dot_general requires contracting dimensions to have the same " "shape, got {} and {}.")
...
def _dot_general_transpose_lhs(g, y, dimension_numbers, precision, swap_ans=False):
  ...
  return transpose(dot_general(g, y, dims, precision=precision), tuple(out_axes))

def _dot_general_transpose_rhs(g, x, dimension_numbers, precision):
  (x_contract, y_contract), (x_batch, y_batch) = dimension_numbers
  swapped_dimension_numbers = ((y_contract, x_contract), (y_batch, x_batch))
  return _dot_general_transpose_lhs(g, x, swapped_dimension_numbers, precision, swap_ans=True)

def _dot_general_batch_rule(batched_args, batch_dims, dimension_numbers, precision):
  ...
  if lbd is not None and rbd is not None:
    ...
    if lbd is not None:
      ...
      if rhs_batch == () or rbd > onp.max(rhs_batch):
        ...
        result_batch_dim = (
            rbd + (lhs.ndim - len(lhs_contract) - len(lhs_batch)) -
...
def _dot_general_polymorphic_shape_rule(shape_exprs, dimension_numbers, precision):
  ...
  lhs_tensorprod_shape = [
      lhs_shape[d] for d in range(lhs_ndim) if d not in lhs_batch and d not in lhs_contract
  ]
  rhs_tensorprod_shape = [
      rhs_shape[d] for d in range(rhs_ndim) if d not in rhs_batch and d not in rhs_contract
  ]
  return ShapeExpr(lhs_batch_shape + lhs_tensorprod_shape + rhs_tensorprod_shape)

def _dot_general_masking_rule(padded_vals, logical_shapes, dimension_numbers, precision):
  lhs, rhs = padded_vals
  ...
  if len(lhs_contract) == 0:
    ...
    masks = [broadcasted_iota(onp.int32, lhs.shape, d) < lhs_shape[d] for d in lhs_contract]
...
broadcast_p = standard_primitive(_broadcast_shape_rule, _input_dtype, 'broadcast')
...
def _broadcast_in_dim_shape_rule(operand, shape, broadcast_dimensions):
  ...
  _check_shapelike('broadcast_in_dim', 'broadcast_dimensions', broadcast_dimensions)
...
def _broadcast_in_dim_batch_rule(batched_args, batch_dims, shape, broadcast_dimensions):
  operand, = batched_args
  bdim, = batch_dims
...
broadcast_in_dim_p = standard_primitive(_broadcast_in_dim_shape_rule, _input_dtype,
                                        'broadcast_in_dim')
...
_clamp_dtype_rule = partial(binop_dtype_rule, _input_dtype, [_any, _any, _any], 'clamp')

clamp_p = standard_primitive(_clamp_shape_rule, _clamp_dtype_rule, 'clamp')
ad.defjvp(
    clamp_p, lambda g, min, operand, max: select(
        bitwise_and(gt(min, operand), lt(min, max)), _brcast(g, operand), _zeros(operand)),
    lambda g, min, operand, max: select(
        bitwise_and(gt(operand, min), lt(operand, max)), g, _zeros(operand)),
    lambda g, min, operand, max: select(lt(max, operand), _brcast(g, operand), _zeros(operand)))
...
def _concatenate_transpose_rule(t, *operands, **kwargs):
  ...
  if t is ad_util.zero:
    ...
    return [
        slice(t, start, limit) if o is ad.undefined_primal else None
...
def _concatenate_batch_rule(batched_args, batch_dims, dimension, operand_shapes):
  size = next(op.shape[bdim] for op, bdim in zip(batched_args, batch_dims) if bdim is not None)
  operands = [
      batching.moveaxis(op, bdim, 0) if bdim is not None else broadcast(op, (size,))
...
concatenate_p = standard_primitive(_concatenate_shape_rule, _concatenate_dtype_rule, 'concatenate',
...
def _pad_transpose(t, operand, padding_value, padding_config):
  if t is ad_util.zero:
    return [
        ad_util.zero if operand is ad.undefined_primal else None,
...
def _reshape_impl(operand, new_sizes, dimensions, old_sizes):
  if (type(operand) is pxla.ShardedDeviceArray and dimensions is None
    ...
    return pxla.ChunkedDeviceArray(old_sizes[0], aval, operand.device_buffers)
        ...
        and _is_axis_split(old_sizes, new_sizes) and operand.axis_size == new_sizes[0]):
    aval = ShapedArray(new_sizes, operand.dtype)
    return pxla.ShardedDeviceArray(aval, operand.device_buffers)
  else:
    return xla.apply_primitive(reshape_p, operand, new_sizes=new_sizes, dimensions=dimensions,
                               old_sizes=old_sizes)
...
def _reshape_transpose_rule(t, new_sizes, dimensions, old_sizes):
  if dimensions is None:
    ...
    return [transpose(reshape(t, onp.take(old_sizes, dimensions)), onp.argsort(dimensions))]
...
reshape_p = standard_primitive(_reshape_shape_rule, _reshape_dtype_rule, 'reshape',
                               _reshape_translation_rule)
...
ad.deflinear(transpose_p, lambda t, permutation: [transpose(t, onp.argsort(permutation))])
...
def _select_transpose_rule(t, pred, on_true, on_false):
  ...
  if t is ad_util.zero:
    return [
        None, ad_util.zero if on_true is ad.undefined_primal else None,
...
def _select_batch_rule(batched_args, batch_dims, **unused_kwargs):
  ...
  size = next(x.shape[i] for x, i in zip(batched_args, batch_dims) if i is not None)
...
ad.defjvp(select_p, None, lambda g, b, x, y: select(b, g, _zeros(g)),
...
def _slice_shape_rule(operand, start_indices, limit_indices, strides, operand_shape):
  ...
  if not onp.all(onp.greater_equal(start_indices, 0)):
    msg = ("slice start_indices must be greater than or equal to zero, " "got start_indices of {}.")
...
def _slice_translation_rule(c, operand, start_indices, limit_indices, strides, operand_shape):
  return c.Slice(operand, start_indices, limit_indices, strides)

def _slice_transpose_rule(t, start_indices, limit_indices, strides, operand_shape):
  if strides is None or onp.all(onp.equal(strides, 1)):
    pads = zip(start_indices, onp.subtract(operand_shape, limit_indices), (0,) * len(start_indices))
  else:
    real_limits = onp.add(
        onp.add(start_indices, 1), onp.multiply(onp.subtract(t.shape, 1), strides))
    pads = zip(start_indices, onp.subtract(operand_shape, real_limits), onp.subtract(strides, 1))
  result = pad(t, _const(t, 0), pads)
  assert result.shape == operand_shape
  return [result]

def _slice_batching_rule(batched_args, batch_dims, start_indices, limit_indices, strides,
                         **unused_kwargs):
  operand, = batched_args
  bdim, = batch_dims
...
slice_p = standard_primitive(_slice_shape_rule, _input_dtype, 'slice', _slice_translation_rule)
...
def _dynamic_slice_shape_rule(operand, *start_indices, **kwargs):
  ...
  if not onp.all(onp.greater_equal(slice_sizes, 0)):
    msg = ("slice slice_sizes must be greater than or equal to zero, " "got slice_sizes of {}.")
...
def _dynamic_slice_transpose_rule(t, operand, *start_indices, **kwargs):
  ...
  return ([dynamic_update_slice(zeros, t, start_indices)] + [None] * len(start_indices))
...
def _batch_dynamic_slice_indices(indices, bdims):
  ...
  indices = concatenate([
      broadcast_in_dim(x, (size, 1), broadcast_dimensions=((0,) if i is not None else ()))
      for x, i in zip(indices, bdims)
  ], dimension=1)
  return indices, 0

def _dynamic_slice_batching_rule(batched_args, batch_dims, slice_sizes, operand_shape):
  ...
  dnums = GatherDimensionNumbers(offset_dims=dims, collapsed_slice_dims=(), start_index_map=dims)
  index, index_bdim = _batch_dynamic_slice_indices(batched_args[1:], batch_dims[1:])
  return _gather_batching_rule([batched_args[0], index], [batch_dims[0], index_bdim], dnums,
                               slice_sizes, operand_shape)

dynamic_slice_p = standard_primitive(_dynamic_slice_shape_rule, _input_dtype, 'dynamic_slice',
...
def _dynamic_update_slice_transpose_rule(t, operand, update, *start_indices, **kwargs):
...
def _dynamic_update_slice_translation_rule(c, operand, update, *start_indices, **kwargs):
...
def _dynamic_update_slice_batching_rule(batched_args, batch_dims, update_shape):
  ...
  dnums = ScatterDimensionNumbers(update_window_dims=dims, inserted_window_dims=(),
                                  scatter_dims_to_operand_dims=dims)
  index, index_bdim = _batch_dynamic_slice_indices(batched_args[2:], batch_dims[2:])
  return _scatter_batching_rule(scatter, (operand, index, update),
                                (operand_bdims, index_bdim, update_bdims), None, None, dnums,
                                update_shape)

dynamic_update_slice_p = standard_primitive(_dynamic_update_slice_shape_rule,
                                            _dynamic_update_slice_dtype_rule,
                                            'dynamic_update_slice',
                                            _dynamic_update_slice_translation_rule)
...
class GatherDimensionNumbers(
    collections.namedtuple("GatherDimensionNumbers",
...
def _gather_shape_rule(operand, start_indices, dimension_numbers, slice_sizes, operand_shape):
  ...
  return tuple(
      next(slice_sizes) if i in dimension_numbers.offset_dims else next(start_indices_shape)
      for i in range(result_rank))

def _gather_translation_rule(c, operand, start_indices, dimension_numbers, slice_sizes,
                             operand_shape):
  indices_shape = c.GetShape(start_indices)
  return c.Gather(operand, start_indices, _gather_dimensions_proto(indices_shape,
                                                                   dimension_numbers), slice_sizes)

def _gather_jvp_rule(g, operand, start_indices, dimension_numbers, slice_sizes, operand_shape):
  return gather(g, start_indices, dimension_numbers, slice_sizes)

def _gather_transpose_rule(t, operand, start_indices, dimension_numbers, slice_sizes,
                           operand_shape):
  assert operand is ad.undefined_primal
  if t is ad_util.zero:
...
def _gather_batching_rule(batched_args, batch_dims, dimension_numbers, slice_sizes, operand_shape):
  ...
  if operand_bdim is not None and start_indices_bdim is None:
    ...
    dnums = GatherDimensionNumbers(offset_dims=offset_dims,
                                   collapsed_slice_dims=collapsed_slice_dims,
                                   start_index_map=start_index_map)
    return gather(operand, start_indices, dimension_numbers=dnums, slice_sizes=slice_sizes), 0
...
gather_p = standard_primitive(_gather_shape_rule, _gather_dtype_rule, 'gather',
...
def _scatter_dimensions_proto(indices_shape, dimension_numbers):
  ...
  proto.scatter_dims_to_operand_dims.extend(dimension_numbers.scatter_dims_to_operand_dims)
  assert indices_shape.rank() > 0
  proto.index_vector_dim = indices_shape.rank() - 1
  return proto
...
def _scatter_translation_rule(c, operand, scatter_indices, updates, update_jaxpr, update_consts,
                              dimension_numbers, updates_shape, backend=None):
  dtype = c.GetShape(operand).numpy_dtype()
  init_value = c.Constant(onp.array(0, dtype))
  update_computation = _reduction_computation(c, update_jaxpr, backend, update_consts, init_value)
...
def _scatter_add_jvp(primals, tangents, update_jaxpr, update_consts, dimension_numbers,
  ...
  val_out = scatter_add_p.bind(operand, scatter_indices, updates, update_jaxpr=update_jaxpr,
  ...
  if g_operand is ad_util.zero and g_updates is ad_util.zero:
    ...
    tangent_out = scatter_add_p.bind(g_operand, scatter_indices, g_updates,
                                     update_jaxpr=update_jaxpr, update_consts=update_consts,
                                     dimension_numbers=dimension_numbers,
                                     updates_shape=updates_shape)
  return val_out, tangent_out

def _scatter_add_transpose_rule(t, operand, scatter_indices, updates, update_jaxpr, update_consts,
  ...
  if updates is ad.undefined_primal:
    ...
    for i in xrange(len(t.shape)):
      if i in dimension_numbers.inserted_window_dims:
        ...
        pos += 1
    update_t = gather(t, scatter_indices, dimension_numbers=gather_dnums, slice_sizes=slice_sizes)
  return [operand_t, None, update_t]

def _scatter_batching_rule(scatter_op, batched_args, batch_dims, update_jaxpr, update_consts,
                           dimension_numbers, updates_shape):
  ...
  if scatter_indices_bdim is None and updates_bdim is not None:
    ...
    dnums = ScatterDimensionNumbers(update_window_dims=update_window_dims,
    ...
    scatter_indices = concatenate([counts, scatter_indices], len(count_shape) - 1)
    ...
    scatter_dims_to_operand_dims = (0,) + tuple(
        onp.add(1, dimension_numbers.scatter_dims_to_operand_dims))
...
scatter_add_p = standard_reduction_primitive(_scatter_shape_rule, _scatter_dtype_rule,
                                             'scatter-add', _scatter_translation_rule)
ad.primitive_jvps[scatter_add_p] = _scatter_add_jvp
ad.primitive_transposes[scatter_add_p] = _scatter_add_transpose_rule
batching.primitive_batchers[scatter_add_p] = (partial(_scatter_batching_rule, scatter_add))

# TODO(jlebar): Add derivatives.
scatter_min_p = standard_reduction_primitive(_scatter_shape_rule, _scatter_dtype_rule,
                                             'scatter-min', _scatter_translation_rule)
batching.primitive_batchers[scatter_min_p] = (partial(_scatter_batching_rule, scatter_min))

# TODO(jlebar): Add derivatives.
scatter_max_p = standard_reduction_primitive(_scatter_shape_rule, _scatter_dtype_rule,
                                             'scatter-max', _scatter_translation_rule)
batching.primitive_batchers[scatter_max_p] = (partial(_scatter_batching_rule, scatter_max))

def _scatter_jvp(primals, tangents, update_jaxpr, update_consts, dimension_numbers, updates_shape):
  operand, scatter_indices, updates = primals
  g_operand, g_scatter_indices, g_updates = tangents
  dnums = dimension_numbers

  if g_operand is ad_util.zero and g_updates is ad_util.zero:
    val_out = scatter_p.bind(operand, scatter_indices, updates, update_jaxpr=update_jaxpr,
  ...
  new_operand = pad(new_operand, _zero(operand), ((0, 1, 0),) + tuple(
      (0, 0, 0) for _ in operand_shape))

  ids_shape = onp.array(updates_shape)
  ids_shape[dnums.update_window_dims,] = 1
  num_ids = onp.prod(ids_shape)
  update_ids = add(reshape(iota(updates_dtype, num_ids), ids_shape), _ones(updates))
  ...
  outputs = scatter_p.bind(new_operand, scatter_indices, updates_and_ids, update_jaxpr=update_jaxpr,
  ...
  gather_dnums = GatherDimensionNumbers(offset_dims=dnums.update_window_dims,
  ...
  for i in xrange(len(scattered_ids.shape)):
    if i in dnums.inserted_window_dims:
      ...
      pos += 1
  gathered_update_ids = gather(scattered_ids, scatter_indices, dimension_numbers=gather_dnums,
                               slice_sizes=slice_sizes)

  # c) mask off input JVP elements that do not correspond to a primal output.
  g_operand = ad.instantiate_zeros(operand, g_operand)
  g_updates = ad.instantiate_zeros(updates, g_updates)
  masked_g_operand = select(eq(scattered_ids, _zeros(scattered_ids)), g_operand, _zeros(g_operand))
  masked_g_updates = select(eq(update_ids, gathered_update_ids), g_updates, _zeros(g_updates))
...
scatter_p = standard_reduction_primitive(_scatter_shape_rule, _scatter_dtype_rule, 'scatter',
                                         _scatter_translation_rule)
ad.primitive_jvps[scatter_p] = _scatter_jvp
batching.primitive_batchers[scatter_p] = (partial(_scatter_batching_rule, scatter))
...
def _reducer_masking_rule(prim, identity, padded_vals, logical_shapes, axes, input_shape):
  del input_shape  # Unused.
  (padded_val,), (logical_shape,) = padded_vals, logical_shapes
  padded_shape = masking.padded_shape_as_value(padded_val.shape)
  masks = [
      broadcasted_iota(onp.int32, padded_shape, i) < d
...
def _reduce_sum_shape_rule(operand, axes, input_shape):
  assert operand.shape == input_shape, ('{} != {}'.format(operand.shape, input_shape))
...
def _reduce_sum_translation_rule(c, operand, axes, input_shape):
  ...
  return c.Reduce(operand, c.Constant(onp.array(0, dtype)),
                  xla.primitive_computation(add_p, scalar, scalar), axes)
...
reduce_sum_p = standard_primitive(_reduce_sum_shape_rule, _input_dtype, 'reduce_sum',
                                  _reduce_sum_translation_rule)
ad.deflinear(reduce_sum_p, _reduce_sum_transpose_rule)
batching.defreducer(reduce_sum_p)
_masking_defreducer(reduce_sum_p, lambda shape, dtype: onp.broadcast_to(onp.array(0, dtype), shape))
...
def _reduce_prod_translation_rule(c, operand, axes):
  ...
  return c.Reduce(operand, c.Constant(onp.array(1, dtype)),
                  xla.primitive_computation(mul_p, scalar, scalar), axes)
...
def _reduce_prod_jvp_rule(tangent, operand, axes):
  ...
  left_products = _reduce_window_prod(
      pad(operand, one, left_padding), window_dims, window_strides, xla_client.PaddingType.VALID)
  right_products = _reduce_window_prod(
      pad(operand, one, right_padding), window_dims, window_strides, xla_client.PaddingType.VALID)

  # Multiply partial products with the tangents and sum.
  return _reduce_sum(mul(tangent, mul(left_products, right_products)), (0,))

reduce_prod_p = standard_primitive(_reduce_prod_shape_rule, _input_dtype, 'reduce_prod',
                                   _reduce_prod_translation_rule)
...
def _reduce_chooser_jvp_rule(g, ans, operand, axes):
  ...
  location_indicators = convert_element_type(_eq_meet(operand, reshape(ans, shape)), g.dtype)
  counts = _reduce_sum(location_indicators, axes)
  return div(_reduce_sum(mul(g, location_indicators), axes), counts)

_reduce_max_translation_rule = partial(_reduce_chooser_translation_rule, max_p, _get_max_identity)
reduce_max_p = standard_primitive(_reduce_chooser_shape_rule, _input_dtype, 'reduce_max',
                                  _reduce_max_translation_rule)
ad.defjvp2(reduce_max_p, _reduce_chooser_jvp_rule)
batching.defreducer(reduce_max_p)

_reduce_min_translation_rule = partial(_reduce_chooser_translation_rule, min_p, _get_min_identity)
reduce_min_p = standard_primitive(_reduce_chooser_shape_rule, _input_dtype, 'reduce_min',
                                  _reduce_min_translation_rule)
...
_reduce_or_translation_rule = partial(_reduce_logical_translation_rule, or_p, _get_max_identity)
reduce_or_p = standard_primitive(_reduce_logical_shape_rule, _fixed_dtype(onp.bool_), 'reduce_or',
                                 _reduce_or_translation_rule)
batching.defreducer(reduce_or_p)

_reduce_and_translation_rule = partial(_reduce_logical_translation_rule, and_p, _get_min_identity)
reduce_and_p = standard_primitive(_reduce_logical_shape_rule, _fixed_dtype(onp.bool_), 'reduce_and',
                                  _reduce_and_translation_rule)
batching.defreducer(reduce_and_p)

def _reduce_window_shape_rule(operand, init_value, jaxpr, consts, window_dimensions, window_strides,
                              padding):
  ...
  return _common_reduce_window_shape_rule(operand, window_dimensions, window_strides, padding)

def _reduce_window_translation_rule(c, operand, init_value, jaxpr, consts, window_dimensions,
                                    window_strides, padding, backend=None):
  xla_computation = _reduction_computation(c, jaxpr, backend, consts, init_value)
  return c.ReduceWindow(operand, init_value, xla_computation, window_dimensions, window_strides,
                        padding)

def _generic_reduce_window_batch_rule(batched_args, batch_dims, jaxpr, consts, window_dimensions,
                                      window_strides, padding):
  operand, init = batched_args
  bdim, init_bdim = batch_dims
  if init_bdim is not None:
    raise NotImplementedError("reduce_window batching is not implemented for " "initial values")

  def reduce_window(x, window_dimensions, window_strides, padding):
    return reduce_window_p.bind(x, init, jaxpr=jaxpr, consts=consts,
                                window_dimensions=window_dimensions, window_strides=window_strides,
                                padding=padding)

  return _reduce_window_batch_rule(reduce_window, (operand,), (bdim,), window_dimensions,
                                   window_strides, padding)

reduce_window_p = standard_reduction_primitive(_reduce_window_shape_rule, _input_dtype,
                                               'reduce_window', _reduce_window_translation_rule)
batching.primitive_batchers[reduce_window_p] = _generic_reduce_window_batch_rule

def _reduce_window_sum_shape_rule(operand, window_dimensions, window_strides, padding, input_shape):
  return _common_reduce_window_shape_rule(operand, window_dimensions, window_strides, padding)

def _reduce_window_sum_translation_rule(c, operand, window_dimensions, window_strides, padding,
                                        input_shape):
  dtype = c.GetShape(operand).numpy_dtype()
  scalar = xla_client.Shape.array_shape(dtype, ())
  return c.ReduceWindow(operand, c.Constant(onp.array(0, dtype)),
                        xla.primitive_computation(add_p, scalar, scalar), window_dimensions,
                        window_strides, padding)

def _reduce_window_sum_transpose_rule(cotangent, window_dimensions, window_strides, padding,
                                      input_shape):
  in_pads = padtype_to_pads(input_shape, window_dimensions, window_strides, padding)
  ones = [1] * len(input_shape)
  pads = _conv_general_vjp_lhs_padding(input_shape, window_dimensions, window_strides,
                                       cotangent.shape, in_pads, ones, ones)
  padding_config = [(lo, hi, stride - 1) for (lo, hi), stride in zip(pads, window_strides)]
  pad_cotangent = pad(cotangent, _zero(cotangent), padding_config)
  result = _reduce_window_sum(pad_cotangent, window_dimensions, ones, xla_client.PaddingType.VALID)
  assert result.shape == input_shape
  return [result]

def _reduce_window_batch_rule(reduce_window, batched_args, bdims, window_dimensions, window_strides,
...
reduce_window_sum_p = standard_primitive(_reduce_window_sum_shape_rule, _input_dtype,
                                         'reduce_window_sum', _reduce_window_sum_translation_rule)
ad.deflinear(reduce_window_sum_p, _reduce_window_sum_transpose_rule)
batching.primitive_batchers[reduce_window_sum_p] = partial(_reduce_window_batch_rule,
                                                           _reduce_window_sum)

def _reduce_window_chooser_translation_rule(prim, identity, c, operand, window_dimensions,
                                            window_strides, padding):
  dtype = c.GetShape(operand).numpy_dtype()
  scalar = xla_client.Shape.array_shape(dtype, ())
  return c.ReduceWindow(operand, c.Constant(identity(dtype)),
                        xla.primitive_computation(prim, scalar, scalar), window_dimensions,
                        window_strides, padding)

def _reduce_window_chooser_jvp_rule(prim, g, operand, window_dimensions, window_strides, padding):
  assert prim is max_p or prim is min_p
  select_prim = ge_p if prim is max_p else le_p
  return _select_and_gather_add(g, operand, select_prim, window_dimensions, window_strides, padding)

def _common_reduce_window_shape_rule(operand, window_dimensions, window_strides, padding):
  ...
  return reduce_window_shape_tuple(operand.shape, window_dimensions, window_strides, padding)

def reduce_window_shape_tuple(operand_shape, window_dimensions, window_strides, padding):
  ...
  t = onp.floor_divide(onp.subtract(operand_padded, window_dimensions), window_strides) + 1
  return tuple(t)

_reduce_window_max_translation_rule = partial(_reduce_window_chooser_translation_rule, max_p,
                                              _get_max_identity)
reduce_window_max_p = standard_primitive(_common_reduce_window_shape_rule, _input_dtype,
                                         'reduce_window_max', _reduce_window_max_translation_rule)
ad.defjvp(reduce_window_max_p, partial(_reduce_window_chooser_jvp_rule, max_p))
batching.primitive_batchers[reduce_window_max_p] = partial(_reduce_window_batch_rule,
                                                           _reduce_window_max)

_reduce_window_min_translation_rule = partial(_reduce_window_chooser_translation_rule, min_p,
                                              _get_min_identity)
reduce_window_min_p = standard_primitive(_common_reduce_window_shape_rule, _input_dtype,
                                         'reduce_window_min', _reduce_window_min_translation_rule)
ad.defjvp(reduce_window_min_p, partial(_reduce_window_chooser_jvp_rule, min_p))

_reduce_window_min_batch_rule = partial(_reduce_window_batch_rule, _reduce_window_min)
batching.primitive_batchers[reduce_window_min_p] = partial(_reduce_window_batch_rule,
                                                           _reduce_window_min)

def _select_and_scatter_shape_rule(operand, source, init_value, select_jaxpr, select_consts,
                                   scatter_jaxpr, scatter_consts, window_dimensions, window_strides,
                                   padding):
...
def _select_and_scatter_translation(c, operand, source, init_value, select_jaxpr, select_consts,
                                    scatter_jaxpr, scatter_consts, window_dimensions,
                                    window_strides, padding, backend=None):
  ...
  return c.SelectAndScatter(operand, select, window_dimensions, window_strides, padding, source,
                            init_value, scatter)

select_and_scatter_p = standard_reduction_primitive(_select_and_scatter_shape_rule, _input_dtype,
                                                    'select_and_scatter',
                                                    _select_and_scatter_translation)

def _select_and_scatter_add_shape_rule(source, operand, select_prim, window_dimensions,
                                       window_strides, padding):
  return operand.shape

def _select_and_scatter_add_translation(c, source, operand, select_prim, window_dimensions,
                                        window_strides, padding):
  dtype = c.GetShape(operand).numpy_dtype()
  scalar = xla_client.Shape.array_shape(dtype, ())
...
def _select_and_scatter_add_jvp(primals, tangents, select_prim, window_dimensions, window_strides,
                                padding):
  source, operand = primals
  g_source, g_operand = tangents
  val_out = _select_and_scatter_add(source, operand, select_prim, window_dimensions, window_strides,
                                    padding)
  del g_operand
  if g_source is ad_util.zero:
    tangent_out = ad_util.zero
  else:
    tangent_out = _select_and_scatter_add(g_source, operand, select_prim, window_dimensions,
                                          window_strides, padding)
  return val_out, tangent_out

def _select_and_scatter_add_transpose(t, source, operand, select_prim, window_dimensions,
                                      window_strides, padding):
  assert source is ad.undefined_primal and operand is not ad.undefined_primal
  source_t = _select_and_gather_add(t, operand, select_prim, window_dimensions, window_strides,
                                    padding)
...
def _select_and_scatter_add_batch_rule(batched_args, batch_dims, **kwargs):
  ...
  if s_bdims is not None and o_bdims is not None:
    ...
    outputs = [_select_and_scatter_add(s, o, **kwargs) for s, o in zip(source, operand)]
    ...
    #TODO(#212): use a map construct instead of unrolling.
    source = batching.moveaxis(source, s_bdims, 0)
    outputs = [_select_and_scatter_add(s, operand, **kwargs) for s in source]
    outputs = [reshape(out, (1,) + out.shape) for out in outputs]
    outputs = concatenate(outputs, 0)
    return outputs, 0
    ...
    #TODO(#212): use a map construct instead of unrolling.
    operand = batching.moveaxis(operand, o_bdims, 0)
    outputs = [_select_and_scatter_add(source, o, **kwargs) for o in operand]
    outputs = [reshape(out, (1,) + out.shape) for out in outputs]
    outputs = concatenate(outputs, 0)
    return outputs, 0

select_and_scatter_add_p = standard_primitive(_select_and_scatter_add_shape_rule, _input_dtype,
...
def _select_and_gather_add_shape_rule(tangents, operand, select_prim, window_dimensions,
                                      window_strides, padding):
  if tangents.shape != operand.shape:
    msg = ("select_and_gather_add tangents and operand shapes must match, " "got {} and {}.")
...
def _select_and_gather_add_translation(c, tangents, operand, select_prim, window_dimensions,
                                       window_strides, padding, max_bits=64):
  ...
  const = lambda c, dtype, x: c.Constant(onp.array(x, dtype=dtype), canonicalize_types=False)
  ...
  if double_word_reduction:
    ...
    # Unpacks the second element of a tuple.
    def snd(t):
      return c.BitcastConvertType(c.ShiftLeft(t, const(c, word_dtype, r_nbits)), etype)

  def reducer():
    c = xla_bridge.make_computation_builder("select_and_gather_pair_reducer")
    x = c.ParameterWithShape(xla_client.Shape.array_shape(onp.dtype(double_word_dtype), ()))
    y = c.ParameterWithShape(xla_client.Shape.array_shape(onp.dtype(double_word_dtype), ()))
  ...
  out = c.ReduceWindow(
      pack(operand, tangents), pack(const(c, dtype, init), const(c, dtype, 0)), reducer(),
      window_dimensions, window_strides, padding)
  return snd(out)

def _select_and_gather_add_jvp(primals, tangents, select_prim, window_dimensions, window_strides,
                               padding):
  source, operand = primals
  g_source, g_operand = tangents
  val_out = _select_and_gather_add(source, operand, select_prim, window_dimensions, window_strides,
                                   padding)
  del g_operand
  if g_source is ad_util.zero:
    tangent_out = ad_util.zero
  else:
    tangent_out = _select_and_gather_add(g_source, operand, select_prim, window_dimensions,
                                         window_strides, padding)
  return val_out, tangent_out

def _select_and_gather_add_transpose(t, tangents, operand, select_prim, window_dimensions,
                                     window_strides, padding):
  assert tangents is ad.undefined_primal and operand is not ad.undefined_primal
  result = _select_and_scatter_add(t, operand, select_prim, window_dimensions, window_strides,
                                   padding)
  return [result, None]

select_and_gather_add_p = standard_primitive(_select_and_gather_add_shape_rule, _input_dtype,
...
class _FilledConstant(xla.DeviceConstant):
  ...
  def constant_handler(c, filled_const, canonicalize_types=True):
    return c.Broadcast(
        c.NumpyArrayConstant(filled_const.fill_value, canonicalize_types), filled_const.shape)
...
class _IotaConstant(xla.DeviceConstant):
  ...
  def _value(self):
    if self._npy_value is None:
      ...
      iota = iota.reshape(
          [self.shape[self.axis] if i == self.axis else 1 for i in range(self.ndim)])
...
class _EyeConstant(xla.DeviceConstant):
  ...
  def _value(self):
    if self._npy_value is None:
      ones = [1] * self.ndim
      iotas = [
          onp.arange(self.shape[axis]).reshape(subvals(ones, [(axis, -1)])) for axis in self.axes
      ]
  ...
  def constant_handler(c, diag_const, canonicalize_types=True):
    ...
    etype = xla_bridge.dtype_to_etype(diag_const.dtype)
    iotas = [c.BroadcastedIota(onp.uint32, diag_const.shape, axis) for axis in diag_const.axes]
...
def padtype_to_pads(in_shape, window_shape, window_strides, padding):
  ...
  if padding == PaddingType.SAME:
    ...
    pad_sizes = [
        _max((out_size - 1) * stride + window_shape - in_size, 0) for out_size, stride,
        window_shape, in_size in zip(out_shape, window_strides, window_shape, in_shape)
    ]
...
def _check_same_dtypes(name, ignore_fp_precision, *dtypes):
  ...
  if ignore_fp_precision:
    dtypes = [
        onp.floating if onp.issubdtype(dtype, onp.floating) else
        onp.complexfloating if onp.issubdtype(dtype, onp.complexfloating) else dtype
        for dtype in dtypes
    ]
  if len({xla_bridge.canonicalize_dtype(t) for t in dtypes}) != 1:
    if ignore_fp_precision:
      msg = ("{} requires arguments to have same dtypes up to floating point " "precision, got {}.")
    else:
...
def conv_transpose_shape_tuple(lhs_shape, rhs_shape, window_strides, padding, dimension_numbers):
  ...
  if isinstance(padding, str):
    padding = [
        _conv_transpose_padding(k, s, padding) for k, s in zip(rhs_trans[2:], window_strides)
    ]
  padding = list(map(onp.sum, padding))
  unpad_out_space = [
      (i - 1) * s - k + 2 for i, k, s in zip(lhs_trans[2:], rhs_trans[2:], window_strides)
  ]
...
def _dynamic_slice_indices(operand, start_indices):
  if not isinstance(start_indices, (tuple, list)):
    if start_indices.ndim != 1:
      raise ValueError("Slice indices must be a 1D sequence, got {}".format(start_indices.shape))
    start_indices = [reshape(slice(start_indices, [i], [i + 1]), ()) for i in range(operand.ndim)]
  else:
    start_indices = [onp.asarray(i) if isinstance(i, int) else i for i in start_indices]
  if len(start_indices) != operand.ndim:
    msg = ("Length of slice indices must match number of operand dimensions ({} " "vs {})")
    raise ValueError(msg.format(len(start_indices, operand.shape)))
  # map int over operand.shape to raise any dynamic-shape errors
  return [
      select(lt(i, _const(i, 0)), add(i, _const(i, int(d))), i)
...
class ConvDimensionNumbers(
    collections.namedtuple("ConvDimensionNumbers", ["lhs_spec", "rhs_spec", "out_spec"])):
...
def _conv_general_vjp_lhs_padding(in_shape, window_dimensions, window_strides, out_shape, padding,
  ...
  pad_after = (onp.add(lhs_dilated_shape, rhs_dilated_shape) - 1 - out_dilated_shape - pad_before)
  return zip(pad_before, pad_after)

def _conv_general_vjp_rhs_padding(in_shape, window_dimensions, window_strides, out_shape, padding,
                                  lhs_dilation, rhs_dilation):
  lhs_dilated_shape = _dilate_shape(in_shape, lhs_dilation)
  rhs_dilated_shape = _dilate_shape(window_dimensions, rhs_dilation)
  out_dilated_shape = _dilate_shape(out_shape, window_strides)
...
def _balanced_eq(x, z, y):
  return div(
      select(_eq_meet(x, z), _ones(z), _zeros(z)), select(_eq_meet(y, z), _twos(z), _ones(z)))
