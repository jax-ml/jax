...
def _canonicalize_shape(shape):
  ...
  """
    ...
    if (
        type(shape) is ShapeExpr
        or type(shape) is tuple
        and any(type(d) is masking.Poly for d in shape)
    ):
        return shape
    try:
...
def convert_element_type(operand, new_dtype):
  ...
  """
    ...
    if old_dtype != new_dtype:
        if onp.issubdtype(old_dtype, onp.complexfloating) and not onp.issubdtype(
            new_dtype, onp.complexfloating
        ):
...
def concatenate(operands, dimension):
  ...
  """
    return concatenate_p.bind(
        *operands, dimension=dimension, operand_shapes=tuple(o.shape for o in operands)
    )
...
def dot_general(lhs, rhs, dimension_numbers, precision=None):
  ...
  """
    ...
    if onp.issubdtype(lhs.dtype, onp.integer):
        ...
        out_ndim = (
            len(lhs_batch_dims) + len(lhs_noncontract_dims) + len(rhs_noncontract_dims)
        )
        return reduce(
            mul(lhs, rhs),
            _zero(lhs),
...
def broadcast_in_dim(operand, shape, broadcast_dimensions):
    ...
    if any(x < 0 or x >= len(shape) for x in broadcast_dimensions):
        msg = (
            "broadcast dimensions must be >= 0 and < ndim(shape), got {} for "
            "shape {}"
        )
        raise ValueError(msg.format(broadcast_dimensions, shape))
    return broadcast_in_dim_p.bind(
        operand, shape=tuple(shape), broadcast_dimensions=tuple(broadcast_dimensions)
    )
...
def reshape(operand, new_sizes, dimensions=None):
  ...
  """
    ...
    same_dims = dimensions is None or tuple(dimensions) == tuple(
        range(onp.ndim(operand))
    )
...
def slice(operand, start_indices, limit_indices, strides=None):
  ...
  """
    if (
        onp.all(onp.equal(start_indices, 0))
...
def dynamic_update_slice(operand, update, start_indices):
  ...
  """
    start_indices = _dynamic_slice_indices(operand, start_indices)
    return dynamic_update_slice_p.bind(
        operand, update, *start_indices, update_shape=update.shape
    )
...
def scatter(operand, scatter_indices, updates, dimension_numbers):
  ...
  """
    jaxpr, consts = _reduction_jaxpr(
        _scatter_reduction_computation, _abstractify(_const(operand, 0))
    )
...
def index_take(src, idxs, axes):
    ...
    dnums = GatherDimensionNumbers(
        offset_dims=offset_dims, collapsed_slice_dims=axes, start_index_map=axes
    )
...
def reduce_window(
    operand, init_value, computation, window_dimensions, window_strides, padding
):
...
def _select_and_scatter(
    ...
    return select_and_scatter_p.bind(
        ...
        source,
        init_value,
        select_jaxpr=select_jaxpr,
        select_consts=select_consts,
        scatter_jaxpr=scatter_jaxpr,
...
def full(shape, fill_value, dtype=None):
  ...
  """
    try:
        ...
        msg = (
            "`full` requires shapes to be concrete. If using `jit`, try using "
...
def conv_with_general_padding(
    lhs, rhs, window_strides, padding, lhs_dilation, rhs_dilation, precision=None
):
    """Convenience wrapper around `conv_general_dilated`.
...
def conv_transpose(
  ...
  """
    ...
    return conv_general_dilated(
        lhs, rhs, one, pads, strides, one, dn, precision=precision
    )
...
def asin(x):
    ...
    return mul(
        _const(x, 2), atan2(x, add(_const(x, 1), sqrt(sub(_const(x, 1), square(x)))))
    )
...
_fixed_dtype = lambda dtype: lambda *args, **kwargs: xla_bridge.canonicalize_dtype(
    dtype
)
...
def standard_reduction_primitive(shape_rule, dtype_rule, name, translation_rule=None):
    ...
    prim.def_impl(partial(xla.apply_primitive, prim))
    prim.def_abstract_eval(partial(standard_abstract_eval, shape_rule, dtype_rule))
    xla.reduction_translations[prim] = translation_rule or partial(
        standard_translate, name
    )
    return prim
...
def binop_dtype_rule(result_dtype, accepted_dtypes, name, *avals, **kwargs):
    ...
    for i, (aval_dtype, types) in enumerate(zip(aval_dtypes, accepted_dtypes)):
        if not any(onp.issubdtype(aval_dtype, t) for t in types):
            msg = (
                "{} does not accept dtype {} at position {}. "
...
def binop(result_dtype, accepted_dtypes, name, translation_rule=None):
    ...
    prim = standard_primitive(
        shape_rule, dtype_rule, name, translation_rule=translation_rule
    )
...
ad.defjvp(
    erf_p,
    lambda g, x: mul(_const(x, 2.0 / onp.sqrt(onp.pi)), mul(g, exp(neg(square(x))))),
)
...
ad.defjvp(
    erfc_p,
    lambda g, x: mul(
        _const(x, 2.0 / onp.sqrt(onp.pi)), mul(neg(g), exp(neg(square(x))))
    ),
)
...
ad.defjvp2(
    erf_inv_p,
    lambda g, ans, x: mul(_const(x, onp.sqrt(onp.pi) / 2.0), mul(g, exp(square(ans)))),
)
...
_complex_dtype = lambda dtype, *args: (
    onp.zeros((), dtype) + onp.zeros((), onp.complex64)
).dtype
    ...
    if _iscomplex(x):
        return _maybe_real(
            mul(
                g,
                div(
                    _maybe_conj(x), _replace_zero(convert_element_type(ans, _dtype(x)))
                ),
            )
        )
    else:
...
ad.defjvp2(
    rsqrt_p,
    lambda g, ans, x: _safe_mul(g, mul(_const(x, -0.5), pow(x, _const(x, -1.5)))),
)
...
def _safe_mul_translation_rule(c, x, y):
    ...
    return c.Select(
        c.Or(c.Eq(x, zero), c.Eq(y, zero)), c.Broadcast(zero, out_shape), c.Mul(x, y)
    )


safe_mul_p = standard_binop(
    [_num, _num], "safe_mul", translation_rule=_safe_mul_translation_rule
)
...
def _minmax_translation_rule(c, x, y, minmax=None, cmp=None):
    ...
    if onp.issubdtype(dtype, onp.complexfloating):
        ...
        return _broadcasting_select(
            c,
            c.Select(
                c.Eq(rx, ry), comparator(c.Imag(x), c.Imag(y)), comparator(rx, ry)
            ),
            x,
            y,
        )
...
max_p = standard_binop(
    [_any, _any],
    "max",
    translation_rule=partial(
...
convert_element_type_p = standard_primitive(
    _convert_element_type_shape_rule,
...
bitcast_convert_type_p = standard_primitive(
    _bitcast_convert_type_shape_rule,
...
def _conv_general_dilated_shape_rule(
    ...
    **unused_kwargs
    ...
    if not feature_group_count > 0:
        msg = (
            "conv_general_dilated feature_group_count "
    ...
    if rhs.shape[dimension_numbers.rhs_spec[0]] % feature_group_count:
        ...
        raise ValueError(
            msg.format(rhs.shape[dimension_numbers.rhs_spec[0]], feature_group_count)
        )
...
def _conv_general_dilated_dtype_rule(
    ...
    **unused_kwargs
    ...
    return binop_dtype_rule(
        _input_dtype, [_float, _float], "conv_general_dilated", lhs, rhs
    )
...
def _conv_general_dilated_transpose_lhs(
    ...
    padding = _conv_general_vjp_lhs_padding(
        onp.take(lhs_shape, lhs_sdims),
        onp.take(rhs_shape, rhs_sdims),
        window_strides,
        onp.take(g.shape, out_sdims),
        padding,
        lhs_dilation,
        rhs_dilation,
    )
    ...
    **unused_kwargs
    ...
    if lhs_bdim is not None and rhs_bdim is not None:
        ...
        if feature_group_count == 1:
            ...
            new_rhs = _reshape_axis_out_of(
                rhs_spec[0] + int(rhs_bdim <= rhs_spec[0]), feature_group_count, rhs
            )
            new_rhs = _reshape_axis_into(
                rhs_bdim + int(rhs_spec[0] < rhs_bdim), rhs_spec[0] + 1, new_rhs
            )
...
def _dot_polymorphic_shape_rule(shape_exprs, precision):
    ...
    if lhs_ndim == rhs_ndim == 1:
        if not lhs_shape == rhs_shape:
            raise ShapeError
        return ShapeExpr(())
    elif lhs_ndim == rhs_ndim == 2:
        if not lhs_shape[1] == rhs_shape[0]:
            raise ShapeError
        ...
        if not lhs_shape[0] == rhs_shape[0]:
            raise ShapeError
...
def _dot_masking_rule(padded_vals, logical_shapes, precision):
    ...
    if lhs_ndim == rhs_ndim == 1:
        masked_lhs = select(
            iota(onp.int32, lhs.shape[0]) < lhs_shape[0], lhs, zeros_like_array(lhs)
        )
        ...
        masked_lhs = select(
            broadcasted_iota(onp.int32, lhs.shape, 1) < lhs_shape[1],
            lhs,
            zeros_like_array(lhs),
        )
        return dot_p.bind(masked_lhs, rhs, precision=precision)
    elif rhs_ndim == 1:
...
dot_p = standard_primitive(
    _dot_shape_rule, _dot_dtype_rule, "dot", _dot_translation_rule
)
...
def _dot_general_transpose_rhs(g, x, dimension_numbers, precision):
    ...
    return _dot_general_transpose_lhs(
        g, x, swapped_dimension_numbers, precision, swap_ans=True
    )
...
def _dot_general_translation_rule(c, lhs, rhs, dimension_numbers, precision):
    return c.DotGeneral(
        lhs, rhs, dimension_numbers, precision_config=_precision_config(precision)
    )
...
def _broadcast_in_dim_shape_rule(operand, shape, broadcast_dimensions):
    ...
    if operand.ndim != len(broadcast_dimensions):
        msg = (
            "broadcast_in_dim broadcast_dimensions must have length equal to "
            "operand ndim, got broadcast_dimensions {} for operand ndim {}."
        )
        raise TypeError(msg.format(broadcast_dimensions, operand.ndim))
    if not set(broadcast_dimensions).issubset(set(range(len(shape)))):
        msg = (
            "broadcast_in_dim broadcast_dimensions must be a subset of output "
...
ad.defjvp(
    ...
    lambda g, min, operand, max: select(
        bitwise_and(gt(min, operand), lt(min, max)),
        _brcast(g, operand),
        _zeros(operand),
    ),
    lambda g, min, operand, max: select(
    ...
    lambda g, min, operand, max: select(
        lt(max, operand), _brcast(g, operand), _zeros(operand)
    ),
)
...
def _concatenate_batch_rule(batched_args, batch_dims, dimension, operand_shapes):
    size = next(
        op.shape[bdim] for op, bdim in zip(batched_args, batch_dims) if bdim is not None
    )
    operands = [
...
def _pad_transpose(t, operand, padding_value, padding_config):
    ...
    def t_op():
        ...
        unpadded = pad(t, onp.array(0.0, t.dtype), unpad_config)
    ...
    t_padv = (
        sub(total(t), total(t_operand))
        if padding_value is ad.undefined_primal
        else None
    )
...
def _reshape_impl(operand, new_sizes, dimensions, old_sizes):
    if (
        type(operand) is pxla.ShardedDeviceArray
        ...
        type(operand) is pxla.ChunkedDeviceArray
        and dimensions is None
...
def _reshape_shape_rule(operand, new_sizes, dimensions, **unused_kwargs):
    ...
    if dimensions is not None:
        if set(dimensions) != set(range(onp.ndim(operand))):
            msg = (
                "reshape dimensions must be a permutation of operand dimensions, "
...
def _reshape_transpose_rule(t, new_sizes, dimensions, old_sizes):
    if dimensions is None:
        ...
        return [
            transpose(
                reshape(t, onp.take(old_sizes, dimensions)), onp.argsort(dimensions)
            )
        ]
...
def _reshape_polymorphic_shape_rule(shape_exprs, new_sizes, dimensions, old_sizes):
    ...
    if masking.prod(shape_expr) != masking.prod(new_sizes):
        raise ShapeError
    return new_sizes


reshape_p = standard_primitive(
    _reshape_shape_rule, _reshape_dtype_rule, "reshape", _reshape_translation_rule
)
...
def _rev_shape_rule(operand, dimensions):
    ...
    if not _max(dimensions) < operand.ndim:
        msg = (
            "rev dimensions must all be less than operand ndim, got dimensions "
            "{} for operand ndim {}."
        )
...
def _transpose_shape_rule(operand, permutation):
    ...
    if tuple(sorted(permutation)) != tuple(range(operand.ndim)):
        msg = (
            "transpose permutation isn't a permutation of operand dimensions, "
...
ad.deflinear(
    transpose_p, lambda t, permutation: [transpose(t, onp.argsort(permutation))]
)
...
def _select_shape_rule(pred, on_true, on_false):
    ...
    if pred.shape and pred.shape != on_true.shape:
        msg = (
            "select pred must be scalar or have the same shape as on_true and "
...
def _slice_shape_rule(operand, start_indices, limit_indices, strides, operand_shape):
    ...
    if strides is None:
        ...
        if len(strides) != operand.ndim:
            msg = (
                "slice strides must have length equal to the number of dimensions "
...
def _slice_transpose_rule(t, start_indices, limit_indices, strides, operand_shape):
    if strides is None or onp.all(onp.equal(strides, 1)):
        pads = zip(
            start_indices,
            onp.subtract(operand_shape, limit_indices),
            (0,) * len(start_indices),
        )
    else:
        real_limits = onp.add(
        ...
        pads = zip(
            start_indices,
            onp.subtract(operand_shape, real_limits),
...
def _slice_batching_rule(
    batched_args, batch_dims, start_indices, limit_indices, strides, **unused_kwargs
):
    operand, = batched_args
    bdim, = batch_dims
...
slice_p = standard_primitive(
    _slice_shape_rule, _input_dtype, "slice", _slice_translation_rule
)
...
def _dynamic_slice_shape_rule(operand, *start_indices, **kwargs):
    ...
    if operand.ndim != len(start_indices):
        msg = (
            "dynamic_slice start_indices must have length equal to the number "
            "of dimensions of the operand, got indices {} for operand shape {}."
        )
        raise TypeError(msg.format(start_indices, operand.shape))
    if len(start_indices) != len(slice_sizes):
        msg = (
            "dynamic_slice slice_sizes must have the same length as "
    ...
    if not onp.all(onp.less_equal(slice_sizes, operand.shape)):
        msg = (
            "slice slice_sizes must be less than or equal to operand shape, "
...
def _dynamic_slice_batching_rule(batched_args, batch_dims, slice_sizes, operand_shape):
    ...
    dnums = GatherDimensionNumbers(
        offset_dims=dims, collapsed_slice_dims=(), start_index_map=dims
    )
    index, index_bdim = _batch_dynamic_slice_indices(batched_args[1:], batch_dims[1:])
    return _gather_batching_rule(
        [batched_args[0], index],
        [batch_dims[0], index_bdim],
        dnums,
        slice_sizes,
        operand_shape,
    )
...
def _dynamic_update_slice_shape_rule(operand, update, *start_indices, **kwargs):
    if operand.ndim != update.ndim:
        msg = (
            "dynamic_update_slice update must have the same rank as operand, "
            "got update shape {} for operand shape {}."
        )
        raise TypeError(msg.format(update.shape, operand.shape))
    if operand.ndim != len(start_indices):
        msg = (
            "dynamic_update_slice start_indices must have length equal to the "
...
def _dynamic_update_slice_transpose_rule(t, operand, update, *start_indices, **kwargs):
    ...
    update_t = (
        ds(t, start_indices, update_shape) if update is ad.undefined_primal else None
    )
    return [operand_t, update_t] + [None] * len(start_indices)


def _dynamic_update_slice_translation_rule(
    c, operand, update, *start_indices, **kwargs
):
...
def _dynamic_update_slice_batching_rule(batched_args, batch_dims, update_shape):
    ...
    return _scatter_batching_rule(
        scatter,
        (operand, index, update),
        (operand_bdims, index_bdim, update_bdims),
        None,
        None,
        dnums,
        update_shape,
...
dynamic_update_slice_p = standard_primitive(
    _dynamic_update_slice_shape_rule,
...
def _gather_shape_rule(
    operand, start_indices, dimension_numbers, slice_sizes, operand_shape
):
...
def _gather_translation_rule(
    c, operand, start_indices, dimension_numbers, slice_sizes, operand_shape
):
    indices_shape = c.GetShape(start_indices)
    return c.Gather(
        operand,
        start_indices,
        _gather_dimensions_proto(indices_shape, dimension_numbers),
        slice_sizes,
    )


def _gather_jvp_rule(
    g, operand, start_indices, dimension_numbers, slice_sizes, operand_shape
):
    return gather(g, start_indices, dimension_numbers, slice_sizes)


def _gather_transpose_rule(
    t, operand, start_indices, dimension_numbers, slice_sizes, operand_shape
):
    assert operand is ad.undefined_primal
    if t is ad_util.zero:
...
def _gather_batching_rule(
    batched_args, batch_dims, dimension_numbers, slice_sizes, operand_shape
):
    ...
    if operand_bdim is not None and start_indices_bdim is None:
        ...
        collapsed_slice_dims = (0,) + tuple(
            onp.add(1, dimension_numbers.collapsed_slice_dims)
        )
        offset_dims = tuple(onp.add(1, dimension_numbers.offset_dims))
...
gather_p = standard_primitive(
    _gather_shape_rule, _gather_dtype_rule, "gather", _gather_translation_rule
)
...
def _scatter_add_jvp(
    primals, tangents, update_jaxpr, update_consts, dimension_numbers, updates_shape
):
...
def _scatter_batching_rule(
    ...
    if scatter_indices_bdim is None and updates_bdim is not None:
        ...
        update_window_dims = (0,) + tuple(
            onp.add(1, dimension_numbers.update_window_dims)
        )
        ...
        inserted_window_dims = (0,) + tuple(
            onp.add(1, dimension_numbers.inserted_window_dims)
        )
...
scatter_add_p = standard_reduction_primitive(
    _scatter_shape_rule, _scatter_dtype_rule, "scatter-add", _scatter_translation_rule
)
ad.primitive_jvps[scatter_add_p] = _scatter_add_jvp
ad.primitive_transposes[scatter_add_p] = _scatter_add_transpose_rule
batching.primitive_batchers[scatter_add_p] = partial(
    _scatter_batching_rule, scatter_add
)

# TODO(jlebar): Add derivatives.
scatter_min_p = standard_reduction_primitive(
    _scatter_shape_rule, _scatter_dtype_rule, "scatter-min", _scatter_translation_rule
)
batching.primitive_batchers[scatter_min_p] = partial(
    _scatter_batching_rule, scatter_min
)

# TODO(jlebar): Add derivatives.
scatter_max_p = standard_reduction_primitive(
    _scatter_shape_rule, _scatter_dtype_rule, "scatter-max", _scatter_translation_rule
)
batching.primitive_batchers[scatter_max_p] = partial(
    _scatter_batching_rule, scatter_max
)
...
def _scatter_jvp(
    ...
    new_dnums = ScatterDimensionNumbers(
        ...
        scatter_dims_to_operand_dims=tuple(
            d + 1 for d in dnums.scatter_dims_to_operand_dims
        ),
    )
    ...
    g_operand = ad.instantiate_zeros(operand, g_operand)
    g_updates = ad.instantiate_zeros(updates, g_updates)
    masked_g_operand = select(
        eq(scattered_ids, _zeros(scattered_ids)), g_operand, _zeros(g_operand)
    )
    masked_g_updates = select(
        eq(update_ids, gathered_update_ids), g_updates, _zeros(g_updates)
    )

    # d) perform a scatter-add to compute the tangent output.
    tangent_out = scatter_add(
        masked_g_operand, scatter_indices, masked_g_updates, dimension_numbers=dnums
    )
    return val_out, tangent_out


scatter_p = standard_reduction_primitive(
    _scatter_shape_rule, _scatter_dtype_rule, "scatter", _scatter_translation_rule
)
...
def _reduce_translation_rule(
    c, operand, init_value, computation, jaxpr, consts, dimensions, backend=None
):
...
def _reduce_batch_rule(
    batched_args, batch_dims, computation, jaxpr, consts, dimensions
):
    ...
    if init_value_bdim is None:
        ...
        new_operand_bdim = operand_bdim - int(
            onp.sum(onp.less(dimensions, operand_bdim))
        )
        return (
            reduce(operand, init_value, computation, new_dimensions),
            new_operand_bdim,
        )
    else:
        raise NotImplementedError  # loop and stack
...
def _reducer_masking_rule(
    prim, identity, padded_vals, logical_shapes, axes, input_shape
):
...
reduce_p = standard_reduction_primitive(
    _reduce_shape_rule, _input_dtype, "reduce", _reduce_translation_rule
)
...
reduce_sum_p = standard_primitive(
    _reduce_sum_shape_rule, _input_dtype, "reduce_sum", _reduce_sum_translation_rule
)
ad.deflinear(reduce_sum_p, _reduce_sum_transpose_rule)
batching.defreducer(reduce_sum_p)
_masking_defreducer(
    reduce_sum_p, lambda shape, dtype: onp.broadcast_to(onp.array(0, dtype), shape)
)
...
reduce_prod_p = standard_primitive(
    _reduce_prod_shape_rule, _input_dtype, "reduce_prod", _reduce_prod_translation_rule
)
...
_reduce_max_translation_rule = partial(
    _reduce_chooser_translation_rule, max_p, _get_max_identity
)
reduce_max_p = standard_primitive(
    _reduce_chooser_shape_rule, _input_dtype, "reduce_max", _reduce_max_translation_rule
)
...
reduce_min_p = standard_primitive(
    _reduce_chooser_shape_rule, _input_dtype, "reduce_min", _reduce_min_translation_rule
)
...
def _reduce_logical_translation_rule(prim, identity, c, operand, axes):
    ...
    return c.Reduce(
        operand,
        c.Constant(identity(onp.bool_)),
        xla.primitive_computation(prim, scalar, scalar),
        axes,
    )


_reduce_or_translation_rule = partial(
    _reduce_logical_translation_rule, or_p, _get_max_identity
)
reduce_or_p = standard_primitive(
    _reduce_logical_shape_rule,
    _fixed_dtype(onp.bool_),
    "reduce_or",
...
_reduce_and_translation_rule = partial(
    _reduce_logical_translation_rule, and_p, _get_min_identity
)
...
def _reduce_window_shape_rule(
    operand, init_value, jaxpr, consts, window_dimensions, window_strides, padding
):
    if operand.dtype != init_value.dtype:
        msg = (
            "reduce_window got inconsistent dtypes for operand and init_value: "
...
def _reduce_window_translation_rule(
    c,
    operand,
    init_value,
    jaxpr,
    ...
    xla_computation = _reduction_computation(c, jaxpr, backend, consts, init_value)
    return c.ReduceWindow(
        operand, init_value, xla_computation, window_dimensions, window_strides, padding
    )


def _generic_reduce_window_batch_rule(
    batched_args, batch_dims, jaxpr, consts, window_dimensions, window_strides, padding
):
    ...
    return _reduce_window_batch_rule(
        reduce_window, (operand,), (bdim,), window_dimensions, window_strides, padding
    )


reduce_window_p = standard_reduction_primitive(
    _reduce_window_shape_rule,
    _input_dtype,
    "reduce_window",
...
def _reduce_window_sum_shape_rule(
    operand, window_dimensions, window_strides, padding, input_shape
):
    return _common_reduce_window_shape_rule(
        operand, window_dimensions, window_strides, padding
    )
...
def _reduce_window_sum_translation_rule(
    c, operand, window_dimensions, window_strides, padding, input_shape
):
    dtype = c.GetShape(operand).numpy_dtype()
    scalar = xla_client.Shape.array_shape(dtype, ())
    return c.ReduceWindow(
...
def _reduce_window_sum_transpose_rule(
    cotangent, window_dimensions, window_strides, padding, input_shape
):
    ...
    padding_config = [
        (lo, hi, stride - 1) for (lo, hi), stride in zip(pads, window_strides)
    ]
    pad_cotangent = pad(cotangent, _zero(cotangent), padding_config)
    result = _reduce_window_sum(
        pad_cotangent, window_dimensions, ones, xla_client.PaddingType.VALID
    )
    assert result.shape == input_shape
    return [result]
...
def _reduce_window_batch_rule(
    ...
    if bdim is not None:
        window_dimensions = window_dimensions[:bdim] + (1,) + window_dimensions[bdim:]
...
def _reduce_window_chooser_jvp_rule(
    prim, g, operand, window_dimensions, window_strides, padding
):
    assert prim is max_p or prim is min_p
    select_prim = ge_p if prim is max_p else le_p
    return _select_and_gather_add(
        g, operand, select_prim, window_dimensions, window_strides, padding
    )
...
def _common_reduce_window_shape_rule(
    ...
    if len(window_strides) != len(window_dimensions):
        msg = (
            "reduce_window got inconsistent window_strides and "
            "window_dimensions: got window_strides {} and window_dimensions {}."
        )
        raise TypeError(msg.format(window_strides, window_dimensions))

    return reduce_window_shape_tuple(
        operand.shape, window_dimensions, window_strides, padding
    )


def reduce_window_shape_tuple(
    operand_shape, window_dimensions, window_strides, padding
):
    ...
    t = (
        onp.floor_divide(
            onp.subtract(operand_padded, window_dimensions), window_strides
        )
        + 1
    )
    return tuple(t)
...
def _select_and_scatter_shape_rule(
    ...
    if len(window_dimensions) != len(window_strides):
        msg = (
            "select_and_scatter got inconsistent window_strides and "
            "window_dimensions: got window_strides {} and window_dimensions {}."
        )
        raise TypeError(msg.format(window_strides, window_dimensions))
    return operand.shape
...
def _select_and_scatter_translation(
    ...
    scatter = _reduction_computation(
        c, scatter_jaxpr, backend, scatter_consts, init_value
    )
    return c.SelectAndScatter(
        operand,
        select,
        window_dimensions,
        window_strides,
...
select_and_scatter_p = standard_reduction_primitive(
    _select_and_scatter_shape_rule,
    _input_dtype,
    "select_and_scatter",
...
def _select_and_scatter_add_translation(
    c, source, operand, select_prim, window_dimensions, window_strides, padding
):
    dtype = c.GetShape(operand).numpy_dtype()
    scalar = xla_client.Shape.array_shape(dtype, ())
...
def _select_and_scatter_add_jvp(
    primals, tangents, select_prim, window_dimensions, window_strides, padding
):
    ...
    if g_source is ad_util.zero:
        ...
        tangent_out = _select_and_scatter_add(
            g_source, operand, select_prim, window_dimensions, window_strides, padding
        )
    return val_out, tangent_out


def _select_and_scatter_add_transpose(
    t, source, operand, select_prim, window_dimensions, window_strides, padding
):
    assert source is ad.undefined_primal and operand is not ad.undefined_primal
    source_t = _select_and_gather_add(
        t, operand, select_prim, window_dimensions, window_strides, padding
    )
...
select_and_scatter_add_p = standard_primitive(
    _select_and_scatter_add_shape_rule,
    _input_dtype,
    "select_and_scatter_add",
    _select_and_scatter_add_translation,
)
ad.primitive_transposes[select_and_scatter_add_p] = _select_and_scatter_add_transpose
...
_UINT_DTYPES = {16: onp.uint16, 32: onp.uint32, 64: onp.uint64}
...
def _select_and_gather_add_translation(
    ...
    const = lambda c, dtype, x: c.Constant(
        onp.array(x, dtype=dtype), canonicalize_types=False
    )
    ...
    if double_word_reduction:
        ...
        # Unpacks the second element of a tuple.
        def snd(t):
            return c.BitcastConvertType(
                c.ShiftLeft(t, const(c, word_dtype, r_nbits)), etype
            )
...
def _select_and_gather_add_transpose(
    t, tangents, operand, select_prim, window_dimensions, window_strides, padding
):
    assert tangents is ad.undefined_primal and operand is not ad.undefined_primal
    result = _select_and_scatter_add(
        t, operand, select_prim, window_dimensions, window_strides, padding
    )
    return [result, None]
...
select_and_gather_add_p = standard_primitive(
    _select_and_gather_add_shape_rule,
    _input_dtype,
    "select_and_gather_add",
    _select_and_gather_add_translation,
)
ad.primitive_jvps[select_and_gather_add_p] = _select_and_gather_add_jvp
ad.primitive_transposes[select_and_gather_add_p] = _select_and_gather_add_transpose
...
def _sort_key_val_transpose_rule(t, keys, values, dimension):
    ...
    values_result = (
        sort_key_val(perm, t_values)[1] if values is ad.undefined_primal else None
    )
...
def _sort_key_val_batch_rule(batched_args, batch_dims, dimension):
    ...
    if keys_bdim == values_bdim:
        ...
        return (
            sort_key_val(keys_trans, values, new_dimension),
            (values_bdim, values_bdim),
        )
...
batching.primitive_batchers[shaped_identity_p] = lambda a, d, shape: (
    shaped_identity(a[0]),
    d[0],
)
...
class _IotaConstant(xla.DeviceConstant):
    ...
    def _value(self):
        if self._npy_value is None:
            ...
            iota = iota.reshape(
                [
                    self.shape[self.axis] if i == self.axis else 1
...
class _EyeConstant(xla.DeviceConstant):
    ...
    def _value(self):
        if self._npy_value is None:
            ones = [1] * self.ndim
            iotas = [
                onp.arange(self.shape[axis]).reshape(subvals(ones, [(axis, -1)]))
    ...
    def constant_handler(c, diag_const, canonicalize_types=True):
        ...
        etype = xla_bridge.dtype_to_etype(diag_const.dtype)
        iotas = [
            c.BroadcastedIota(onp.uint32, diag_const.shape, axis)
...
def padtype_to_pads(in_shape, window_shape, window_strides, padding):
    ...
    if padding == PaddingType.SAME:
        ...
        pad_sizes = [
            _max((out_size - 1) * stride + window_shape - in_size, 0)
            for out_size, stride, window_shape, in_size in zip(
                out_shape, window_strides, window_shape, in_shape
            )
        ]
...
def _check_same_dtypes(name, ignore_fp_precision, *dtypes):
    ...
    if len({xla_bridge.canonicalize_dtype(t) for t in dtypes}) != 1:
        if ignore_fp_precision:
            msg = (
                "{} requires arguments to have same dtypes up to floating point "
                "precision, got {}."
            )
        else:
...
def conv_transpose_shape_tuple(
    ...
    unpad_out_space = [
        (i - 1) * s - k + 2
        for i, k, s in zip(lhs_trans[2:], rhs_trans[2:], window_strides)
    ]
...
def _check_shapelike(fun_name, arg_name, obj):
    ...
    if (
        type(obj) is masking.ShapeExpr
        or type(obj) is tuple
        and any(type(d) is masking.Poly for d in obj)
    ):
        return obj
...
def _dynamic_slice_indices(operand, start_indices):
    if not isinstance(start_indices, (tuple, list)):
        ...
        start_indices = [
            reshape(slice(start_indices, [i], [i + 1]), ()) for i in range(operand.ndim)
        ]
    else:
        start_indices = [
            onp.asarray(i) if isinstance(i, int) else i for i in start_indices
        ]
...
def conv_general_permutations(dimension_numbers):
    ...
    for i, (a, b) in enumerate(charpairs):
        if not dimension_numbers[i].count(a) == dimension_numbers[i].count(b) == 1:
            msg = (
                "convolution dimension_numbers[{}] must contain the characters "
                "'{}' and '{}' exatly once, got {}."
            )
            raise TypeError(msg.format(i, a, b, dimension_numbers[i]))
        if len(dimension_numbers[i]) != len(set(dimension_numbers[i])):
            msg = (
                "convolution dimension_numbers[{}] cannot have duplicate "
                "characters, got {}."
            )
            raise TypeError(msg.format(i, dimension_numbers[i]))
    if not (
        set(lhs_spec) - set(lhs_char)
        == set(rhs_spec) - set(rhs_char)
        == set(out_spec) - set(out_char)
    ):
        msg = (
            "convolution dimension_numbers elements must each have the same "
...
def _conv_general_vjp_lhs_padding(
    ...
    pad_after = (
        onp.add(lhs_dilated_shape, rhs_dilated_shape)
        - 1
        - out_dilated_shape
        - pad_before
    )
...
def _balanced_eq(x, z, y):
    return div(
        select(_eq_meet(x, z), _ones(z), _zeros(z)),
...
def _check_user_dtype_supported(dtype, fun_name=None):
    if dtype is not None and onp.dtype(dtype) != xla_bridge.canonicalize_dtype(dtype):
        msg = (
            "Explicitly requested dtype {} {} is not available, "
