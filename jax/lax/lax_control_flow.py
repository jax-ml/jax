...
def fori_loop(lower, upper, body_fun, init_val):
  ...
  """
    _, _, result = while_loop(
        _fori_cond_fun, _fori_body_fun(body_fun), (lower, upper, init_val)
    )
    return result
...
def _while_loop_translation_rule(c, axis_env, *args, **kwargs):
    ...
    if batched:
        ...
        pred = cond_c.Reduce(
            pred,
            cond_c.Constant(onp.array(False)),
            or_,
...
def _while_loop_batching_rule(
    args, dims, cond_nconsts, cond_jaxpr, body_nconsts, body_jaxpr
):
    size, = {x.shape[d] for x, d in zip(args, dims) if d is not batching.not_mapped}
    orig_batched = [d is not batching.not_mapped for d in dims]
    cconst_bat, bconst_bat, init_bat = split_list(
        orig_batched, [cond_nconsts, body_nconsts]
    )
...
def cond(pred, true_operand, true_fun, false_operand, false_fun):
  ...
  """
    ...
    false_jaxpr, false_consts, out_tree2 = _initial_style_jaxpr(
        false_fun, false_tree, false_avals
    )
    if out_tree != out_tree2:
        msg = (
            "true_fun and false_fun outputs must have identical tree structure, "
            "got {} and {}."
        )
        raise TypeError(msg.format(out_tree, out_tree2))
    if not all(_map(typematch, true_jaxpr.out_avals, false_jaxpr.out_avals)):
        msg = (
            "true_fun and false_fun outputs must have identical types, "
            "got {} and {}."
        )
...
def _cond_batching_rule(
    args, dims, true_jaxpr, false_jaxpr, true_nconsts, false_nconsts
):
    ...
    _, false_out_bat = batching.batch_jaxpr(
        false_jaxpr, size, fconst_bat + f_bat, False
    )
    out_bat = [a or b for a, b in zip(true_out_bat, false_out_bat)]

    true_jaxpr_batched, _ = batching.batch_jaxpr(
        true_jaxpr, size, tconst_bat + t_bat, out_bat
    )
    false_jaxpr_batched, _ = batching.batch_jaxpr(
        false_jaxpr, size, fconst_bat + f_bat, out_bat
    )
    ...
    if pred_bat:
        ...
        return (
            [_cond_pred_bcast_select(pred, t, f) for t, f in zip(true_out, false_out)],
            [0] * len(true_out),
        )
    else:
        ...
        return (
            cond_p.bind(
                *itertools.chain(
                    [pred], true_consts, true_ops, false_consts, false_ops
                ),
...
def scan(f, init, xs):
  ...
  """
    ...
    out = scan_p.bind(
        *itertools.chain(consts, in_flat),
        forward=True,
        length=length,
        jaxpr=jaxpr,
...
def _scan_partial_eval(trace, *tracers, **kwargs):
    ...
    linear_2 = [lin or not uk for uk, lin in zip(unknowns, linear)] + [False] * len(
        residual_tracers
    )
    eqn = pe.new_jaxpr_eqn(
        new_tracers + residual_tracers,
        out_tracers,
        scan_p,
        (),
        dict(
...
def _transpose_jaxpr(num_c, num_res, jaxpr):
    ...
    def transposed(*cbar_bbar_res):
        ...
        c_bar = _map(
            ad.instantiate_zeros_aval, c_avals, _map(ad.add_tangents, c_bar, new_c_bar)
        )
        return c_bar + a_bar
...
def _scan_batching_rule(
    args, dims, forward, length, jaxpr, num_consts, num_carry, linear
):
    num_ys = len(jaxpr.out_avals) - num_carry
    size, = {x.shape[d] for x, d in zip(args, dims) if d is not batching.not_mapped}
    orig_batched = [d is not batching.not_mapped for d in dims]
    ...
    new_init = [
        ...
        for x, d, was_batched, now_batched in zip(
            init, init_bdims, init_batched, carry_batched
        )
    ]
    new_xs = [
...
def _scan_polymorphic_shape_rule(
...
def _scan_masking_rule(
    ...
    out_shape = _scan_polymorphic_shape_rule(
        shape_exprs, forward, length, jaxpr, num_consts, num_carry, linear
    )
...
def _masked_scan_jaxpr(jaxpr, num_consts, num_carry):
    ...
    def masked(*args):
        ...
        new_carry = [
            lax.select(i < dynamic_length, new_c, c)
            for new_c, c in zip(new_carry, carry)
        ]
        return [i + 1] + new_carry + ys

    aval = ShapedArray((), onp.int64)
    const_avals, carry_avals, x_avals = split_list(
        jaxpr.in_avals, [num_consts, num_carry]
    )
    return _make_typed_jaxpr(
        masked, [aval] + const_avals + [aval] + carry_avals + x_avals
    )
...
def scan_bind(*args, **kwargs):
    ...
    consts_avals, init_avals, x_avals = split_list(
        jaxpr.in_avals, [num_consts, num_carry]
    )
...
def _concat_masking_rule(padded_vals, logical_shapes, dimension, operand_shapes):
    ...
    for padded_val, logical_shape in zip(padded_vals, logical_shapes):
        result = _memcpy(
            dimension, logical_shape[dimension], padded_val, result, offset
        )
...
def _flatten_higher_order_func(f, tree, error_template="Expected {}, got {}"):
...
def _root_tree_error_template(func_name):
    return (
        func_name
        + "() output pytree structure must match initial_guess, "
        + "got {} and {}."
    )
...
def root(f, initial_guess, solve, tangent_solve):
  ...
  """
    ...
    out_flat = root_p.bind(
        *itertools.chain(consts, guess_flat),
        num_consts=len(consts),
        jaxpr=jaxpr,
        solve=solve_flat,
