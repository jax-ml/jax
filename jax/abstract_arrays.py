...
def make_shaped_array(x):
  ...
  return ShapedArray(np.shape(x), dtype, weak_type=dtypes.is_weakly_typed(x))

def zeros_like_array(x):
  dtype = dtypes.canonicalize_dtype(dtypes.result_type(x))
  return zeros_like_shaped_array(ShapedArray(np.shape(x), dtype, weak_type=dtypes.is_weakly_typed(x)))
