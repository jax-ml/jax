...
"""Utilities for building JAX-related Python packages."""
...
def copy_file(
    ...
    for src_file in src_files:
        ...
        shutil.copyfile(src_file_rloc, dst_file) if is_windows() else shutil.copy(src_file_rloc, dst_file)


def platform_tag(cpu: str) -> str:
    """Return the platform tag based on the system and CPU architecture."""
...
def build_wheel(
    ...
    """Build a wheel in `output_path` using the source tree in `sources_path`."""
    env = dict(os.environ)
    if git_hash:
    ...
    for wheel in glob.glob(os.path.join(sources_path, "dist", "*.whl")):
        ...
        sys.stderr.write(f"To install the newly-built {package_name} wheel on hermetic Python, run:\n")
        sys.stderr.write(f'  echo -e "\\n{output_file}" >> build/requirements.in\n')
        sys.stderr.write(f"  bazel run //build:requirements.update --repo_env=HERMETIC_PYTHON_VERSION={py_version}\n\n")
...
def update_setup_with_cuda_version(file_dir: pathlib.Path, cuda_version: str):
    """Update the setup.py file with the specified CUDA version."""
