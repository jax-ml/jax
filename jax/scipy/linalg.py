...
def cho_solve(c_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    if not (
        c_ndims >= 2
        and c_shape[-1] == c_shape[-2]
        and (c_ndims == b_ndims or c_ndims == b_ndims + 1)
    ):
        msg = (
...
def _lu_solve(lu, pivots, b, trans):
    ...
    if len(lu_shape) != 2 or lu_shape[0] != lu_shape[1]:
        raise ValueError(
            "LU decomposition must be a square matrix, got shape {}".format(lu_shape)
        )
    if len(b_shape) < 1:
    ...
    if b_shape[0] != lu_shape[0]:
        raise ValueError(
            "Dimension of LU decomposition matrix (shape {}) must "
