...
if __name__ == '__main__':
    ...
    a = jax.random.normal(jax.random.key(0), (1, BLOCK, N_BLOCKS), dtype=jnp.float32)
    b = jax.random.normal(jax.random.key(1), (1, BLOCK, N_BLOCKS), dtype=jnp.float32)
    ...
    # ── SAXPY ─────────────────────────────────────────────────────────
    x = jax.random.normal(jax.random.key(2), (1, BLOCK, N_BLOCKS), dtype=jnp.float32)
    y = jax.random.normal(jax.random.key(3), (1, BLOCK, N_BLOCKS), dtype=jnp.float32)
    alpha = 2.5
    call = cjax.cutlass_call(
        launch_saxpy,
    ...
    x = jax.random.normal(jax.random.key(4), (N_ELEM,), dtype=jnp.float32)
    call = cjax.cutlass_call(
        launch_relu,
        output_shape_dtype=jax.ShapeDtypeStruct(x.shape, x.dtype),
        N=N_ELEM,
    ...
    # ── Fused Bias + ReLU ─────────────────────────────────────────────
    ROWS, COLS = 16, 64
    x = jax.random.normal(jax.random.key(5), (ROWS * COLS,), dtype=jnp.float32)
    bias = jax.random.normal(jax.random.key(6), (COLS,), dtype=jnp.float32)
    call = cjax.cutlass_call(
    ...
    # ── GEMM ──────────────────────────────────────────────────────────
    M, N, K = 128, 128, 64
    A = jax.random.normal(jax.random.key(7), (M * K,), dtype=jnp.float32)
    B = jax.random.normal(jax.random.key(8), (K * N,), dtype=jnp.float32)
    call = cjax.cutlass_call(
        launch_gemm,
    ...
    # ── Elementwise Add (2-D) ─────────────────────────────────────────
    M, N = 16, 256
    a = jax.random.normal(jax.random.key(9), (M, N), dtype=jnp.float32)
    b = jax.random.normal(jax.random.key(10), (M, N), dtype=jnp.float32)
    call = cjax.cutlass_call(
