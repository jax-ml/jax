...
id: f-6sxUlqrlBn
outputId: f55396c9-c241-43fe-d399-762ef1076122
---
import jax
...
colab:
  base_uri: https://localhost:8080/
id: OJFnf7FGp6Lj
outputId: 0fbf5f62-71e9-47f9-ec76-b3ef944abfec
---
# Create a 4x8 array
...
def func(x):
    ...
    x = with_sharding_constraint(x, PartitionSpec('x'))
    return x + 1
```
...
input = jnp.ones((1000, 1000,), dtype=jnp.float32) * 0.0001  # Input matrix: 1000 x 1000
w1 = jnp.ones((10, 1000, 8000), dtype=jnp.float32) * 0.0001 # 10 layers of 1000 x 8000 matrices
w2 = jnp.ones((10, 8000, 1000), dtype=jnp.float32) * 0.0001 # 10 layers of 8000 x 1000 matrices
...
def layer(x, w):
  # Move model parameters w1 and w2 to the device via device_put
...
w1 = jax.device_put(w1, s_host)
w2 = jax.device_put(w2, s_host)

# Compile and compute gradients of the scanned function
f = jax.jit(jax.grad(scanned))  # Apply JIT compilation to gradient computation
result_both = f((w1, w2), input) # Execute with both activation and parameter offloading
...
For device memory analysis, refer to {ref}`device_memory_profiling`. The profiling tools described in {ref}`profiling` can help measure memory savings and performance impact from host offloading.
