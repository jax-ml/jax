...
This JAX Enhancement Proposal (or [JEP](https://docs.jax.dev/en/latest/jep/index.html)) proposes the introduction of a Stateful PRNG API into {mod}`jax.random`.
...
With this implementation exposed in the {mod}`jax.random` namespace, usage is virtually identical to that of {func}`numpy.random.default_rng`:

```python
>>> import jax
>>> rng = jax.random.stateful_rng(1701)
>>> rng.random((5,))
...
Because the statefulness in {class}`jax.random.StatefulPRNG` is tracked via mutable refs, the random state will correctly update even if the generator is used in transformations like {func}`jax.jit`, which typically require pure functional semantics.
...
**Refs cannot be return values:** Mutable refs cannot be present in the return values of transformed JAX functions, and the proposed stateful RNG object would inherit this limitation. This is also an explicit limitation: attempting to return a `random.StatefulPRNG` from a transformed function would lead to an explicit error.

## Evaluation

Our judgment is that the advantages of the stateful PRNG API outweigh the limitations, and that a fully-implemented version of {func}`jax.random.stateful_rng` and {class}`jax.random.StatefulRNG` should be added to the public API, with aliases in a new {mod}`jax.numpy.random` module replicating the {func}`numpy.random.default_rng` API.
