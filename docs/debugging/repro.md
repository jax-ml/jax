...
```
   ...
   try:
     ...
     repro.save(repro_source, "jax_repro")  # Will log the path to the repro
```
...
There are a few known differences though:
  ...
  * JAX cache tracing is foiled, so you are going to see functions traced
    multiple times.
    E.g., JAX will normally avoid tracing a function repeatedly with similar
    arguments, but when repros are enabled this cache is disabled.
...
There are two phases: repro collection, when we intercept certain function calls
...
its own USER functions among arguments. This is the
...
on the shapes and types of the arguments, static arguments, etc.
...
Furthermore, because `jax_jit_call` will wrap the first argument
