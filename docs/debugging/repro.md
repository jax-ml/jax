...
IF JAX encounters an uncaught exception under a JAX API call, e.g., `jax.jit`,
and the `JAX_REPRO_DIR` is set to a directory,  JAX will save in that directory
a Python source file that when executed should
reproduce the error.
That source file is standalone except for importing `jax`.

When `JAX_REPRO_DIR` is set, JAX tracks the sequence of nested JAX API calls,
capturing all user-functions that JAX traces, their nested calls to JAX APIs,
and so on.
In large JAX programs we have seen the nested sequence of JAX API calls
grow to a depth of 30.
If an uncaught exception arises then we save a repro that should result in
the same API call tree up to the point of the exception.
If this exception is coming from JAX (tracing, lowering, compilation, or
execution), then the saved repro will hopefully reproduce the error.
We save a repro even if the uncaught exception is from the user program, but
in that case the saved repro will only indicate where the error appeared, but
won't reproduce the error, because the user program is staged out.
One can get the path and source code of the saved repro in the current
thread by calling `repro.last_saved_repro()`.

The above use case is the "implicit" repro generation, when the program
encounters an uncaught exception. The only thing you need to do is to set
`JAX_REPRO_DIR`
.
You can also generate repros "explicitly", even in absence of errors,
by using the `repro.collect_and_save` function transformation:

```
fn = repro.collect_and_save(fn, repro_name_prefix="name")
# Once we trace `fn` we save `$JAX_REPRO_DIR/name_<counter>.py` even if there
# is no uncaught exception.
# `repro.last_saved_repro()` returns the path and source code of the saved repro.
```

There is an even lower-level explicit-mode API:
```
col = repro.collector(fn)
try:
  return col(*args, **kwargs)
finally:
  source = col.source()
  repro.save(source, repro_name_prefix="name")
  if col.deferred_error is not None:  # If use use error_mode="defer"
    raise col.deferred_error
```

There is a shortcut for the above:
```
col_fn = repro.collect_and_save(fn, repro_name_prefix="name)
col_fn(*args, **kwargs)
```

You should use the `repro.collector` mechanism when:
  * you want direct access to the generated repro source, perhaps without
    even saving it.
  * you want to generate multiple versions of the source for different
    source emitting strategies. This is used by the repro reducer mechanism.
...
`jax.jit(f).trace(*args).jaxpr` (or the old `jax.make_jaxpr`), except that:
  ...
  * it works even if there are errors encountered before a Jaxprs is produced,
    e.g., for JAX tracing errors. The repro source may reproduce the error.
  * the repro is higher-level than the Jaxpr. E.g., instead of seeing the
    ...
    often have complicated parameters, and sometimes even have references to
    Python callables.
 ...
 * `JAX_REPRO_DIR` denotes the directory where reproducers are saved. A
   non-empty value also triggers the collection of the call tree, so that a
   reproducer is saved on error. The value can be `sponge` for use in internal
   Google tests.
 * `JAX_REPRO_FLAGS` contains comma-separated flags that configure details of
    ...
    * `log_calls` (default 0). An integer value that controls the repro
      collection logging (for debugging the repro internals). The recognized
      values are: 0 (no logging, default), 1 (log all calls except the JAX
      primitive.bind), 2 (log all calls).
    * `log_calls_details` (default ""). A sequence of call ids for which to log
      more details (for debugging the repro internals). E.g.,
      `log_calls_details=3+5+6`.
    * `error_mode` (default "log"). Configures the handling of repro
      collection and generation errors and warnings. The possible values are:
       * "ignore" -- the errors and warnings are not even logged.
       * "log" -- the errors and warnings are logged with up to
         `log_traceback_frames` stack frames, or until the nearest enclosing
         `jax.jit`.
         This is the least intrusive error mode but you'd have to look at the
         logs to see is anything went wrong.
      * "defer" -- like the "log" mode, but the at the end of repro tracking
         for a top-level call, if there were deferred errors then a repro source
         is saved and a `repro.ReproError` exception is raised. This happens
         in implicit mode. When using a collector it is the user's
         responsibility to check `collector.deferred_error`.
      * "raise_tracking" -- if there are errors during repro tracking raise
         a `repro.ReproError`. This gives you the full stack trace at the
         time of the error. If there are errors during repro emitting then
         they are deferred (as for the "defer" mode).
       * "raise" -- like "raise_tracking" but also raises eagerly a
         `repro.ReproError` during repro emitting. This is probably less useful
         because for errors during repro emitting, this prevents a repro
         source being saved and also because the stack trace at the time of
         the emit error is entirely in the repro emitting code.
    ...
    * `save_repro_on_success` (default False) controls the saving of a repro
      at the end of a successful top-level JAX API call.
    * `save_repro_on_uncaught_exception` (default True) controls the saving
      of a repro at the end of a top-level JAX API call with an uncaught
      exception.
...
In the simplest cases, the arguments of `FunctionDef` are tracers which are
arguments to JAX primitive calls that produce tracers. So, we only need to
worry about following the flow of tracers. We do this by maintaining an
environment that maps tracers by their `id()` to a variable name.

A more complicated situation is when the arguments are PyTress of tracers.
In that case we define in the environment accessors for all the leaves.

One special case is for `TransformedRef`. These objects have some internal
structure that can be accessed via properties. Operations such as reshape
and slicing are not performed with JAX primitives, but by transforming their
internal structure. We handle these by defining accessors for the different
parts of the internal structure, similar to the case of PyTrees.
