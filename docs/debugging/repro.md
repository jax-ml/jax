...
Do you believe that there is a small JAX program, without additional libraries,
...
you can set `JAX_REPRO_DIR` to a directory where JAX will attempt to save a Python source
...
One can get the path and source code of the saved repro by calling repro.last_repro().
...
In the implicit use, successful JAX calls at the top-level are not retained.
...
(or the old `jax.make_jaxpr`), except that:
  ...
  * the repro is higher-level than the Jaxpr: the higher-order Jaxpr primitives
    ...
    you will see a call to `lax.scan`.
