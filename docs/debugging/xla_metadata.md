...
1. [Tag an individual operation by wrapping its output value](#individual-operations)
2. [Tag a block of operations using a context manager](#context-manager)
...
# Usage
## Tagging Individual Operations <a id="individual-operations"></a>
...
```
## Tagging a Block of Code with a Context Manager <a id="context-manager"></a>
...
*   **Forward-pass only:** Metadata is not currently automatically propagated to gradients <u>when tagging individual operations</u> in the backward pass. A `custom_vjp` must be used in order to tag gradients in this case. See [above](#custom-vjp) for example.
