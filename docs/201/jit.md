...
({ref}`jax-101-tracing`) and hands the whole jaxpr to
[XLA](https://www.openxla.org/xla/), which compiles it
...
([SELU](https://proceedings.neurips.cc/paper/6698-self-normalizing-neural-networks.pdf)),
...
3. We timed the compiled version. Subsequent calls with the same input types
   never run the Python function `selu` at all; they go straight to the
   compiled executable.
...
A jaxpr records the function *as executed on the JAX types it was traced with*.
Python-level control flow and side effects are resolved during tracing. If
a function branches on a static property like rank, the jaxpr knows only the
...
For a rank-1 input, the compiled function is just the identity. That's fine,
because the cached executable is only reused for inputs of the same JAX type; a
rank-2 input triggers a fresh trace that takes the other branch.
...
survives compilation, use {func}`jax.debug.print`; for general side effects at
a performance cost, see {doc}`callbacks`. To check for leaked tracers from
side effects, use {func}`jax.check_tracer_leaks`.
...
*inside* compiled code, see {doc}`control-flow`.
...
The compiled executable is specialized to the *value* of each static argument,
so JAX recompiles whenever a static argument takes a value it hasn't seen.
Marking an argument static is only a good strategy when the function will see a
limited set of values for it. Static argument values must be hashable, since
they become part of the cache key.

## JIT and caching

The first call to a jitted function performs tracing and compilation.
Subsequent applications of the function to the same input types will instead
use cached compilations. Understanding how the compilation cache works is
important: several common patterns silently defeat it.
...
- the object identity of the function `g` itself.
...
problem, {doc}`slow-compilation` is a field guide to diagnosing it.
...
The persistent compilation cache ({ref}`jax-501-compilation-cache`) keeps
compiled executables across process restarts; in the other direction,
{func}`jax.clear_caches` drops every cached trace and executable.
...
If your class is a dataclass, {func}`jax.tree_util.register_dataclass` gets you
the same result with much less ceremony; see {ref}`jax-101-custom-pytrees`.
...
A ref ({ref}`jax-101-refs`) passed into a jitted function represents mutable
memory: writes through it happen in place, and no fresh buffer is allocated. We
can watch the buffer address stay fixed across the call:
...
Working with refs does mean writing the function differently: mutation through
indexed writes, rather than functional updates. For code already written
functionally (which is most existing JAX code, and the convention across the
ecosystem), buffer donation can achieve the in-place effect without changing
the function.
