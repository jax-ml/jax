...
import jax
jax.config.update('jax_num_cpu_devices', 8)  # simulate 8 devices, for the sharding section
```
...
This can make `debug.callback` more useful for general-purpose debugging than
either `pure_callback` or `io_callback`. It's covered in more depth, along
with `jax.debug.print`, in {doc}`debugging`.
...
gathered onto a single device, and the callback runs once, on that device's
host, with the full global value:

```{code-cell}
def host_fn(x):
  print(f"callback received shape {x.shape}")
  return np.asarray(x) * 2

def f(x):
  return jax.pure_callback(host_fn, jax.ShapeDtypeStruct.like(x), x)

mesh = jax.make_mesh((8,), ('i',))

with jax.set_mesh(mesh):
  x = jax.device_put(jnp.arange(8.0), jax.P('i'))
  out = jax.jit(f)(x)  # one callback, with the full gathered array
out
```
...
```{code-cell}
with jax.set_mesh(mesh):
  out = jax.shard_map(f, in_specs=jax.P('i'), out_specs=jax.P('i'))(x)  # one callback per device
out
```
