...
XLA implements SPMD-based pipeline parallelism. This is a scaling technique
where the forward and backward pass are split into multiple pipeline stages.
Each device (or device group) processes the partial result of the previous
...
when the wall time of the computation is larger than communication. At compile
time, the operations will be rearranged to overlap communication with
computation as much as possible.

For an optimized schedule, we recommend these flags:
```
--xla_gpu_enable_latency_hiding_scheduler=true
