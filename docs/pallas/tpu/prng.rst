...
PRNG keys can be generated inside of the kernel using :func:`jax.random.key`. However, the more likely scenario is that a key will be passed into the kernel from the caller. In such a case, the key can be passed into the kernel by unwrapping the key using :func:`jax.random.key_data` and passing the key in via VMEM:

.. code-block:: python

  def body(key_ref, o_ref):
    # Key is passed in as a VMEM Ref - we "wrap" the array back to a key.
