...
All our benchmarks here use arrays with iid normal float16 entries.
```
...
Full implementations can be found in [our test file](https://github.com/jax-ml/jax/blob/main/tests/pallas/mgpu_examples_test.py).
...
transforms to get the SMEM data format to match [what's expected by MMA instructions](./reference.md#memory-space-a-b-mma).

The kernel implementation itself is relatively short. The first part sets up
a compute/memory pipeline using {py:func}`plgpu.emit_pipeline <jax.experimental.pallas.mosaic_gpu.emit_pipeline>`. At each step, the compute function (`do_mma`) consumes a
...
from TMEM, write it to SMEM ([remembering to call `plgpu.commit_smem`](./reference.md#commit-smem)),
...
our arithmetic intensity are [collective MMAs](./reference.md#collective-mma).
...
As the comment indicates, since [TMEM loads are asynchronous](./reference.md#tmem-loads),
