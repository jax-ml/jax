...
def gaussian_kl(mu, sigmasq):
    ...
    return -0.5 * np.sum(1.0 + np.log(sigmasq) - mu ** 2.0 - sigmasq)
...
def bernoulli_logpdf(logits, x):
    ...
    return -np.sum(np.logaddexp(0.0, np.where(x, -1.0, 1.0) * logits))
...
def image_sample(rng, params, nrow, ncol):
    ...
    sampled_images = random.bernoulli(img_rng, np.logaddexp(0.0, logits))
...
def image_grid(nrow, ncol, imagevecs, imshape):
    ...
    return np.vstack(
        [np.hstack([next(images).T for _ in range(ncol)][::-1]) for _ in range(nrow)]
    ).T
...
decoder_init, decode = stax.serial(Dense(512), Relu, Dense(512), Relu, Dense(28 * 28))
