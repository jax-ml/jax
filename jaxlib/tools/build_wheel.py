...
_MAIN_RUNFILES_DIR = "__main__/"


def _get_file_path(src_file, runfiles=None, transitive_sources=None):
  if runfiles:
    return runfiles.Rlocation(src_file)
  elif transitive_sources:
    return build_utils.find_transitive_source_file(
        src_file.replace(_MAIN_RUNFILES_DIR, ""), transitive_sources
    )
  else:
    raise RuntimeError(
...
def patch_copy_mlir_import(
    src_file, dst_dir, runfiles=None, transitive_sources=None
):
  src_file = _get_file_path(src_file, runfiles, transitive_sources)
...
def patch_copy_xla_extension_stubs(
    dst_dir, runfiles=None, transitive_sources=None
):
...
def verify_mac_libraries_dont_reference_chkstack(
    runfiles=None, transitive_sources=None
):
  ...
  file_path = _get_file_path(
      "__main__/jaxlib/xla/xla_extension.so", runfiles, transitive_sources
  )
  nm = subprocess.run(
      ["nm", "-g", file_path],
      capture_output=True,
...
def prepare_wheel(wheel_sources_path: pathlib.Path, *, cpu, transitive_sources):
  ...
  if transitive_sources:
    ...
    runfiles = r
    source_file_prefix = _MAIN_RUNFILES_DIR
  ...
  verify_mac_libraries_dont_reference_chkstack(
      runfiles=runfiles, transitive_sources=transitive_sources
  )
  copy_files(
  ...
  copy_files(
      ...
      src_files=[
          ...
          f"{source_file_prefix}jaxlib/xla/xla_extension.{pyext}",
      ],
  )
  ...
  patch_copy_xla_extension_stubs(
      jaxlib_dir, runfiles=runfiles, transitive_sources=transitive_sources
  )
  ...
  if build_utils.is_windows():
    capi_so = f"{source_file_prefix}jaxlib/mlir/_mlir_libs/jaxlib_mlir_capi.dll"
  else:
    so_ext = "dylib" if _is_mac() else "so"
    capi_so = f"{source_file_prefix}jaxlib/mlir/_mlir_libs/libjaxlib_mlir_capi.{so_ext}"
  ...
  copy_files(
      ...
      src_files=[
          ...
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlir.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_chlo.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlirHlo.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlirDialectsSparseTensor.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlirSparseTensorPasses.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mosaic_gpu_ext.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_tpu_ext.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_sdy.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_stablehlo.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/register_jax_dialects.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlirDialectsGPU.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlirDialectsLLVM.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlirDialectsNVGPU.{pyext}",
          f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_mlirGPUPasses.{pyext}",
      ]
      + (
          []
          if build_utils.is_windows()
          else [
              f"{source_file_prefix}jaxlib/mlir/_mlir_libs/_triton_ext.{pyext}",
