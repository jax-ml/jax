  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (element_type.isFloat() && element_type.getIntOrFloatBitWidth() <= 8) {
      ...
      if (auto cast_op =
        ...
        auto expected_ty = mlir::VectorType::get(
            op.getSource().getType().getShape(), element_type);
        if (cast_op->getNumOperands() != 1 ||
            cast_op->getOperand(0).getType() != expected_ty) {
          return rewriter.notifyMatchFailure(
              op, "expected a single integer-vector operand type");
        }
        source = cast_op->getOperand(0);
      }
    }
