...
// 1. It lowers gpu.launch_func into a call to mosaic_gpu_build_kernel_spec,
//    which fills in a MosaicKernelSpec (see launch_config.h).
//    The spec is an out-param passed as an extra first argument to the
//    enclosing host function.
...
// and keeps the resulting kernel handle. At execution time it calls the host
// function to obtain the kernel spec, and decides how to run it: either an
// immediate cuLaunchKernelEx, or recording it as a command for a command
// buffer.
...
// operand, 0 for pointers and byte size for non-pointer arguments.
    ...
    for (mlir::Operation& op : *module.getBody()) {
      if (auto func = mlir::dyn_cast<mlir::func::FuncOp>(&op)) {
        ...
        auto result = getOperation()->walk([&](mlir::gpu::LaunchFuncOp launch)
          ...
          // Drop the stream argument if it exists.
          // Remove this block once min supported JAX version is 0.12.0
          // See core.py#_lower_as_gpu_kernel for details.
          if (func.getNumArguments() > 0 &&
              launch.getAsyncObject() == func.getArgument(0)) {
            launch.getAsyncObjectMutable().clear();
            if (func.eraseArgument(0).failed()) {
              return mlir::WalkResult::interrupt();
            }
          }

          // Add a new function argument for pointer to MosaicKernelSpec.
          if (func.insertArgument(0, ptr_ty,
