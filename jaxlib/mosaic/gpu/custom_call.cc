...
mlir::FailureOr<mlir::OpPassManager> GetPassPipeline(
  ...
  std::string cuda_root =
      std::string(tsl::io::Dirname(tsl::io::Dirname(ptxas_path.value())));
  std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!cuda_root: " << cuda_root << std::endl;
  return mlir::parsePassPipeline(absl::StrCat(
      ...
      // R"(
      //     builtin.module(
      //         func.func(nvvm.attach.target, cse),
      //         nvvm.target(
      //           chip=)" +
      //     sm + " features=" + ptx_isa +
      //     R"( triple=nvptx64-nvidia-cuda O=3,
      //           fast-math,
      //           ptx=)" +
      //     std::to_string(target.ptx_version) + R"(,
      //           toolkit-path=)" +
      //     cuda_root + R"(,
      //           fatbin-options=--maxrregcount=128
      //         )
      //     )
      //     )",
