...
absl::Status InitializeBarrier(
  ...
  if (!device_state.barrier_signal ||
    ...
    // Barrier signal buffer should be allocated witin a collective memory
    // space (kCollective) since it's going to be registered as a collective
...
absl::Status MosaicGpuInitialize(
  ...
  param_to_peers.resize(buffers.size() * clique_key.num_devices());
  bool is_all_parameters_vmm =
      absl::c_all_of(buffers, [&](const ffi::AnyBuffer& buffer) {
        return collective_params->executor->IsVmmMemory(buffer.device_memory());
      });
  for (int i = 0; i < buffers.size(); ++i) {
    ...
    if (is_all_parameters_vmm) {
      ...
      // also need to subtract the offset from the parameter base address to
      ...
      for (int device_rank = 0; device_rank < clique_key.num_devices();
        ...
        if (device_rank == rank.value()) {
          ...
          TF_ASSIGN_OR_RETURN(se::DeviceAddressBase peer_address,
                              symmetric_memory->peer_addr(device_rank));
          param_to_peers[parameter_offset + device_rank] =
              peer_address.opaque();
        }
      }
  ...
  // If all parameters are in the collective memory space, we can use NCCL to
  // get peer addresses.
  if (!is_all_parameters_vmm) {
    XLA_VLOG_DEVICE(6, device_ordinal)
