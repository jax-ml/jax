...
// used with multimem instruction or across several processes.
absl::StatusOr<std::vector<bool>> ParseCollectiveMemoryParameters(
  ...
  for (int64_t arg = 0; arg < num_buffers; ++arg) {
    if (collective_memory_parameters[arg] == 1) {
      result[arg] = true;
    }
  }
  return result;
...
absl::StatusOr<xla::gpu::GpuCliqueKey> GetCliqueKey(
    ...
    const xla::ffi::Dictionary& attributes) {
  ASSIGN_OR_RETURN(std::vector<int64_t> replica_ids,
...
absl::Status MosaicGpuInitialize(
  ...
  if (!all_parameters_in_collective_memory) {
    ...
    ASSIGN_OR_RETURN(param_to_peers,
