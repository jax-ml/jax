...
class Tiling {
 public:
  ...
  std::vector<int64_t> TileShape(const std::vector<int64_t>& shape) const;

  // Compute the shape of an array before tiling from its tiled shape.
  std::vector<int64_t> UntileShape(const std::vector<int64_t>& shape) const;
  ...
  // ((2 * 64 * 64, 64), (64 * 64, 1)).
  std::pair<std::vector<Tile>, std::vector<Tile>> TileNestedShapeStrides(
  ...
  std::vector<bool> TileDimension(int dim) const;

  // Returns a tiling with the given dimension removed.
  Tiling RemoveDimension(int dim) const;
