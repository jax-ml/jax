...
TEST_F(TransformsTest, TileTransformConstructionRoundingDown) {
...
TEST_F(TransformsTest, TileTransformShapes) {
  TileTransform transform;
...
TEST_F(TransformsTest, ApplyFailsOnNonDivisible) {
  mlir::Type f32 = builder_.getF32Type();
  ...
  EXPECT_THAT(transform.Apply(builder_, input),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("no rounding mode is specified")));
}
