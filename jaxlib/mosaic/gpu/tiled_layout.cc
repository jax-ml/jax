...
Tiling::Tiling(std::vector<Tile> tiles) : tiles_(std::move(tiles)) {
  ...
  for (const Tile& tile : tiles_) {
    CHECK(tile.size() <= last_tile_rank) << "Tiles must have a decreasing rank";
    CHECK(!tile.empty()) << "Tiles must not be empty";
    for (int64_t d : tile) {
      CHECK(d > 0) << "Tile shape must only have positive sizes";
    }
    last_tile_rank = tile.size();
  }
}

std::vector<int64_t> Tiling::TileShape(
    const std::vector<int64_t>& shape) const {
  std::vector<int64_t> current_shape = shape;
  for (const Tile& tile : tiles_) {
    CHECK(tile.size() <= current_shape.size())
        << "Tiling does not apply to shape";
    ...
    for (size_t i = 0; i < tile.size(); ++i) {
      ...
      int64_t t = tile[i];
      CHECK(dim % t == 0) << "Dimension not divisible by tile size";
...
std::vector<int64_t> Tiling::UntileShape(
    const std::vector<int64_t>& shape) const {
  std::vector<int64_t> current_shape = shape;
  for (auto it = tiles_.rbegin(); it != tiles_.rend(); ++it) {
    const Tile& tile = *it;
    CHECK(tile.size() * 2 <= current_shape.size()) << "Invalid tiled shape";
    ...
    for (size_t i = 0; i < tile.size(); ++i) {
      ...
      CHECK(inner == tile[i]) << "Tiling dimension mismatch";
...
Tiling::TileNestedShapeStrides(
  ...
  for (const Tile& tile : tiles_) {
    ...
    for (size_t i = 0; i < tile.size(); ++i) {
      ...
      for (size_t j = 0; j < dim_shape.size(); ++j) {
        ...
        if (d < t) {
          CHECK(t % d == 0) << "Dimension not divisible by tile size";
          t /= d;
          minor_dim_shape_rev.push_back(d);
          minor_dim_stride_rev.push_back(s);
        } else if (t != 1) {
          CHECK(d % t == 0) << "Dimension not divisible by tile size";
      ...
      CHECK(t == 1) << "Tile size too large for dimension";
  ...
  return {std::move(current_shape), std::move(current_strides)};
}

std::vector<bool> Tiling::TileDimension(int dim) const {
  size_t tiling_rank = tiles_[0].size();
  CHECK(dim >= 0 && dim < tiling_rank) << "Invalid dimension";
...
Tiling Tiling::RemoveDimension(int dim) const {
  size_t tiling_rank = tiles_[0].size();
  CHECK(dim >= 0 && dim < tiling_rank) << "Invalid dimension";
  int dim_in_tile = dim;
  ...
  for (Tile t : tiles_) {
    CHECK(last_tile_rank >= t.size()) << "Rank invariant violated";
