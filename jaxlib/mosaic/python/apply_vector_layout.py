...
class VectorLayout:
  ...
  def __post_init__(self):
    ...
    assert self.bitwidth == 16 or self.bitwidth == 32
...
def relayout(v: ir.Value, src: VectorLayout, dst: VectorLayout) -> ValueLike:
  ...
  if "in_layout" in op.attributes:
    ...
    for idx, (v, li) in enumerate(zip(op.operands, layout_in)):
      ...
      with ir.InsertionPoint(op), op.location:
        new_v = relayout(v, src=lo, dst=li).result
...
def _arith_extf_rule(  # pylint: disable=missing-function-docstring
  ...
  if type(layout_in) != type(layout_out):  # pylint: disable=unidiomatic-typecheck
    raise NotImplementedError("Changing layout type inside a cast")
  ...
  if layout_in.implicit_dim is not None:
    if (
        layout_in.implicit_dim != ImplicitDim.SECOND_MINOR
        or layout_out.implicit_dim != ImplicitDim.SECOND_MINOR
    ):
...
def _arith_truncf_rule(  # pylint: disable=missing-function-docstring
  ...
  if layout_in.implicit_dim is None and layout_out.implicit_dim is None:
    ...
    if layout_out.tiling == TARGET_SHAPE:
      for idx in np.ndindex(output_vregs.shape):
        first = input_vregs[(*idx[:-1], idx[-1] * 2)]
        if idx[-1] * 2 + 1 == input_vregs.shape[-1]:
          second = first  # OOB, so we can pack any data lying around.
        else:
          second = input_vregs[(*idx[:-1], idx[-1] * 2 + 1)]
        output_vregs[idx] = tpu.PackSubelementsOp(res_vreg_ty, [first, second])
    elif layout_out.tiling == (16, 128):
      for idx in np.ndindex(output_vregs.shape):
...
def native_vreg_ty(el_ty: ir.Type) -> ir.Type:
  if ir.F32Type.isinstance(el_ty):
    return ir.VectorType.get(TARGET_SHAPE, el_ty)
  elif ir.BF16Type.isinstance(el_ty):
    return ir.VectorType.get((*TARGET_SHAPE, 2), el_ty)
  elif ir.IntegerType.isinstance(el_ty):
    el_ty = ir.IntegerType(el_ty)
    if el_ty.width == 32:
      return ir.VectorType.get(TARGET_SHAPE, el_ty)
  raise NotImplementedError(el_ty)
