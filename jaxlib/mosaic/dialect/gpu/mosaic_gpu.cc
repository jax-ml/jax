...
absl::StatusOr<MemRefType> TileMemRefTypeImpl(MemRefType source_type,
  ...
  if (source_type.getLayout().isIdentity()) {
...
::llvm::LogicalResult UntileShapeOp::inferReturnTypes(
  ...
  result_shape.reserve(num_untiled_dims + tiling_rank);
  for (int64_t i = 0; i < num_untiled_dims; ++i) {
  ...
  if (tiled_type.getLayout().isIdentity()) {
  ...
  result_strides.reserve(num_untiled_dims + tiling_rank);
  {
    for (int64_t i = 0; i < num_untiled_dims; ++i) {
...
llvm::LogicalResult UntileShapeOp::canonicalize(
    UntileShapeOp op, mlir::PatternRewriter& rewriter) {
  mlir::Operation* producer = op.getSource().getDefiningOp();
  if (auto slice_smem = llvm::dyn_cast<SliceSMEMOp>(producer)) {
    rewriter.replaceOpWithNewOp<SliceSMEMOp>(op, op.getResult().getType(),
                                             slice_smem.getOffset());
    return llvm::success();
  }

  if (auto tile_shape = llvm::dyn_cast<TileShapeOp>(producer)) {
    MemRefType out_type = op.getType();
    MemRefType source_type = tile_shape.getSource().getType();
    // We only check shape equality here, because given a memref type with
    // an unset layout and a memref type with a contiguous strided layout,
    // the types would not compare as equal---even though they are functionally
    // equivalent.
    //
    // Since type inference takes care of deriving the strides for both
    // tile_shape and untile_shape ops, verification should already ensure that
    // this transformation is valid.
    if (source_type.getShape() == out_type.getShape()) {
      rewriter.replaceOp(op, tile_shape.getSource());
      return llvm::success();
    }
  }
  return rewriter.notifyMatchFailure(op, "no canonicalization pattern found");
}
