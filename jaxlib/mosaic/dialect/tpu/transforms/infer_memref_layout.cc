...
LogicalResult inferOp(Operation &op, int hardware_generation);

LogicalResult inferBlock(Block &block, const int hardware_generation) {
  if (block.empty()) {
    return success();
  }
  for (auto op_it = block.begin(); op_it != block.end(); ++op_it) {
    Operation &op = *op_it;
    if (auto alloca_op = dyn_cast<memref::AllocaOp>(op)) {
      ...
      if (memref_ty != new_memref_ty) {
        OpBuilder builder(&block, std::next(op_it));
...
LogicalResult inferOp(Operation &op, const int hardware_generation) {
  for (Region &region : op.getRegions()) {
    for (Block &block : region.getBlocks()) {
      if (failed(inferBlock(block, hardware_generation))) {
        return failure();
      }
    }
  }
