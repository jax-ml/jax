...
#define GEN_PASS_DECL_PREINFEROPTIMIZATIONPASS
#define GEN_PASS_DEF_PREINFEROPTIMIZATIONPASS
...
struct RhsTraversalResult {
  ...
  // Note - this is kinda arbitrary, but it's a good example of how we can
  // look for ops that might be removable here, and it's a real world case.
  // In the future, we should extend this to casts that elide out, and to
  // elementwise, so we can "peek" through them to the transpose.
  arith::SIToFPOp sitofp_op = nullptr;
  arith::FPToSIOp fptosi_op = nullptr;
  Value pre_cast_rhs;
};
...
std::optional<RhsTraversalResult> walkRhsForFusibleTranspose(Value rhs) {
  ...
  while (Operation* defining_op = current_operand.getDefiningOp()) {
    ...
    } else if (auto fptosi_op = dyn_cast<arith::FPToSIOp>(defining_op)) {
      if (!result.fptosi_op) {
        result.fptosi_op = fptosi_op;
        current_operand = fptosi_op.getOperand();
        continue;
      }
  ...
  if (auto sitofp_op =
          dyn_cast_or_null<arith::SIToFPOp>(current_operand.getDefiningOp())) {
...
std::optional<std::tuple<Value, bool, tpu::DotDimensionNumbersAttr>>
tryFuseRhsTranspose(tpu::MatmulOp op, bool current_transpose_rhs,
  ...
  // Start with the value that is the input to the transpose.
  ...
  if (trace.sitofp_op && trace.fptosi_op) {
    ...
    if (pre_cast_type.getElementType() == post_cast_type.getElementType()) {
      ...
      // The casts form a perfect round-trip, so we can safely use the
      // original value from before the sitofp.
      current_val = trace.pre_cast_rhs;
    }
  }

  // If we didn't elide the casts, we may need to reconstruct the fptosi.
  if (!cancelled_casts && trace.fptosi_op) {
    ...
    current_val = builder.create<arith::FPToSIOp>(new_fptosi_type, current_val,
                                                  trace.fptosi_op->getAttrs());
  }
  ...
  return std::make_tuple(new_rhs, !current_transpose_rhs,
                         new_dimension_numbers);
}

struct PreInferOptimizationPass
    : impl::PreInferOptimizationPassBase<PreInferOptimizationPass> {
  void runOnOperation() override {
    // We collect all ops that are part of a fused chain and erase them at the
    // end to avoid iterator invalidation issues. A SmallPtrSet ensures we don't
    // try to erase the same op twice.
    llvm::SmallPtrSet<Operation*, 8> ops_to_erase;
    ...
    getOperation().walk([&](tpu::MatmulOp op) {
      ...
      if (auto fusion_result =
              tryFuseRhsTranspose(op, op.getTransposeRhs(), builder)) {
        auto [new_rhs_val, new_transpose_rhs, new_dnums] = *fusion_result;

        if (auto new_rhs = dyn_cast<TypedValue<VectorType>>(new_rhs_val)) {
          // Collect the original op chain for deletion.
          Operation* current_op = op.getRhs().getDefiningOp();
          while (current_op && current_op->hasOneUse()) {
            ops_to_erase.insert(current_op);
            if (current_op->getNumOperands() > 0) {
              // Be careful with multi-operand ops, but for this chain it's
              // fine.
              current_op = current_op->getOperand(0).getDefiningOp();
            } else {
              break;
            }
          }
          ...
          op.setTransposeRhs(new_transpose_rhs);
    ...
    // Now that the walk is complete, we can safely erase the operations.
    // Erasing in reverse topological order is safest.
...
std::unique_ptr<OperationPass<func::FuncOp>> createPreInferOptimizationPass() {
  return std::make_unique<PreInferOptimizationPass>();
}
