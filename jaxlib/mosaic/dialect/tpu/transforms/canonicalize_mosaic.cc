...
LogicalResult tpu_matmul_rule(tpu::MatmulOp op) {
  ...
  // 3) defined and default -> no-op for canonicalization of dims
  ...
  auto transposition_fn = [&builder](auto &lhs) -> auto {
    ...
    // This function must run on vectors with rank >= 2
    CHECK_GE(rank, 2);
  ...
  auto dot_dim_matmul = [&](auto lhs, auto rhs, auto acc) {
    ...
    if (transpose_lhs) {
      lhs = transposition_fn(lhs);
    }
    // We must update the lhs operand of the matmul op to the transposed lhs.
    // While it is correct to proceed without this, it will cause problems
    // later when we try to infer the layout of the matmul op.
    op.setOperand(0, lhs);
