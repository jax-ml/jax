...
FailureOr<Value> canonicalize_fptosi(const CanonicalizeContext &ctx,
  ...
  // - f32->s8 and f32->s4 casts on TPU 7x+.
  bool bf16_to_s8_or_s4 = src_vty.getElementType().isBF16() &&
                          (dst_vty.getElementType().isSignlessInteger(8) ||
                           dst_vty.getElementType().isSignlessInteger(4));
  bool f32_to_s8_or_s4 = src_vty.getElementType().isF32() &&
                         (dst_vty.getElementType().isSignlessInteger(8) ||
                          dst_vty.getElementType().isSignlessInteger(4));
  if (is_vector && ((ctx.hardware_generation >= 6 && bf16_to_s8_or_s4) ||
