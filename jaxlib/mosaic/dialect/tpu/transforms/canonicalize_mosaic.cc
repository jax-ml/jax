...
FailureOr<Value> canonicalize_reshape(const CanonicalizeContext &ctx,
  ...
  if (!memref_ty || !isContiguousMemref(ref)) {
    return raw_op.getResult(0);
  }

  // This pattern assumes the load is rank-preserving.
  if (memref_ty.getRank() != src_ty.getRank()) {
    return raw_op.getResult(0);
  }
  ...
  // Make sure we fill a tile, theres 2 layouts we can use here:
  // (1, sublane * packing * lane) or (sublane * packing, lane)
  auto full_vreg_last_2_dims = (sublane * packing * lane);
  auto tgt_sublane = *(tgt_ty.getShape().end() - 2);
  auto tgt_lane = *(tgt_ty.getShape().end() - 1);
  if (tgt_sublane % (sublane * packing) != 0) {
    // If the sublane is not a multiple of the packing, we can only use the full
    // vreg!
    if (tgt_lane % full_vreg_last_2_dims != 0) {
      return raw_op.getResult(0);
    }
  }
  ...
  // Steps forward only if the dimension is a full load, a broadcast, or a
  // constant slice, otherwise aborts canon.
  for (int i = 0; i < src_ty.getRank(); ++i) {
    int64_t mem_d = memref_ty.getShape()[i];
    int64_t vec_d = src_ty.getShape()[i];
    ...
    std::optional<int64_t> off = getIntConst(indices[i]);
    if (off.has_value()) {
      auto off_val = off.value();
      if (off_val >= 0 && off_val + vec_d <= mem_d) {
        continue;
      }
    }
  ...
  // vector's prefix.
  SmallVector<int64_t> mem_prefix(memref_ty.getShape().begin(),
                                  memref_ty.getShape().begin() + split_point);
  SmallVector<int64_t> vec_prefix(src_ty.getShape().begin(),
  ...
  SmallVector<int64_t> reshaped_mem_shape = mem_prefix;
  reshaped_mem_shape.push_back(sublane_prod);
  reshaped_mem_shape.push_back(lane);
  Value reshaped_ref = b.create<tpu::MemRefReshapeOp>(
      MemRefType::get(reshaped_mem_shape, memref_ty.getElementType()), ref);

  // Bitcast this view to i32 for packed loading.
  int64_t num_i32_rows = sublane_prod / packing;
  *(reshaped_mem_shape.end() - 2) = num_i32_rows;
  Value i32_view = b.create<tpu::MemRefBitcastOp>(
      MemRefType::get(reshaped_mem_shape, i32_type), reshaped_ref);
  ...
  SmallVector<int32_t> strides(reshaped_mem_shape.size(), 1);
  ...
  // Collapse the '1' dimension from the loaded chunk.
  SmallVector<int64_t> collapsed_shape = vec_prefix;
