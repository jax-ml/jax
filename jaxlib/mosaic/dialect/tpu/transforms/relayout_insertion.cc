...
FailureOr<TypedValue<VectorType>> changeBitwidth(
  ...
  if (src.bitwidth() == dst.bitwidth() || src.tiling() != dst.tiling() ||
...
FailureOr<TypedValue<VectorType>> relayout(
  ...
  if (v.getType().getElementType() == builder.getI1Type() &&
    ...
    auto tmp_vreg_slice =
        src.vregSlice(target_shape, dst.bitwidth(), tmp_tiling);
    auto tmp_offsets = LayoutOffsets{
        src.offsets()[0].has_value() ? *src.offsets()[0] % tmp_vreg_slice[0]
                                     : LayoutOffset(),
        src.offsets()[1].has_value() ? *src.offsets()[1] % tmp_vreg_slice[1]
                                     : LayoutOffset(),
    };
    auto tmp_src = VectorLayout(src.bitwidth(), tmp_offsets, tmp_tiling,
                                dst.implicit_dim());
    auto tmp_dst = VectorLayout(dst.bitwidth(), tmp_offsets, tmp_tiling,
                                dst.implicit_dim());

    if (src != tmp_src) {
      auto relayout_op =
          builder.create<tpu::RelayoutOp>(v.getLoc(), v.getType(), v);
      setLayout(relayout_op, src, tmp_src);
      v = cast<TypedValue<VectorType>>(relayout_op.getResult());
    }
    FAILUREOR_ASSIGN_OR_RETURN(
        TypedValue<VectorType> new_v,
        changeBitwidth(builder, v, tmp_src, tmp_dst, hardware_generation,
                       target_shape));
    if (dst != tmp_dst) {
      auto relayout_op =
          builder.create<tpu::RelayoutOp>(v.getLoc(), v.getType(), new_v);
      setLayout(relayout_op, tmp_dst, dst);
