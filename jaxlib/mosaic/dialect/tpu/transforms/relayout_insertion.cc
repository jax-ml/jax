...
FailureOr<TypedValue<VectorType>> relayout(
  ...
  if (v.getType().getElementType() == builder.getI1Type() &&
    ...
    if (src.bitwidth() == 32 && dst.bitwidth() == 16 &&
      ...
      auto identity_op =
          builder.create<tpu::IdentityOp>(v.getLoc(), v.getType(), v);
      setLayout(identity_op, src, dst_bitwidth_layout);
      return cast<TypedValue<VectorType>>(identity_op.getResult());
    }
