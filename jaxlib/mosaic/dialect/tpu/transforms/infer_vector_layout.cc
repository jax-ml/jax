...
#define NYI(msg)                            \
  ...
  LogicalResult inferBlock(
    ...
    for (Operation &any_op : block.without_terminator()) {
      ...
      } else if (isa<arith::ExtFOp, arith::ExtSIOp, tpu::ExtFOp>(any_op)) {
      ...
      } else if (isa<arith::TruncFOp, arith::TruncIOp, tpu::TruncFOp>(any_op)) {
  ...
  LogicalResult inferExt(Operation *op) {
    ...
    TPU_CHECK_OP(some_layout.has_value(), "missing vector layout");
    if (isa<arith::ExtFOp, tpu::ExtFOp>(op)) {
