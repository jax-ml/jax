...
#define NYI(msg)                            \
  ...
  LogicalResult infer(vector::MultiDimReductionOp op) {
    ...
    // When shape_invariant_numerics is enabled, force zero offsets to the
    // reduction dimensions for the source layout. This is because we want to
    // guarantee more stable numerics by disabling some optimizations in
    // apply-vector-layout pass which may change the numerics.
    if (shape_invariant_numerics_) {
      src_layout = VectorLayout(src_layout.bitwidth(),
