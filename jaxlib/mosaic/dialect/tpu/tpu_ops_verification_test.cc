  ...
  Value ConstantIndexVector(ArrayRef<int64_t> shape, ArrayRef<int64_t> values) {
    return Create<arith::ConstantOp>(
               ...
               /*value=*/dyn_cast<TypedAttr>(
                   builder().getIndexArrayAttr(values)))
        .getResult();
  }
...
TEST_F(TpuOpsVerificationTest, ScatterStoreVerificationWorks) {
...
TEST_F(TpuOpsVerificationTest, ScatterStoreInvalidMemorySpace) {
...
TEST_F(TpuOpsVerificationTest, ScatterStoreInvalidElementType) {
  Value memref =
...
TEST_F(TpuOpsVerificationTest, ScatterStoreInvalidIndicesDimension) {
  Value memref = AllocaI32({8}, MemorySpace::kVmem);
  Value vector_to_store = ConstantI32Vector(/*shape=*/{8},
                                            /*values=*/{1});
...
TEST_F(TpuOpsVerificationTest, ScatterStoreValidMask) {
  Value memref = AllocaI32({8}, MemorySpace::kVmem);
  Value vector_to_store = ConstantI32Vector(/*shape=*/{8},
                                            /*values=*/{1});
  Value indices = ConstantIndexVector(/*shape=*/{8},
...
TEST_F(TpuOpsVerificationTest, ScatterStoreInvalidMaskShape) {
  Value memref = AllocaI32({8}, MemorySpace::kVmem);
  Value vector_to_store = ConstantI32Vector(/*shape=*/{8},
                                            /*values=*/{1});
  Value indices = ConstantIndexVector(/*shape=*/{8},
