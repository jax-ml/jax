...
int64_t TiledLayoutAttr::getUntiledRank() const {
  ...
  int64_t untiled_rank = getRank();
  int64_t tiled_rank = getRank();
  for (const xla::Tile& tile : getTiles()) {
...
LogicalResult TiledLayoutAttr::verify(
  ...
  // across first tiles (e.g. T(8, 128)T(2, 4, 64)), and this has no
  // applications anyway.
  if (auto it = llvm::find_if(tiles.drop_front(),
                              [&](const xla::Tile& tile) {
                                return tile.dimensions().size() >
                                       first_tile.dimensions().size();
                              });
      it != tiles.end()) {
    return emitError() << "Not implemented: Nested tile " << it->ToString()
                       << " has more dimensions than the first tile ("
                       << first_tile.ToString() << ").";
  }
