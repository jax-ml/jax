...
FailureOr<TypedValue<VectorType>> getFullLikeVreg(ImplicitLocOpBuilder &builder,
                                                  Type vreg_ty,
                                                  Attribute value) {
  auto vty = dyn_cast<VectorType>(vreg_ty);
  if (!vty) {
    return builder.emitError() << "Expected a vector type";
  }
  DenseElementsAttr dense_attr = dyn_cast<DenseElementsAttr>(value);
  if (!dense_attr) {
    dense_attr = DenseElementsAttr::get(vty, value);
  } else if (!dense_attr.isSplat()) {
    return builder.emitError()
           << "Expected a splat attribute. Got: " << value << "\n";
  }
  return cast<TypedValue<VectorType>>(
      builder.create<arith::ConstantOp>(dense_attr).getResult());
}

FailureOr<TypedValue<VectorType>> getFullLikeVreg(ImplicitLocOpBuilder &builder,
                                                  Value vreg, Attribute value) {
  return getFullLikeVreg(builder, vreg.getType(), value);
}

FailureOr<TypedValue<VectorType>> getZerosLikeVreg(
    ImplicitLocOpBuilder &builder, Type vreg_ty) {
  return getFullLikeVreg(builder, vreg_ty, builder.getZeroAttr(vreg_ty));
}

FailureOr<TypedValue<VectorType>> getZerosLikeVreg(
    ImplicitLocOpBuilder &builder, Value vreg) {
  return getZerosLikeVreg(builder, vreg.getType());
}

FailureOr<TypedValue<VectorType>> getX32VmaskByPaddingEnd(
    ImplicitLocOpBuilder &builder, Value padding,
    const std::array<int64_t, 2> target_shape, int64_t dim, int64_t stride) {
  ...
  Value padding_vreg;
  if (auto const_op = padding.getDefiningOp<arith::ConstantOp>()) {
    if (auto integer_attr = dyn_cast<IntegerAttr>(const_op.getValue())) {
      int64_t padding_cst = integer_attr.getValue().getSExtValue();
      if (padding_cst < 0 || padding_cst > target_shape[dim]) {
        return builder.emitError()
               << "Expected padding to be in [0, " << target_shape[dim] << "]";
      }
      FAILUREOR_ASSIGN_OR_RETURN(
          padding_vreg,
          getFullLikeVreg(builder, i32_vreg_ty,
                          builder.getI32IntegerAttr(padding_cst)));
    }
  } else {
    padding_vreg = builder.create<vector::BroadcastOp>(i32_vreg_ty, padding);
  }

  if (stride > 0) {
    FAILUREOR_ASSIGN_OR_RETURN(
        Value stride_vreg, getFullLikeVreg(builder, i32_vreg_ty,
                                           builder.getI32IntegerAttr(stride)));
    auto offset = builder.create<arith::MulIOp>(
        i32_vreg_ty,
        builder.create<tpu::IotaOp>(
            i32_vreg_ty, builder.getI32IntegerAttr(dim == 0 ? 1 : 0)),
        stride_vreg);
    padding_vreg =
        builder.create<arith::AddIOp>(i32_vreg_ty, padding_vreg, offset);
  }
...
FailureOr<TypedValue<VectorType>> getX32VmaskByPaddingEnd(
    ...
    const std::array<int64_t, 2> target_shape, int64_t dim, int64_t stride) {
  return getX32VmaskByPaddingEnd(
      builder,
      builder.create<arith::ConstantOp>(builder.getI32Type(),
                                        builder.getI32IntegerAttr(padding)),
...
LogicalResult maskNativeTilingVregs(ImplicitLocOpBuilder &builder,
  ...
  VectorType i32_vreg_ty =
      getNativeVregType(builder.getI32Type(), target_shape);
  FAILUREOR_ASSIGN_OR_RETURN(Value i32_zeros_vreg,
                             getZerosLikeVreg(builder, i32_vreg_ty));
  FAILUREOR_ASSIGN_OR_RETURN(
      Value i32_max_vreg,
      getFullLikeVreg(builder, i32_vreg_ty,
  ...
  if (padding_bottom > 0) {
    ...
    FAILUREOR_ASSIGN_OR_RETURN(
        Value mask_top, getX32VmaskByPaddingEnd(
                            builder, target_shape[0] - (x32_padding_bottom + 1),
                            target_shape, /*dim=*/0));
    FAILUREOR_ASSIGN_OR_RETURN(
        Value mask_bottom,
        getX32VmaskByPaddingEnd(builder, target_shape[0] - x32_padding_bottom,
                                target_shape,
                                /*dim=*/0));
  ...
  if (padding_right > 0) {
    FAILUREOR_ASSIGN_OR_RETURN(
        Value mask_right,
        getX32VmaskByPaddingEnd(builder, target_shape[1] - padding_right,
                                target_shape, /*dim=*/1));
