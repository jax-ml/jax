  ...
  if (getDimensionNumbers().has_value()) {
    ...
    auto check_and_mark_dims = [&](const std::vector<int64_t> &dims,
      ...
      for (int64_t dim : dims) {
        if (seen_dims[dim]) {
          emitOpError("Illegal: Dim found in multiple places in ") << operand;
          return failure();
        }
        seen_dims[dim] = true;
      }
    ...
    if (output_dim_order.size() % 2 != 0) {
      emitOpError("Illegal: output dim order must be of size 2");
      return failure();
    }
    ...
    std::unordered_set<int64_t> lhs_needed(lhs_non_contracting_dims.begin(),
                                           lhs_non_contracting_dims.end());
    std::unordered_set<int64_t> rhs_needed(rhs_non_contracting_dims.begin(),
    ...
    for (int dim_pos = batch_size.has_value() ? 2 : 0;
      ...
      if (is_lhs) {
        dim -= lhs_adjustment;
        if (lhs_needed.count(dim)) {
          lhs_needed.erase(dim);
        } else if (std::count(lhs_non_contracting_dims.begin(),
                              lhs_non_contracting_dims.end(), dim) > 0) {
          emitError("Dimension appears more than once in lhs: ") << dim;
          return failure();
        }
      } else {
        dim -= rhs_adjustment;
        if (rhs_needed.count(dim)) {
          rhs_needed.erase(dim);
        } else if (std::count(rhs_non_contracting_dims.begin(),
                              rhs_non_contracting_dims.end(), dim) > 0) {
          emitError("Dimension appears more than once in rhs: ") << dim;
          return failure();
        }
      }
    }
