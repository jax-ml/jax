...
AttributeMap AttributeMapFromPyDict(nb::dict dict) {
  ...
  for (auto [key, value] : dict) {
    if (!nb::isinstance<nb::str>(key)) {
      PyErr_SetString(PyExc_TypeError, "Custom option keys must be strings");
      throw nb::python_error();
    }
...
void PopulateExecutionOptions(xla::ifrt::ExecuteOptions& options) {
  ...
  AttributeMap custom_options =
      AttributeMapFromPyDict(nb::cast<nb::dict>(state));
  if (!options.custom_options.has_value()) {
    options.custom_options = std::move(custom_options);
    return;
  }
