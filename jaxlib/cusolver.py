...
def trsm(
    c, a, b, left_side=False, lower=False, trans_a=False, conj_a=False, diag=False
):
  ...
  the batched case."""
    ...
    if batch_dims + (k, k) != a_shape.dimensions() or a_shape.element_type() != dtype:
        raise ValueError(
            "Argument mismatch for trsm, got {} and {}".format(a_shape, b_shape)
        )
    ...
    out = c.CustomCall(
        ...
        shape_with_layout=_Shape.tuple_shape(
            (
                ...
                _Shape.array_shape(
                    dtype, batch_dims + (min(m, n),), tuple(range(num_bd, -1, -1))
                ),
                _Shape.array_shape(
                    np.dtype(np.int32), batch_dims, tuple(range(num_bd - 1, -1, -1))
                ),
        ...
        out = c.CustomCall(
            ...
            shape_with_layout=_Shape.tuple_shape(
                (
                    ...
                    _Shape.array_shape(
                        singular_vals_dtype, batch_dims + (min(m, n),), vector_layout
                    ),
