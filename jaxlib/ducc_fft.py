...
def ducc_fft_xhlo(a, dtype, *, fft_type: FftType, fft_lengths: List[int]):
  ...
  if 0 in a_type.shape or 0 in out_shape:
    zero = xhlo.ConstantOp(
        ir.DenseElementsAttr.get(np.array(0, dtype=out_dtype), type=out_type))
    return xhlo.BroadcastOp(
        zero, ir.DenseElementsAttr.get(np.asarray(out_shape, np.int64))).result

  u8_type = ir.IntegerType.get_unsigned(8)
  descriptor = xhlo.ConstantOp(
