...
// A ABSL-like open-addressing hash set that is safe to use where
...
//
// See https://www.youtube.com/watch?v=ncHmEUmJZf4 for a nice talk about the
// ABSL table design.
//
...
//  * Eq may make reentrant calls to the set, including indirectly by releasing
//    a lock and allowing other threads to mutate the set. If eq mutates the
//    set, then the operation that triggered it will be restarted.
//  * if Eq directly or indirectly mutates the set, this invalidates the
...
//  * Hash must not mutate the table, release any locks or make any reentrant
//    calls. However, this is an easy requirement to satisfy since one can
//    simply precompute the hash as part of the Key.
...
class ReentrantHashSet {
 ...
 private:
  ...
  FindResult FindInternal(const K& key, size_t hash) const {
    ...
    int8_t h2 = H2(hash);
    size_t capacity_mask = capacity_ - 1;
    size_t curr = H1(hash) & capacity_mask;  // Equivalent to % capacity_
    size_t step = 0;
    size_t target_idx = ~size_t{0};

    while (true) {
      Group g(ctrl_.get() + curr);
      for (int i : g.Match(h2)) {
        size_t idx = (curr + i) & capacity_mask;
      ...
      if (target_idx == ~size_t{0}) {
        ...
        if (match_eod) {
          target_idx = (curr + match_eod.LowestBitSet()) & capacity_mask;
        }
      }
      if (g.MatchEmpty()) {
        break;
      }
      // Quadratic probing with triangular numbers, see
      // https://en.wikipedia.org/wiki/Quadratic_probing
      step += Group::kWidth;
      curr = (curr + step) & capacity_mask;
    }
  ...
  void Initialize(size_t capacity) {
    ...
    capacity_ = capacity;
    if (capacity_ == 0) {
      ctrl_.reset(new int8_t[Group::kWidth + 1]);
      std::memset(ctrl_.get(), kEmpty, Group::kWidth + 1);
      ctrl_[Group::kWidth] = kSentinel;
      slots_.reset();
    } else {
      ctrl_.reset(new int8_t[capacity_ + Group::kWidth + 1]);
      std::memset(ctrl_.get(), kEmpty, capacity_ + Group::kWidth + 1);
      ctrl_[capacity_ + Group::kWidth] = kSentinel;
      slots_.reset(static_cast<Key*>(::operator new(capacity_ * sizeof(Key))));
    }
    ++version_;
  }
    ...
    if (old_capacity > 0) {
      for (size_t i = 0; i < old_capacity; ++i) {
        if (IsFull(old_ctrl[i])) {
          ...
          size_t capacity_mask = capacity_ - 1;
          size_t curr = H1(hash) & capacity_mask;
          size_t step = 0;
          while (true) {
            Group g(ctrl_.get() + curr);
            auto match_eod = g.MatchEmptyOrDeleted();
            if (match_eod) {
              size_t idx = (curr + match_eod.LowestBitSet()) & capacity_mask;
  ...
  // group is mirrored at the end of the array. The mirroring ensures that we
  // can read a 16-byte Group at any point in the array, without worrying about
  // reading past the end of the metadata.
  //
  // The sign bit of each entry in ctrl_ is negative if the slot is empty or
  // deleted. If the value is positive, the entry is present and the metadata
  // stores the lowest 7 bits of the hash of the key in the slot (H2). This
  // design allows for SIMD accelerated lookups that scan, say, 16 elements at a
  // time.
  std::unique_ptr<int8_t[]> ctrl_;

  struct SlotDeleter {
    void operator()(Key* ptr) const { ::operator delete(ptr); }
  };
  // The array of slots.
  std::unique_ptr<Key[], SlotDeleter> slots_;
