...
void PopulateCallLocation(xla::ifrt::ExecuteOptions& options) {
  ...
  std::optional<Traceback> traceback =
      GetTraceback(xla::ifrt::UserContextScope::current().get());
...
void BuildCallLocationSubmodule(nb::module_& m) {
  ...
  nb::enum_<RuntimeTracebackMode>(m, "RuntimeTracebackMode")
      .value("OFF", RuntimeTracebackMode::kOff)
      .value("ON", RuntimeTracebackMode::kOn)
      .value("FULL", RuntimeTracebackMode::kFull);
  m.def("set_send_traceback_to_runtime_global", [](RuntimeTracebackMode mode) {
