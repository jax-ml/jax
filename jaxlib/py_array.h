...
struct PyHostValue {
  PyHostValue() = default;
  explicit PyHostValue(xla::nb_numpy_ndarray value) : value(std::move(value)) {}
  ~PyHostValue() {
    if (value.ptr() != nullptr) {
      GlobalPyRefManager()->AddGarbage(std::move(value));
    }
  }
  PyHostValue& operator=(const PyHostValue&) = delete;
  // Declared but undefined so that `tsl::Future<PyHostValue>` remains copyable.
  PyHostValue(const PyHostValue&);
  PyHostValue(PyHostValue&&) = default;
  PyHostValue& operator=(PyHostValue&&) = default;
...
struct PyArray_Storage {
  ...
  nanobind::object py_arrays ABSL_GUARDED_BY(mu) = nanobind::none();
  tsl::Future<PyHostValue> host_value ABSL_GUARDED_BY(mu);
  std::optional<std::vector<int64_t>> dynamic_shape ABSL_GUARDED_BY(mu) =
      std::nullopt;
  ...
  // array.
  xla::Future<> result_status ABSL_GUARDED_BY(mu);
};
...
class PyArray : public nanobind::object {
 public:
  ...
  absl::StatusOr<std::vector<int64_t>> dynamic_shape() const;

  absl::StatusOr<std::shared_ptr<const xla::PjRtLayout>> layout() {
    ABSL_ASSIGN_OR_RETURN(xla::ifrt::ArrayRef ifrt_array_ptr, GetIfrtArray());
  ...
  nanobind::object npy_value() const {
    const Storage& storage = GetStorage();
    ft_lock_guard lock(storage.mu);
    return storage.npy_value;
  }
  ...
  // Returns the underlying IFRT array, or an InvalidArgument error if the array
  // has been deleted or donated. Unlike `ifrt_array_ref()`, which returns a
  // null pointer when deleted, this method returns a non-null `ArrayRef` or a
  // formatted error status.
  absl::StatusOr<xla::ifrt::ArrayRef> GetIfrtArray(
  ...
  nanobind::object py_arrays_cached();
