...
NB_MODULE(_mosaic_gpu_ext, m) {
  ...
  mlir::python::nanobind_adaptors::mlir_attribute_subclass(
      m, "TileTransformAttr", MosaicGpuIsATileTransformAttr)
      .def_classmethod(
          "get",
          [](nb::object cls, std::vector<int32_t>& tiling, MlirContext ctx) {
            return cls(MosaicGpuTileTransformAttrGet(ctx, tiling.data(),
                                                     tiling.size()));
          },
      ...
      .def_property_readonly("tiling", [](MlirAttribute self) {
        ...
        for (int i = 0; i < MosaicGpuTileTransformAttrGetTilingSize(self);
             ++i) {
          result.push_back(MosaicGpuTileTransformAttrGetTiling(self, i));
        }
        return result;
      });

  mlir::python::nanobind_adaptors::mlir_attribute_subclass(
      m, "TransposeTransformAttr", MosaicGpuIsATransposeTransformAttr)
      .def_classmethod(
          "get",
          [](nb::object cls, std::vector<int32_t>& permutation,
             MlirContext ctx) {
            return cls(MosaicGpuTransposeTransformAttrGet(
      ...
      .def_property_readonly("permutation", [](MlirAttribute self) {
        std::vector<int32_t> result;
        for (int i = 0;
             i < MosaicGpuTransposeTransformAttrGetPermutationSize(self); ++i) {
          result.push_back(
              MosaicGpuTransposeTransformAttrGetPermutation(self, i));
        }
        return result;
      });
