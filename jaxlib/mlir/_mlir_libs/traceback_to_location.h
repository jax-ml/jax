...
class TracebackToLocationCache {
 public:
  ...
  explicit TracebackToLocationCache(nanobind::callable code_to_filename);

  // Returns an MLIR location for the given traceback.
  // If the traceback is empty, returns an unknown location.
  mlir::Location Get(const Traceback& traceback, mlir::MLIRContext* context);
 ...
 private:
  ...
  struct TracebackPointerHash {
    size_t operator()(const Traceback& traceback) const {
      return absl::HashOf(traceback.ptr());
    }
  };
  struct TracebackPointerEqual {
    bool operator()(const Traceback& a, const Traceback& b) const {
      return a.ptr() == b.ptr();
    }
  };
  absl::flat_hash_map<Traceback, std::optional<mlir::Location>,
                      TracebackPointerHash, TracebackPointerEqual>
