...
TracebackToLocationCache::TracebackToLocationCache(
    nanobind::callable code_to_filename)
    : code_to_filename_(std::move(code_to_filename)) {}

mlir::Location TracebackToLocationCache::Get(const Traceback& traceback,
                                             mlir::MLIRContext* context) {
  auto& traceback_cache_entry = traceback_to_location_cache_[traceback];
  if (!traceback_cache_entry.has_value()) {
    std::optional<mlir::Location> loc;
    for (const TracebackEntry& frame : traceback.RawFrames()) {
      ...
      if (!frame_cache_entry.has_value()) {
        ...
        frame_cache_entry = mlir::NameLoc::get(
            mlir::StringAttr::get(context, function_name),
            mlir::FileLineColRange::get(
                mlir::StringAttr::get(context, filename), start_line,
                start_column, end_line, end_column));
      }
      if (loc.has_value()) {
        loc = mlir::CallSiteLoc::get(*loc, *frame_cache_entry);
      } else {
        loc = *frame_cache_entry;
      }
    }
    traceback_cache_entry =
        loc.has_value() ? *loc : mlir::UnknownLoc::get(context);
  }
