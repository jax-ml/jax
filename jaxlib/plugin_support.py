...
def import_from_plugin(
  ...
  return maybe_import_plugin_submodule(
      [f"jax.jaxlib.{plugin_name}", _PLUGIN_MODULE_NAME[plugin_name]],
      submodule_name,
      check_version=check_version,
  )


def import_plugin_extension(
    plugin_name: str, *, check_version: bool = True
) -> ModuleType | None:
  """Import the extension for a known plugin with version checking.
  ...
  Returns:
    The imported plugin extension, or None if the plugin is not installed or if
    the versions are incompatible.
  """
  if plugin_name not in _PLUGIN_MODULE_NAME:
    raise ValueError(f"Unknown plugin: {plugin_name}")
  return maybe_import_plugin_submodule(
      ["jax.jaxlib", _PLUGIN_MODULE_NAME[plugin_name]],
      f"{plugin_name}_plugin_extension",
      check_version=check_version,
  )
...
def maybe_import_plugin_submodule(
  ...
  for plugin_module_name in plugin_module_names:
    try:
      module = importlib.import_module(f"{plugin_module_name}.{submodule_name}")
    except ImportError:
      continue
    else:
