...
absl::Mutex runtime_error_type_lock(absl::kConstInit);
nb::handle py_jax_runtime_error_type ABSL_GUARDED_BY(runtime_error_type_lock);

void register_runtime_error_bindings(nb::module_& m) {
  absl::MutexLock ml(runtime_error_type_lock);
  if (py_jax_runtime_error_type.is_valid()) {
    return;
  }
...
void translate_xla_runtime_error(const std::exception_ptr& p, void*) {
  try {
    ...
    try {
      ...
      nb::object exc_inst = py_jax_runtime_error_type(msg, code);

      // Set the current Python error to this new exception instance.
      PyErr_SetObject(py_jax_runtime_error_type.ptr(), exc_inst.ptr());
