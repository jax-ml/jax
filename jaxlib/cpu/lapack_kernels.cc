...
// lapack geqp3
template <ffi::DataType dtype>
ffi::Error PivotingQrFactorization<dtype>::Kernel(ffi::Buffer<dtype> x,
                                          ffi::Buffer<LapackIntDtype> jpvt,
                                          ffi::ResultBuffer<dtype> x_out,
                                          ffi::ResultBuffer<LapackIntDtype> jpvt_out,
                                          ffi::ResultBuffer<dtype> tau) {
  FFI_ASSIGN_OR_RETURN((auto [batch_count, x_rows, x_cols]),
                        SplitBatch2D(x.dimensions()));
  auto* x_out_data = x_out->typed_data();
  ...
  const int64_t tau_step{std::min(x_rows, x_cols)};
  for (int64_t i=0; i < batch_count; ++i) {
    fn(&x_rows_v, &x_cols_v, x_out_data, &x_leading_dim_v, jpvt_out_data, tau_data,
      work_data.get(), &workspace_dim_v, &info);
    x_out_data += x_out_step;
...
template <ffi::DataType dtype>
ffi::Error PivotingQrFactorizationComplex<dtype>::Kernel(ffi::Buffer<dtype> x,
                                          ffi::Buffer<LapackIntDtype> jpvt,
                                          ffi::ResultBuffer<dtype> x_out,
                                          ffi::ResultBuffer<LapackIntDtype> jpvt_out,
                                          ffi::ResultBuffer<dtype> tau) {
  ...
  for (int64_t i=0; i < batch_count; ++i) {
    fn(&x_rows_v, &x_cols_v, x_out_data, &x_leading_dim_v, jpvt_out_data, tau_data,
      work_data.get(), &workspace_dim_v, rwork_data.get(), &info);
    x_out_data += x_out_step;
    jpvt_out_data += jpvt_step;
    tau_data += tau_step;
  }
...
int64_t PivotingQrFactorization<dtype>::GetWorkspaceSize(lapack_int x_rows,
  ...
  lapack_int info = 0;
  lapack_int workspace_query = -1;
  fn(&x_rows, &x_cols, nullptr, &x_leading_dim_v, nullptr, nullptr, &optimal_size,
    &workspace_query, &info);
  return info == 0 ? static_cast<int64_t>(std::real(optimal_size)) : -1;
}

template <ffi::DataType dtype>
int64_t PivotingQrFactorizationComplex<dtype>::GetWorkspaceSize(lapack_int x_rows,
                                                 lapack_int x_cols) {
  ValueType optimal_size{};
  lapack_int x_leading_dim_v = x_rows;
  lapack_int info = 0;
...
template struct PivotingQrFactorizationComplex<ffi::DataType::C64>;
template struct PivotingQrFactorizationComplex<ffi::DataType::C128>;
...
#define JAX_CPU_DEFINE_GEQP3_COMPLEX(name, data_type)            \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(                                 \
      name, PivotingQrFactorizationComplex<data_type>::Kernel,   \
      ::xla::ffi::Ffi::Bind()                                    \
          .Arg<::xla::ffi::Buffer<data_type>>(/*x*/)             \
          .Arg<::xla::ffi::Buffer<LapackIntDtype>>(/*jpvt*/)     \
          .Ret<::xla::ffi::Buffer<data_type>>(/*x_out*/)         \
...
JAX_CPU_DEFINE_GEQP3_COMPLEX(lapack_cgeqp3_ffi, ::xla::ffi::DataType::C64);
JAX_CPU_DEFINE_GEQP3_COMPLEX(lapack_zgeqp3_ffi, ::xla::ffi::DataType::C128);
...
#undef JAX_CPU_DEFINE_GEQP3_COMPLEX
