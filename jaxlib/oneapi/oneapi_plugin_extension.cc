...
NB_MODULE(oneapi_plugin_extension, m) {
  ...
  m.def(
      ...
      [](std::intptr_t data_value) {
        // TODO(Intel-tf): Implement using SYCL USM pointer query API to
        // retrieve the device ordinal for the given device pointer.
        xla::ThrowIfError(absl::UnimplementedError(
            "get_device_ordinal not yet implemented for OneAPI"));
      },
      nb::arg("data_value"));
}
}  // namespace jax
