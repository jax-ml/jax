...
#include "jaxlib/xla/py_program.h"
...
absl::StatusOr<std::unique_ptr<ifrt::CompileOptions>> MakeXlaCompileOptions(
    CompileOptions options, jax::PyDeviceList& py_execution_devices,
  ...
  TF_ASSIGN_OR_RETURN(ifrt::DeviceListRef execution_devices,
                      py_execution_devices.ifrt_device_list());
  return std::make_unique<ifrt::XlaCompileOptions>(
      std::move(options), std::move(execution_devices),
...
void BuildIfrtProgramsSubmodule(nanobind::module_& m) {
  ...
  sub_module
      ...
      .def("make_xla_compile_options",
           ...
           nb::arg("execution_devices"), nb::arg("host_callbacks"))
