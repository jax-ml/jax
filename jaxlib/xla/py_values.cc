...
// Makes an IFRT Array from `Shard`s using an array assembly API (slow path).
...
absl::StatusOr<DevicePutResult> DevicePutWithDevice(
    nanobind::handle arg, ifrt::Client* ifrt_client, ifrt::Device* ifrt_device,
    ifrt::MemoryKind ifrt_memory_kind, const DevicePutOptions& options) {
  ...
  TF_ASSIGN_OR_RETURN(
      ShardFn shard_fn,
      MakeShardFn(arg, ifrt_client, ifrt_device, ifrt_memory_kind, options));
...
absl::StatusOr<DevicePutResult> DevicePutWithSharding(
    absl::Span<const nanobind::handle> args, ifrt::Client* ifrt_client,
    const nb_dtype& dtype, absl::Span<const int64_t> shape,
  ...
  if (args.size() != ifrt_addressable_devices.size()) {
    ...
    return InvalidArgument(
        ...
        args.size(), ifrt_addressable_devices.size());
  }
  if (is_pmap_sharding && args.empty()) {
    return InvalidArgument(
  ...
  shard_fns.reserve(args.size());
  for (int i = 0; i < args.size(); ++i) {
    TF_ASSIGN_OR_RETURN(ShardFn shard, MakeShardFn(args[i], ifrt_client,
                                                   ifrt_addressable_devices[i],
