...
// `device_put_result` will be consumed.
///
...
MakeSingleDeviceIfrtArrayFromDevicePutResult(
    ...
    const tsl::RCReference<ifrt::UserContext>& user_context) {
  if (auto* ifrt_array = std::get_if<tsl::RCReference<ifrt::Array>>(
    ...
    auto host_buffer_shard = std::get<ifrt::Client::HostBuffer>(
        std::move(device_put_result.ifrt_array_or_host_buffer));
    ...
    return ifrt_client->MakeArrayFromHostBuffer(
        ...
        std::move(host_buffer_shard.on_done), user_context);
  }
}

// Makes an IFRT Array from `DevicePutResult`s using a batched array creation
// API. `device_put_results` will be consumed.
//
// Expected to be called without holding GIL.
absl::StatusOr<tsl::RCReference<ifrt::Array>>
...
// `device_put_results` will be consumed.
//
// Expected to be called without holding GIL.
absl::StatusOr<tsl::RCReference<ifrt::Array>>
  ...
  for (int64_t i = 0; i < device_put_results.size(); ++i) {
    TF_ASSIGN_OR_RETURN(
        ...
        MakeSingleDeviceIfrtArrayFromDevicePutResult(
            ...
            device_put_results[i], user_context));
...
absl::StatusOr<tsl::RCReference<ifrt::Array>> MakeIfrtArrayFromDevicePut(
  ...
  if (!ifrt_device_list->IsFullyAddressable()) {
  ...
  return MakeSingleDeviceIfrtArrayFromDevicePutResult(
      ifrt_client, ifrt_device_list->devices().front(), ifrt_memory_kind,
      device_put_result, std::move(ifrt_user_context));
}
