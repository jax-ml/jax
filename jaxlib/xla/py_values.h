...
#ifndef JAXLIB_XLA_PY_VALUES_H_
#define JAXLIB_XLA_PY_VALUES_H_
...
//
// `args` is a list of buffer-like objects to be copied to the devices specified
// in `sharding`.
//
// `shape` and `sharding` determine the shape and sharding of the returned IFRT
// Array.
//
// The size of `args` must match the number of addressable devices in
// `sharding`. For a Pmap sharding, there must be at least one addressable
// device.
//
// Requires GIL. Will release GIL while calling `shard_fns` and
