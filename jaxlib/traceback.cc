...
int traceback_tp_traverse(PyObject* self, visitproc visit, void* arg) {
  TracebackObject* tb = reinterpret_cast<TracebackObject*>(self);
  Py_VISIT(Py_TYPE(self));
  for (Py_ssize_t i = 0; i < Py_SIZE(self); ++i) {
    Py_VISIT(tb->frames[i].code);
  }
  return 0;
}

int traceback_tp_clear(PyObject* self) {
  TracebackObject* tb = reinterpret_cast<TracebackObject*>(self);
  for (Py_ssize_t i = 0; i < Py_SIZE(self); ++i) {
    Py_CLEAR(tb->frames[i].code);
  }
  return 0;
}
...
PyType_Slot traceback_slots_[] = {
    ...
    {Py_tp_traverse, reinterpret_cast<void*>(traceback_tp_traverse)},
    {Py_tp_clear, reinterpret_cast<void*>(traceback_tp_clear)},
...
// Feel free to turn this on if you like, but it might break at any time!
  ...
  Traceback traceback = nb::steal<Traceback>(
      PyObject_GC_NewVar(PyObject, traceback_type_, count));
  ...
  PyObject_GC_Track(traceback.ptr());
  return traceback;
}
...
void BuildTracebackSubmodule(nb::module_& m) {
  ...
  m.def(
      ...
      [](nb::object tb) {
        if (!tb.is_none() && !traceback_check(tb.ptr())) {
