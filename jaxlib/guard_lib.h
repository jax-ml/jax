...
// thread guard state is set with the current thread ID. If `set_thread_id` is
// false, the global thread guard state is cleared. A global mutex ensures the
// update is atomic.
// A failed status is returned if `set_thread_id` is true and the global thread
// ID is already set and not equal to the current thread ID.
