...
void fixstrides(stride_t &str, size_t size)
  ...
  for (auto &s: str)
    {
    auto tmp = s/ssize;
    if (tmp*ssize != s) throw "Bad stride";
    s = tmp;
    }
  }
...
void PocketFft(void* out, void** in) {
  ...
  shape_t shape(descriptor->shape()->begin(),
  ...
  shape_t axes(descriptor->axes()->begin(),
               descriptor->axes()->end());
  ...
  switch (descriptor->fft_type()) {
    case PocketFftType_C2C:
      if (descriptor->dtype() == PocketFftDtype_COMPLEX64) {
        ...
        ducc0::cfmav<std::complex<float>> m_in(reinterpret_cast<std::complex<float>*>(in[1]),
          shape, stride_in);
        ducc0::vfmav<std::complex<float>> m_out(reinterpret_cast<std::complex<float>*>(out),
          shape, stride_out);
        ...
        ducc0::cfmav<std::complex<double>> m_in(reinterpret_cast<std::complex<double>*>(in[1]),
          shape, stride_in);
        ducc0::vfmav<std::complex<double>> m_out(reinterpret_cast<std::complex<double>*>(out),
          shape, stride_out);
        ducc0::c2c(m_in, m_out, axes, descriptor->forward(),
    ...
    case PocketFftType_C2R:
      if (descriptor->dtype() == PocketFftDtype_COMPLEX64) {
        ...
        ducc0::cfmav<std::complex<float>> m_in(reinterpret_cast<std::complex<float>*>(in[1]),
          shape_in, stride_in);
        ducc0::vfmav<float> m_out(reinterpret_cast<float *>(out),
          shape, stride_out);
        ...
        ducc0::cfmav<std::complex<double>> m_in(reinterpret_cast<std::complex<double>*>(in[1]),
          shape_in, stride_in);
        ducc0::vfmav<double> m_out(reinterpret_cast<double *>(out),
          shape, stride_out);
        ducc0::c2r(m_in, m_out, axes, descriptor->forward(),
                   static_cast<double>(descriptor->scale()));
      }
    ...
    case PocketFftType_R2C:
      if (descriptor->dtype() == PocketFftDtype_COMPLEX64) {
        ...
        ducc0::cfmav<float> m_in(reinterpret_cast<float *>(in[1]),
          shape, stride_in);
        ducc0::vfmav<std::complex<float>> m_out(reinterpret_cast<std::complex<float>*>(out),
          shape_out, stride_out);
        ...
        auto shape_out = shape;
        shape_out[axes.back()]=shape_out[axes.back()]/2+1;
        ducc0::cfmav<double> m_in(reinterpret_cast<double *>(in[1]),
          shape, stride_in);
        ducc0::vfmav<std::complex<double>> m_out(reinterpret_cast<std::complex<double>*>(out),
          shape_out, stride_out);
        ducc0::r2c(m_in, m_out, axes, descriptor->forward(),
